...
- Library-count 1UP: clear the animating episode when the digit roll finishes so a later fetch can animate again (sticky `isSurfaceAnimating` left mega-hero / chart handoff stuck). Align roll duration with the +1 CSS flight; ease large capped trains; keep timed spawn timers cancelable after finish.
