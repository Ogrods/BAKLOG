...
_SKIP_PROFILE_SUFFIXES = ('.lock', '.tmp', '-journal', '-wal', '-shm')
_SKIP_PROFILE_NAMES = frozenset({'LOCK', 'SingletonLock', 'SingletonCookie', 'SingletonSocket', 'lockfile'})
...
class ImportSummary:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {'providers_imported': list(self.providers_imported), 'profiles_imported': list(self.profiles_imported), 'bytes_written': self.bytes_written, 'dry_run': self.dry_run}
...
def _derive_key(passphrase: str, salt: bytes, *, n_log: int=SCRYPT_N_LOG, r: int=SCRYPT_R, p: int=SCRYPT_P) -> bytes:
    maxmem = 128 * 2 ** n_log * r + (1 << 20)
    return scrypt(passphrase.encode('utf-8'), salt=salt, n=2 ** n_log, r=r, p=p, dklen=32, maxmem=maxmem)
...
def _build_inner_payload(*, include_profiles: bool) -> dict[str, Any]:
    payload: dict[str, Any] = {'bundle_format': INNER_FORMAT, 'exported_at': _now_iso(), 'app_version': _app_version(), 'secrets_doc': load_doc(), 'profiles': _collect_profiles() if include_profiles else {}, 'note': 'Generated by BAKLOG. Local-only. See PRIVACY.md.'}
    return payload
...
def _encrypt_payload(passphrase: str, inner: dict[str, Any]) -> bytes:
    ...
    header = MAGIC + bytes([BUNDLE_VERSION, SCRYPT_N_LOG, SCRYPT_R, SCRYPT_P]) + salt + nonce
...
def import_bundle(blob: bytes, passphrase: str, *, dry_run: bool=False) -> ImportSummary:
    ...
    providers = sorted((k for k, v in (secrets_doc.get('providers') or {}).items() if isinstance(v, dict) and v))
...
def parse_bundle_header(blob: bytes) -> dict[str, Any]:
    ...
    if blob[:len(MAGIC)] != MAGIC:
        raise BadMagic('invalid bundle magic')
    return {'magic': blob[:len(MAGIC)].decode('ascii'), 'version': blob[len(MAGIC)], 'scrypt_n_log': blob[len(MAGIC) + 1], 'scrypt_r': blob[len(MAGIC) + 2], 'scrypt_p': blob[len(MAGIC) + 3]}