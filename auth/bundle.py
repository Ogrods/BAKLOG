...
@dataclass
class ImportSummary:
    providers_imported: "Any" = field(default_factory=list)
    profiles_imported: "Any" = field(default_factory=list)
    bytes_written: "Any" = 0
    dry_run: "Any" = False
...
def _build_inner_payload(*, include_profiles):
    payload = {
...
def _encrypt_payload(passphrase, inner):
    salt = os.urandom(16)
    nonce = os.urandom(12)
...
def _decrypt_payload(blob, passphrase):
...
def export_bundle(passphrase, *, include_profiles=True):
...
def _restore_profiles(profiles, *, dry_run):
    imported = []
    bytes_written = 0
...
def import_bundle(blob, passphrase, *, dry_run=False):
    _validate_passphrase(passphrase)
