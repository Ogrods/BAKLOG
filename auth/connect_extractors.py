...
class HeaderSniffer:
    ...
    def __init__(self, *, url_substr: str, fields: dict[str, str], normalize: Callable[[str | None], str | None] | None=None) -> None:
...
class GraphqlResponseSniffer:
    ...
    def __init__(self, *, url_match: Callable[[str], bool], accept: Callable[[dict[str, Any]], bool]) -> None:
...
def battlenet_connect_hint(page: Any) -> str:
    ...
    return "On your Games page? Wait until your library list finishes loading, then we'll save the session automatically."

def extract_ea_bearer_session(context: Any, *, sniffer: HeaderSniffer) -> dict[str, str] | None:
...
def build_ea_header_sniffer() -> HeaderSniffer:
    ...
    return HeaderSniffer(url_substr=EA_GRAPHQL_HOST, fields={'authorization': 'EA_BEARER_TOKEN'}, normalize=normalize_bearer)

class DeferredGraphqlResponseSniffer:

    def __init__(self, *, url_match: Callable[[str], bool], accept: Callable[[dict[str, Any]], bool], debug_entry: Callable[[str, dict[str, Any]], dict[str, Any]] | None=None) -> None:
        self.url_match = url_match
        self.accept = accept
...
def build_epic_wishlist_graphql_sniffer() -> DeferredGraphqlResponseSniffer:
    ...
    return DeferredGraphqlResponseSniffer(url_match=is_epic_graphql_url, accept=wishlist_graphql_ok, debug_entry=graphql_debug_entry)
UBISOFT_API_HOST = 'public-ubiservices.ubi.com'

def build_ubisoft_header_sniffer() -> HeaderSniffer:
    return HeaderSniffer(url_substr=UBISOFT_API_HOST, fields={'authorization': 'UBISOFT_AUTH', 'ubi-sessionid': 'UBISOFT_SESSION_ID', 'ubi-appid': 'UBISOFT_APP_ID'})
...
def ubisoft_connect_hint(*, urls: list[str], seen_success: bool, captured: bool) -> str:
    joined = ' '.join(urls)
