...
def providers_in_auth_cooldown(history: list[dict[str, Any]], now: float, *, cooldown_sec: float=AUTH_COOLDOWN_SEC) -> set[str]:
    out: set[str] = set()
...
def run_connection_probe(profile_id: str, *, now: float | None=None, history: list[dict[str, Any]] | None=None) -> dict[str, str]:
    now = time.time() if now is None else now
    if history is None:
        history = _load_history(profile_id)
    cooldown = providers_in_auth_cooldown(history, now)
    state = load_probe_state(profile_id)
    strikes: dict[str, int] = {k: int(v) for k, v in (state.get('strikes') or {}).items() if isinstance(v, (int, float))}
