...
_LEGACY_ENV_ALIASES: dict[str, tuple[str, ...]] = {'battlenet': ('BATTLENET_COOKIE',), 'nintendo': ('NINTENDO_COOKIE',)}
...
def _with_profile_secrets(profile_id: str):
    ...
    def _cm():
        with _secrets._lock:
            saved = (_secrets.AUTH_DIR, _secrets.SECRETS_FILE, _secrets.MASTER_KEY_FILE, _secrets._cache, _secrets.PROFILE_ID_OVERRIDE)
            ...
            try:
                ...
                _secrets.AUTH_DIR, _secrets.SECRETS_FILE, _secrets.MASTER_KEY_FILE, _secrets._cache, _secrets.PROFILE_ID_OVERRIDE = saved
    return _cm()
...
def subprocess_env_for_profile(profile_id: str) -> dict[str, str]:
    env: dict[str, str] = {'PYTHONUNBUFFERED': '1', 'PYTHONIOENCODING': 'utf-8', 'BAKLOG_PROFILE': profile_id}
    for k in ('PATH', 'HOME', 'USERPROFILE', 'SYSTEMROOT', 'TEMP', 'TMP', 'APPDATA', 'LOCALAPPDATA', 'COMSPEC', 'LANG', 'LC_ALL'):
        v = os.environ.get(k)
        if v:
            env[k] = v
...
def get_status() -> list[dict[str, Any]]:
    ...
    for key, spec in PROVIDERS.items():
        ...
        rows.append({'key': key, 'label': spec.label, 'kind': spec.kind, 'description': spec.description, 'tips': list(spec.tips), 'status': state, 'connected_at': blob.get('connected_at'), 'last_verified': blob.get('last_verified'), 'last_error': blob.get('last_error'), 'expiry_days': spec.expiry_days, 'platforms': list(spec.platforms), 'available': platform_supported(spec.platforms), 'form_fields': [{'key': f.key, 'label': f.label, 'secret': f.secret, 'placeholder': f.placeholder} for f in spec.form_fields], 'fetcher_keys': list(spec.fetcher_keys)})
    return rows
...
def has_active_sessions() -> bool:
    with _sessions_lock:
        active = [{'session_id': sid, 'provider': s.provider, 'finished': s._finished.is_set()} for sid, s in _active_sessions.items() if not s._finished.is_set()]
        result = bool(active)
    return result
...
def set_form_credentials(provider: str, fields: dict[str, str]) -> dict[str, Any]:
    ...
    if provider == 'epic':
        ...
        if len(code) < 16 or not re.fullmatch('[A-Za-z0-9_\\-]+', code):
            raise ValueError("That doesn't look like an Epic authorizationCode. Copy just the value between the quotes (no quotes, no commas, no spaces).")
        try:
            client = EpicClient(auth_code=code, cache_dir=default_epic_cache_dir())
            client.login()
        except EpicCorrectiveActionError as e:
            raise ValueError('Epic needs you to accept its privacy policy. In the Epic sign-in window, accept the privacy policy / complete the prompt, then refresh the page and paste a fresh authorizationCode here.') from e
        except EpicAuthError as e:
            msg = str(e)
            if 'OAuth 400' in msg or 'invalid_grant' in msg or 'expired' in msg.lower():
                raise ValueError('That authorizationCode is invalid or already used. Open in browser, refresh the page so a new code appears, then paste it here.') from e
...
def start_browser_auth(provider: str, *, fresh: bool=False) -> str:
    ...
    if existing is not None:
        raise ValueError(f'A sign-in window for {spec.label} is already open. Finish or close it before starting again.')
