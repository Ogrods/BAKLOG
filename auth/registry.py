...
class ProviderSpec:
    ...
    form_fields: "Any" = ()
    login_url: "Any" = ""
    success_url_pattern: "Any" = ""
    expiry_days: "Any" = None
    fetcher_keys: "Any" = ()
    tips: "Any" = ()
    platforms: "Any" = ()
    connect_strategy: "Any" = ""
    preserve_profile_on_reconnect: "Any" = False
    post_connect_probe: "Any" = "required"
    login_success_url: "Any" = ""
...
PROVIDERS = {
    "steam": ProviderSpec(
        ...
        success_url_pattern="steamcommunity\\.com/dev/apikey",
    ...
    "gog": ProviderSpec(
        ...
        success_url_pattern="gog\\.com/(account|library|en)",
        expiry_days=14,
        ...
        tips=(
            ...
            "On Windows/macOS with GOG Galaxy installed, the local Galaxy source below is preferred — no browser sign-in needed.",
        ),
    ...
    "psn": ProviderSpec(
        ...
        success_url_pattern="store\\.playstation\\.com",
        expiry_days=30,
        fetcher_keys=("psn", "wishlistPsn"),
        tips=(
            "Use the Sign In button at the top-right of the store page, then keep the window open until it closes on its own.",
            "Easiest sign-in: if you have a passkey, choose “Sign in with a passkey” and approve it on your phone — no password or 2FA codes to type.",
    ...
    "epic": ProviderSpec(
        ...
        login_url="https://www.epicgames.com/id/login?lang=en&redirectUrl=https%3A%2F%2Fwww.epicgames.com%2Fid%2Fapi%2Fredirect%3FclientId%3D34a02cf8f4414e29b15921876da36f9a%26responseType%3Dcode",
        success_url_pattern="id/api/redirect",
        expiry_days=30,
        fetcher_keys=("epic",),
        tips=(
    ...
    "epic_wishlist": ProviderSpec(
        ...
        success_url_pattern="store\\.epicgames\\.com",
        expiry_days=7,
    ...
    "amazon": ProviderSpec(
        ...
        tips=(
            ...
            "Nothing leaves your machine for this source; it's a local file read only.",
            "Windows only — the launcher database uses Windows DPAPI. On macOS/Linux use “Amazon (Prime Gaming, web)” below.",
        ),
    ),
    "amazon_web": ProviderSpec(
        ...
        kind="browser",
        description="Prime Gaming claims delivered to Amazon Games (no external redemption keys). Works on any OS; complements the Windows launcher source above.",
        env_keys=("AMAZON_WEB_PROFILE",),
        login_url=amazon_signin_url(),
        success_url_pattern="luna\\.amazon\\.com|gaming\\.amazon\\.com|amazon\\.com",
        expiry_days=30,
        fetcher_keys=("amazon",),
        tips=(
    ...
    "xbox": ProviderSpec(
        ...
        label="Xbox",
        kind="browser",
        description="Your Xbox play history (every title you've launched on Xbox network).",
        env_keys=("XBL_API_KEY",),
        form_fields=(FormField("XBL_API_KEY", "Or paste OpenXBL API key (from xbl.io dashboard)", secret=True),),
        login_url="https://xbl.io/login",
        success_url_pattern="xbl\\.io/(dashboard|app)",
        fetcher_keys=("xbox",),
        tips=(
            "Sign in at xbl.io with your Microsoft account — we save the OpenXBL API key for you.",
            "Stuck on the Microsoft login? Click “Sign in another way” and have it email you a one-time code — no password or authenticator needed.",
    ...
    "xbox_wishlist": ProviderSpec(
        ...
        success_url_pattern="xbox\\.com/(en-us/)?wishlist",
        expiry_days=30,
        ...
        tips=(
            ...
            "Easiest login: click “Sign in another way” and have Microsoft email you a one-time code — no password or authenticator needed.",
        ),
    ...
    "battlenet": ProviderSpec(
        ...
        success_url_pattern="account\\.battle\\.net/(games|overview|details)",
        expiry_days=7,
    ...
    "nintendo": ProviderSpec(
        ...
        label="Nintendo",
        kind="browser",
        description="Your Nintendo digital library from Virtual Game Cards plus eShop purchase dates (~2 years of receipts). BAKLOG keeps older purchases across syncs.",
        env_keys=("NINTENDO_COOKIE",),
        login_url="https://ec.nintendo.com/my/transactions/",
        success_url_pattern="ec\\.nintendo\\.com",
        expiry_days=14,
    ...
    "nintendo_wishlist": ProviderSpec(
        ...
        success_url_pattern="nintendo\\.com/(us/)?wish-list",
        expiry_days=30,
    ...
    "humble": ProviderSpec(
        ...
        success_url_pattern="humblebundle\\.com/(home|store)",
        expiry_days=30,
    ...
    "ubisoft": ProviderSpec(
        ...
        success_url_pattern="(ubisoft\\.com|ubi\\.com)",
        expiry_days=7,
        ...
        tips=(
            ...
            "If 2FA/verify leaves a blank tab, close it and click Connect again — the sign-in still completes in the main window.",
