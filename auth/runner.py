...
from auth.cdp_browser import STEALTH_INIT_SCRIPT, ConnectBrowserClosed, abort_if_browser_closed, auth_banner_init_script, is_blank_browser_url, launch_persistent_profile
...
def _extract_battlenet_inline(page, context, session: AuthSession | None=None) -> dict[str, str]:
    ...
    return run_connect_poll(context=context, session=session, deadline=time.time() + SUCCESS_WAIT_SEC, poll_sec=POLL_SEC, check=lambda: extract_battlenet_session(context), hint=lambda: battlenet_connect_hint(page), on_signed_in=_on_signed_in, timeout_message='Could not verify a Battle.net library session — sign in at account.battle.net/games, wait until your Games list loads, then click Connect again.')
...
def _extract_nintendo_inline(page, context, session: AuthSession | None=None) -> dict[str, str]:
    ...
    if creds:
        return creds
    raise RuntimeError('No Nintendo session captured — sign in, then make sure the eShop transactions page at ec.nintendo.com finished loading. Close any blank tab and click Connect again if needed.')
...
def _extract_epic_inline(page, context, session: AuthSession | None=None) -> dict[str, str]:
    ...
    while time.time() < deadline:
        ...
        if redirect_page is not None:
            ...
            if code:
                ...
                try:
                    ...
                    raise RuntimeError(f'Epic rejected the captured code ({exc}). Refresh the Epic page so a new code appears, or paste the authorizationCode into the fallback field below.') from exc
        ...
        page.wait_for_timeout(int(POLL_SEC * 1000))
    raise RuntimeError('Could not capture your Epic authorization code in time. If the Epic page shows an authorizationCode, paste it into the fallback field below and click Save key.')
...
def _extract_gog_inline(page, context, session: AuthSession | None=None) -> dict[str, str]:
    ...
    return run_connect_poll(context=context, session=session, deadline=time.time() + SUCCESS_WAIT_SEC, poll_sec=POLL_SEC, check=lambda: extract_gog_session(context), hint=lambda: gog_connect_hint(page), on_signed_in=_on_signed_in, timeout_message='Could not capture a GOG session in time — sign in at gog.com in the browser window, then click Connect again.')
    ...
    while time.time() < deadline:
        ...
        if _psn_on_blocked_account_page(url, body):
            if session and now - last_msg > 6:
                last_msg = now
                session.emit('waiting_for_user', {'message': 'Use Sign In on the PlayStation Store (top-right) to continue'})
            try:
        ...
        if session and now - last_msg > 6:
            last_msg = now
            session.emit('waiting_for_user', {'message': 'Signed in to PlayStation? Click anything on the page so Sony issues a fresh session cookie.' if 'store.playstation.com' in url.lower() else 'Sign in on the PlayStation Store (Sign In, top-right).'})
        page.wait_for_timeout(int(POLL_SEC * 1000))
    raise RuntimeError('Could not capture a PSN session — sign in on the PlayStation Store (Sign In, top-right) and keep this window open until it closes.')
...
def _extract_epic_wishlist_inline(page, context, session) -> dict[str, str]:
    ...
    def _write_debug() -> None:
        ...
        try:
            ...
            _debug_path.write_text(json.dumps(sniffer.debug_log[:50], indent=2, default=str, ensure_ascii=False), encoding='utf-8')
        except Exception:
            pass
    sniffer.attach(page)
    try:
    ...
    while time.time() < deadline:
        ...
        if session and now - last_msg > 8:
            ...
            session.emit('waiting_for_user', {'message': msg})
        page.wait_for_timeout(int(POLL_SEC * 1000))
    raise RuntimeError('Could not detect Epic wishlist sign-in — sign in at store.epicgames.com/wishlist, clear any Cloudflare challenge, and let your wishlist finish loading.')
...
def _xbox_url_is_login(url: str) -> bool:
    ...
    if _xbox_url_on_wishlist(url):
        return False
    return 'login.live.com' in u or 'login.microsoftonline' in u or 'account.microsoft' in u or ('signin' in u and 'xbox.com' not in u)
...
def _extract_xbox_wishlist_inline(page, context, session) -> dict[str, str]:
    ...
    while time.time() < deadline:
        ...
        mid_exchange = not handoff_done and ('/auth/msa' in url_low and 'action=login' in url_low or '#code=' in url_low or ('oauth20' in url_low and _xbox_url_is_login(url)))
        ...
        if handoff_done and (not on_wishlist):
            ...
            now = time.time()
            need_refresh = not on_wishlist or (not token_ready and (not dom_signed_in) and (now - last_ssr_refresh >= 8.0))
            if need_refresh:
                last_ssr_refresh = now
                try:
                    active.goto(XBOX_WISHLIST_URL, wait_until='commit', timeout=15000)
    ...
    raise RuntimeError('Could not detect an Xbox sign-in — sign in to xbox.com/wishlist and keep the window open until it closes.')
...
def _extract_nintendo_wishlist_inline(page, context, session) -> dict[str, str]:
    ...
    raise RuntimeError('Could not detect a Nintendo wish-list session — sign in at nintendo.com/us/wish-list/ and keep the window open until it closes.')
...
def _extract_humble_inline(page, context, session) -> dict[str, str]:
    ...
    try:
        ...
        pass
    return run_connect_poll(context=context, session=session, deadline=time.time() + SUCCESS_WAIT_SEC, poll_sec=HUMBLE_POLL_SEC, check=lambda: extract_humble_session(context, page), hint=lambda: humble_connect_hint(page), timeout_message='Could not detect a Humble sign-in - sign in at humblebundle.com/home/library and keep the window open until it closes.')
...
def _extract_ubisoft(page, context, session: AuthSession | None=None) -> dict[str, str]:
    from auth.connect_extractors import build_ubisoft_header_sniffer, extract_ubisoft_session, ubisoft_connect_hint, ubisoft_session_captured
    ...
    while time.time() < deadline:
        ...
        if session and now - last_hint > 10:
            ...
            session.emit('waiting_for_user', {'message': msg})
        page.wait_for_timeout(int(POLL_SEC * 1000))
    raise RuntimeError('Ubisoft API headers not captured — sign in at ubisoft.com, complete 2FA, and open your games library. If a verify step left a blank tab, close it and click Connect again.')

def _extract_ea(page, context, session: AuthSession | None=None) -> dict[str, str]:
    from clients.ea_session import EA_COOKIE_SESSION, EA_DEALS_URL, EA_GRAPHQL_HOST, EA_LOGIN_URL, _ensure_ea_deals_ready, _merge_owned_items, drain_ea_graphql_hook, ensure_ea_graphql_hook, fetch_owned_games_inpage, fetch_owned_games_playwright_request, install_ea_graphql_hook, is_ea_session_expired_page, normalize_bearer, probe_ea_token, write_ea_connect_snapshot
    ...
    while time.time() < deadline:
        ...
        sustained_cookie = auth_ok and _login_witnessed() and (consecutive_auth >= 2 or bool(owned_accum) or deals_burst_ok)
        if sustained_cookie:
        ...
        if session and now - last_hint > 10:
            ...
            session.emit('waiting_for_user', {'message': msg})
        page.wait_for_timeout(int(POLL_SEC * 1000))
    raise RuntimeError('EA session not confirmed ΓÇö sign in at ea.com and wait for the deals page to finish loading before the window closes.')

def _extract_amazon_web(page, context, session: AuthSession | None=None) -> dict[str, str]:
    from clients.amazon_web_client import COLLECTION_URLS, _capture_claims_from_response, _poll_prime_collection, filter_codeless_claims, raw_dump_path, scrub_claim_codes
    raw_claims: list[dict[str, Any]] = []
    captured: dict[str, bool] = {'done': False, 'claims_captured': False, 'session_only_captured': False}
    candidates: list[Any] = []

    def on_response(resp: Any) -> None:
        _capture_claims_from_response(resp, candidates, raw_claims, captured)
    page.on('response', on_response)
    _poll_prime_collection(page, context, deadline=time.time() + SUCCESS_WAIT_SEC, candidates=candidates, raw_claims=raw_claims, captured=captured, allow_session_only=True, start_at_signin=True, session=session, poll_interval_ms=int(POLL_SEC * 1000), session_only_grace_s=2.0)
    if captured['done']:
        if captured.get('claims_captured'):
            path = raw_dump_path()
            path.parent.mkdir(parents=True, exist_ok=True)
            safe_claims = scrub_claim_codes(raw_claims)
            path.write_text(json.dumps({'urls': list(COLLECTION_URLS), 'raw_claim_count': len(safe_claims), 'raw_claims': safe_claims, 'codeless_count': len(filter_codeless_claims(safe_claims)), 'capture_reason': 'headed_connect'}, indent=2, ensure_ascii=False), encoding='utf-8')
        return {'AMAZON_WEB_PROFILE': 'ready'}
    raise RuntimeError('Prime Gaming session not confirmed — sign in on the Amazon page, wait for My Collection (or gaming.amazon.com) to load, then try Connect again.')
INLINE_PROVIDERS = {'psn', 'steam', 'itch', 'itad', 'xbox', 'xbox_wishlist', 'ubisoft', 'ea', 'epic_wishlist', 'nintendo', 'nintendo_wishlist', 'epic', 'humble', 'amazon_web', 'battlenet', 'gog'}
...
def run_browser_auth(provider: str, session: AuthSession) -> dict[str, str] | None:
    ...
    _CONNECT_HINTS = {'psn': 'Sign in to PlayStation Store (Sign In, top-right). Keep this window open.', 'steam': "Sign in to Steam. We'll save your API key automatically.", 'itch': "Sign in to itch.io. We'll save your API key automatically.", 'itad': "Sign in to IsThereAnyDeal. We'll register an app and save your API key.", 'xbox': "Click “Sign in with Xbox Live” on the xbl.io page, then sign in with your Microsoft account. We'll save your API key automatically once you have it.", 'xbox_wishlist': "Sign in to xbox.com with your Microsoft account. We'll detect your wishlist session automatically — no need to refresh the page.", 'epic_wishlist': "Sign in to Epic in this window — you'll be returned to your wishlist automatically. Clear any Cloudflare check if shown, and keep this window open until your wishlist finishes loading.", 'ubisoft': "Sign in to Ubisoft and complete 2FA in the browser (use the login popup if it stays open). Close DevTools if you see a yellow 'Debugger paused' banner.", 'nintendo': "Sign in to your Nintendo Account. We'll automatically open your eShop transactions page to capture the session — no need to navigate yourself.", 'epic': "Sign in to your Epic account in the browser window. We'll capture and exchange your authorization code automatically — no copy/paste needed.", 'nintendo_wishlist': "Sign in to nintendo.com with your Nintendo Account. We'll detect your wish list session automatically — stay on the wish list page until it loads.", 'humble': "Sign in to Humble Bundle (humblebundle.com). We'll open your library page to capture the session — complete any CAPTCHA in the browser window.", 'ea': "Sign in to your EA account. We'll open the EA deals page to capture your library token automatically.", 'amazon_web': "Sign in on the Amazon page in the browser window. After login we'll open My Collection and save your session automatically.", 'battlenet': "Sign in at account.battle.net and open your Games list. We'll verify the library API before saving your session.", 'gog': "Sign in to your GOG account in the browser window. We'll save your session automatically once you're logged in."}
    connect_hint = _CONNECT_HINTS.get(provider, f'Sign in to {spec.label} in the browser window, then keep it open until it closes automatically.')
    ...
    with launch_persistent_profile(user_data, headless=False, initial_url=spec.login_url if provider == 'ea' else None) as context:
        try:
            ...
            if provider in INLINE_PROVIDERS:
                if provider == 'psn':
                    ...
                    try:
                        ...
                        pass
                    api_extractors = {'steam': extract_steam, 'itch': extract_itch, 'itad': extract_itad, 'xbox': extract_xbox}
