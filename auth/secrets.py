...
def _warn_plaintext_master_key(*, action: str) -> None:
    ...
    print(f'[auth] WARNING: OS keyring unavailable — {action} encryption key in plaintext at {path}. Prefer system keyring or set a master password.', file=sys.stderr)
...
def _derive_key_from_password(password: str, salt: bytes, *, n_log: int=MPW_SCRYPT_N_LOG, r: int=MPW_SCRYPT_R, p: int=MPW_SCRYPT_P) -> bytes:
...
def _restrict_file_permissions(path: Path) -> None:
    if os.name == 'nt':
        try:
            ...
            if not user:
                return
            subprocess.run(['icacls', str(path), '/inheritance:r', '/grant:r', f'{user}:F'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False)
        except Exception:
            pass
    else:
        try:
...
def _profile_key_from_master(master: bytes, profile_id: str | None=None) -> bytes:
    ...
    return HKDF(algorithm=hashes.SHA256(), length=32, salt=None, info=b'baklog-profile:' + pid.encode('utf-8')).derive(master)
