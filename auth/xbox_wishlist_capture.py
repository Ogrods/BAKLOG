...
_API_HINTS = ('emerald.xboxservices.com', 'xboxservices.com', 'collections.mp.microsoft.com', 'displaycatalog.mp.microsoft.com', 'cllpwa.microsoft.com')
...
class WishlistApiSniffer:
    ...
    def on_response(self, resp: Any) -> None:
        try:
            ...
            entry: dict[str, Any] = {'url': url, 'method': 'POST' if post else 'GET', 'status': getattr(resp, 'status', 0), 'authorization': _redact_auth(auth), 'ms_cv_present': 'ms-cv' in headers, 'signature_present': 'signature' in headers, 'post_data': post[:_MAX_BODY], 'is_wishlist_candidate': is_wishlisty}
            if is_wishlisty:
                try:
    ...
    def dump(self, *, profile_id: str | None=None) -> Path | None:
        try:
            ...
            payload = {'captured_at': datetime.now(UTC).isoformat(), 'token_captured': bool(self.token), 'wishlist_url': self.wishlist_url, 'entry_count': len(self.entries), 'entries': self.entries}
