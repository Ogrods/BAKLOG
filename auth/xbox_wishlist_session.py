...
def _capture_once(*, headless, timeout_s):
    profile = profile_dir("xbox_wishlist")
    if not profile.exists():
        raise RuntimeError(
            "No saved Xbox wishlist profile at cache/auth/profiles/xbox_wishlist. Open the Connections page and connect 'Xbox Store wishlist' first."
        )
...
def capture_xbox_wishlist_preloaded_state(*, headless="legacy", timeout_s=30):
    return _capture_once(headless=headless, timeout_s=timeout_s)


def validate_xbox_wishlist_state(state, *, headless=True):
    user = state.get("user") or {}
    if not user.get("isSignedIn"):
