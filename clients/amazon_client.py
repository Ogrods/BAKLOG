...
def _is_external_prime_claim(ent):
...
class AmazonGamesClient:
    ...
    def _connect_ro(self, filename):
        path = self.sql_dir / filename
        if not path.is_file():
            raise FileNotFoundError(path)
        return sqlite3.connect(f"file:{path}?mode=ro", uri=True)

    def _load_product_details_by_title(self):
    ...
    def _load_last_played_by_adg_id(self):
    ...
    def get_library_records(self):
        ...
        for ent in self.get_entitlements():
            ...
            library_img = _pick_image_url(
                cached.get("Background"), cached.get("Background2"), cached.get("ProductIconUrl"), icon
            )
