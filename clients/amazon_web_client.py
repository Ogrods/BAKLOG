...
_AMAZON_SESSION_COOKIE_NAMES = frozenset({'session-id', 'session-id-time', 'at-main', 'sess-at-main', 'ubid-main', 'session-token', 'x-main'})
_AMAZON_HOST_MARKERS = ('amazon.com', 'luna.amazon.com', 'gaming.amazon.com')
_EXTERNAL_DESTINATION_TYPES = frozenset({'EPIC', 'STEAM', 'GOG', 'UBISOFT', 'BATTLENET', 'BLIZZARD', 'ORIGIN', 'EA', 'XBOX', 'MICROSOFT', 'NINTENDO', 'RIOT', 'ROCKSTAR', 'BETHESDA', 'HUMBLE', 'ITCH'})
...
def amazon_signin_url(return_to: str=LUNA_CLAIMS_URL) -> str:
    params = {'openid.pape.max_auth_age': '3600', 'openid.return_to': return_to, 'openid.identity': _OPENID_IDENTIFIER_SELECT, 'openid.assoc_handle': 'tempo_us', 'openid.mode': 'checkid_setup', 'language': 'en_US', 'openid.claimed_id': _OPENID_IDENTIFIER_SELECT, 'pageId': 'tempo_us', 'openid.ns': _OPENID_NS}
...
def is_luna_error_page(html: str) -> bool:
    lower = (html or '').lower()
    return 'technical difficulties' in lower or 'having issues with our service' in lower or 'please come back later' in lower
...
def claim_to_record(claim: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {'amazon_product_id': product_id, 'amazon_entitlement_id': claim.get('orderId'), 'amazon_adg_id': None, 'name': title, 'asin': claim.get('productAsin') or claim.get('asin'), 'product_sku': None, 'product_line': 'prime_claim', 'header_image': icon, 'library_image': icon, 'genres': [], 'release_date': release, 'last_played': None, 'store_url': store_url, 'publisher': None, 'prime_claim_order_state': claim.get('orderState')}
...
def _capture_claims_from_response(resp: Any, candidates: list[Any], raw_claims: list[dict[str, Any]], captured: dict[str, bool]) -> None:
    if captured['done']:
        return
    try:
...
def _drain_claim_candidates(candidates: list[Any], raw_claims: list[dict[str, Any]], captured: dict[str, bool], *, max_per_tick: int=6) -> bool:
...
def _try_capture_from_page_html(html: str, url: str, context: Any, raw_claims: list[dict[str, Any]], captured: dict[str, bool], *, allow_session_only: bool, session_only_grace_ok: bool) -> bool:
...
def _poll_prime_collection(page: Any, context: Any, *, deadline: float, candidates: list[Any], raw_claims: list[dict[str, Any]], captured: dict[str, bool], allow_session_only: bool=False, session_only_grace_s: float=8.0, start_at_signin: bool=False, session: Any=None, poll_interval_ms: int=500, log_progress: bool=False, log_prefix: str='Amazon web') -> None:
    ...
    while _now_s() < deadline and (not captured['done']):
        ...
        if _try_capture_from_page_html(html, url, context, raw_claims, captured, allow_session_only=allow_session_only, session_only_grace_ok=session_only_grace_ok):
            break
...
def sniff_claims(*, timeout_s: int=60, dump_path: Path | str | None=None) -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    from auth.cdp_browser import launch_persistent_profile
    from auth.secrets import profile_dir
    profile = profile_dir(PROFILE_KEY)
    if not profile.is_dir():
        raise AmazonWebAuthError('No saved Prime Gaming web profile. Open Connections and connect “Amazon (Prime Gaming, web)” first.')
    raw_claims: list[dict[str, Any]] = []
    candidates: list[Any] = []

    def _outcome_template() -> dict[str, Any]:
        return {'capture_ok': False, 'claims_captured': False, 'session_only_captured': False, 'signed_in': False, 'final_url': None, 'headless': True, 'reason': None}
    ...
    def _dump_raw(reason: str, *, forced_path: Path | None=None) -> None:
        ...
        path.write_text(json.dumps({'urls': list(COLLECTION_URLS), 'raw_claim_count': len(raw_claims), 'raw_claims': raw_claims, 'codeless_count': len(filter_codeless_claims(raw_claims)), 'capture_reason': reason}, indent=2, ensure_ascii=False), encoding='utf-8')
    ...
    def _run_once(*, headless: bool, run_timeout_s: int) -> dict[str, Any]:
        ...
        with launch_persistent_profile(str(profile), headless=headless) as ctx:
            ...
            _poll_prime_collection(page, ctx, deadline=deadline, candidates=candidates, raw_claims=raw_claims, captured=captured, allow_session_only=False, start_at_signin=False, log_progress=True, log_prefix='Amazon web')
            ...
            outcome['reason'] = 'claims_captured' if outcome['capture_ok'] else 'session_only' if captured.get('session_only_captured') else 'no_claims'
            print(f"Amazon web: final headless={headless} capture_ok={outcome['capture_ok']} claims_captured={outcome['claims_captured']} signed_in={outcome['signed_in']} reason={outcome['reason']} url={final_url}", flush=True)
        return outcome
    ...
    if not headless_outcome['signed_in']:
        raise AmazonWebAuthError('Could not capture Prime Gaming claims — session may have expired. Reconnect “Amazon (Prime Gaming, web)” on Connections.')
