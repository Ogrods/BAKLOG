...
_BROWSER_LOADERS: dict[str, object] = {'edge': bc3.edge, 'chrome': bc3.chrome, 'brave': bc3.brave, 'firefox': bc3.firefox}
...
class BattleNetClient:
    ...
    def __init__(self, cookie_header: str, user_agent: str | None=None):
        ...
        if not cookie:
            raise BattleNetAuthError('No Battle.net session cookie available. Sign in at https://account.battle.net/ in Edge (default), or set BATTLENET_COOKIE in .env as a fallback.')
        self.session = requests.Session()
        headers = {'Cookie': cookie, 'Accept': 'application/json, text/plain, */*', 'Accept-Language': 'en-US,en;q=0.9', 'Referer': 'https://account.battle.net/games', 'User-Agent': user_agent or 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36'}
    ...
    def from_browser(cls, browser: str='edge', **kw) -> BattleNetClient:
        ...
        try:
            ...
            hint = "Modern Edge/Chrome (v127+) use app-bound cookie encryption that browser-cookie3 can't decrypt on Windows, even from an elevated shell. Recommended: paste the Cookie header into BATTLENET_COOKIE in .env and run with --browser env." if name in ('edge', 'chrome', 'brave') else f'Ensure {name} is installed and signed in at account.battle.net.'
        ...
        if not cookies:
            raise BattleNetAuthError(f'No battle.net cookies found in {name}. Sign in at https://account.battle.net/ in that browser, then retry.')
    ...
    def get_raw_account(self) -> dict:
        ...
        if resp.status_code == 500:
            raise BattleNetAuthError('Battle.net returned 500. This endpoint is unofficial and Blizzard intermittently breaks it; retry later.')
        if resp.status_code >= 400:
            raise BattleNetAuthError(f'Battle.net {resp.status_code}: {resp.text[:200]}')
        try:
            return resp.json()
        except ValueError as e:
            raise BattleNetAuthError(f"Battle.net returned non-JSON ({resp.headers.get('content-type')}). Session likely expired — sign in at account.battle.net in Edge.") from e