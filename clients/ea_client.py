...
def _persisted_url(operation, variables, sha256_hash):
...
def owned_games_full_document_body(*, limit, offset="0", preload=True):
    _ = preload
    return {
        "operationName": "getPreloadedOwnedGames",
        "query": OWNED_GAMES_QUERY,
        "variables": {"limit": limit, "next": offset},
    }


class EaClient:
    def __init__(self, access_token="", cookies=None):
    ...
    def _raise_graphql_errors(self, data, resp):
    ...
    def _graphql_get(self, operation, variables, sha256_hash):
        self._throttle()
    ...
    def _graphql_post(self, operation, query, variables):
        self._throttle()
        resp = self.session.post(
            GRAPHQL_URL, json={"operationName": operation, "query": query, "variables": variables}, timeout=60
        )
        data = resp.json()
        self._raise_graphql_errors(data, resp)
        return data

    def _graphql_apq_post(self, operation, variables, sha256_hash):
        self._throttle()
        resp = self.session.post(
            GRAPHQL_URL,
            json={
    ...
    def probe_owned_games(self):
        ...
        self._graphql_get(
            ...
            {**_OWNED_VARIABLES_BASE, "addFieldsToPreloadGames": False, "limit": 1, "next": "0"},
            OWNED_GAMES_HASH,
        )

    def _owned_page_variables(self, *, limit, offset, preload):
        return {**_OWNED_VARIABLES_BASE, "addFieldsToPreloadGames": preload, "limit": limit, "next": offset}

    def _owned_items_from_root(self, root):
    ...
    def _owned_full_page_variables(self, *, limit, offset, preload):
        _ = preload
        return {"limit": limit, "next": offset}

    def _get_owned_games_full(self):
        out = []
        offset = "0"
        while True:
    ...
    @staticmethod
    def _should_fallback_owned_document(exc):
        msg = str(exc)
