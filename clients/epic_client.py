...
PLAYTIME_ALL_URL = 'https://library-service.live.use1a.on.epicgames.com/library/api/public/playtime/account/{account_id}/all'
...
def build_epic_oauth_login_url(redirect_uri: str, state: str='') -> str:
    ...
    inner = f"https://www.epicgames.com/id/api/redirect?clientId={CLIENT_ID}&responseType=code&redirectUrl={quote(target, safe='')}"
    return f"https://www.epicgames.com/id/login?lang=en&redirectUrl={quote(inner, safe='')}"
BASIC_AUTH = 'basic ' + base64.b64encode(f'{CLIENT_ID}:{CLIENT_SECRET}'.encode()).decode()
...
class EpicCorrectiveActionError(EpicAuthError):
    ...
    def __init__(self, message: str, *, corrective_action: str | None=None, continuation: str | None=None, error_code: str | None=None) -> None:
...
def is_corrective_action(fields: dict[str, str]) -> bool:
    return 'corrective_action_required' in (fields.get('errorCode') or '') or bool(fields.get('correctiveAction'))
...
class EpicClient:
    ...
    def _load_session(self) -> dict | None:
        ...
        if data and data.get('refresh_token'):
            try:
                from auth import secrets as _secrets
                _secrets.set_provider_blob(self._SECRETS_PROVIDER, {'refresh_token': data.get('refresh_token'), 'account_id': data.get('account_id')})
                self._purge_plaintext_session()
            except Exception:
                pass
        return data
    ...
    def _request_token(self, params: dict) -> dict:
        self._throttle()
        resp = self.session.post(OAUTH_URL, data=params, headers={'Authorization': BASIC_AUTH, 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'steam-backlog/1.0'}, timeout=30)
        if resp.status_code >= 400:
            ...
            if is_corrective_action(fields):
                ...
                raise EpicCorrectiveActionError(f"Epic requires you to complete '{action}' before continuing. Accept Epic's privacy policy in the sign-in window, then refresh and reconnect.", corrective_action=fields.get('correctiveAction') or None, continuation=fields.get('continuation') or None, error_code=fields.get('errorCode') or None)
    ...
    def login(self) -> None:
        ...
        if cached and cached.get('refresh_token'):
            try:
                data = self._request_token({'grant_type': 'refresh_token', 'refresh_token': cached['refresh_token']})
        ...
        if not self._auth_code:
            raise EpicAuthError('No valid Epic session. Run: python fetch_epic.py --auth-help')
        data = self._request_token({'grant_type': 'authorization_code', 'code': self._auth_code, 'token_type': 'eg1'})
        self._apply_tokens(data)
    ...
    def get_library_records(self) -> list[dict]:
        ...
        while True:
            ...
            resp = self.session.get(LIBRARY_URL, params=params, headers=self._auth_headers(), timeout=30)
    ...
    def get_playtime(self) -> dict[str, int]:
        ...
        self._throttle()
        try:
            resp = self.session.get(PLAYTIME_ALL_URL.format(account_id=self._account_id), headers=self._auth_headers(), timeout=30)
            if resp.status_code == 401:
    ...
    def get_catalog_item(self, namespace: str, catalog_id: str, country: str='US', locale: str='en-US') -> dict | None:
        cache_key = f'{namespace}_{catalog_id}'
        cached = self._read_catalog_cache(cache_key)
        if cached is not None:
            return cached
        self._throttle()
        resp = self.session.get(f'https://{CATALOG_HOST}/catalog/api/shared/namespace/{namespace}/bulk/items', params={'id': catalog_id, 'includeDLCDetails': 'true', 'includeMainGameDetails': 'true', 'country': country, 'locale': locale}, headers=self._auth_headers(), timeout=30)
        if resp.status_code == 401:
    ...
    def get_catalog_bulk(self, namespace: str, catalog_ids: list[str], country: str='US', locale: str='en-US') -> dict:
        out: dict = {}
...
class EpicStoreClient:
    ...
    def __init__(self, cookie: str):
        ...
        if not cookie:
            raise EpicAuthError('EPIC_STORE_COOKIE is empty. See README for instructions.')
        self._cookie = cookie
        self._last_request = 0.0
        self._lock = threading.Lock()
    ...
    def _headers(self) -> dict:
        return {'Cookie': self._cookie, 'Content-Type': 'application/json', 'Accept': '*/*', 'Origin': 'https://store.epicgames.com', 'Referer': 'https://store.epicgames.com/', 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0 Safari/537.36'}

    def graphql(self, query: str, variables: dict, operation_name: str) -> dict:
        self._throttle()
        resp = self.session.post(STORE_GRAPHQL_URL, json={'query': query, 'operationName': operation_name, 'variables': variables}, headers=self._headers(), timeout=30)
        if resp.status_code in (401, 403):
            raise EpicAuthError(f'Storefront GraphQL {resp.status_code}: cookie expired or wrong (grab a fresh EPIC_STORE_COOKIE from DevTools — see README).')
        if resp.status_code >= 400:
            body = resp.text[:1500] if resp.text else '<empty>'
            raise EpicAuthError(f'Storefront GraphQL {resp.status_code}: {body}')
        payload = resp.json()
    ...
    def get_wishlist(self, country: str='US', locale: str='en-US') -> list[dict]:
        all_elements: list[dict] = []
        seen_ids: set[str] = set()
        for start in (0, 200, 400, 600, 800):
            data = self.graphql(_WISHLIST_QUERY, {'country': country, 'locale': locale, 'start': start, 'count': 200}, 'getWishlistQuery')
            elements = ((data.get('Wishlist') or {}).get('wishlistItems') or {}).get('elements', [])
            if not elements:
                break
            added_this_page = 0
            for el in elements:
...
def validate_store_wishlist_cookie(cookie: str) -> bool:
    try:
        ...
        client.graphql(_WISHLIST_QUERY, {'country': 'US', 'locale': 'en-US', 'start': 0, 'count': 1}, 'getWishlistQuery')
        return True
    except EpicAuthError:
        return False
    except Exception:
