...
_PROMO_SUFFIX_RES: tuple[re.Pattern[str], ...] = (re.compile('\\s*-\\s*Amazon Luna\\s*$', re.IGNORECASE), re.compile('\\s*-\\s*Amazon Prime\\s*$', re.IGNORECASE), re.compile('\\s*-\\s*Prime Giveaway\\s*$', re.IGNORECASE))
_YEAR_QUALIFIER_RE = re.compile('\\s*\\(\\d{4}\\)\\s*$')
_EDITION_VARIANT_SUBTITLE_RE = re.compile('\\b(edition|deluxe|upgrade|complete|musical|trilogy|collection|pack|bundle|goty|definitive|remastered|remaster|ultimate)\\b', re.IGNORECASE)
_PACK_REGISTRY: tuple[tuple[re.Pattern[str], tuple[str, ...]], ...] = ((re.compile('^Silver Box Classics$', re.IGNORECASE), ('Heroes of the Lance', 'Dragons of Flame', 'War of the Lance', 'Shadow Sorcerer')), (re.compile('^Forgotten Realms: The Archives - Collection One$', re.IGNORECASE), ('Eye of the Beholder', 'Eye of the Beholder II: The Legend of Darkmoon', 'Eye of the Beholder III: Assault on Myth Drannor')), (re.compile('^Forgotten Realms: The Archives - Collection Two$', re.IGNORECASE), ('Pool of Radiance', 'Curse of the Azure Bonds', 'Hillsfar', 'Secret of the Silver Blades', 'Pools of Darkness', 'Gateway to the Savage Frontier', 'Treasures of the Savage Frontier')), (re.compile('^Forgotten Realms: The Archives - Collection Three$', re.IGNORECASE), ('Dungeon Hack', 'Menzoberranzan')), (re.compile('^Alone in the Dark: The Trilogy 1\\+2\\+3$', re.IGNORECASE), ('Alone in the Dark 1', 'Alone in the Dark 2', 'Alone in the Dark 3')))
...
def collapse_pack_dupes(rows: list[dict[str, Any]], *, pack_component_keys: dict[str, set[str]] | None=None, owned_release_keys: set[str] | None=None) -> list[dict[str, Any]]:
