...
class GogGalaxyClient:
    ...
    def _owned_release_keys(self, conn: sqlite3.Connection) -> list[str]:
        if self._table_exists(conn, 'ProductPurchaseDates'):
            rows = conn.execute("SELECT DISTINCT gameReleaseKey FROM ProductPurchaseDates WHERE gameReleaseKey LIKE 'gog\\_%' ESCAPE '\\'").fetchall()
            keys = [str(r[0]) for r in rows if r and r[0]]
            if keys:
                return keys
        if self._table_exists(conn, 'LibraryReleases'):
            rows = conn.execute("SELECT DISTINCT releaseKey FROM LibraryReleases WHERE releaseKey LIKE 'gog\\_%' ESCAPE '\\'").fetchall()
            return [str(r[0]) for r in rows if r and r[0]]
        raise GogGalaxyError('GOG Galaxy database has no ProductPurchaseDates or LibraryReleases table.')

    def _load_pieces_for_keys(self, conn: sqlite3.Connection, release_keys: list[str], type_ids: dict[str, int]) -> dict[str, dict[str, object]]:
    ...
    def _load_last_played(self, conn: sqlite3.Connection, release_keys: list[str]) -> dict[str, str]:
    ...
    def _load_parent_keys(self, conn: sqlite3.Connection, release_keys: list[str], type_ids: dict[str, int]) -> set[str]:
        if not release_keys or not self._table_exists(conn, 'GamePieces'):
            return set()
    ...
    def _load_dlc_keys(self, conn: sqlite3.Connection, type_ids: dict[str, int]) -> set[str]:
    ...
    def _load_product_link_components(self, conn: sqlite3.Connection, release_keys: list[str], type_ids: dict[str, int]) -> dict[str, set[str]]:
    ...
    def get_library_records(self) -> list[dict]:
        ...
        for rk in release_keys:
            ...
            records.append({'gog_id': gog_id, 'release_key': rk, 'name': title, 'raw_image': cover, 'header_image': cover, 'library_image': cover, 'release_date': _release_date_from_meta(meta), 'last_played': last_played.get(rk), 'genres': _genres_from_meta(meta), 'store_url': _meta_store_url(meta, gog_id)})
