    ...
    def _get(self, path: str, params: dict | None=None) -> dict:
        ...
        for attempt in range(len(_RETRY_BACKOFF) + 1):
            ...
            try:
                ...
                last_exc = exc
                if exc.response is not None and exc.response.status_code in _RETRYABLE_HTTP and (attempt < len(_RETRY_BACKOFF)):
                    time.sleep(_RETRY_BACKOFF[attempt])
                    continue
                raise
    ...
    def prices_for_ids(self, game_ids: list[str]) -> dict[str, dict]:
        ...
        for chunk_idx, i in enumerate(range(0, len(game_ids), chunk_size), 1):
            ...
            for entry in entries:
                ...
                is_year = price_amt is not None and year_low is not None and (price_amt <= year_low + 0.01) and (not is_all_time)
                currency = normalize_currency_code(price.get('currency'), country=self.country)
                out[gid] = {'shop': (best.get('shop') or {}).get('name'), 'price': price_amt, 'currency': currency, 'price_str': format_price(price_amt, currency), 'regular': regular.get('amount'), 'cut': best.get('cut', 0), 'url': best.get('url'), 'history_low_all': all_low, 'history_low_year': year_low, 'is_historical_low': is_all_time, 'is_historical_low_year': is_year}
        return out
