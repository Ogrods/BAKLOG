...
class ItchLocalClient:
            ...
            if stats:
                ...
                if prev is None:
                    ...
                    if lp and (not prev.get('last_played') or lp > prev['last_played']):
                        prev['last_played'] = lp
                    prev['playtime_minutes'] = max(prev.get('playtime_minutes') or 0, stats.get('playtime_minutes') or 0)
        return out
    ...
    def get_library_records(self) -> list[dict]:
        ...
        for row in rows:
            ...
            records.append({'itch_id': gid, 'download_key_id': row_map.get('download_key_id'), 'name': title, 'header_image': cover, 'library_image': cover, 'store_url': row_map.get('url'), 'release_date': _iso_date(row_map.get('published_at')), 'classification': row_map.get('classification'), 'short_text': row_map.get('short_text'), 'min_price': row_map.get('min_price'), 'in_press_system': bool(row_map.get('in_press_system')), 'last_played': stats.get('last_played'), 'playtime_minutes': stats.get('playtime_minutes', 0)})
