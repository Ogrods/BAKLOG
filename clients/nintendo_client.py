...
NINTENDO_SESSION_COOKIE_NAMES = frozenset({'MIST', 'JViDD', '_gh_sess', 'NASID', 'ecsid', '__Secure-next-auth.session-token'})
...
def _merge_graphql_payload(payload: dict[str, Any], collected: list[dict[str, Any]], seen_ids: set[str]) -> int:
...
def _drain_graphql_candidates(candidates: list[Any], collected: list[dict[str, Any]], seen_ids: set[str]) -> int:
    added = 0
    while candidates:
...
def probe_session_id_token(http_get, *, timeout: float=REQUESTS_TIMEOUT_SEC) -> dict[str, Any]:
    out: dict[str, Any] = {'ok': False, 'status': None, 'id_token_present': False, 'error': None}
    try:
...
class NintendoClient:
    ...
    def __init__(self, cookie_header: str='', *, profile_path: Path | None=None, user_agent: str | None=None, headless: bool=True, dump_debug_path: Path | None=None) -> None:
    ...
    def fetch_all_transactions(self) -> list[dict]:
        ...
        raise NintendoAuthError('Nintendo is not connected. Open Connections, connect Nintendo, and complete sign-in on ec.nintendo.com (transactions page).')

    def _fetch_via_cookie_requests(self) -> list[dict]:
        session = requests.Session()
        session.headers.update({'Cookie': self._cookie, 'Accept': 'application/json', 'Accept-Language': 'en-US,en;q=0.9', 'Referer': TRANSACTIONS_PAGE, 'Origin': 'https://ec.nintendo.com', 'User-Agent': self._user_agent})
        auth = session.get(SESSION_URL, timeout=REQUESTS_TIMEOUT_SEC)
        if auth.status_code in (401, 403):
            raise NintendoAuthError(f'Nintendo rejected the session ({auth.status_code}). Reconnect Nintendo in Connections.')
        if not auth.ok:
            raise NintendoAuthError(f'Could not load Nintendo session ({auth.status_code}). Reconnect Nintendo in Connections.')
        try:
            body = auth.json()
        ...
        if not body.get('idToken'):
            raise NintendoAuthError('Nintendo cookie is incomplete (no idToken). Reconnect Nintendo in Connections so the full eShop session is saved to the browser profile.')
        raise NintendoAuthError('Nintendo now requires the saved browser profile for purchase history. Reconnect Nintendo in Connections, then run the fetcher again.')
    ...
    def _fetch_via_direct_graphql(self, context, collected: list[dict[str, Any]], seen_ids: set[str]) -> int:
        ...
        if not id_token:
            return 0
        headers = {'Accept': 'application/json', 'Accept-Language': 'en-US,en;q=0.9', 'Referer': TRANSACTIONS_PAGE, 'Origin': 'https://ec.nintendo.com', 'User-Agent': self._user_agent, 'Authorization': f'Bearer {id_token}'}
        ...
        for _ in range(200):
            ...
            query = urlencode({'operationName': 'TransactionsClientRootClient', 'variables': variables, 'extensions': extensions})
    ...
    def _fetch_via_browser_profile(self, profile_path: Path) -> list[dict]:
        ...
        candidates: list[Any] = []
        debug: dict[str, Any] = {'profile': str(profile_path), 'headless': self._headless, 'graphql_candidates_seen': 0, 'graphql_rows_merged': 0, 'session_probe': None, 'final_url': None, 'page_title': None, 'login_page_detected': False, 'direct_fallback_rows': 0}
        ...
        with launch_persistent_profile(profile_path, headless=self._headless) as context:
            ...
            if not collected:
                ...
                raise NintendoCaptureError('Purchase history could not be captured from the transactions page. Try: python fetch_nintendo.py --skip-hltb --headed --dump-debug (see cache/nintendo/fetch_debug.json). Reconnect in Connections if the headed browser shows a sign-in page.')
        self._write_debug(debug)
        return [_map_graphql_item(item) for item in collected]

    def _paginate_transactions_ui(self, page) -> None:
        try:
            labels = page.evaluate("() => [...document.querySelectorAll('button, a')]\n                    .map(el => (el.innerText || '').trim())\n                    .filter(t => /^\\d+$/.test(t))\n                    .map(t => parseInt(t, 10))\n                    .filter(n => n > 1)")
        except Exception:
            return
...
def _map_graphql_item(item: dict[str, Any]) -> dict[str, Any]:
    ...
    return {'title': title, 'date': date, 'transaction_id': str(tid) if tid is not None else '', 'device_type': device_type, 'content_type': item_type, 'transaction_type': tx_type}