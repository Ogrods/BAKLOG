...
def validate_npsso(npsso: str) -> None:
    ...
    try:
        ...
        raise PsnAuthError('NPSSO rejected — sign in at playstation.com, open the ssocookie link, and paste a fresh token.') from exc
    except PSNAWPForbiddenError as exc:
        raise PsnAuthError('PSN profile is private. Set trophies/games visibility to Anyone in PSN privacy settings.') from exc
...
_SUFFIX_NOISE = ('\\bps4\\s*(?:and|&|\\+)?\\s*ps5\\b', '\\bps5\\s*(?:and|&|\\+)?\\s*ps4\\b', '\\bfor\\s+ps5\\b', '\\bfor\\s+ps4\\b', '\\bps5\\b', '\\bps4\\b', '\\bps3\\b', '\\bps\\s*vita\\b', '\\btrophy\\s+set\\b', '\\btrophies\\b')
_SUFFIX_RE = re.compile('|'.join(_SUFFIX_NOISE))
_ROMAN_WORDS = {'i': '1', 'ii': '2', 'iii': '3', 'iv': '4', 'v': '5', 'vi': '6', 'vii': '7', 'viii': '8', 'ix': '9', 'x': '10', 'xi': '11', 'xii': '12', 'xiii': '13', 'xiv': '14', 'xv': '15'}
...
def _display_name(name: str) -> str:
    ...
    n = re.sub('\\bT elltale\\b', 'Telltale', n, flags=re.I)
    return n
...
def _trophy_counts_from_title(trophy) -> tuple[int | None, int | None, bool, bool]:
    ...
    return (earned if total else None, total if total else None, has_platinum, platinum_earned)

def _new_stat_agg() -> dict[str, dict]:
    return defaultdict(lambda: {'minutes': 0, 'play_count': None, 'last': None, 'first': None})
...
def _apply_stat_to_entry(entry: PsnGameEntry, stat: object | None, stat_agg: dict[str, dict]) -> None:
    ...
    last = agg and agg['last'] or (_iso(getattr(stat, 'last_played_date_time', None)) if stat is not None else None)
    if last:
        entry.last_played = last
    first = agg and agg['first'] or (_iso(getattr(stat, 'first_played_date_time', None)) if stat is not None else None)
    if first:
...
_TITLE_ID_PLATFORM_PREFIX = {'PPSA': 'PS5', 'CUSA': 'PS4', 'PCSE': 'PS Vita', 'PCSA': 'PS Vita', 'PCSB': 'PS Vita', 'PCSC': 'PS Vita', 'PCSD': 'PS Vita', 'PCSF': 'PS Vita', 'PCSG': 'PS Vita', 'PCSH': 'PS Vita'}
...
class PsnClient:
    ...
    def __init__(self, npsso: str):
        ...
        try:
            ...
            raise PsnAuthError('PSN session expired or invalid. Sign in at playstation.com and update PSN_NPSSO in .env.') from exc

    def validate_session(self) -> str:
        try:
            return self._client.online_id
        except PSNAWPForbiddenError as exc:
            raise PsnAuthError('PSN profile is private or inaccessible. Set profile/trophies to Public in PSN privacy settings.') from exc

    def collect_wishlist(self) -> list[PsnWishlistEntry]:
        import json
        params = {'operationName': WISHLIST_OPERATION, 'variables': json.dumps({}), 'extensions': json.dumps({'persistedQuery': {'version': 1, 'sha256Hash': WISHLIST_PERSISTED_HASH}})}
        headers = {'x-apollo-operation-name': WISHLIST_OPERATION, 'apollographql-client-name': 'PlayStationApp-Web', 'content-type': 'application/json'}
        try:
            resp = self._psnawp.authenticator.get(url=WISHLIST_GQL_URL, params=params, headers=headers)
        except PSNAWPAuthenticationError as exc:
            raise PsnAuthError('PSN session expired or invalid. Reconnect PSN on the Connections page.') from exc
        payload = resp.json()
        ...
        for item in raw_items:
            ...
            entries.append(PsnWishlistEntry(id=pid, name=_display_name(item.get('name') or f'PSN {pid}'), image_url=image, platforms=[str(p) for p in item.get('platforms') or [] if p], store_classification=item.get('storeDisplayClassification'), localized_classification=item.get('localizedStoreDisplayClassification'), kind=str(item.get('__typename') or 'Product'), price=price, price_initial=price_initial, discount_percent=discount, store_url=_wishlist_store_url(pid)))
        return entries
    ...
    def _walk_trophy_entries(self) -> tuple[dict[str, PsnGameEntry], int]:
        ...
        for trophy in self._client.trophy_titles(limit=None):
            ...
            trophies_earned, trophies_total, has_platinum, platinum_earned = _trophy_counts_from_title(trophy)
            entry = PsnGameEntry(id=comm_id, np_communication_id=comm_id, title_id=trophy.np_title_id, concept_id=None, name=name, image_url=trophy.title_icon_url, platforms=_platform_labels(trophy.title_platform), trophy_progress=trophy.progress, trophies_earned=trophies_earned, trophies_total=trophies_total, has_platinum=has_platinum, platinum_earned=platinum_earned, playtime_minutes=0, last_played=_iso(trophy.last_updated_datetime), first_played=None, play_count=None, store_url=_store_url(None, comm_id))
    ...
    def collect_library(self) -> list[PsnGameEntry]:
        ...
        for entitlement in entitlements:
            ...
            if not title_id or title_id in seen_title_ids:
                continue
            name = _display_name(title_meta.get('name') or concept_meta.get('name') or game_meta.get('name') or f'PSN {title_id}')
            concept_id = concept_meta.get('conceptId')
            image = title_meta.get('imageUrl') or concept_meta.get('iconUrl') or game_meta.get('iconUrl')
            entry_id = title_id
            entries[entry_id] = PsnGameEntry(id=entry_id, np_communication_id=None, title_id=title_id, concept_id=concept_id, name=name, image_url=image, platforms=_platform_from_title_id(title_id), trophy_progress=None, trophies_earned=None, trophies_total=None, has_platinum=False, platinum_earned=False, playtime_minutes=0, last_played=None, first_played=None, play_count=None, store_url=_store_url(concept_id, None))
    ...
    def _dedupe_by_name(entries: list[PsnGameEntry]) -> list[PsnGameEntry]:
        ...
        def sort_key(entry: PsnGameEntry) -> tuple:
            return (_platform_priority(entry.platforms), 1 if entry.trophy_progress is not None else 0, 1 if entry.playtime_minutes else 0, 1 if entry.title_id else 0)
        ...
        for group in groups.values():
            ...
            for sibling in group[1:]:
                ...
                if sibling.last_played and (not keeper.last_played or sibling.last_played > keeper.last_played):
                    keeper.last_played = sibling.last_played
                if sibling.first_played and (not keeper.first_played or sibling.first_played < keeper.first_played):
