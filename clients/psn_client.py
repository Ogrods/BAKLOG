...
class PsnWishlistEntry:
    ...
    image_url: "Any"
    platforms: "Any"
    store_classification: "Any"
    localized_classification: "Any"
    kind: "Any"
    price: "Any"
    price_initial: "Any"
...
def _trophy_counts_from_title(trophy):
...
def _accumulate_stat_into_agg(stat_agg, stat):
...
def _apply_stat_to_entry(entry, stat, stat_agg):
...
def _platform_from_title_id(title_id):
    if not title_id:
        return ["PlayStation"]
...
def _platform_priority(platforms):
    return max((_PLATFORM_RANK.get(p, 0) for p in platforms), default=0)


def _store_url(concept_id, np_communication_id):
    if concept_id:
...
def _wishlist_store_url(product_id):
...
def _parse_wishlist_price(price_obj):
    if not price_obj:
...
class PsnClient:
    ...
    def collect_wishlist(self):
        import json

        params = {
            "operationName": WISHLIST_OPERATION,
            "variables": json.dumps({}),
            "extensions": json.dumps({"persistedQuery": {"version": 1, "sha256Hash": WISHLIST_PERSISTED_HASH}}),
        }
        headers = {
    ...
    def probe_library_fingerprint(self):
        count = 0
        max_last = None
    ...
    def _walk_entitlements_raw(self):
        raw = []
        count = 0
    ...
    def collect_library(self):
        ...
        for entitlement in entitlements:
            ...
            if not title_id or title_id in seen_title_ids:
                continue
            name = _display_name(
                title_meta.get("name") or concept_meta.get("name") or game_meta.get("name") or f"PSN {title_id}"
            )
