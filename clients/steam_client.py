...
def _get_with_retry(url: str, params: dict, *, timeout: int=30, retries: int=3, backoff: tuple[int, ...]=_STORE_RETRY_BACKOFF) -> requests.Response:
...
class SteamClient:
    ...
    def get_owned_games(self) -> list[dict]:
        ...
        params = {'key': self.api_key, 'steamid': self.steam_id, 'include_appinfo': 1, 'include_played_free_games': 1, 'format': 'json'}
    ...
    def get_app_details_batch(self, appids: list[int], refresh: bool=False) -> dict[int, dict | None]:
        ...
        for appid in appids:
            ...
            if not refresh:
                ...
                if cached is not None:
                    ...
                    continue
            results[appid] = self.get_app_details(appid, refresh=refresh)
        return results
    ...
    def get_review_summary(self, appid: int, refresh: bool=False) -> dict | None:
        ...
        result = {'total_reviews': summary.get('total_reviews', 0), 'total_positive': summary.get('total_positive', 0), 'total_negative': summary.get('total_negative', 0), 'review_score': summary.get('review_score', 0), 'review_score_desc': summary.get('review_score_desc', '')}
        if result['total_reviews'] > 0:
            result['percent_positive'] = round(100 * result['total_positive'] / result['total_reviews'], 1)
        else:
