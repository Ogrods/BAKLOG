...
def _get_with_retry(url, params, *, timeout=30, retries=3, backoff=_STORE_RETRY_BACKOFF):
    last_exc = None
...
class SteamClient:
    def __init__(self, api_key, steam_id, cache_dir=None):
    ...
    def _read_cache(self, kind, key):
        path = self._cache_path(kind, key)
        if path.exists():
            return json.loads(path.read_text(encoding="utf-8"))
        return None

    def _write_cache(self, kind, key, data):
        path = self._cache_path(kind, key)
    ...
    def get_app_details(self, appid, refresh=False):
        cache_key = str(appid)
        if not refresh:
    ...
    def get_app_details_batch(self, appids, refresh=False):
        results = {}
        for appid in appids:
            cache_key = str(appid)
            if not refresh:
    ...
    def get_review_summary(self, appid, refresh=False):
        cache_key = str(appid)
        if not refresh:
