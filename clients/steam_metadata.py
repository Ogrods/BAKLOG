...
def coop_flags_from_categories(categories):
    if not categories:
        return (False, False)
...
def early_access_from_appdetails(details):
    if not details:
        return False
...
def enrichment_from_appdetails(details):
    empty = {
        "coop_online": False,
        "coop_local": False,
        "genres": [],
...
ALWAYS_WRITE_FIELDS = frozenset({"coop_online", "coop_local"})
FILL_IF_MISSING_FIELDS = frozenset(
    {"genres", "release_date", "metacritic_score", "developers", "publishers", "controller_support", "early_access"}
)
...
def apply_enrichment_to_row(row, enrichment):
