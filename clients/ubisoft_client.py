...
class UbisoftClient:
    ...
    def __init__(self, auth: str, session_id: str, app_id: str | None=None, user_agent: str | None=None) -> None:
        auth = (auth or '').strip()
        session_id = (session_id or '').strip()
        if not auth or not session_id:
            raise UbisoftAuthError('Set UBISOFT_AUTH and UBISOFT_SESSION_ID in .env. From DevTools → Network on ubisoft.com, copy Authorization (Ubi_v1 t=...) and Ubi-SessionId.')
        ...
        self.session.headers.update({'Authorization': auth, 'Ubi-AppId': self.app_id, 'Ubi-SessionId': session_id, 'Ubi-Localecode': 'en-US', 'Ubi-Country': 'US', 'Ubi-RequestedPlatformType': 'uplay', 'Accept': 'application/json', 'Content-Type': 'application/json', 'User-Agent': user_agent or 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36', 'Origin': 'https://www.ubisoft.com', 'Referer': 'https://www.ubisoft.com/'})
    ...
    def _get(self, path: str) -> tuple[int, dict | list | None, str]:
        ...
        if resp.status_code in (401, 403):
            raise UbisoftAuthError(f'Ubisoft rejected the credentials ({resp.status_code}). Sign in again at ubisoft.com, then refresh UBISOFT_AUTH and UBISOFT_SESSION_ID from DevTools.')
        try:
            body = resp.json()
        except ValueError:
            body = None
    ...
    def get_library(self) -> tuple[dict, str]:
        ...
        if status >= 400 or not isinstance(apps_body, dict):
            raise UbisoftAuthError(f'Could not load applications ({status}). Refresh UBISOFT_AUTH / UBISOFT_SESSION_ID from DevTools.')
