...
STORE_FILES: list[tuple[Path, str, Callable[[dict], bool] | None]] = [(Path('games_gog.json'), 'gog', None), (Path('games_psn.json'), 'psn', None), (Path('games_epic.json'), 'epic', None), (Path('games_amazon.json'), 'amazon', None), (Path('games_xbox.json'), 'xbox', None), (Path('games_battlenet.json'), 'battlenet', None), (Path('games_ubisoft.json'), 'ubisoft', None), (Path('games_nintendo.json'), 'nintendo', None), (Path('games_humble.json'), 'humble', None), (Path('games_itch.json'), 'itch', _itch_is_videogame), (Path('games_ea.json'), 'ea', None), (Path('games_wishlist.json'), 'wishlist', None), (Path('games_wishlist_gog.json'), 'wishlist', None), (Path('games_wishlist_epic.json'), 'wishlist', None), (Path('games_wishlist_psn.json'), 'wishlist', None), (Path('games_wishlist_ubisoft.json'), 'wishlist', None), (Path('games_wishlist_xbox.json'), 'wishlist', None), (Path('games_wishlist_nintendo.json'), 'wishlist', None), (Path('games_wishlist_humble.json'), 'wishlist', None)]
...
def main() -> int:
    ...
    parser.add_argument('--retry-misses', action='store_true', help='Re-search Steam for rows previously cached as having no Steam match.')
    parser.add_argument('--upgrade-lowres', action='store_true', help='Re-source native-store art (non-steamstatic URLs) to sharper Steam CDN images.')
    ...
    for rel, store, row_filter in STORE_FILES:
        ...
        if not todo:
            ...
            continue
        skip_cached = sum((1 for g in todo if f"{store}:{g.get('id')}" in no_steam_match or (args.upgrade_lowres and needs_lowres_upgrade(g) and (f"{store}:{g.get('id')}" in lowres_checked))))
        fresh = len(todo) - skip_cached
        if fresh == 0:
            print(f'{path.name}: {len(todo)} need cover but all are cached as "no Steam match" — skipping (use --retry-misses to retry).', flush=True)
            continue
    ...
    meta.write_text(json.dumps({'fetched_at': datetime.now(UTC).isoformat(), 'last_updated': updated, 'no_steam_match': sorted(no_steam_match), 'lowres_checked': sorted(lowres_checked), 'steam_validated': sorted(steam_validated)}, indent=2), encoding='utf-8')
    print(f'\nUpdated {updated} games with Steam CDN images. {len(no_steam_match)} rows cached as "no Steam match".', flush=True)
