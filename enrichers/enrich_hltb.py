...
HEARTBEAT_EVERY = 25
STORE_FILES = [('games_steam.json', 'steam', None), ('games_gog.json', 'gog', None), ('games_psn.json', 'psn', None), ('games_epic.json', 'epic', None), ('games_amazon.json', 'amazon', None), ('games_xbox.json', 'xbox', None), ('games_battlenet.json', 'battlenet', None), ('games_ubisoft.json', 'ubisoft', None), ('games_nintendo.json', 'nintendo', None), ('games_wishlist.json', 'wishlist', None), ('games_humble.json', 'humble', None), ('games_itch.json', 'itch', _itch_is_videogame), ('games_ea.json', 'ea', None)]
HLTB_FIELDS = ('hltb_id', 'hltb_name', 'hltb_main_hours', 'hltb_main_extra_hours', 'hltb_completionist_hours', 'hltb_match_confidence')
...
def main() -> int:
    ...
    parser.add_argument('--retry-misses', action='store_true', help='Re-attempt rows previously cached as having no HLTB match.')
    ...
    for filename, store, row_filter in STORE_FILES:
        if args.store and args.store != store:
            continue
        path = catalog_file(Path(filename))
        if not path.exists():
            continue
    ...
    print(f'  ~{pending_lookups} HLTB lookups pending (est. {int(est_sec // 60)}m at ~{QUERY_DELAY_SEC + 7.5:.1f}s each)', flush=True)
        ...
        for i, g in enumerate(missing, 1):
            ...
            if cached is False and (not args.retry_misses):
                ...
                if processed % HEARTBEAT_EVERY == 0:
                    hb.tick(f'[{i}/{len(missing)}] skipped {store_skipped} cached misses, {store_lookups} lookups (+{store_hits} hits) — still working')
                    hb.reset()
                continue
            if isinstance(cached, dict):
                hit = cached
            ...
            stats.ok += 1
            if updated % 25 == 0:
                print(f"  [{i}/{len(missing)}] {updated} updated so far ({g.get('name')} -> {hit.get('hltb_main_hours')}h, match {hit.get('hltb_match_confidence')})", flush=True)
                hb.reset()
        data['game_count'] = len(games)
        write_catalog_text(rel, json.dumps(data, indent=2, ensure_ascii=False))
        print(f'  saved {updated} HLTB updates to {filename} ({store_lookups} lookups: {store_hits} hits, {store_misses} no-match, {store_skipped} skipped from cache)', flush=True)
        save_mapping(mapping)
    save_mapping(mapping)
