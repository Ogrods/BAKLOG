...
def summary_to_row_fields(summary):
    ...
    return {
        "protondb_tier": tier,
        "protondb_confidence": summary.get("confidence"),
        "protondb_report_count": int(total) if total is not None else None,
        "protondb_score": float(score) if score is not None else None,
        "protondb_trending_tier": trending if trending not in (None, "pending") else None,
    }
