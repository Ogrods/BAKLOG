...
def main() -> int:
    ...
    parser.add_argument('--stores', nargs='+', choices=['gog', 'epic', 'psn', 'amazon', 'xbox', 'battlenet', 'ubisoft', 'nintendo', 'humble', 'itch'], metavar='STORE', help='Only process these stores (default: all). Example: --stores nintendo')
    parser.add_argument('--retry-misses', action='store_true', help='Re-attempt rows previously cached as "no Steam app match" (appid 0).')
    parser.add_argument('--refresh-empty', action='store_true', help='Re-fetch Steam review summaries for rows that have a mapped appid but steam_review_percent is still null (skips store search).')
    ...
    for filename, store, row_filter in store_files:
        ...
        for i, g in enumerate(games, 1):
            ...
            stats.ok += 1
            if updated % 25 == 0:
                print(f"  [{i}/{len(games)}] {updated} updated so far ({g['name']} -> {reviews['percent_positive']}%)", flush=True)
