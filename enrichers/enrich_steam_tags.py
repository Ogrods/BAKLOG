...
from clients.steam_metadata import ALWAYS_WRITE_FIELDS, FILL_IF_MISSING_FIELDS, apply_enrichment_to_row, enrichment_from_appdetails
...
STORE_FILES: list[tuple[str, str, Callable[[dict], bool] | None]] = [('games_gog.json', 'gog', None), ('games_epic.json', 'epic', None), ('games_psn.json', 'psn', None), ('games_amazon.json', 'amazon', None), ('games_xbox.json', 'xbox', None), ('games_battlenet.json', 'battlenet', None), ('games_ubisoft.json', 'ubisoft', None), ('games_ea.json', 'ea', None), ('games_nintendo.json', 'nintendo', None), ('games_itch.json', 'itch', _itch_is_videogame)]
...
def main(argv: list[str] | None=None) -> int:
    ...
    parser = argparse.ArgumentParser(description='Backfill coop tags + missing genres/release_date on non-Steam library rows from Steam appdetails.')
    parser.add_argument('--stores', nargs='+', choices=[s[1] for s in STORE_FILES], metavar='STORE', help='Only process these stores (default: all).')
    ...
    if not mapping:
        stats.error('cache/steam_review_map.json is missing or empty. Run enrich_steam_reviews.py first to build the appid map.')
        return stats.finish('enrich_steam_tags', t0, exit_code=1)
    ...
    for filename, store, row_filter in store_files:
        ...
        print(f'  appid-mapped: {rows_with_appid}  updated: {rows_updated}  coop_true: {coop_set}  genres_filled: {genres_filled}  release_filled: {release_filled}' + ('  [dry run, not written]' if args.dry_run else ''), flush=True)
    ...
    if not args.dry_run:
        ...
        meta.write_text(json.dumps({'fetched_at': datetime.now(UTC).isoformat(), 'rows_updated': total_updated, 'rows_with_appid': total_appid_mapped, 'rows_coop': total_coop}, indent=2, ensure_ascii=False), encoding='utf-8')
