...
from shared.free_claims_sources import CLAIM_ENRICH_FIELDS, EPIC_MOBILE_STORE, GAMERPOWER_ATTRIBUTION, claim_match_keys, has_valid_claim_links, is_epic_mobile_store, merge_manual_and_auto, norm_title, normalize_claim_urls
...
ITAD_GAME_SLUG_RE = re.compile('isthereanydeal\\.com/game/([^/\\"\'>]+)/info', re.IGNORECASE)
...
def _clean_blurb(raw: object) -> str | None:
    ...
    text = re.sub('\\s*\\|?\\s*(unknown expiry|expires on[^|]*)\\s*\\|?\\s*go to giveaway\\s*', ' ', text, flags=re.IGNORECASE)
    text = re.sub('https?://\\S+', ' ', text, flags=re.IGNORECASE)
...
def _slug_id(store: str, title: str, appid: int | None=None) -> str:
    ...
    base = re.sub('[^a-z0-9]+', '-', (title or 'game').lower()).strip('-')
...
def _steam_app_details(appid: int, last_call: list[float]) -> dict | None:
    _throttle(last_call)
    try:
        resp = requests.get('https://store.steampowered.com/api/appdetails', params={'appids': appid, 'l': 'english'}, timeout=30)
...
def _steam_review_percent(appid: int, last_call: list[float]) -> int | None:
    _throttle(last_call)
    try:
        resp = requests.get(f'https://store.steampowered.com/appreviews/{appid}', params={'json': 1, 'language': 'all', 'purchase_type': 'all', 'num_per_page': 0}, timeout=30)
        resp.raise_for_status()
...
def _steam_storesearch(term: str, last_call: list[float]) -> list[dict]:
    ...
    _throttle(last_call)
    try:
        resp = requests.get(STEAM_STORESEARCH_URL, params={'term': term, 'l': 'english', 'cc': 'us'}, headers=STEAM_HEADERS, timeout=30)
        resp.raise_for_status()
...
def _resolve_steam_appid_by_title(title: str, last_call: list[float], *, blurb: object=None) -> int | None:
...
def _resolve_steam_appid(*, store: str, title: str, claim_url: str, blurb: object=None, last_call: list[float]) -> int | None:
...
_ITCH_HINT = re.compile('itch\\.?io', re.IGNORECASE)
_INDIEGALA_HINT = re.compile('indiegala', re.IGNORECASE)
_MOBILE_EPIC_HINT = re.compile('mobile', re.IGNORECASE)
_EGS_EPIC_HINT = re.compile('\\b(epic|egs)\\b', re.IGNORECASE)
...
def _enrich_item_publish_skip(store: str, appid: int | None, header: object, genres: list, review: object) -> bool:
...
def _enrich_item(raw: dict, last_call: list[float], cover_lookup: dict[str, str] | None=None, *, now: datetime | None=None, upgrade_covers: bool=True, prior_row: dict | None=None, is_manual: bool=False) -> dict:
    claim_url = str(raw.get('claim_url') or '').strip()
    title = (raw.get('title') or '').strip()
    store = _infer_store_from_text(str(raw.get('store') or '').strip().lower(), title, raw.get('blurb'), claim_url)
    ...
    if appid is None:
        appid = _resolve_steam_appid(store=store, title=title, claim_url=claim_url, blurb=raw.get('blurb'), last_call=last_call)
    if cover_lookup:
    ...
    if appid:
        header_str = str(header or '').strip()
        publish_skip = not upgrade_covers and _enrich_item_publish_skip(store, appid, header_str, genres, review)
        if publish_skip:
            needs_details = False
            needs_portrait = False
        else:
            needs_details = review is None or not header_str or (store in ('steam', '') and (not genres))
    ...
    out = {'id': item_id, 'store': store, 'title': title or item_id, **_claim_links_payload(raw, store), 'header_image': header, 'genres': genres[:6] if isinstance(genres, list) else [], 'blurb': _clean_blurb(raw.get('blurb')), 'ends_at': _resolve_ends_at(raw, now=now)}
    if appid:
    ...
    if source:
        out['source'] = source
    first_seen = _resolve_first_seen(raw, prior_row=prior_row, now=now, is_manual=is_manual)
    if first_seen:
...
def _carry_forward_missing_approved(published_items: list[dict], *, approved_ids: set[str], dismissed_ids: set[str], prior_rows_by_id: dict[str, dict], now: datetime) -> list[dict]:
...
def filter_manual_items_for_publish(manual_items: list[dict], *, require_manual_approval: bool) -> list[dict]:
    return [item for item in manual_items if isinstance(item, dict) and manual_row_publishable(item, require_manual_approval=require_manual_approval)]

def _apply_premium_only(items: list[dict], *, premium_only_ids: set[str], manual_items: list[dict]) -> None:
...
def _auto_items_by_id(auto_items_all: list[dict]) -> dict[str, dict]:
    return {str(item.get('id') or '').strip(): item for item in auto_items_all if isinstance(item, dict) and str(item.get('id') or '').strip()}

def _keys_for_approved_id(item_id: str, *, auto_by_id: dict[str, dict], field_overrides: dict[str, dict[str, str]]) -> set[str]:
...
def _resolve_approved_keys(approved_ids: set[str], auto_items_all: list[dict], field_overrides: dict[str, dict[str, str]]) -> set[str]:
    auto_by_id = _auto_items_by_id(auto_items_all)
    keys: set[str] = set()
    for item_id in approved_ids:
        keys |= _keys_for_approved_id(item_id, auto_by_id=auto_by_id, field_overrides=field_overrides)
    return keys

def _build_key_override_maps(approved_ids: set[str], *, auto_by_id: dict[str, dict], store_overrides: dict[str, str], field_overrides: dict[str, dict[str, str]]) -> tuple[dict[str, str], dict[str, dict[str, str]]]:
...
def _is_approved_item(item: dict, *, approved_ids: set[str], approved_keys: set[str]) -> bool:
    item_id = str(item.get('id') or '').strip()
...
def _lookup_store_override(item: dict, *, store_overrides: dict[str, str], store_overrides_by_key: dict[str, str]) -> str | None:
    item_id = str(item.get('id') or '').strip()
...
def _lookup_field_overrides(item: dict, *, field_overrides: dict[str, dict[str, str]], field_overrides_by_key: dict[str, dict[str, str]]) -> dict[str, str] | None:
    item_id = str(item.get('id') or '').strip()
...
def _apply_store_overrides(items: list[dict], *, store_overrides: dict[str, str], store_overrides_by_key: dict[str, str] | None=None) -> None:
    store_overrides_by_key = store_overrides_by_key or {}
    for item in items:
        override = _lookup_store_override(item, store_overrides=store_overrides, store_overrides_by_key=store_overrides_by_key)
        if override:
            item['store'] = override

def _apply_field_overrides(items: list[dict], field_overrides: dict[str, dict[str, str]], field_overrides_by_key: dict[str, dict[str, str]] | None=None) -> None:
    field_overrides_by_key = field_overrides_by_key or {}
    for item in items:
        overrides = _lookup_field_overrides(item, field_overrides=field_overrides, field_overrides_by_key=field_overrides_by_key)
        if not overrides:
            continue
        for field, value in overrides.items():
            item[field] = value

def _effective_ends_at(item: dict, *, field_overrides: dict[str, dict[str, str]] | None=None, field_overrides_by_key: dict[str, dict[str, str]] | None=None) -> object:
    overrides = _lookup_field_overrides(item, field_overrides=field_overrides or {}, field_overrides_by_key=field_overrides_by_key or {})
    if overrides and 'ends_at' in overrides:
        return overrides['ends_at']
    return item.get('ends_at')

def _select_approved_auto_items(auto_items_all: list[dict], *, approved_ids: set[str], approved_keys: set[str], now: datetime, dismissed_ids: set[str] | None=None, field_overrides: dict[str, dict[str, str]] | None=None, field_overrides_by_key: dict[str, dict[str, str]] | None=None) -> tuple[list[dict], set[str]]:
    ...
    for item in auto_items_all:
        ...
        if item_id and item_id in dismissed_ids:
            continue
        if not _is_approved_item(item, approved_ids=approved_ids, approved_keys=approved_keys):
            continue
        ends_at = _effective_ends_at(item, field_overrides=field_overrides, field_overrides_by_key=field_overrides_by_key)
...
def _resolve_first_seen(raw: dict, *, prior_row: dict | None, now: datetime | None, is_manual: bool) -> str | None:
...
def _enrich_item_light(raw: dict, cover_lookup: dict[str, str] | None=None, live_item: dict | None=None, review_lookup: dict[str, int] | None=None, *, now: datetime | None=None, prior_row: dict | None=None, is_manual: bool=False) -> dict:
    claim_url = str(raw.get('claim_url') or '').strip()
    title = (raw.get('title') or '').strip()
    store = _infer_store_from_text(str(raw.get('store') or '').strip().lower(), title, raw.get('blurb'), claim_url)
    appid = raw.get('steam_appid')
    ...
    item_id = (raw.get('id') or '').strip() or _slug_id(store, title, appid)
    out: dict = {'id': item_id, 'store': store, 'title': title or item_id, **_claim_links_payload(raw, store), 'header_image': header, 'genres': genres[:6] if isinstance(genres, list) else [], 'blurb': _clean_blurb(raw.get('blurb')), 'ends_at': _resolve_ends_at(raw, now=now)}
    if appid:
        out['steam_appid'] = appid
    if review is not None:
        out['review_percent'] = review
    ...
    if source:
        out['source'] = source
    first_seen = _resolve_first_seen(raw, prior_row=prior_row or live_item, now=now, is_manual=is_manual)
    if first_seen:
        out['first_seen'] = first_seen
    return out

def preview_publish_items(*, manual_items: list[dict], auto_items_all: list[dict], approved_ids: set[str], store_overrides: dict[str, str] | None=None, field_overrides: dict[str, dict[str, str]] | None=None, dismissed_ids: set[str] | None=None, live_items: list[dict] | None=None, premium_only_ids: set[str] | None=None, require_manual_approval: bool=False, now: datetime | None=None) -> list[dict]:
    ...
    store_by_key, field_by_key = _build_key_override_maps(approved_ids, auto_by_id=auto_by_id, store_overrides=store_overrides, field_overrides=field_overrides)
    auto_items, _ = _select_approved_auto_items(auto_items_all, approved_ids=approved_ids, approved_keys=approved_keys, now=now, dismissed_ids=dismissed_ids, field_overrides=field_overrides, field_overrides_by_key=field_by_key)
    if store_overrides or store_by_key:
        _apply_store_overrides(auto_items, store_overrides=store_overrides, store_overrides_by_key=store_by_key)
    if field_overrides or field_by_key:
        _apply_field_overrides(auto_items, field_overrides, field_overrides_by_key=field_by_key)
    publish_manual = filter_manual_items_for_publish(manual_items, require_manual_approval=require_manual_approval)
    manual_ids = {str(it.get('id') or '').strip() for it in publish_manual if str(it.get('id') or '').strip()}
    ...
    for raw in raw_items:
        ...
        items.append(_enrich_item_light(raw, cover_lookup, live_by_id.get(item_id), review_lookup=review_lookup, now=now, prior_row=live_by_id.get(item_id), is_manual=item_id in manual_ids))
    carried = _carry_forward_missing_approved(items, approved_ids=approved_ids, dismissed_ids=dismissed_ids or set(), prior_rows_by_id=live_by_id, now=now)
    items.extend(carried)
    _apply_premium_only(items, premium_only_ids=premium_only_ids or set(), manual_items=manual_items)
    return items
...
def merge_enriched_items_into_auto_feed(auto_path: Path, enriched_items: list[dict]) -> int:
    ...
    if not isinstance(items, list):
        return 0
    by_id = {str(item.get('id') or '').strip(): item for item in enriched_items if isinstance(item, dict) and str(item.get('id') or '').strip()}
    if not by_id:
        return 0
    updated = 0
    for item in items:
...
def merge_enriched_items_into_input_feed(input_path: Path, enriched_items: list[dict]) -> int:
...
def rekey_approved_state(*, ids: list[str], store_overrides: dict[str, str], field_overrides: dict[str, dict[str, str]], premium_only_ids: set[str], auto_items_all: list[dict], prior_rows_by_id: dict[str, dict] | None=None) -> tuple[list[str], dict[str, str], dict[str, dict[str, str]], set[str]]:
    ...
    for item_id in ids:
        ...
        if old_id not in auto_by_id:
            keys = _keys_for_approved_id(old_id, auto_by_id=auto_by_id, field_overrides=field_overrides)
...
def parse_approved_put_payload(payload: dict) -> dict:
    ...
    return {'ids': ids, 'store_overrides': store_overrides, 'field_overrides': field_overrides, 'premium_only_ids': premium_only_ids, 'dismissed': dismissed, 'blocked': blocked}

def prepare_approved_document(*, ids: list[str], store_overrides: dict[str, str], field_overrides: dict[str, dict[str, str]], premium_only_ids: set[str], dismissed: list[str], auto_items: list[dict], blocked: list[str] | None=None, prior_rows_by_id: dict[str, dict] | None=None) -> dict:
    rekeyed_ids, store, fields, premium = rekey_approved_state(ids=ids, store_overrides=store_overrides, field_overrides=field_overrides, premium_only_ids=premium_only_ids, auto_items_all=auto_items, prior_rows_by_id=prior_rows_by_id)
    ...
    auto_ids = {str(it.get('id') or '').strip() for it in auto_items or [] if isinstance(it, dict) and str(it.get('id') or '').strip()}
    pruned_dismissed = [d for d in dismissed if d not in blocked_set and (not auto_ids or d in auto_ids)]
...
def _prune_expired_from_approved(path: Path, expired_ids: set[str]) -> int:
    ...
    if isinstance(store_overrides, dict):
        doc['store_overrides'] = {k: v for k, v in store_overrides.items() if str(k).strip() not in expired_ids}
    field_overrides = doc.get('field_overrides')
    if isinstance(field_overrides, dict):
        doc['field_overrides'] = {k: v for k, v in field_overrides.items() if str(k).strip() not in expired_ids}
...
def main() -> int:
    ...
    parser.add_argument('--no-profile', action='store_true', help="Skip writing the active profile's free_claims.json (publish-only)")
    parser.add_argument('--dry-run', action='store_true')
    parser.add_argument('--allow-empty', action='store_true', help='Allow overwriting the published feed with zero items')
    parser.add_argument('--require-manual-approval', action='store_true', help='Only publish manual rows with approved: true (default: publish unless approved: false)')
    ...
    prior_published_rows = _load_prior_published_rows([args.output, FALLBACK_PATH, free_claims_path()])
    rekeyed_ids, store_overrides, field_overrides, premium_only_ids = rekey_approved_state(ids=sorted(approved_ids), store_overrides=store_overrides, field_overrides=field_overrides, premium_only_ids=premium_only_ids, auto_items_all=auto_items_all, prior_rows_by_id=prior_published_rows)
    approved_ids = set(rekeyed_ids)
    approved_changed = approved_ids != orig_approved_ids or store_overrides != orig_store_overrides or field_overrides != orig_field_overrides or (premium_only_ids != orig_premium_only_ids)
    ...
    approved_keys = _resolve_approved_keys(approved_ids, auto_items_all, field_overrides)
    store_by_key, field_by_key = _build_key_override_maps(approved_ids, auto_by_id=auto_by_id, store_overrides=store_overrides, field_overrides=field_overrides)
    auto_items, expired_approved_ids = _select_approved_auto_items(auto_items_all, approved_ids=approved_ids, approved_keys=approved_keys, now=now, dismissed_ids=dismissed_ids, field_overrides=field_overrides, field_overrides_by_key=field_by_key)
    if store_overrides or store_by_key:
        _apply_store_overrides(auto_items, store_overrides=store_overrides, store_overrides_by_key=store_by_key)
    if field_overrides or field_by_key:
        _apply_field_overrides(auto_items, field_overrides, field_overrides_by_key=field_by_key)
    ...
    if auto_items_all:
        stats.warn(f'auto feed: {len(auto_items)} approved of {len(auto_items_all)} available')
    if DEBUG_CLAIMS:
        _debug_claims(f'approved selection: {len(auto_items)} of {len(auto_items_all)} auto; dismissed={len(dismissed_ids)} expired_approved={len(expired_approved_ids)}')
    publish_manual = filter_manual_items_for_publish(manual_items, require_manual_approval=require_manual_approval)
    skipped_manual = max(0, len(manual_items) - len(publish_manual))
    if skipped_manual:
        stats.warn(f'skipped {skipped_manual} unapproved manual item(s) (require_manual_approval={require_manual_approval})')
    if publish_manual:
        shipped_manual_ids = [str(item.get('id') or '').strip() for item in publish_manual if str(item.get('id') or '').strip()]
        if shipped_manual_ids:
            stats.warn(f'manual rows publishing ({len(shipped_manual_ids)}): ' + ', '.join(shipped_manual_ids))
    raw_items = merge_manual_and_auto(publish_manual, auto_items)
    if auto_items:
    ...
    last_call = [0.0]
    items: list[dict] = []
    enrich_total = sum((1 for raw in raw_items if isinstance(raw, dict) and has_valid_claim_links(raw) and raw.get('store') and (not _is_expired(_resolve_ends_at(raw, now=now), now))))
    ...
    for raw in raw_items:
        ...
        items.append(_enrich_item(raw, last_call, cover_lookup, now=now, upgrade_covers=False, prior_row=prior_published_rows.get(item_id), is_manual=item_id in manual_ids))
    carried = _carry_forward_missing_approved(items, approved_ids=approved_ids, dismissed_ids=dismissed_ids, prior_rows_by_id=prior_published_rows, now=now)
    if carried:
        stats.warn(f"carried forward {len(carried)} approved claim(s) missing from the source feed: {', '.join((str(c.get('id')) for c in carried))}")
        if DEBUG_CLAIMS:
    ...
    if not args.dry_run:
        ...
        to_persist_auto = [it for it in items if str(it.get('id') or '').strip() in auto_ids]
        to_persist_manual = [it for it in items if str(it.get('id') or '').strip() in manual_ids]
    ...
    profile_payload = {'fetched_at': generated_at, 'source_url': 'build_free_claims.py', 'generated_at': generated_at, 'items': items}
    if has_gamerpower:
    ...
    if args.dry_run:
        ...
        empty_code = refuse_empty_result(items, label='build_free_claims publish set', allow_empty=args.allow_empty, output_path=args.output)
        if empty_code:
