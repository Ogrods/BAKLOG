...
from fetchers._base import add_allow_empty_arg, add_no_carry_arg, apply_carry_forward, carry_enrichment, catalog_file, configure_stdout, merge_cached_row, write_catalog_guarded
...
def resolve_source(requested: str, sql_dir: Path | None) -> str:
    ...
    if _web_profile_ready():
        return 'web'
    if sys.platform == 'win32':
        raise RuntimeError('Amazon Games launcher database not found and no Prime Gaming web session saved.\nInstall/sign in to the Amazon Games app on this PC, or connect “Amazon (Prime Gaming, web)” on the Connections page.')
    raise RuntimeError('Prime Gaming web session not found. Connect “Amazon (Prime Gaming, web)” on the Connections page (works on macOS/Linux).')
...
def merge_amazon_sources(current_rows: list[dict], carried_rows: list[dict], current_source: str) -> list[dict]:
...
def refuse_amazon_source_drift(new_same_source_count: int, *, source: str, allow_drift: bool, output_path: Path, threshold: float=0.5) -> int | None:
    if allow_drift:
        return None
    ...
    print(f'ERROR: Amazon {source} slice returned {new_same_source_count} rows, but the previous {source} slice had {prev} (≈{pct:.0f}% — under the {int(threshold * 100)}% floor).\nLikely a broken auth or upstream API. If this drop is real, re-run with --allow-drift.', file=sys.stderr, flush=True)
    return 3

def _build_row(rec: dict, hltb: dict | None, source: str) -> dict:
    pid = rec['amazon_product_id']
    row = {'store': 'amazon', 'id': pid, 'amazon_id': pid, 'amazon_entitlement_id': rec.get('amazon_entitlement_id'), 'amazon_adg_id': rec.get('amazon_adg_id'), 'name': rec['name'], 'source': source, 'playtime_minutes': 0, 'last_played': rec.get('last_played'), 'header_image': rec.get('header_image'), 'library_image': rec.get('library_image'), 'release_date': rec.get('release_date'), 'genres': rec.get('genres') or [], 'tags': [], 'steam_review_percent': None, 'steam_review_count': None, 'steam_review_desc': None, 'hltb_main_hours': None, 'hltb_main_extra_hours': None, 'hltb_completionist_hours': None, 'hltb_match_confidence': None, 'hltb_name': None, 'store_url': rec.get('store_url'), 'type': 'game', 'price': None, 'price_initial': None, 'discount_percent': None, 'currency': None, 'asin': rec.get('asin'), 'product_line': rec.get('product_line'), 'publisher': rec.get('publisher')}
    if hltb:
...
def _web_outcome_kind_from_capture(*, raw_claims: list[dict], outcome: dict, fallback_claims: list[dict] | None=None) -> str | None:
...
def _load_web_records(*, dump_raw: bool) -> tuple[list[dict], str | None]:
    from amazon_web_client import filter_codeless_claims, raw_dump_max_age_s, raw_dump_path, sniff_claims
    ...
    try:
        ...
        raise
    print('Reading Prime Gaming claims (Amazon-fulfilled only, no external keys).', flush=True)
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Import Amazon library (Windows launcher DB or Prime Gaming web)')
    parser.add_argument('--source', choices=('auto', 'launcher', 'web'), default=os.getenv('AMAZON_SOURCE', 'auto').strip().lower() or 'auto', help='auto: launcher DB on Windows when present, else web profile (default: auto)')
    parser.add_argument('--sql-dir', type=Path, default=None, help='Override Amazon Games Sql folder (launcher source only)')
    ...
    parser.add_argument('--dump-raw', action='store_true', help=f'Web source only: write {AMAZON_RAW_DUMP} with full claims payload')
    ...
    try:
        ...
        if source == 'launcher':
            if sys.platform != 'win32':
                ...
                return stats.finish('fetch_amazon', t0, exit_code=1)
            records = run_with_heartbeat(lambda: _load_launcher_records(sql_dir), 'Amazon launcher library')
        else:
            records, web_outcome_kind = run_with_heartbeat(lambda: _load_web_records(dump_raw=args.dump_raw), 'Amazon web library')
    ...
    if carried_rows:
        print(f'Keeping {len(carried_rows)} row(s) from the other Amazon source.', flush=True)
    ...
    for i, rec in enumerate(records, 1):
        ...
        current_rows.append(merge_cached_row(_build_row(rec, hltb, source), cached, authoritative=AMAZON, hltb_updated=hltb_updated))
        loop_hb.tick_progress(i, len(records), 'Amazon library', name[:40])
    drift_exit = None
    if not (source == 'web' and web_outcome_kind == 'signed_in_empty'):
        drift_exit = refuse_amazon_source_drift(len(current_rows), source=source, allow_drift=args.allow_drift, output_path=GAMES_AMAZON_JSON)
    ...
    payload = {'fetched_at': datetime.now(UTC).isoformat(), 'store': 'amazon', 'source': source, 'game_count': len(games_out), 'games': sorted(games_out, key=lambda g: g['name'].lower())}
    guard_allow_empty = args.allow_empty or (source == 'web' and web_outcome_kind == 'signed_in_empty')
    refused = write_catalog_guarded(GAMES_AMAZON_JSON, json.dumps(payload, indent=2, ensure_ascii=False), count=games_out, label='Amazon library', allow_empty=guard_allow_empty, allow_drift=True)
    if refused:
