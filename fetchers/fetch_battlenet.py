...
HLTB_DELAY_SEC = 1.0
FRANCHISE_STORE_URLS: dict[str, str] = {'world-of-warcraft.svg': 'https://worldofwarcraft.blizzard.com/', 'hearthstone.svg': 'https://hearthstone.blizzard.com/', 'overwatch-2.svg': 'https://overwatch.blizzard.com/', 'overwatch.svg': 'https://overwatch.blizzard.com/', 'diablo-ii-resurrected.svg': 'https://diablo2.blizzard.com/', 'diablo-iii.svg': 'https://diablo3.blizzard.com/', 'diablo-iv.svg': 'https://diablo4.blizzard.com/', 'starcraft-remastered.svg': 'https://starcraft.blizzard.com/', 'starcraft-ii.svg': 'https://starcraft2.blizzard.com/', 'warcraft-rumble.svg': 'https://warcraftrumble.blizzard.com/', 'heroes-of-the-storm.svg': 'https://heroesofthestorm.blizzard.com/'}
_TM_CHARS = ''.maketrans({'®': '', '™': '', '©': ''})
...
def _build_row(item: dict, hltb: dict | None) -> dict:
    ...
    if (item.get('gameAccountStatus') or '').lower() == 'good':
        tags.append('active')
    row = {'store': 'battlenet', 'id': bid, 'battlenet_id': bid, 'name': name, 'playtime_minutes': None, 'last_played': _last_played_iso(item), 'header_image': None, 'library_image': None, 'release_date': None, 'genres': [], 'tags': list(dict.fromkeys(tags)), 'steam_review_percent': None, 'steam_review_count': None, 'steam_review_desc': None, 'hltb_main_hours': None, 'hltb_main_extra_hours': None, 'hltb_completionist_hours': None, 'hltb_match_confidence': None, 'hltb_name': None, 'store_url': _store_url(item, name), 'type': 'game', 'price': None, 'price_initial': None, 'discount_percent': None, 'currency': None}
    if hltb:
...
def _build_client(browser: str, env_cookie: str) -> BattleNetClient:
    ...
    try:
        ...
        if env_cookie:
            ...
            return BattleNetClient(env_cookie)
        raise BattleNetAuthError(f'{e}\nAs a fallback, set BATTLENET_COOKIE in .env (DevTools → Network → /api/games-and-subs → Cookie header), or run with --browser env after setting it.') from e
...
def main() -> int:
    ...
    parser.add_argument('--browser', default=default_browser, choices=['edge', 'chrome', 'brave', 'firefox', 'env'], help='Where to read session cookies from (default: edge, or BATTLENET_BROWSER env).')
    parser.add_argument('--dump-raw', action='store_true', help=f'Also write the raw API response to {BATTLENET_RAW_DUMP} for debugging.')
    ...
    try:
        ...
        err = str(e)
        if args.browser == 'env' and env_cookie and ('401' in err or '403' in err or 'rejected the session' in err.lower()):
            ...
            try:
                ...
                print(f'warning: stored BATTLENET_COOKIE was rejected; using {fallback_browser} browser session instead.', file=sys.stderr)
    ...
    empty_exit = refuse_empty_result(deduped, label='Battle.net library', allow_empty=args.allow_empty, output_path=GAMES_BATTLENET_JSON)
    if empty_exit is not None:
        stats.error(f'No game records found in the response. Re-run with --dump-raw and inspect {BATTLENET_RAW_DUMP} to confirm the cookie hit the right account.')
    ...
    for i, item in enumerate(deduped, 1):
        ...
        if args.only_new and cached:
            ...
            loop_hb.tick_progress(i, len(deduped), 'Battle.net library', 'cached')
            continue
    ...
    drift_exit = refuse_drift_result(games_out, label='Battle.net library rows', allow_drift=args.allow_drift, output_path=GAMES_BATTLENET_JSON)
    ...
    payload = {'fetched_at': datetime.now(UTC).isoformat(), 'store': 'battlenet', 'game_count': len(games_out), 'games': sorted(games_out, key=lambda g: g['name'].lower())}
