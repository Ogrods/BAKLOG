...
from shared.free_claims_sources import EPIC_FREE_GAMES_URL, GAMERPOWER_ATTRIBUTION, GAMERPOWER_URL, ITAD_GIVEAWAYS_RSS, carry_claim_enrichment, dedup_claim_items_by_id, norm_title, parse_epic_payload, parse_gamerpower_payload, parse_itad_rss
...
def _fetch_text(url: str) -> str:
    resp = requests.get(url, headers={'User-Agent': USER_AGENT, 'Accept': 'application/rss+xml, application/xml, text/xml'}, timeout=REQUEST_TIMEOUT)
    resp.raise_for_status()
    return resp.text
...
def collect_claims(sources: set[str], *, now: datetime | None=None, stats: RunStats | None=None) -> tuple[list[dict], dict[str, int]]:
    counts: dict[str, int] = {}
    collected: list[dict] = []
    if 'epic' in sources:
        try:
    ...
    if DEBUG_CLAIMS:
        ...
        if dup_titles:
            _debug_claims(f'title collisions (kept for admin DUPE stamp): {len(dup_titles)} title(s), e.g. {list(dup_titles.items())[:3]}')
...
def _stamp_first_seen(items: list[dict], existing_by_id: dict[str, dict], fetched_at: str) -> None:
    for item in items:
...
def main() -> int:
    ...
    parser.add_argument('--source', choices=('all', 'epic', 'gamerpower', 'itad'), default='all', help='Which source(s) to fetch (default: all)')
    parser.add_argument('--dry-run', action='store_true')
    parser.add_argument('--allow-empty', action='store_true', help='Allow writing an empty feed (e.g. genuinely no live giveaways).')
    parser.add_argument('--allow-drift', action='store_true', help='Allow a feed sharply smaller than the prior run (partial source outage).')
    ...
    if existing_by_id:
        ...
        if failed_sources and counts:
            ...
            carried = [row for row in existing_by_id.values() if str(row.get('source') or '') in failed_sources and str(row.get('id') or '').strip() not in present_ids]
            if carried:
                stats.warn(f"carried forward {len(carried)} item(s) from failed source(s): {', '.join(sorted(failed_sources))}")
                items.extend(carried)
    _stamp_first_seen(items, existing_by_id, fetched_at)
    code = refuse_empty_result(items, label='fetch_claim_sources', allow_empty=args.allow_empty, output_path=args.output)
    if code is not None:
    ...
    if vanished_sources:
        ...
        if unrecovered and (not args.allow_drift):
            stats.error(f"source(s) disappeared from fetch with no recovered rows: {', '.join(sorted(unrecovered))}. Re-run with --allow-drift if this drop is real.")
            return stats.finish('fetch_claim_sources', t0, exit_code=3)
    if not args.allow_drift and prior_count > 0:
        floor = max(1, prior_count // 2)
        if len(items) < floor:
            stats.error(f'fetch_claim_sources collected {len(items)} item(s) but the prior feed had {prior_count} (under the 50% floor) — likely a partial source outage. Re-run with --allow-drift if this drop is real.')
            return stats.finish('fetch_claim_sources', t0, exit_code=3)
    has_gamerpower = any((item.get('source') == 'gamerpower' for item in items))
    payload = {'fetched_at': fetched_at, 'sources': counts, 'attribution': [GAMERPOWER_ATTRIBUTION] if has_gamerpower else [], 'items': items}
