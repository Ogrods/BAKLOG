...
from fetchers._base import add_allow_empty_arg, add_no_carry_arg, add_only_new_arg, apply_carry_forward, catalog_file, configure_stdout, merge_cached_row, refuse_drift_result, refuse_empty_result, row_key_by_id, write_catalog_text
from fetchers._progress import EXIT_CODE_AUTH, HeartbeatTimer, RunStats, started
from shared.library_noise import catalog_game_count, is_entitlement_slug, maybe_tag_library_noise_row, should_auto_hide_by_title, tag_noise_row
...
def _can_reuse_cached_epic_row(cached: dict, catalog_item: dict | None, *, skip_hltb: bool=False) -> bool:
...
def _build_game_row(catalog_id: str, namespace: str, item: dict, hltb: dict | None) -> dict | None:
    ...
    for info in item.get('releaseInfo') or []:
        if isinstance(info, dict) and info.get('date'):
            release = info['date']
            break
    row = {'store': 'epic', 'id': f'{namespace}:{catalog_id}', 'epic_namespace': namespace, 'epic_catalog_id': catalog_id, 'name': name, 'playtime_minutes': 0, 'last_played': None, 'header_image': header, 'library_image': library, 'release_date': release, 'genres': _extract_genres(item), 'tags': [], 'steam_review_percent': None, 'steam_review_count': None, 'steam_review_desc': None, 'hltb_main_hours': None, 'hltb_main_extra_hours': None, 'hltb_completionist_hours': None, 'hltb_match_confidence': None, 'hltb_name': None, 'store_url': store_url, 'type': 'game', 'price': None, 'price_initial': None, 'discount_percent': None, 'currency': None, 'acquired_at': None}
    if hltb:
...
def _build_game_row_from_record(rec: dict, catalog_item: dict | None, hltb: dict | None) -> dict | None:
    ...
    row = {'store': 'epic', 'id': f'{ns}:{cid}', 'epic_namespace': ns, 'epic_catalog_id': cid, 'name': name, 'playtime_minutes': 0, 'last_played': None, 'header_image': None, 'library_image': None, 'release_date': None, 'genres': [], 'tags': [], 'steam_review_percent': None, 'steam_review_count': None, 'steam_review_desc': None, 'hltb_main_hours': None, 'hltb_main_extra_hours': None, 'hltb_completionist_hours': None, 'hltb_match_confidence': None, 'hltb_name': None, 'store_url': _epic_store_url_from_record(rec, name), 'type': 'game', 'price': None, 'price_initial': None, 'discount_percent': None, 'currency': None, 'acquired_at': _acquired_at_from_record(rec)}
    if hltb:
        row.update({'hltb_main_hours': hltb.get('hltb_main_hours'), 'hltb_main_extra_hours': hltb.get('hltb_main_extra_hours'), 'hltb_completionist_hours': hltb.get('hltb_completionist_hours'), 'hltb_match_confidence': hltb.get('hltb_match_confidence'), 'hltb_name': hltb.get('hltb_name')})
    _tag_epic_row(row)
    return row
...
def main() -> int:
    ...
    print(f'  {len(records)} entitlements, {len(apps)} applications (dropped {ue_dropped} UE marketplace assets)', flush=True)
    ...
    empty_exit = refuse_empty_result(apps, label='Epic library', allow_empty=args.allow_empty, output_path=GAMES_EPIC_JSON)
    if empty_exit is not None:
        return stats.finish('fetch_epic', t0, exit_code=empty_exit)
    drift_exit = refuse_drift_result(apps, label='Epic library', allow_drift=args.allow_drift, output_path=GAMES_EPIC_JSON)
    ...
    if catalog_file(GAMES_EPIC_JSON).exists():
        try:
            prev_hash = json.loads(catalog_file(GAMES_EPIC_JSON).read_text(encoding='utf-8')).get('entitlement_set_hash')
    ...
    if args.only_new and (not args.refresh) and prev_hash and (set_hash == prev_hash):
        ...
        apps_needing_catalog = apps if args.refresh else [rec for rec in apps if _needs_catalog_fetch(rec, existing, args)]
        catalog_skipped = len(apps) - len(apps_needing_catalog)
    catalog: dict[tuple[str, str], dict] = {}
    if apps_needing_catalog:
        if catalog_skipped:
            print(f'Fetching catalog metadata for {len(apps_needing_catalog)} entitlements ({catalog_skipped} cached, skipping API) — {CATALOG_WORKERS} workers...', flush=True)
        else:
    ...
    if playtime_by_artifact:
        for rec in apps:
            ...
            nm = ((item or {}).get('title') or rec.get('sandboxName') or rec.get('appName') or str(rec.get('catalogItemId'))).strip().lower()
    ...
    for i, rec in enumerate(apps_sorted, 1):
        ...
        if not args.refresh and row_id in existing:
            ...
            may_reuse = args.skip_hltb or (cached_early.get('hltb_main_hours') is not None and cached_early.get('library_image'))
    ...
    for g in games_out:
        ...
        deduped.append(g)
    games_out = deduped
    drift_exit = refuse_drift_result(catalog_game_count(games_out), label='Epic library rows', allow_drift=args.allow_drift, output_path=GAMES_EPIC_JSON)
    if drift_exit is not None:
        return stats.finish('fetch_epic', t0, exit_code=drift_exit)
    ...
    payload = {'fetched_at': datetime.now(UTC).isoformat(), 'store': 'epic', 'game_count': catalog_game_count(games_out), 'entitlement_set_hash': set_hash, 'games': sorted(games_out, key=lambda g: g['name'].lower())}
    write_catalog_text(GAMES_EPIC_JSON, json.dumps(payload, indent=2, ensure_ascii=False))
    print(f'\nWrote {catalog_game_count(games_out)} games to {GAMES_EPIC_JSON} (skipped {skipped}, tagged {tagged_non_game} library noise, collapsed {collapsed} duplicate entitlements).', flush=True)
