...
from auth.epic_wishlist_session import cloudflare_interstitial, enrich_wishlist_elements_with_catalog, extract_wishlist_payloads_from_html, graphql_debug_entry, is_epic_graphql_url, storefront_auth_blocked, storefront_auth_error_message, storefront_signed_out, wishlist_capture_complete_from_html, wishlist_graphql_ok
...
def _price_fields(offer: dict | None) -> dict:
    ...
    if final_cents == 0 and (not price_str):
        price_str = 'Free'
    return {'price': price_str, 'price_initial': price_initial_str, 'discount_percent': discount_pct, 'currency': currency}
...
def _build_row(element: dict, hltb: dict | None) -> dict | None:
    ...
    row = {'store': 'wishlist', 'wishlist_store': 'epic', 'id': f'epic-{namespace}:{offer_id}', 'epic_namespace': namespace, 'epic_offer_id': offer_id, 'name': name, 'wishlist_added': element.get('created'), 'playtime_minutes': 0, 'last_played': None, 'header_image': header, 'library_image': library, 'release_date': _release(offer), 'genres': _genres(offer), 'tags': [], 'steam_review_percent': None, 'steam_review_count': None, 'steam_review_desc': None, 'hltb_main_hours': (hltb or {}).get('hltb_main_hours'), 'hltb_main_extra_hours': (hltb or {}).get('hltb_main_extra_hours'), 'hltb_completionist_hours': (hltb or {}).get('hltb_completionist_hours'), 'hltb_match_confidence': (hltb or {}).get('hltb_match_confidence'), 'hltb_name': (hltb or {}).get('hltb_name'), 'store_url': _store_url(offer, name), 'type': 'game'}
...
_CDP_TRANSPORT_MSG = ('cdp command timed out', 'websocket', 'socket', 'connection aborted', 'debugging endpoint', 'browser', '10053', 'established connection was aborted')
_CDP_TRANSPORT_TYPES = ('websocketconnectionclosedexception', 'connectionabortederror', 'connectionreseterror', 'brokenpipeerror')
...
def _drain_wishlist_candidates(candidates: list[Any], seen: list[dict[str, Any]] | None=None) -> list[Any]:
    found: list[Any] = []
    while candidates:
...
def _fetch_with_profile_once(*, dump: bool=False, timeout_s: int=45) -> tuple[str, str, list[Any]]:
    ...
    if not profile.exists():
        raise RuntimeError("No saved Epic wishlist profile at cache/auth/profiles/epic_wishlist. Open the Connections page and connect 'Epic (wishlist)' first.")
    ...
    with launch_persistent_profile(str(profile), headless=False, window_position=(-32000, 0), window_size=(1280, 900)) as ctx:
        ...
        if dump:
            ...
            dump_json().write_text(json.dumps({'url': url, 'api_payload_count': len(api_payloads), 'api_payloads': api_payloads[:20], 'graphql_seen': seen_graphql[:50], 'element_count': len(parse_wishlist_sources(html, api_payloads))}, indent=2, default=str, ensure_ascii=False), encoding='utf-8')
...
def main() -> int:
    ...
    try:
        html, url, api_payloads = run_with_heartbeat(lambda: _fetch_with_profile(dump=args.dump), 'Epic wishlist capture')
    except Exception as exc:
        msg = str(exc)
    ...
    empty_exit = refuse_empty_result(elements, label='Epic wishlist', allow_empty=args.allow_empty, output_path=GAMES_WISHLIST_EPIC_JSON)
    if empty_exit is not None:
        return stats.finish('fetch_epic_wishlist', t0, exit_code=empty_exit)
    drift_exit = refuse_drift_result(elements, label='Epic wishlist', allow_drift=args.allow_drift, output_path=GAMES_WISHLIST_EPIC_JSON)
    ...
    for i, el in enumerate(elements, 1):
        ...
        if row is None:
            continue
        rows.append(row)
    payload = {'fetched_at': datetime.now(UTC).isoformat(), 'store': 'wishlist_epic', 'game_count': len(rows), 'games': sorted(rows, key=lambda g: (g.get('name') or '').lower())}
