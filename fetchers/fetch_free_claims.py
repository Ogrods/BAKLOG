...
def main() -> int:
    ...
    parser.add_argument('--url', default=os.environ.get('BAKLOG_CLAIMS_URL', DEFAULT_URL), help='Hosted feed URL (default: BAKLOG_CLAIMS_URL or baklog.app)')
    parser.add_argument('--dry-run', action='store_true', help='Validate only; do not write')
    parser.add_argument('--allow-empty', action='store_true', help='Allow writing an empty claims file (e.g. genuinely no live giveaways).')
    ...
    if valid == 0 and (not args.allow_empty):
        stats.error('feed produced 0 valid claim(s) (need id, store, and valid claim link(s)) — refusing to overwrite. Re-run with --allow-empty if there are genuinely no live giveaways.')
        return stats.finish('fetch_free_claims', t0, exit_code=2)
    payload = {'fetched_at': datetime.now(UTC).isoformat(), 'source_url': args.url, 'generated_at': data.get('generated_at'), 'items': valid_items}
