...
from fetchers._base import STEAM_CREDENTIALS_HINT, STEAM_PRIVATE_PROFILE_HINT, add_allow_empty_arg, add_no_carry_arg, apply_carry_forward, catalog_file, configure_stdout, refuse_drift_result, refuse_empty_result, row_key_by_appid, write_catalog_text
...
def _build_game_row(owned: dict, details: dict | None, reviews: dict | None, hltb: dict | None) -> dict | None:
    appid = owned['appid']
    name = owned.get('name') or (details or {}).get('name', f'App {appid}')
    if details is None:
        return _finalize_steam_row({'appid': appid, 'name': name, 'playtime_minutes': owned.get('playtime_forever', 0), 'last_played': owned.get('rtime_last_played', 0) or None, 'header_image': f'https://cdn.akamai.steamstatic.com/steam/apps/{appid}/header.jpg', 'library_image': f'https://cdn.akamai.steamstatic.com/steam/apps/{appid}/library_600x900.jpg', 'release_date': None, 'genres': [], 'tags': [], 'steam_review_percent': None, 'steam_review_count': None, 'steam_review_desc': None, 'hltb_main_hours': None, 'hltb_main_extra_hours': None, 'hltb_completionist_hours': None, 'hltb_match_confidence': None, 'hltb_name': None, 'store_url': f'https://store.steampowered.com/app/{appid}', 'type': 'unknown', 'price': None, 'price_initial': None, 'discount_percent': None, 'currency': None})
    ...
    row = {'appid': appid, 'name': details.get('name', name), 'playtime_minutes': owned.get('playtime_forever', 0), 'last_played': owned.get('rtime_last_played', 0) or None, 'header_image': details.get('header_image') or f'https://cdn.akamai.steamstatic.com/steam/apps/{appid}/header.jpg', 'library_image': f'https://cdn.akamai.steamstatic.com/steam/apps/{appid}/library_600x900.jpg', 'release_date': _parse_release_date(details), 'genres': genres, 'tags': tags, 'coop_online': coop_online, 'coop_local': coop_local, 'steam_review_percent': (reviews or {}).get('percent_positive'), 'steam_review_count': (reviews or {}).get('total_reviews'), 'steam_review_desc': (reviews or {}).get('review_score_desc'), 'hltb_main_hours': None, 'hltb_main_extra_hours': None, 'hltb_completionist_hours': None, 'hltb_match_confidence': None, 'hltb_name': None, 'store_url': f'https://store.steampowered.com/app/{appid}', 'type': details.get('type', 'game'), 'price': price.get('final_formatted'), 'price_initial': price.get('initial_formatted'), 'discount_percent': price.get('discount_percent'), 'currency': price.get('currency')}
    meta = enrichment_from_appdetails(details)
    row.update({'metacritic_score': meta['metacritic_score'], 'developers': meta['developers'], 'publishers': meta['publishers'], 'controller_support': meta['controller_support'], 'early_access': meta['early_access']})
    if hltb:
...
def _row_from_cached_catalog(owned: dict, cached_row: dict) -> dict:
    return _finalize_steam_row({**cached_row, 'playtime_minutes': owned.get('playtime_forever', cached_row.get('playtime_minutes', 0)), 'last_played': owned.get('rtime_last_played', 0) or cached_row.get('last_played')})

def _fetch_store_data(steam: SteamClient, appid: int, *, refresh: bool, cached_row: dict | None, skip_reviews: bool=False) -> tuple[dict | None, dict | None]:
    ...
    try:
        ...
        if not skip_reviews:
            ...
            reviews = {'percent_positive': cached_row.get('steam_review_percent'), 'total_reviews': cached_row.get('steam_review_count'), 'review_score_desc': cached_row.get('steam_review_desc')}
        ...
        if cached_row:
            ...
            if cached_row.get('type') == 'game' or cached_row.get('genres'):
                details_data = {'type': cached_row.get('type', 'game'), 'name': cached_row.get('name'), 'genres': [{'description': g} for g in cached_row.get('genres', [])], 'categories': [{'description': t} for t in cached_row.get('tags', [])], 'header_image': cached_row.get('header_image'), 'release_date': {'date': cached_row.get('release_date')} if cached_row.get('release_date') else {}}
        else:
            reviews = None
...
def main() -> int:
    ...
    if not args.appid:
        ...
        empty_exit = refuse_empty_result(owned_games, label='Steam owned-games API', allow_empty=args.allow_empty, output_path=GAMES_STEAM_JSON)
    ...
    for i, owned in enumerate(owned_games, 1):
        ...
        if use_cached_only:
            ...
            details_data, reviews = _fetch_store_data(steam, appid, refresh=args.refresh, cached_row=cached_row, skip_reviews=args.only_new and (not args.refresh) and (not args.appid))
        hltb = None
        ...
        games_out.append(row)
        loop_hb.tick_progress(i, len(owned_games), 'Steam library', name[:40])
    empty_exit = refuse_empty_result(games_out, label='Steam library rows', allow_empty=args.allow_empty, output_path=GAMES_STEAM_JSON)
    if empty_exit is not None:
        return stats.finish('fetch_games', t0, exit_code=empty_exit)
    drift_exit = refuse_drift_result(games_out, label='Steam library rows', allow_drift=args.allow_drift, output_path=GAMES_STEAM_JSON)
    ...
    payload = {'fetched_at': datetime.now(UTC).isoformat(), 'steam_id': steam_id, 'game_count': len(games_out), 'games': sorted(games_out, key=lambda g: g['name'].lower())}
