...
from fetchers._base import add_allow_empty_arg, add_no_carry_arg, apply_carry_forward, carry_enrichment, catalog_file, configure_stdout, merge_cached_row, refuse_empty_result, write_catalog_text
...
def _build_game_row(product: dict, details: dict | None, hltb: dict | None) -> dict | None:
    ...
    image = product.get('image') or product.get('img') or (details or {}).get('backgroundImage') or (details or {}).get('image')
    ...
    if isinstance(price_block, dict):
        ...
        currency = None
        discount = None
    row = {'store': 'gog', 'id': gog_id, 'gog_id': gog_id, 'name': name, 'playtime_minutes': _playtime_minutes(details), 'last_played': None, 'header_image': header_url, 'library_image': library_url, 'release_date': release, 'genres': _extract_genres(product, details), 'tags': [], 'steam_review_percent': None, 'steam_review_count': None, 'steam_review_desc': None, 'hltb_main_hours': None, 'hltb_main_extra_hours': None, 'hltb_completionist_hours': None, 'hltb_match_confidence': None, 'hltb_name': None, 'store_url': _normalize_gog_store_url(product, gog_id), 'type': 'game', 'price': price_str, 'price_initial': None, 'discount_percent': discount, 'currency': currency}
    if hltb:
...
def merge_gog_cached_row(fresh: dict, cached: dict | None, *, source: str, hltb_updated: bool=False) -> dict:
...
def merge_gog_sources(current_rows: list[dict], carried_rows: list[dict], current_source: str) -> list[dict]:
...
def refuse_gog_source_drift(new_same_source_count: int, *, source: str, allow_drift: bool, output_path: Path, threshold: float=0.5) -> int | None:
    if allow_drift:
        return None
    ...
    print(f'ERROR: GOG {source} slice drift refused — new={new_same_source_count}, previous={prev}, floor={floor} (≥{int(threshold * 100)}% of prior), ratio≈{pct:.0f}%.\nReason: same-source row count fell below the drift guard (broken auth/API, or a legitimate library shrink from filtering). Re-run with --allow-drift if the drop is expected.', file=sys.stderr, flush=True)
    return 3
...
def _build_game_row_from_local(rec: dict, hltb: dict | None, source: str) -> dict:
    ...
    row = {'store': 'gog', 'id': gog_id, 'gog_id': gog_id, 'name': rec['name'], 'source': source, 'playtime_minutes': 0, 'last_played': rec.get('last_played'), 'header_image': header_url, 'library_image': library_url, 'release_date': rec.get('release_date'), 'genres': rec.get('genres') or [], 'tags': [], 'steam_review_percent': None, 'steam_review_count': None, 'steam_review_desc': None, 'hltb_main_hours': None, 'hltb_main_extra_hours': None, 'hltb_completionist_hours': None, 'hltb_match_confidence': None, 'hltb_name': None, 'store_url': rec.get('store_url') or f'https://www.gog.com/en/game/{gog_id}', 'type': 'game', 'price': None, 'price_initial': None, 'discount_percent': None, 'currency': None}
    if hltb:
        row.update({'hltb_main_hours': hltb.get('hltb_main_hours'), 'hltb_main_extra_hours': hltb.get('hltb_main_extra_hours'), 'hltb_completionist_hours': hltb.get('hltb_completionist_hours'), 'hltb_match_confidence': hltb.get('hltb_match_confidence'), 'hltb_name': hltb.get('hltb_name')})
    maybe_tag_library_noise_row(row, 'gog')
    return row
...
def main() -> int:
    ...
    parser.add_argument('--source', choices=('auto', 'web', 'local'), default=os.getenv('GOG_SOURCE', 'auto').strip().lower() or 'auto', help='auto: Galaxy DB when present, else web cookie (default: auto)')
    parser.add_argument('--db-path', type=Path, default=None, help='Override GOG Galaxy galaxy-2.0.db path (local source only)')
    ...
    if source == 'local':
        try:
            ...
            stats.error(str(e))
            return stats.finish('fetch_gog', t0, exit_code=1)
        empty_exit = refuse_empty_result(records, label='GOG Galaxy library', allow_empty=args.allow_empty, output_path=GAMES_GOG_JSON)
        ...
        for i, rec in enumerate(records, 1):
            ...
            current_rows.append(merge_gog_cached_row(_build_game_row_from_local(rec, hltb, source), cached, source=source, hltb_updated=hltb_updated))
    else:
        ...
        empty_exit = refuse_empty_result(products, label='GOG library', allow_empty=args.allow_empty, output_path=GAMES_GOG_JSON)
        if empty_exit is not None:
            return stats.finish('fetch_gog', t0, exit_code=empty_exit)
        if args.gog_id:
        ...
        for i, product in enumerate(products, 1):
            ...
            if row is None:
                skipped += 1
                continue
            current_rows.append(merge_gog_cached_row(_tag_web_row(row), cached_row, source=source, hltb_updated=hltb_updated))
        if skipped:
    ...
    drift_exit = refuse_gog_source_drift(len(current_rows), source=source, allow_drift=args.allow_drift, output_path=GAMES_GOG_JSON)
    ...
    if len(games_out) < before_merge:
        print(f'Post-merge: dropped {before_merge - len(games_out)} pack/promo duplicate(s) across mixed gog_id sources.', flush=True)
    ...
    payload = {'fetched_at': datetime.now(UTC).isoformat(), 'store': 'gog', 'source': source, 'game_count': playable, 'games': sorted(games_out, key=lambda g: g['name'].lower())}
