...
def _item_from_lookup(obj: dict) -> WishlistItem | None:
    ...
    return WishlistItem(product_id=machine, title=title, image_url=image, store_url=store_url, price=price, price_initial=price_initial, discount_percent=discount, currency=currency)
...
def _fetch_wishlist(*, dump: bool=False) -> tuple[list[WishlistItem], bool]:
    ...
    with launch_persistent_profile(str(profile), headless=False, window_position=_WL_WINDOW_POS, window_size=_WL_WINDOW_SIZE) as ctx:
        ...
        if dump:
            ...
            dump_json().write_text(json.dumps({'slug_count': len(slugs), 'slugs': slugs, 'result_count': len(results), 'results': results[:20]}, indent=2, default=str, ensure_ascii=False), encoding='utf-8')
            print(f'  wrote {dump_json()}', flush=True)
        return (items, False)

def _build_row(item: WishlistItem, hltb: dict | None) -> dict:
    return {'store': 'wishlist', 'wishlist_store': 'humble', 'id': f'humble-{item.product_id}', 'humble_product_id': item.product_id, 'name': item.title, 'playtime_minutes': 0, 'last_played': None, 'header_image': item.image_url, 'library_image': item.image_url, 'release_date': None, 'genres': [], 'tags': [], 'steam_review_percent': None, 'steam_review_count': None, 'steam_review_desc': None, 'hltb_main_hours': (hltb or {}).get('hltb_main_hours'), 'hltb_main_extra_hours': (hltb or {}).get('hltb_main_extra_hours'), 'hltb_completionist_hours': (hltb or {}).get('hltb_completionist_hours'), 'hltb_match_confidence': (hltb or {}).get('hltb_match_confidence'), 'hltb_name': (hltb or {}).get('hltb_name'), 'store_url': item.store_url, 'type': 'game', 'price': item.price, 'price_initial': item.price_initial, 'discount_percent': item.discount_percent, 'currency': item.currency}
...
def main() -> int:
    ...
    empty_exit = refuse_empty_result(items, label='Humble wishlist', allow_empty=args.allow_empty, output_path=GAMES_HUMBLE_WISHLIST_JSON)
    if empty_exit is not None:
        return stats.finish('fetch_humble_wishlist', t0, exit_code=empty_exit)
    drift_exit = refuse_drift_result(items, label='Humble wishlist', allow_drift=args.allow_drift, output_path=GAMES_HUMBLE_WISHLIST_JSON)
    ...
    payload = {'fetched_at': datetime.now(UTC).isoformat(), 'store': 'wishlist_humble', 'game_count': len(rows), 'games': sorted(rows, key=lambda g: (g.get('name') or '').lower())}
