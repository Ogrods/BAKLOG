...
LIBRARY_FILES = ['games_steam.json', 'games_gog.json', 'games_psn.json', 'games_epic.json', 'games_amazon.json', 'games_nintendo.json']
WISHLIST_FILE = 'games_wishlist.json'

def _collect_titles(include_library: bool, *, library_stores: set[str] | None=None, include_wishlist: bool=True) -> list[tuple[str, str]]:
    seen: set[str] = set()
...
def main() -> int:
    ...
    parser.add_argument('--include-library', action='store_true', help='Also look up every owned game (slow; default is wishlist only).')
    parser.add_argument('--stores', nargs='+', metavar='STORE', help='With --include-library, only price these library stores (e.g. nintendo).')
    parser.add_argument('--skip-wishlist', action='store_true', help='Skip Steam wishlist titles (library-only ITAD run).')
    ...
    titles = _collect_titles(include_library=args.include_library, library_stores=set(args.stores) if args.stores else None, include_wishlist=not args.skip_wishlist)
    if args.limit:
    ...
    if not titles:
        if args.include_library:
            stats.warn('No wishlist or library titles found to price yet. Add games to a store wishlist (or import a library), then run ITAD prices again.')
        else:
            stats.warn('Your wishlist is empty, so there is nothing for ITAD to price. ITAD tracks price drops on wishlist titles - add games to a store wishlist and re-fetch it, then run ITAD prices again. To also track prices for games you own, run with --include-library.')
    ...
    try:
        ...
        empty_exit = refuse_empty_result(plain_by_key, label='ITAD price resolution', allow_empty=args.allow_empty, output_path=ITAD_JSON)
    ...
    payload = {'fetched_at': datetime.now(UTC).isoformat(), 'country': args.country, 'currency': country_to_currency(args.country), 'count': len(merged_by_key), 'by_key': merged_by_key}
    ...
    if fx_rows:
        print(f'FX: converted {fx_rows} wishlist row(s) across {fx_files} catalog(s) to {country_to_currency(args.country)}.', flush=True)
    stats.ok = len(by_key)
