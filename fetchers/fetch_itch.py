...
from fetchers._base import add_allow_empty_arg, add_dry_run_arg, add_hltb_args, add_no_carry_arg, apply_carry_forward, carry_enrichment, catalog_file, configure_stdout, load_existing_games, merge_cached_row, print_id_diff, refuse_empty_result, write_games_json
...
FETCHER_AUTHORITATIVE = frozenset({'store', 'id', 'itch_id', 'name', 'header_image', 'library_image', 'release_date', 'genres', 'store_url', 'type', 'price', 'price_initial', 'discount_percent', 'currency', 'publisher', 'short_text', 'classification', 'min_price', 'in_press_system', 'download_key_id', 'purchase_id', 'source', 'playtime_minutes', 'last_played'})
...
def _release_date(game: dict) -> str | None:
    for key in ('published_at', 'created_at'):
        ...
        if isinstance(raw, str) and raw:
            return raw[:10]
    return None
_ITCH_NOISE_GENRES = {'default', 'html', 'html5', 'flash', 'java', 'unity', 'godot', 'physical_game', 'physical game', 'assets', 'asset_pack', 'asset pack', 'tool', 'book', 'comic', 'soundtrack', 'other', 'game'}
...
def _build_row(entry: dict, hltb: dict | None) -> dict | None:
    ...
    row = {'store': 'itch', 'id': int(gid), 'itch_id': int(gid), 'name': (game.get('title') or 'Untitled').strip(), 'playtime_minutes': 0, 'last_played': None, 'header_image': cover, 'library_image': cover, 'release_date': _release_date(game), 'genres': _genres(game), 'tags': [], 'steam_review_percent': None, 'steam_review_count': None, 'steam_review_desc': None, 'hltb_main_hours': None, 'hltb_main_extra_hours': None, 'hltb_completionist_hours': None, 'hltb_match_confidence': None, 'hltb_name': None, 'store_url': game.get('url'), 'type': 'game', 'price': None, 'price_initial': None, 'discount_percent': None, 'currency': None, 'publisher': user.get('username') or user.get('display_name'), 'short_text': game.get('short_text'), 'classification': game.get('classification'), 'min_price': game.get('min_price'), 'in_press_system': bool(game.get('in_press_system')), 'download_key_id': entry.get('id'), 'purchase_id': entry.get('purchase_id')}
    if hltb:
...
def merge_itch_sources(current_rows: list[dict], carried_rows: list[dict], current_source: str) -> list[dict]:
...
def refuse_itch_source_drift(new_same_source_count: int, *, source: str, allow_drift: bool, output_path: Path, threshold: float=0.5) -> int | None:
    if allow_drift:
        return None
    ...
    print(f'ERROR: itch.io {source} slice drift refused — new={new_same_source_count}, previous={prev}, floor={floor} (≥{int(threshold * 100)}% of prior), ratio≈{pct:.0f}%.\nReason: same-source row count fell below the drift guard. Re-run with --allow-drift if the drop is expected.', file=sys.stderr, flush=True)
    return 3
...
def resolve_source(requested: str, db_path: Path | None) -> str:
    ...
    if _api_creds_ready():
        return 'api'
    raise RuntimeError('itch app database not found and no ITCH_API_KEY saved.\nInstall/sign in to the itch desktop app, or paste an API key on the Connections page.')

def _build_row_from_local(rec: dict, hltb: dict | None, source: str) -> dict:
    gid = int(rec['itch_id'])
    row = {'store': 'itch', 'id': gid, 'itch_id': gid, 'name': rec['name'], 'source': source, 'playtime_minutes': rec.get('playtime_minutes') or 0, 'last_played': rec.get('last_played'), 'header_image': rec.get('header_image'), 'library_image': rec.get('library_image'), 'release_date': rec.get('release_date'), 'genres': _genres({'tags': [], 'classification': rec.get('classification')}), 'tags': [], 'steam_review_percent': None, 'steam_review_count': None, 'steam_review_desc': None, 'hltb_main_hours': None, 'hltb_main_extra_hours': None, 'hltb_completionist_hours': None, 'hltb_match_confidence': None, 'hltb_name': None, 'store_url': rec.get('store_url'), 'type': 'game', 'price': None, 'price_initial': None, 'discount_percent': None, 'currency': None, 'publisher': None, 'short_text': rec.get('short_text'), 'classification': rec.get('classification'), 'min_price': rec.get('min_price'), 'in_press_system': rec.get('in_press_system'), 'download_key_id': rec.get('download_key_id'), 'purchase_id': None}
    if hltb:
        row.update({'hltb_main_hours': hltb.get('hltb_main_hours'), 'hltb_main_extra_hours': hltb.get('hltb_main_extra_hours'), 'hltb_completionist_hours': hltb.get('hltb_completionist_hours'), 'hltb_match_confidence': hltb.get('hltb_match_confidence'), 'hltb_name': hltb.get('hltb_name')})
    return row
...
def main() -> int:
    ...
    parser.add_argument('--source', choices=('auto', 'api', 'local'), default=os.getenv('ITCH_SOURCE', 'auto').strip().lower() or 'auto', help='auto: butler.db when present, else API key (default: auto)')
    ...
    parser.add_argument('--min-price', type=int, default=None, help='Skip games whose listed min_price is below this (in cents). Useful to drop free jam games.')
    add_dry_run_arg(parser)
    add_allow_empty_arg(parser)
    add_no_carry_arg(parser)
    parser.add_argument('--games-only', action='store_true', help='Only write rows classified as videogames (tools/soundtracks/etc. omitted).')
    parser.add_argument('--enrich-details', action='store_true', help='Fetch full game/<id> docs (API source only) to merge description and tags.')
    ...
    if source == 'local':
        try:
            ...
            stats.error(str(e))
            return stats.finish('fetch_itch', t0, exit_code=1)
        empty_exit = refuse_empty_result(records, label='itch.io local library', allow_empty=args.allow_empty, output_path=GAMES_ITCH_JSON)
        ...
        for i, rec in enumerate(filtered, 1):
            ...
            current_rows.append(merge_cached_row(_build_row_from_local(rec, hltb, source), cached, authoritative=FETCHER_AUTHORITATIVE, hltb_updated=hltb_updated))
        ...
        try:
            ...
            return stats.finish('fetch_itch', t0, exit_code=1)
        empty_exit = refuse_empty_result(keys, label='itch.io API library', allow_empty=args.allow_empty, output_path=GAMES_ITCH_JSON)
        if empty_exit is not None:
            return stats.finish('fetch_itch', t0, exit_code=empty_exit)
        ...
        for i, entry in enumerate(filtered_entries, 1):
            ...
            if row:
                current_rows.append(merge_cached_row(_tag_api_row(row), cached, authoritative=FETCHER_AUTHORITATIVE, hltb_updated=hltb_updated))
    ...
    preserved_enrichment = sum((1 for g in games_out if existing.get(str(g['id'])) and (g.get('steam_review_percent') is not None or g.get('hltb_main_hours') is not None)))
    ...
    drift_exit = refuse_itch_source_drift(len(current_rows), source=source, allow_drift=args.allow_drift, output_path=GAMES_ITCH_JSON)
