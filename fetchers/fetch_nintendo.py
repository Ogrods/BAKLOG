...
from clients.nintendo_vgc import NintendoVgcAuthError, NintendoVgcCaptureError, NintendoVgcClient, resolve_nintendo_icon_url
from fetchers._authoritative import NINTENDO
from fetchers._base import LAST_SEEN_FIELD, STALE_FIELD, STALE_SINCE_FIELD, add_allow_empty_arg, add_no_carry_arg, add_only_new_arg, catalog_file, configure_stdout, merge_cached_row, refuse_empty_result, row_key_by_id, write_catalog_text
from fetchers._progress import EXIT_CODE_AUTH, RunStats, run_with_heartbeat, started
from fetchers.nintendo_hybrid import finalize_nintendo_library_rows, find_existing_row, index_existing_rows, is_nintendo_playable_game, match_nintendo_title_key, merge_vgc_with_transactions, nintendo_store_url, norm_nintendo_title
    ...
    for tx in transactions:
        ...
        if key not in by_title:
            ...
            if ctype in SKIP_CONTENT_TYPES and 'noise' not in tags:
                tags.append('noise')
            by_title[key] = {'name': name, 'id': str(tid), 'nintendo_id': str(tid), 'purchase_date': date, 'device_type': tx.get('device_type'), 'content_type': tx.get('content_type'), 'tags': tags}
            continue
        row = by_title[key]
...
def _nintendo_drift_baseline(output_path: Path) -> int | None:
    ...
    if not isinstance(games, list):
        return None
    non_legacy = sum((1 for row in games if isinstance(row, dict) and (not row.get(NINTENDO_LEGACY_FIELD)) and (not row.get(STALE_FIELD))))
    if non_legacy > 0:
        return non_legacy
...
def refuse_nintendo_source_drift(items: list[dict], *, label: str, allow_drift: bool, output_path: Path) -> int | None:
    new_count = len(items)
    ...
    print(f'ERROR: {label} returned {new_count} items{where}, but the previous fresh slice had {prev} (≈{pct:.0f}% — under the {int(NINTENDO_DRIFT_THRESHOLD * 100)}% floor).\nLikely a broken auth or upstream API. If this drop is real, re-run with --allow-drift.', file=sys.stderr, flush=True)
    return 3

def carry_forward_nintendo_legacy(fresh_rows: list[dict], existing_rows: list[dict], *, dropped_ids: set[str], key_fn, now_iso: str | None=None) -> list[dict]:
...
def repair_nintendo_stale_catalog(games: list[dict], *, dropped_ids: set[str] | None=None) -> tuple[list[dict], int]:
...
def maybe_repair_nintendo_catalog_on_disk(dropped_ids: set[str] | None=None) -> int:
    ...
    if not isinstance(payload.get('fresh_count'), int):
        payload['fresh_count'] = catalog_game_count([row for row in games if isinstance(row, dict) and (not row.get(NINTENDO_LEGACY_FIELD))])
...
def maybe_backfill_nintendo_catalog_meta() -> bool:
    ...
    computed_fresh = catalog_game_count([row for row in games if isinstance(row, dict) and (not row.get(NINTENDO_LEGACY_FIELD))])
    ...
    if not needs_write:
        return False
    write_catalog_text(GAMES_NINTENDO_JSON, json.dumps(payload, indent=2, ensure_ascii=False))
    print(f"  Backfilled Nintendo catalog meta (fresh_count={payload['fresh_count']}, game_count={payload['game_count']}).", flush=True)
    return True
...
def _apply_vgc_metadata(row: dict, item: dict) -> None:
    ...
    for src, dst in (('has_application', 'nintendo_has_application'), ('has_addon_contents', 'nintendo_has_addon_contents'), ('has_upgrade', 'nintendo_has_upgrade'), ('has_nx_application', 'nintendo_has_nx_application'), ('has_nx_addon_contents', 'nintendo_has_nx_addon_contents'), ('has_ounce_application', 'nintendo_has_ounce_application'), ('has_ounce_addon_contents', 'nintendo_has_ounce_addon_contents')):
        if item.get(src):
            row[dst] = True
...
def _build_row(item: dict, hltb: dict | None) -> dict:
    ...
    icon_sizes = item.get('icon_sizes')
    header_icon = resolve_nintendo_icon_url(item.get('icon_url') or item.get('icon_url_standard'), icon_sizes, prefer_large=True)
    library_icon = resolve_nintendo_icon_url(item.get('icon_url_standard') or item.get('icon_url'), icon_sizes, prefer_large=False)
    row = {'store': 'nintendo', 'id': row_id, 'nintendo_id': str(nid) if nid else None, 'name': name, 'playtime_minutes': None, 'last_played': None, 'header_image': header_icon, 'library_image': library_icon, 'release_date': item.get('purchase_date'), 'genres': [], 'tags': list(item.get('tags') or []), 'steam_review_percent': None, 'steam_review_count': None, 'steam_review_desc': None, 'hltb_main_hours': None, 'hltb_main_extra_hours': None, 'hltb_completionist_hours': None, 'hltb_match_confidence': None, 'hltb_name': None, 'store_url': nintendo_store_url(str(app_id) if app_id else None, name), 'type': 'game', 'price': None, 'price_initial': None, 'discount_percent': None, 'currency': None, 'nintendo_platform': item.get('nintendo_platform') or item.get('device_type')}
    if app_id:
...
def main() -> int:
    ...
    parser.add_argument('--skip-vgc', action='store_true', help='Use eShop transactions only (skip Virtual Game Cards entitlements)')
    ...
    parser.add_argument('--headed', action='store_true', help='Open the saved browser profile visibly (debug capture issues)')
    ...
    if not _nintendo_connected():
        stats.error('Nintendo is not connected. Open Connections → Nintendo → Connect and sign in at ec.nintendo.com/my/transactions/ (saved browser profile required).')
    ...
    if args.skip_vgc:
        ...
        try:
            vgc_rows = run_with_heartbeat(lambda: NintendoVgcClient(profile_path=prof, headless=not args.headed).fetch_all_cards(), 'Nintendo VGC')
    ...
    if vgc_rows_n:
        print(f'VGC-primary library: {vgc_rows_n} entitlement(s), {len(merged)} row(s) after merging receipts.', flush=True)
    else:
        print(f'Hybrid library slice: {len(merged)} title(s).', flush=True)
    empty_exit = refuse_empty_result(merged, label='Nintendo library', allow_empty=args.allow_empty, output_path=GAMES_NINTENDO_JSON)
    if empty_exit is not None:
        stats.error(f'No games found. Check {NINTENDO_RAW_DUMP} — session may be valid but account has no entitlements or eShop purchases in range.')
    ...
    for i, item in enumerate(merged, 1):
        cached = find_existing_row(item, existing=existing, by_title=existing_by_title, by_app_id=existing_by_app_id, by_nintendo_id=existing_by_nintendo_id)
        ...
        games_out.append(merge_cached_row(_build_row(item, hltb), cached, authoritative=NINTENDO, hltb_updated=hltb_updated))
    drift_exit = refuse_nintendo_source_drift(catalog_game_count(games_out), label='Nintendo library rows', allow_drift=args.allow_drift, output_path=GAMES_NINTENDO_JSON)
    ...
    if args.no_carry:
        ...
        final_games = carry_forward_nintendo_legacy(games_out, list(existing.values()), dropped_ids=dropped_ids, key_fn=row_key_by_id)
    if dropped_ids:
        final_games = [row for row in final_games if row_key_by_id(row) not in dropped_ids]
    payload = {'fetched_at': datetime.now(UTC).isoformat(), 'store': 'nintendo', 'fresh_count': fresh_count, 'game_count': catalog_game_count(final_games), 'note': 'Primary: Virtual Game Cards (entitlements, icons, platform flags, publisher). Secondary: eShop transaction receipts for purchase dates (~2 year window). No cartridge-only games. Older purchases kept as nintendo_legacy.', 'games': sorted(final_games, key=lambda g: g['name'].lower())}
    write_catalog_text(GAMES_NINTENDO_JSON, json.dumps(payload, indent=2, ensure_ascii=False))
