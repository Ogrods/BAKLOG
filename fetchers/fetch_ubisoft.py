...
def _extract_records(payload) -> list[dict]:
    ...
    for game in catalog:
        ...
        key = name.lower()
        if key not in seen:
            seen[key] = {'name': name, 'spaceId': str(sid), 'id': str(sid), 'last_played': last_by_space.get(str(sid)), 'displayName': name}
...
def _build_row(item: dict, hltb: dict | None) -> dict:
    ...
    row = {'store': 'ubisoft', 'id': uid, 'ubisoft_id': uid, 'name': name, 'playtime_minutes': None, 'last_played': _last_played_iso(item), 'header_image': image, 'library_image': image, 'release_date': None, 'genres': [], 'tags': [], 'steam_review_percent': None, 'steam_review_count': None, 'steam_review_desc': None, 'hltb_main_hours': None, 'hltb_main_extra_hours': None, 'hltb_completionist_hours': None, 'hltb_match_confidence': None, 'hltb_name': None, 'store_url': _store_url(item, name), 'type': 'game', 'price': None, 'price_initial': None, 'discount_percent': None, 'currency': None}
    if hltb:
...
def main() -> int:
    ...
    parser.add_argument('--dump-raw', action='store_true', help=f'Also write the raw API response to {UBISOFT_RAW_DUMP} for debugging.')
    ...
    empty_exit = refuse_empty_result(deduped, label='Ubisoft library', allow_empty=args.allow_empty, output_path=GAMES_UBISOFT_JSON)
    if empty_exit is not None:
        stats.error(f'No game records found in the response. Re-run with --dump-raw and inspect {UBISOFT_RAW_DUMP} to confirm the endpoint hit your library.')
    ...
    for i, item in enumerate(deduped, 1):
        ...
        games_out.append(merge_cached_row(_build_row(item, hltb), cached, authoritative=UBISOFT, hltb_updated=hltb_updated))
        loop_hb.tick_progress(i, len(deduped), 'Ubisoft library', (name or '')[:40])
    drift_exit = refuse_drift_result(games_out, label='Ubisoft library rows', allow_drift=args.allow_drift, output_path=GAMES_UBISOFT_JSON)
    ...
    payload = {'fetched_at': datetime.now(UTC).isoformat(), 'store': 'ubisoft', 'endpoint': endpoint, 'game_count': len(games_out), 'games': sorted(games_out, key=lambda g: g['name'].lower())}
