...
_TILE_OPEN_RE = re.compile(
    '<div\\b[^>]*\\bclass="[^"]*\\bproduct-tile\\b[^"]*\\bwishlist-product-tile\\b[^"]*"[^>]*>', re.IGNORECASE
)
...
_EMPTY_PHRASE = re.compile(
    "Log in to access your wishlist|Your wishlist is empty|Looking for new games to add", re.IGNORECASE
)
_GAME_EDITION_RE = re.compile(
    "\\b(edition|standard|resynced|definitive|anniversary|remaster|remastered|reckoning|year\\s*\\d+|gold|deluxe|ultimate|elite|premium|complete)\\b",
    re.IGNORECASE,
)
...
def _fetch_wishlist_html(timeout_s=45):
    from auth.cdp_browser import launch_persistent_profile

    profile = profile_dir("ubisoft")
    if not profile.exists():
        raise RuntimeError(
            "No saved Ubisoft profile at cache/auth/profiles/ubisoft. Open the Connections page and connect Ubisoft first."
        )
...
def _classify_kind(name, edition):
    ...
    for match in _TILE_OPEN_RE.finditer(html):
        ...
        items.append(
            WishlistItem(
                ...
                store_url=product.get("url") or f"https://store.ubisoft.com/us/{item_id}.html?lang=en_US",
...
def main():
    ...
    if dropped:
        print(
            f"  filtered {len(dropped)} non-game tiles (DLC/skins/currency); pass --include-dlc to keep them",
            flush=True,
        )
    empty_exit = refuse_empty_result(
        kept, label="Ubisoft wishlist", allow_empty=args.allow_empty, output_path=GAMES_UBISOFT_WISHLIST_JSON
    )
    if empty_exit is not None:
        return stats.finish("fetch_ubisoft_wishlist", t0, exit_code=empty_exit)
    drift_exit = refuse_drift_result(
        kept, label="Ubisoft wishlist", allow_drift=args.allow_drift, output_path=GAMES_UBISOFT_WISHLIST_JSON
    )
