...
HLTB_DELAY_SEC = 1.0
_TILE_OPEN_RE = re.compile('<div\\b[^>]*\\bclass="[^"]*\\bproduct-tile\\b[^"]*\\bwishlist-product-tile\\b[^"]*"[^>]*>', re.IGNORECASE)
_ATTR_RE = re.compile('\\bdata-([a-z][a-z0-9-]*)="([^"]*)"', re.IGNORECASE)
_PRODUCT_BLOB_RE = re.compile('var\\s+product\\s*=\\s*(\\{[^\\n]+?\\})\\s*;', re.MULTILINE)
_TITLE_GO_TO_RE = re.compile('title="Go to product:\\s*([^"]+)"', re.IGNORECASE)
_IMG_ALT_RE = re.compile('<img[^>]*\\bclass="[^"]*\\bproduct_image\\b[^"]*"[^>]*\\balt="([^"]+)"', re.IGNORECASE)
_TC100_NAME_RE = re.compile('data-tc100="[^"]*&quot;productName&quot;:&quot;([^"&]+)&quot;', re.IGNORECASE)
_EMPTY_PHRASE = re.compile('Log in to access your wishlist|Your wishlist is empty|Looking for new games to add', re.IGNORECASE)
_GAME_EDITION_RE = re.compile('\\b(edition|standard|resynced|definitive|anniversary|remaster|remastered|reckoning|year\\s*\\d+|gold|deluxe|ultimate|elite|premium|complete)\\b', re.IGNORECASE)
...
def _wishlist_page_ready(html: str) -> bool:
    if _TILE_OPEN_RE.search(html):
        return True
    if 'wishlist-items-list' in html and len(html) > 200000 and _EMPTY_PHRASE.search(html):
        return True
    return False
...
def _fetch_wishlist_html(timeout_s: int=45) -> tuple[str, str]:
    ...
    if not profile.exists():
        raise RuntimeError('No saved Ubisoft profile at cache/auth/profiles/ubisoft. Open the Connections page and connect Ubisoft first.')
...
def _classify_kind(name: str, edition: str | None) -> str:
    ...
    if edition_s and _GAME_EDITION_RE.search(edition_s):
        return 'game'
    dlc_signals = ('pack', 'skin', 'bundle', 'currency', 'credits', 'charm', 'emblem', 'booster', 'season pass', 'year pass')
...
def _parse_tiles(html: str) -> list[WishlistItem]:
    ...
    for match in _TILE_OPEN_RE.finditer(html):
        ...
        items.append(WishlistItem(store_id=item_id, mdm_id=attrs.get('mdmiid') or None, brand=attrs.get('brand') or product.get('brand') or None, name=name, edition=edition, genre_text=product.get('genre') or None, image_url=product.get('image_url') or None, store_url=product.get('url') or f'https://store.ubisoft.com/us/{item_id}.html?lang=en_US', currency=product.get('currency'), unit_price=product.get('unit_price'), unit_sale_price=product.get('unit_sale_price'), platform=product.get('platform') or None, kind=kind))
    return items
...
def _build_row(item: WishlistItem, hltb: dict | None) -> dict:
    ...
    genres: list[str] = []
    if item.genre_text:
        for piece in re.split('[,/]', item.genre_text):
            label = piece.strip()
            if label:
                genres.append(label)
    discount_pct = None
    if item.unit_price is not None and item.unit_sale_price is not None and (item.unit_price > 0) and (item.unit_sale_price < item.unit_price):
    ...
    return {'store': 'wishlist', 'wishlist_store': 'ubisoft', 'id': f'ubisoft-{item.store_id}', 'ubisoft_product_id': item.store_id, 'ubisoft_mdm_id': item.mdm_id, 'ubisoft_kind': item.kind, 'name': item.name, 'playtime_minutes': 0, 'last_played': None, 'header_image': item.image_url, 'library_image': item.image_url, 'release_date': None, 'genres': list(dict.fromkeys(genres)), 'tags': tags, 'steam_review_percent': None, 'steam_review_count': None, 'steam_review_desc': None, 'hltb_main_hours': (hltb or {}).get('hltb_main_hours'), 'hltb_main_extra_hours': (hltb or {}).get('hltb_main_extra_hours'), 'hltb_completionist_hours': (hltb or {}).get('hltb_completionist_hours'), 'hltb_match_confidence': (hltb or {}).get('hltb_match_confidence'), 'hltb_name': (hltb or {}).get('hltb_name'), 'store_url': item.store_url, 'type': 'game', 'price': price, 'price_initial': price_initial, 'discount_percent': discount_pct, 'currency': item.currency, 'edition': item.edition}
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Fetch Ubisoft Store wishlist into games_wishlist_ubisoft.json')
    parser.add_argument('--hltb', action='store_true', help='Look up HowLongToBeat hours (slow)')
    add_only_new_arg(parser)
    parser.add_argument('--include-dlc', action='store_true', help='Also write DLC/cosmetic/currency tiles (default keeps only base games)')
    ...
    if 'Sign in' in title or (_EMPTY_PHRASE.search(html or '') and '<div' not in (html or '')):
        msg = 'Ubisoft storefront session is missing or expired. Open the Connections page, click Ubisoft → Connect, and sign in to store.ubisoft.com inside the launched browser window.'
    ...
    if dropped:
        print(f'  filtered {len(dropped)} non-game tiles (DLC/skins/currency); pass --include-dlc to keep them', flush=True)
    empty_exit = refuse_empty_result(kept, label='Ubisoft wishlist', allow_empty=args.allow_empty, output_path=GAMES_UBISOFT_WISHLIST_JSON)
    if empty_exit is not None:
        return stats.finish('fetch_ubisoft_wishlist', t0, exit_code=empty_exit)
    drift_exit = refuse_drift_result(kept, label='Ubisoft wishlist', allow_drift=args.allow_drift, output_path=GAMES_UBISOFT_WISHLIST_JSON)
    ...
    payload = {'fetched_at': datetime.now(UTC).isoformat(), 'store': 'wishlist_ubisoft', 'game_count': len(rows), 'games': sorted(rows, key=lambda g: (g.get('name') or '').lower())}
    write_catalog_text(GAMES_UBISOFT_WISHLIST_JSON, json.dumps(payload, indent=2, ensure_ascii=False))
    print(f'\nWrote {len(rows)} games to {GAMES_UBISOFT_WISHLIST_JSON}.', flush=True)
    print('Reload the dashboard to see Ubisoft items in the Wishlist tab.', flush=True)
    stats.ok = len(rows)
    return stats.finish('fetch_ubisoft_wishlist', t0, exit_code=0, extra=f'{len(rows)} games')
