...
from fetchers._base import STEAM_CREDENTIALS_HINT, add_allow_empty_arg, add_only_new_arg, configure_stdout, load_existing_games, refuse_drift_result, refuse_empty_result, write_catalog_text
...
def main() -> int:
    ...
    try:
        ...
        return stats.finish('fetch_wishlist', t0, exit_code=1)
    empty_exit = refuse_empty_result(items, label='Steam wishlist', allow_empty=args.allow_empty, output_path=GAMES_WISHLIST_JSON)
    ...
    for i, item in enumerate(items, 1):
        ...
        row = {'store': 'wishlist', 'id': appid, 'appid': appid, 'name': name, 'wishlist_priority': item.get('priority'), 'wishlist_added': item.get('date_added'), 'playtime_minutes': 0, 'last_played': None, 'header_image': (data or {}).get('header_image') or f'https://cdn.akamai.steamstatic.com/steam/apps/{appid}/header.jpg', 'library_image': f'https://cdn.akamai.steamstatic.com/steam/apps/{appid}/library_600x900_2x.jpg', 'release_date': ((data or {}).get('release_date') or {}).get('date') or None, 'release_coming_soon': bool(((data or {}).get('release_date') or {}).get('coming_soon')), 'genres': [g['description'] for g in (data or {}).get('genres', [])], 'tags': [c['description'] for c in categories[:16]], 'coop_online': coop_online, 'coop_local': coop_local, 'steam_review_percent': reviews.get('percent_positive') if reviews else None, 'steam_review_count': reviews.get('total_reviews') if reviews else None, 'steam_review_desc': reviews.get('review_score_desc') if reviews else None, 'hltb_main_hours': hltb_data.get('hltb_main_hours') if hltb_data else None, 'hltb_main_extra_hours': hltb_data.get('hltb_main_extra_hours') if hltb_data else None, 'hltb_completionist_hours': hltb_data.get('hltb_completionist_hours') if hltb_data else None, 'hltb_match_confidence': hltb_data.get('hltb_match_confidence') if hltb_data else None, 'hltb_name': hltb_data.get('hltb_name') if hltb_data else None, 'store_url': f'https://store.steampowered.com/app/{appid}/', 'type': 'game', 'price': format_price(price_block.get('final', 0) / 100 if price_block.get('final') else None, normalize_currency_code(price_block.get('currency'))), 'price_initial': format_price(price_block.get('initial', 0) / 100 if price_block.get('initial') else None, normalize_currency_code(price_block.get('currency'))), 'discount_percent': price_block.get('discount_percent'), 'currency': normalize_currency_code(price_block.get('currency'))}
        games_out.append(row)
        loop_hb.tick_progress(i, len(items), 'Steam wishlist', name[:40])
    empty_exit = refuse_empty_result(games_out, label='Steam wishlist rows', allow_empty=args.allow_empty, output_path=GAMES_WISHLIST_JSON)
    if empty_exit is not None:
        return stats.finish('fetch_wishlist', t0, exit_code=empty_exit)
    drift_exit = refuse_drift_result(games_out, label='Steam wishlist rows', allow_drift=args.allow_drift, output_path=GAMES_WISHLIST_JSON)
    if drift_exit is not None:
        return stats.finish('fetch_wishlist', t0, exit_code=drift_exit)
    payload = {'fetched_at': datetime.now(UTC).isoformat(), 'store': 'wishlist', 'game_count': len(games_out), 'games': sorted(games_out, key=lambda g: g['name'].lower())}
