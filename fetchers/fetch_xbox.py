...
def _build_row(title: dict, hltb: dict | None) -> dict:
    ...
    row = {'store': 'xbox', 'id': tid, 'xbox_title_id': tid, 'name': title.get('name') or 'Unknown', 'playtime_minutes': None, 'last_played': hist.get('lastTimePlayed'), 'header_image': image, 'library_image': image, 'release_date': None, 'genres': [], 'tags': list(dict.fromkeys(tags)), 'steam_review_percent': None, 'steam_review_count': None, 'steam_review_desc': None, 'hltb_main_hours': None, 'hltb_main_extra_hours': None, 'hltb_completionist_hours': None, 'hltb_match_confidence': None, 'hltb_name': None, 'trophy_progress': ach.get('progressPercentage'), 'store_url': _store_url(title), 'type': (title.get('type') or 'game').lower(), 'price': None, 'price_initial': None, 'discount_percent': None, 'currency': None, 'xbox_gamerscore_current': ach.get('currentGamerscore'), 'xbox_gamerscore_total': ach.get('totalGamerscore')}
    if hltb:
...
def main() -> int:
    ...
    empty_exit = refuse_empty_result(games, label='Xbox library', allow_empty=args.allow_empty, output_path=GAMES_XBOX_JSON)
    if empty_exit is not None:
        return stats.finish('fetch_xbox', t0, exit_code=empty_exit)
    drift_exit = refuse_drift_result(games, label='Xbox library', allow_drift=args.allow_drift, output_path=GAMES_XBOX_JSON)
    ...
    for i, title in enumerate(games, 1):
        ...
        games_out.append(merge_cached_row(_build_row(title, hltb), cached, authoritative=XBOX, hltb_updated=hltb_updated))
        stats.ok += 1
    ...
    payload = {'fetched_at': datetime.now(UTC).isoformat(), 'store': 'xbox', 'gamertag': gt, 'game_count': len(games_out), 'games': sorted(games_out, key=lambda g: g['name'].lower())}
