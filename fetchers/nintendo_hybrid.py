...
def find_existing_row(item: dict[str, Any], *, existing: dict[str, dict], by_title: dict[str, dict], by_app_id: dict[str, dict], by_nintendo_id: dict[str, dict]) -> dict | None:
...
def lookup_transaction_for_vgc(vgc_name: str, tx_by_title: dict[str, dict[str, Any]]) -> dict[str, Any] | None:
...
def _hybrid_from_tx_only(tx: dict[str, Any]) -> dict[str, Any]:
    ...
    return {'name': tx['name'], 'id': tx_id, 'application_id': None, 'nintendo_id': tx_id or None, 'vgc_id': None, 'purchase_date': tx.get('purchase_date'), 'device_type': tx.get('device_type'), 'content_type': tx.get('content_type'), 'tags': list(tx.get('tags') or []), 'nintendo_platform': tx.get('device_type'), 'ownership_source': 'transaction', 'transaction_name': tx.get('name')}
...
def merge_vgc_with_transactions(vgc_rows: list[dict[str, Any]], tx_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
