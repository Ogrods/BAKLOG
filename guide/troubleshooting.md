...
| **Cloud sync** | Pro opt-in: enable **Cloud sync to your account** on Connections (requires account sign-in and `BAKLOG_CAP_CLOUD_MIRROR=live` while the capability defaults to soon). After a store fetch or personal save, catalog JSON uploads (~30s debounce, 25 MiB per object). Browse read-only at [baklog.app/mirror](https://baklog.app/mirror); use **Import from cloud mirror** on another PC. Credentials stay local. Claimables are not mirrored. Comp-Pro: sign out/in once after a grant so Storage RLS sees `plan=pro` on the JWT. |
