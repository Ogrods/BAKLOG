...
| **Cloud sync** | Pro opt-in: enable **Cloud sync to your account** on Connections (requires account sign-in). Confirm the risk dialog, then click **Sync now** to upload catalog JSON (25 MiB per object). Browse read-only at [baklog.app/mirror](https://baklog.app/mirror); use **Import from cloud mirror** on another PC. Credentials stay local. Turning sync off stops uploads; use **Clear cloud library** to delete existing cloud objects (or remove them in account storage). Claimables are not mirrored. Comp-Pro: sign out/in once after a grant so Storage RLS sees `plan=pro` on the JWT. |
