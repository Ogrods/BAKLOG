...
export function setAddGameTarget(target) {
  addGameTarget =
    target === "wishlist" ? "wishlist" : target === "itch" ? "itch" : "library";
  document.querySelectorAll(".add-target-btn").forEach((b) => {
    b.classList.toggle("active", b.dataset.target === addGameTarget);
  });
  document
    .getElementById("addGameWishlistFields")
    .classList.toggle("hidden", addGameTarget !== "wishlist");
  ...
  document
    .getElementById("addGameCountWrap")
    ?.classList.toggle("hidden", addGameTarget === "wishlist");
  ...
  if (addGameTarget === "wishlist") {
    ...
    hint.textContent =
      "Tracking a deal? Add an optional price and/or discount % and store URL. Discount-only entries still match On sale / Min discount filters even without a price.";
  } else if (addGameTarget === "itch") {
    titleEl.textContent = "Add to itch.io";
    hint.textContent =
      "Type a title and click Search Steam to import cover/rating, or save without a match. The game will be saved under your chosen platform.";
  } else {
    titleEl.textContent = "Add a game";
    hint.textContent =
      "Type a title and click Search Steam. Pick the closest match to import its cover, Steam rating, and store link. The game will be saved under your chosen platform.";
  }
}
...
async function steamSearch(term) {
  const url = `/api/proxy/steam-search?term=${encodeURIComponent(term)}`;
  const res = await fetch(url);
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
    throw new Error(
      `Steam search failed: ${body.error || `HTTP ${res.status}`}`,
    );
  }
...
function readWishlistFields() {
  ...
  const discountRaw = document
    .getElementById("addGameWishDiscount")
    .value.trim();
  ...
  const discount = discountRaw
    ? Math.max(0, Math.min(100, parseInt(discountRaw, 10) || 0))
    : null;
  return {
    price:
      priceNum != null && !isNaN(priceNum) ? `$${priceNum.toFixed(2)}` : null,
...
async function importSteamMatch(title, platform, match) {
  ...
  const doImport = async () => {
    ...
    const game = {
      ...
      id:
        (isWishlist ? "wish-" : "") +
        (platform === "steam" ? match.id : manualSlug(title || match.name)),
      name: title || match.name,
      header_image:
        match.tiny_image ||
        `https://cdn.akamai.steamstatic.com/steam/apps/${match.id}/header.jpg`,
...
function importTitleOnly() {
  ...
  const doImport = () => {
    ...
    const where = isWishlist ? `wishlist (${platform})` : platform;
    document.getElementById("addGameStatus").textContent =
      `Saved "${title}" under ${where} (no Steam data).`;
    refreshAfterManualChange();
    setTimeout(closeAddGameModal, 700);
  };
...
export function bindAddGameModal() {
  ...
  document
    ...
    .addEventListener("click", openAddGameModal);
  document
    .getElementById("addGameClose")
    ...
    try {
      ...
      if (!matches.length) {
        statusEl.textContent =
          "No Steam matches. Save without a match to add the title only.";
        return;
      }
  ...
  resultsEl.addEventListener("click", async (e) => {
    ...
    const match = (await steamSearch(titleEl.value.trim())).find(
      (m) => m.id === appid,
    );
    if (!match) {
      statusEl.textContent = "Couldn't refetch match details.";
      return;
    }
    await importSteamMatch(titleEl.value.trim(), platformEl.value, match);
  });
  document
    .getElementById("addGameSkipSteam")
    .addEventListener("click", importTitleOnly);

  document
    .getElementById("addGameDupCancel")
    ?.addEventListener("click", hideDuplicateWarn);
  ...
  document.getElementById("addGameExport").addEventListener("click", () => {
    download(
      "steam-backlog-manual-games.json",
      JSON.stringify(loadManualGames(), null, 2),
      "application/json",
    );
  });
  document
    .getElementById("addGameImport")
    .addEventListener("change", async (e) => {
      ...
      try {
        ...
        if (!Array.isArray(incoming))
          throw new Error("File must be an array of manual games.");
        const merged = [...loadManualGames()];
        for (const g of incoming) {
          const idx = merged.findIndex(
            (m) => m.store === g.store && m.id === g.id,
          );
