...
import { initDashboard, scheduleDashboardRender } from "./dashboard.js";
...
import {
  reloadGames,
  reloadAfterFetcher,
  finishEmptyLibraryLoad,
  repairBulkFirstSeenStamps,
...
} from "./library-count-animation.js";
...
async function bootstrap() {
  ...
  try {
    ...
    console.warn(
      "[personalStore] init failed, falling back to localStorage",
      err,
    );
  ...
  stripLegacyTags();
  seedPreHiddenDefaults();
  state.prefs.genreFilters = (state.prefs.genreFilters || []).map(
    aliasCanonicalGenre,
  );
  ...
  if (rowHeroBackdropEl)
    rowHeroBackdropEl.checked = !!state.prefs.rowHeroBackdrop;
  ...
  if (quickWinMaxValEl)
    quickWinMaxValEl.textContent = state.prefs.quickWinMaxHours;
  applyPicksCollapsedState();
  ...
  if (itchShowNonGamesEl)
    itchShowNonGamesEl.checked = !state.sessionPrefs.itchHideNonGames;
    ...
    try {
      ...
      if (cfgRes.ok) {
        ...
        if (cfg.running_from_temp) {
          ...
          if (banner) {
            banner.innerHTML =
              '<div class="migration-banner-body"><span class="text-amber-400">BAKLOG is running from a temporary folder. Install or unzip it to Desktop or Documents. Your library data is stored in a separate folder and is not lost when Windows cleans up temp files.</span></div>';
  ...
  const reloadPromise = reloadGames().catch(async () => {
    ...
    if (banner) {
      banner.innerHTML =
        '<div class="migration-banner-body"><span class="text-amber-400">No library data yet. Open <strong>Connections</strong>, connect a store, and your games will appear here.</span></div>';
      banner.classList.remove("hidden");
    }
  ...
  try {
    ...
    if (!fetcherRunner.isApiAvailable()) {
      void import("./claimable.js").then((m) =>
        m.startClaimableReadOnlyPolling(),
      );
    }
  }
  ...
  try {
    ...
    if (state.activeView === "dashboard") {
      ...
      else if (demo === "count-small")
        setTimeout(() => runLibraryCountSmallDemo(), 1200);
    }
  } catch (_) {}
}
