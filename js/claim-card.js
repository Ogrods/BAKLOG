...
export function dedupeClaims(list) {
  ...
  for (const c of list) {
    ...
    const titleNorm = normalizeNameForDedup(
      stripClaimTitleDecorations(c.title || ""),
    );
  ...
  for (const c of list) {
    ...
    if (
      ...
      (c.steam_appid == null || c.steam_appid === "")
    ) {
    ...
    const curRank =
      CLAIM_SOURCE_RANK[String(cur.source || "").toLowerCase()] ?? 99;
...
export function sanitizeBlurb(raw) {
  if (!raw) return "";
  // Decode HTML entities ITAD emits in plain-text blurbs so the decoded
  // characters are re-encoded by the caller's escapeHtml() before innerHTML.
  // CodeQL flags this as potential double-unescaping, but the output is
  // always passed through escapeHtml() (see claimDetailPanelHtml line 388).
  // codeql:ignore - intentional decode-then-re-encode pipeline
  let t = String(raw)
    .replace(/<[^>]*>/g, " ")
    .replace(/&/g, "&")
    .replace(/</g, "<")
    ...
    .replace(/&#0*39;|'/g, "'");
  t = t.replace(
    /\s*\|?\s*(unknown expiry|expires on[^|]*)\s*\|?\s*go to giveaway\s*/i,
    " ",
  );
...
export function claimAttributionHtml(attribution) {
  ...
  const itad =
    '<a href="https://isthereanydeal.com/" target="_blank" rel="noopener noreferrer">ITAD</a>';
...
export function claimCardHtml(claim) {
  ...
  const blurb = blurbText
    ? `<p class="claim-hero-blurb text-sm text-slate-400 mt-1">${escapeHtml(blurbText)}</p>`
    : "";
  ...
  const endsHtml = ends
    ? `<span class="text-xs text-amber-300/90">Ends ${escapeHtml(ends)}</span>`
    : "";
  const genres = (claim.genres || []).slice(0, 2).map(escapeHtml).join(" · ");
  const genreHtml = genres
    ? `<span class="deal-hero-genres">${genres}</span>`
    : "";
...
export function claimHiddenRowHtml(
  claim,
  { actionHtml = "", rowClass = "" } = {},
) {
  const title = escapeHtml(claim.title || "Free game");
...
export function claimDetailPanelHtml(
  claim,
  { owned = false, attribution = null } = {},
) {
  const ends = formatEndsAt(claim.ends_at);
  ...
  const blurbText = sanitizeBlurb(claim.blurb);
  const blurb = blurbText
    ? `<p class="claim-detail-blurb">${escapeHtml(blurbText)}</p>`
    : "";
  const endsHtml = ends
 ...
 */
export function claimableModuleMarkup(
  claims,
  {
