 ...
 * @returns {string}
 */
export function formatLastUploadedBy(localUploadState) {
  const device = String(localUploadState?.device_id || '').trim();
  const last = String(localUploadState?.last_upload_at || '').trim();
  if (!device && !last) return '';
  const when = last ? formatMirrorTimestamp(last) : '';
  if (device && when) return `Last uploaded by ${device} at ${when}.`;
  if (device) return `Last uploaded by ${device}.`;
