...
def _load_font(size: int, bold: bool=False) -> ImageFont.FreeTypeFont | ImageFont.ImageFont:
    ...
    mac_font = '/System/Library/Fonts/Supplemental/Arial Bold.ttf' if bold else '/System/Library/Fonts/Supplemental/Arial.ttf'
...
def _render_app_icon_raw(size: int) -> Image.Image:
    ...
    ImageDraw.Draw(stroke).rounded_rectangle((1, 1, size - 2, size - 2), radius=radius, outline=(*ACCENT, 90), width=max(1, size // 64))
...
def _make_wizard_large_at(w: int, h: int) -> Image.Image:
    ...
    draw.line([(int(24 * scale), int(156 * scale)), (int(140 * scale), int(156 * scale))], fill=ACCENT, width=max(1, int(2 * scale)))
    draw.multiline_text((cx, int(176 * scale)), 'One honest backlog\nacross every store.', font=tag_font, fill=ACCENT, anchor='mm', align='center', spacing=max(2, int(4 * scale)))
    draw.multiline_text((cx, int(252 * scale)), 'Local-only beta\nbaklog.app', font=small_font, fill=MUTED, anchor='mm', align='center', spacing=max(2, int(3 * scale)))
    return img
