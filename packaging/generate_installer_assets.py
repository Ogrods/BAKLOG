...
def _draw_mark_layer(size, *, pad=0.12):
...
def _render_app_icon_raw(size):
    ...
    ImageDraw.Draw(stroke).rounded_rectangle(
        (1, 1, size - 2, size - 2), radius=radius, outline=(*ACCENT, 90), width=max(1, size // 64)
    )
...
def _composite_on_bg(rgba, bg_color):
