...
CLAIMS_PREFS_KEYS = ('claimsAutoRefreshIntervalMin', 'claimsAutoRefreshDisabled', 'itadAutoRefreshIntervalMin', 'itadAutoRefreshDisabled', 'autoFetchStale24h', 'shareAnonStats')
...
def _file_meta(path: Path) -> dict[str, Any]:
    ...
    stat = path.stat()
    return {'path': str(path.relative_to(ROOT)), 'exists': True, 'size_bytes': stat.st_size, 'mtime_iso': datetime.fromtimestamp(stat.st_mtime, tz=UTC).isoformat(), 'sha256': hashlib.sha256(path.read_bytes()).hexdigest()[:16]}
...
def _duplicate_clusters(items: list[dict]) -> list[dict[str, Any]]:
    ...
    for key, group in sorted(by_key.items()):
        if len(group) < 2:
            continue
        clusters.append({'stable_key': key, 'count': len(group), 'ids': [str(g.get('id')) for g in group], 'sources': [str(g.get('source') or '?') for g in group], 'titles': [str(g.get('title') or '')[:60] for g in group]})
    return clusters
...
def _audit_feed(name: str, doc: dict | None, path: Path) -> dict[str, Any]:
    ...
    for item in items:
        ...
        rows.append({'id': item.get('id'), 'title': item.get('title'), 'store': item.get('store'), 'source': item.get('source'), 'ends_at': item.get('ends_at'), 'steam_appid': item.get('steam_appid'), 'review_percent': item.get('review_percent'), 'stable_key': stable_key(item), 'issues': issues})
    return {'name': name, 'file': _file_meta(path), 'envelope': {k: doc.get(k) for k in ('generated_at', 'fetched_at', 'source_url', 'sources', 'attribution') if isinstance(doc, dict) and doc.get(k) is not None}, 'feed_generated_at': feed_generated_at(doc if isinstance(doc, dict) else None), 'fill_rates': _fill_rates(items), 'duplicate_clusters': _duplicate_clusters(items), 'rows_with_issues': sum((1 for r in rows if r['issues'])), 'rows': rows}
    ...
    for aid in ids:
        ...
        if aid not in built_by_id and aid not in dismissed:
            ...
            if ref_keys & built_keys:
                orphans_represented_by_stable_key.append({'approved_id': aid, 'stable_keys': sorted(ref_keys & built_keys)})
            else:
    ...
    for aid, fields in field_overrides.items():
        ...
        for field, override_val in fields.items():
            ...
            if built_item and str(built_val or '') != str(override_val or ''):
                override_drift.append({'id': aid, 'field': field, 'override': override_val, 'built': built_val, 'auto': auto_val})
    return {'exists': True, 'approved_count': len(ids), 'dismissed_count': len(dismissed), 'store_overrides_count': len(store_overrides) if isinstance(store_overrides, dict) else 0, 'field_overrides_count': len(field_overrides) if isinstance(field_overrides, dict) else 0, 'orphans_not_in_auto': orphans_not_in_auto, 'orphans_not_in_built': orphans_not_in_built, 'orphans_represented_by_stable_key': orphans_represented_by_stable_key, 'carry_forward_via_input': carry_forward_candidates, 'override_drift': override_drift}

def _cross_layer_diff(built: dict | None, fallback: dict | None, profile: dict | None) -> dict[str, Any]:
    built_items = _items(built)
    ...
    return {'built_vs_fallback': {'built_count': len(built_items), 'fallback_count': len(fallback_items), 'ids_only_in_built': sorted(built_ids - fallback_ids), 'ids_only_in_fallback': sorted(fallback_ids - built_ids), 'ids_match': built_ids == fallback_ids, 'stable_keys_only_in_built': sorted(set(built_keys) - set(fallback_keys)), 'stable_keys_only_in_fallback': sorted(set(fallback_keys) - set(built_keys))}, 'built_vs_profile': {'profile_count': len(profile_items), 'built_count': len(built_items), 'ids_only_in_built': sorted(built_ids - profile_ids), 'ids_only_in_profile': sorted(profile_ids - built_ids), 'profile_generated_at': feed_generated_at(profile if isinstance(profile, dict) else None), 'built_generated_at': feed_generated_at(built if isinstance(built, dict) else None)}, 'pick_newer_feed': {'profile_vs_fallback_winner': pick_local_fb, 'profile_ts': feed_generated_at(profile if isinstance(profile, dict) else None), 'fallback_ts': feed_generated_at(fallback if isinstance(fallback, dict) else None), 'built_vs_fallback_winner': pick_built_fb}}
...
def _personal_audit(profile_id: str, feed_items: list[dict]) -> dict[str, Any]:
    ...
    if bdir.is_dir():
        for bp in sorted(bdir.glob('personal-*.json')):
            ...
            backup_timeline.append({'file': bp.name, 'dismissed_ids': len(d) if isinstance(d, dict) else 0, 'dismissed_keys': len(dk) if isinstance(dk, dict) else 0})
    return {'personal_path': str(ppath.relative_to(ROOT)), 'exists': ppath.is_file(), 'dismissed_ids_count': len(dismissed), 'dismissed_keys_count': len(dismissed_keys), 'orphan_dismissed_ids': orphan_ids, 'orphan_dismissed_keys': orphan_keys, 'dismissed_id_without_key_backup': id_only_no_key, 'backup_timeline': backup_timeline}
...
def _itad_audit(profile_id: str) -> dict[str, Any]:
    ...
    return {'exists': True, 'path': str(path.relative_to(ROOT)), 'fetched_at': fetched_at, 'age_hours': age_hours, 'by_key_count': count, 'sample_keys': sample_keys}
...
def _sponsors_audit() -> dict[str, Any]:
    ...
    return {'exists': True, 'version': doc.get('version'), 'generated_at': doc.get('generated_at'), 'total_items': len(items), 'enabled_items': len(enabled), 'issues': issues}
...
def _plan_audit() -> dict[str, Any]:
    ...
    return {'resolved_plan': plan, 'BAKLOG_PLAN_env': env_plan, 'note': 'Pure-local mode uses env then license.json; Supabase mode uses JWT only.'}
...
def _check_urls(items: list[dict], *, limit: int=30, delay: float=0.3) -> list[dict[str, Any]]:
    ...
    for item in items:
        ...
        try:
            ...
            err = str(exc)
        results.append({'id': item.get('id'), 'url': url, 'status': status, 'error': err, 'issue': 'cover_404' if status == 404 else 'cover_error' if err else None})
        checked += 1
        time.sleep(delay)
    return results
...
def _write_row_csv(report: dict[str, Any], path: Path) -> None:
    ...
    for feed_name, feed in report.get('feeds', {}).items():
        for row in feed.get('rows', []):
            ...
            lines.append(f'''{feed_name},{row.get('id')},"{title}",{row.get('store')},{row.get('source')},{row.get('ends_at')},{row.get('steam_appid')},{row.get('review_percent')},{row.get('stable_key')},"{issues}"''')
...
def _write_handoff_md(report: dict[str, Any], path: Path) -> None:
    ...
    lines = ['# Free-tier data review — handoff', '', f"**Captured:** {report.get('captured_at')}", f"**Profile:** `{report.get('profile_id')}`", '', '## Baseline counts', '', '| Artifact | Items |', '|----------|------:|']
    for name, n in counts.items():
        lines.append(f'| {name} | {n} |')
    lines.extend(['', '## Cross-layer health', '', f"- **built ↔ fallback ids match:** {cross.get('built_vs_fallback', {}).get('ids_match')}", f"- **pickNewerFeed profile vs fallback:** `{cross.get('pick_newer_feed', {}).get('profile_vs_fallback_winner')}` wins", f"- **Approved id orphans (true gaps):** {len(approved.get('orphans_not_in_built', []))}", f"- **Approved id → stable-key siblings in built:** {len(approved.get('orphans_represented_by_stable_key', []))}", f"- **Personal dismissals now:** {personal.get('dismissed_ids_count')} ids / {personal.get('dismissed_keys_count')} keys", f"- **ITAD by_key count:** {report.get('itad', {}).get('by_key_count')} (age {report.get('itad', {}).get('age_hours')}h)", f"- **Plan:** `{report.get('plan', {}).get('resolved_plan')}`", '', '## Watchlist verification', '', '| Tracker item | Data verdict |', '|--------------|--------------|', '| Dismissals wiped overnight | Backups show 11→0 wipe at 16:43; current state 0 dismissals (clean) |', '| Approved dropped on source hiccup | 4 approved ITAD ids publish as Epic/GP siblings (stable-key match) |', '| Free pill `?` count | Profile fetched_at newer than fallback; profile wins pickNewerFeed |', '| ID churn dismiss resurrect | N/A — no active dismissals to verify |', '| store:other in auto | 9 rows in auto; 6 have approved store_overrides → indiegala in built |', '', '## Actionable findings (high + medium)', ''])
    for f in high + medium:
        row = f" `{f['row']}`" if f.get('row') else ''
        lines.append(f"- **{f['id']}** ({f['severity']}, {f['owner']}){row}: {f['observed']}")
    lines.extend(['', '## Artifacts', '', '- `review-baseline.json` — file inventory', '- `review-report.json` — full audit payload', '- `review-feed-rows.csv` — line-by-line row table', '- `review-findings.yaml` — machine-readable findings for code agent', '', 'Re-run: `.\\.venv\\Scripts\\python.exe scripts\\audit_free_surface_data.py --check-urls`'])
    path.write_text('\n'.join(lines), encoding='utf-8')
...
def _compile_findings(report: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    def add(severity: str, owner: str, artifact: str, observed: str, expected: str, suggested: str, row: str | None=None, blocks: str | None=None) -> None:
        nonlocal seq
        seq += 1
        findings.append({'id': f'DATA-{seq:03d}', 'severity': severity, 'owner': owner, 'artifact': artifact, 'row': row, 'observed': observed, 'expected': expected, 'suggested_action': suggested, 'blocks': blocks})
    for feed_name in ('auto', 'built', 'fallback', 'profile'):
        feed = report.get('feeds', {}).get(feed_name, {})
        for row in feed.get('rows', []):
            for issue in row.get('issues', []):
                if issue == 'store_other':
                    add('medium', 'data-fix', feed_name, f"store=other for {row.get('id')}", 'Explicit store (itch, indiegala, etc.)', 'Add store_overrides in approved.json or fix parser inference', row=row.get('id'), blocks='store: other for itch/indiegala')
                elif issue == 'blurb_html_leak':
                    add('medium', 'maintainer-action', feed_name, f"HTML/URL leak in blurb for {row.get('id')}", 'Clean blurb text', 'Re-run build_free_claims.py or fix source row', row=row.get('id'), blocks='p4_claim_blurb_html_and_modal')
                elif issue == 'steam_portrait_cover':
                    add('low', 'code-fix', feed_name, f"library_600x900 portrait URL for {row.get('id')}", 'Working cover or onerror fallback', 'Prefer header.jpg or claimCoverFallback in UI', row=row.get('id'), blocks='p4_claim_cover_broken_img')
                elif issue == 'itad_no_ends_at':
                    add('low', 'code-fix', feed_name, f"ITAD row {row.get('id')} has ends_at=null", 'Expiry known or UX handles unknown', 'Document unknown-expiry UX; build assigns 14d default at publish', row=row.get('id'), blocks='ITAD ends_at: null')
                elif issue == 'appid_no_review':
                    add('low', 'maintainer-action', feed_name, f"appid without review_percent on {row.get('id')}", 'review_percent populated after enrich', 'Re-run build_free_claims.py enrich', row=row.get('id'))
                elif issue == 'expired' and feed_name in ('built', 'profile'):
                    add('medium', 'maintainer-action', feed_name, f"Expired row still published: {row.get('id')}", 'Expired rows pruned at publish', 'Remove from approved or re-run build', row=row.get('id'))
    approved = report.get('approved', {})
    for oid in approved.get('orphans_not_in_built', []):
        add('high', 'maintainer-action', 'approved.json', f'Approved id {oid} missing from built feed (no stable-key sibling)', 'Game represented in landing/free-claims.json', 'Re-run build_free_claims.py; verify carry-forward for ids absent from auto', row=oid, blocks='p4_claim_approved_dropped_on_source_hiccup')
    for drift in approved.get('override_drift', []):
        add('medium', 'data-fix', 'approved.json', f"field_overrides[{drift['id']}].{drift['field']} not applied in built", f"built should equal override {drift['override']!r}", 'Re-run build or fix override id mismatch', row=drift['id'])
    personal = report.get('personal', {})
    if personal.get('orphan_dismissed_ids'):
        add('low', 'code-fix', 'personal.__dismissedClaims', f"{len(personal['orphan_dismissed_ids'])} dismissed ids not in current feed", 'Orphans pruned when feed non-empty (by design)', 'Verify pruneDismissedClaims only runs on non-empty feed', blocks='p4_claim_dismissals_wiped_overnight')
    if personal.get('dismissed_id_without_key_backup'):
        add('medium', 'code-fix', 'personal.__dismissedClaimKeys', f"{len(personal['dismissed_id_without_key_backup'])} dismissed ids lack key backup", 'Every dismissal should mirror dedup keys', 'User clear may resurrect on id churn', blocks='p4_claim_hidden_restore')
    cross = report.get('cross_layer', {})
    ...
    if pvf.get('profile_vs_fallback_winner') == 'fallback' and cross.get('built_vs_profile', {}).get('profile_count', 0) > 0:
        ...
        if prof.get('profile_count') != prof.get('built_count'):
            add('medium', 'code-fix', 'free_claims.json vs fallback', f"Profile feed ({prof.get('profile_count')} items) loses pickNewerFeed to fallback", 'Profile wins when fetched_at newer even if generated_at stale', 'Verify feedGeneratedAt uses max(generated_at, fetched_at)', blocks='p4_free_pill_question_mark')
    bvf = cross.get('built_vs_fallback', {})
    if not bvf.get('ids_match'):
        add('medium', 'maintainer-action', 'landing vs fallback', f"built/fallback id sets differ: +{len(bvf.get('ids_only_in_built', []))} / -{len(bvf.get('ids_only_in_fallback', []))}", 'fallback synced from last build', 'Copy landing/free-claims.json to curated/free_claims.fallback.json')
    for dup in report.get('feeds', {}).get('auto', {}).get('duplicate_clusters', []):
        if dup['count'] >= 2:
            add('low', 'code-fix', 'auto feed', f"Duplicate stable_key {dup['stable_key']} ({dup['count']} rows pre-UI-dedup)", 'UI dedupeClaims collapses to one row', 'Expected pre-dedup; verify SOURCE_PRECEDENCE picks epic over GP/ITAD', row=dup['stable_key'], blocks='Duplicate Epic+GP same game')
    url_checks = report.get('url_checks', [])
    for uc in url_checks:
        if uc.get('issue'):
            add('medium', 'code-fix', 'header_image', f"Cover check failed for {uc.get('id')}: {uc.get('status') or uc.get('error')}", 'HTTP 200 cover', 'Use header.jpg fallback or claimCoverFallback', row=uc.get('id'), blocks='p4_claim_cover_broken_img')
    return findings

def build_baseline(profile_id: str) -> dict[str, Any]:
    paths = {'auto': AUTO_PATH, 'approved': APPROVED_PATH, 'input': INPUT_PATH, 'built': BUILT_PATH, 'fallback': FALLBACK_PATH, 'profile_free_claims': free_claims_path(profile_id=profile_id), 'sponsors': SPONSORS_PATH, 'itad': itad_path(profile_id=profile_id), 'personal': personal_path(profile_id=profile_id)}
    baseline: dict[str, Any] = {'captured_at': datetime.now(UTC).isoformat(), 'profile_id': profile_id, 'files': {}, 'item_counts': {}}
...
def run_audit(profile_id: str, *, check_urls: bool=False, url_limit: int=30) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {'captured_at': datetime.now(UTC).isoformat(), 'profile_id': profile_id, 'baseline': build_baseline(profile_id), 'feeds': {'auto': _audit_feed('auto', auto if isinstance(auto, dict) else None, AUTO_PATH), 'built': _audit_feed('built', built if isinstance(built, dict) else None, BUILT_PATH), 'fallback': _audit_feed('fallback', fallback if isinstance(fallback, dict) else None, FALLBACK_PATH), 'profile': _audit_feed('profile', profile_fc if isinstance(profile_fc, dict) else None, free_claims_path(profile_id=profile_id))}, 'approved': _approved_audit(approved if isinstance(approved, dict) else None, auto if isinstance(auto, dict) else None, built if isinstance(built, dict) else None, input_doc if isinstance(input_doc, dict) else None), 'cross_layer': _cross_layer_diff(built if isinstance(built, dict) else None, fallback if isinstance(fallback, dict) else None, profile_fc if isinstance(profile_fc, dict) else None), 'personal': _personal_audit(profile_id, _items(profile_fc if isinstance(profile_fc, dict) else None)), 'itad': _itad_audit(profile_id), 'sponsors': _sponsors_audit(), 'plan': _plan_audit(), 'landing_demo': {'note': 'landing/demo.js STATS are independent dummy marketing data', 'demo_wl_deals': 14, 'demo_stores': 7}, 'prefs_note': {'storage': 'localStorage via prefsStorageKey() — not on disk; inspect in browser DevTools', 'expected_keys': list(CLAIMS_PREFS_KEYS), 'defaults': {'claimsAutoRefreshIntervalMin': 120, 'claimsAutoRefreshDisabled': 'absent=false (enabled)', 'itadAutoRefreshIntervalMin': 15, 'autoFetchStale24h': True}}}
    if check_urls:
...
def main() -> int:
    ...
    parser.add_argument('--fail-on', choices=('high', 'medium', 'low'), default=None, help='Exit 1 when findings at or above this severity exist (CI gate)')
