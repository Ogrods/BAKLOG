...
JS_DIR = ROOT / 'js'
GLOBAL_LS_ALLOWLIST = frozenset({'baklog-active-profile', 'baklog-debug', 'baklog-debug-pro', 'baklog-perf', 'baklog-error-log', 'steam-backlog-personal', 'baklog-admin.knownAutoIds'})
GLOBAL_SS_ALLOWLIST = frozenset({'baklog-stale-chunk-reload'})
EXPECTED_SCOPED_LS_BASES = frozenset({'steam-backlog-ui-prefs', 'baklog-itad-snapshot', 'baklog-claims-snapshot', 'steam-backlog-personal', 'steam-backlog-manual-games', 'steam-backlog-library-first-seen', 'baklog-known-library-keys', 'baklog-spotlight-recent', 'baklog-fetcher-auth-cooldown', 'baklog-reconnect-dismissed', 'baklog-itad-last-auto-run', 'baklog-claims-last-auto-run', 'baklog-auto-stale-last-run', 'baklog-library-watch', 'baklog-ad-cursors', 'baklog-color-theme', 'baklog-fetcher-stat-layout', 'baklog.coverGalleryMode', 'baklog-dash-failed-covers', 'baklog-landscape-covers', 'baklog-metrics-rendered', 'baklog-metrics-untapped-batch-seeded'})
EXPECTED_SCOPED_SS_BASES = frozenset({'fetcher-suppressed-run-ids', 'fetcher-last-seq-by-run', '__baklogMetricSeed', 'baklog-pro-welcome'})
    ...
    for i, a in enumerate(ids):
        for b in ids[i + 1:]:
            for field_key in PERSONAL_BLEED_KEYS:
                ...
                report.add('high', code, f'profiles/{a} and profiles/{b} share {len(identical)} {field_key} id(s) with identical values (likely legacy bleed). Sample: {identical[:3]}')
...
def audit_scoped_registry(report: AuditReport) -> None:
    ...
    for base in EXPECTED_SCOPED_LS_BASES | EXPECTED_SCOPED_SS_BASES:
        if base not in text:
            report.add('medium', 'REGISTRY_DRIFT', f'Expected profile-scoped base {base!r} missing from js/profiles.js / state.js')
    ...
    for js_path in sorted(JS_DIR.glob('*.js')):
        ...
        for match in pattern.finditer(text):
            ...
            if key in GLOBAL_LS_ALLOWLIST or key in EXPECTED_SCOPED_LS_BASES:
                continue
            report.add('medium', 'LS_UNSCOPED', f'localStorage key {key!r} in {js_path.relative_to(ROOT)} without profile scoping')
    ...
    for js_path in sorted(JS_DIR.glob('*.js')):
        ...
        for match in pattern.finditer(text):
            ...
            if key in GLOBAL_SS_ALLOWLIST or key in EXPECTED_SCOPED_SS_BASES:
                continue
            report.add('medium', 'SS_UNSCOPED', f'sessionStorage key {key!r} in {js_path.relative_to(ROOT)} without profile scoping')
...
def main(argv: list[str] | None=None) -> int:
    ...
    parser.add_argument('--fail-on', choices=('high', 'medium', 'low'), help='Exit 1 when findings at or above this severity exist')
    parser.add_argument('--ignore-disk-bleed', action='store_true', help='Skip DISMISS_BLEED / PERSONAL_BLEED checks on profiles/ disk data (CI)')
