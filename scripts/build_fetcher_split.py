...
def patch_reconnect_body(body):
    body = body.replace("async function handleFetcherAuthOutcome", "export async function handleFetcherAuthOutcome")
    body = body.replace(
    ...
    body = body.replace("      renderDashboardFetcherHealth();", "      _renderDashboard();")
...
def main():
    ...
    misc_body = re.sub("export \\{\\n  itadLastAutoRunKey,.*?\\n\\};\\n\\n", "", misc_body, count=1, flags=re.DOTALL)
    ...
    source_meta = source_meta.replace(
        "let reloadGamesFn = async () => {};", "export let reloadGamesFn = async () => {};"
    )
    source_meta = source_meta.replace("let reloadAfterFetcherFn = null;", "export let reloadAfterFetcherFn = null;")
    source_meta = source_meta.replace(
        "let runStaleCooldownUntil = 0;", "export const batchRunCooldowns = { staleUntil: 0, failedUntil: 0 };"
    )
    source_meta = source_meta.replace("let runFailedCooldownUntil = 0;", "")
    source_meta = source_meta.replace(
        "export let runStaleCooldownUntil = 0;", "export const batchRunCooldowns = { staleUntil: 0, failedUntil: 0 };"
    )
    source_meta = source_meta.replace("export let runFailedCooldownUntil = 0;", "")
    source_meta = source_meta.replace(
        "const MAX_SSE_HINT = 'max 8 live streams';", "export const MAX_SSE_HINT = 'max 8 live streams';"
    )
    source_meta = source_meta.replace("const ENRICH_KEYS = new Set", "export const ENRICH_KEYS = new Set")
    source_meta = source_meta.replace(
        "const BOOT_DEFERRED_FETCHER_KEYS = new Set", "export const BOOT_DEFERRED_FETCHER_KEYS = new Set"
    )
    source_meta = source_meta.replace("const GROUP_ORDER = ['library'", "export const GROUP_ORDER = ['library'")
    source_meta = source_meta.replace("const GROUP_LABELS = {", "export const GROUP_LABELS = {")
    source_meta = source_meta.replace("const GROUP_LABEL_TIPS = {", "export const GROUP_LABEL_TIPS = {")
    source_meta = source_meta.replace("const COUNT_PILL_TITLES = {", "export const COUNT_PILL_TITLES = {")
    source_meta = source_meta.replace("const ENRICH_ORDER = ['steamTags'", "export const ENRICH_ORDER = ['steamTags'")
    source_meta = source_meta.replace("const COUNT_FNS = {", "export const COUNT_FNS = {")
    source_meta = source_meta.replace("function clickHintFor(src) {", "export function clickHintFor(src) {")
    source_meta = source_meta.replace("function refreshHintFor(src) {", "export function refreshHintFor(src) {")
    source_meta = source_meta.replace("function coverageLabel(key) {", "export function coverageLabel(key) {")
    source_meta = source_meta.replace(
        "function coverageTooltipLine(key) {", "export function coverageTooltipLine(key) {"
    )
    ...
    stats_body = (
        read("render_stats.txt")
        .replace("function fetcherStatTotals(rows) {", "export function fetcherStatTotals(rows) {")
        .replace(
    ...
    counts["js/fetcher/auto-refresh-wire.js"] = write(ROOT / "js/fetcher/auto-refresh-wire.js", auto_refresh_wire)
    ...
    runner_body = (
        read("runner_body.txt")
        .replace("import('./error-boundary.js')", "import('../../error-boundary.js')")
        .replace("runStaleCooldownUntil = Date.now() + 2000;", "batchRunCooldowns.staleUntil = Date.now() + 2000;")
        .replace("runFailedCooldownUntil = Date.now() + 2000;", "batchRunCooldowns.failedUntil = Date.now() + 2000;")
    )
    counts["js/fetcher/runner/index.js"] = write(ROOT / "js/fetcher/runner/index.js", runner_imports + runner_body)
