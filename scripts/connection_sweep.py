...
FETCHER_PROVIDER_GROUP: dict[str, tuple[str, ...]] = {'amazon': ('amazon_web', 'amazon'), 'gog': ('gog', 'gog_galaxy'), 'itch': ('itch', 'itch_local')}
...
def run_fetcher(key: str, entry: dict, profile_id: str, timeout_s: float) -> dict:
    ...
    t0 = time.monotonic()
    try:
        proc = popen_fetcher(cmd, cwd=str(bundle_root()), env=env, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding='utf-8', errors='replace', creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0))
        try:
            out, _ = proc.communicate(timeout=timeout_s)
        except subprocess.TimeoutExpired:
            proc.kill()
            out, _ = proc.communicate(timeout=10)
            return {'ok': False, 'exit_code': -9, 'elapsed_s': round(time.monotonic() - t0, 1), 'error': f'timeout after {timeout_s}s', 'tail': (out or '')[-500:]}
        exit_code = proc.returncode
        return {'ok': exit_code == 0, 'exit_code': exit_code, 'elapsed_s': round(time.monotonic() - t0, 1), 'tail': (out or '')[-500:]}
