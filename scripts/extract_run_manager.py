...
text = text.replace('def _load_run_history() -> list[dict[str, Any]]:\n    return _load_run_history_from(RUN_HISTORY_FILE)', 'def _load_run_history() -> list[dict[str, Any]]:\n    return _load_run_history_from(_run_history_path())')
text = text.replace('def _save_run_history(entries: list[dict[str, Any]]) -> None:\n    _save_run_history_to(RUN_HISTORY_FILE, entries)', 'def _save_run_history(entries: list[dict[str, Any]]) -> None:\n    _save_run_history_to(_run_history_path(), entries)')
text = text.replace('runs_dir: Path = RUNS_DIR,', 'runs_dir_path: Path | None = None,')
text = text.replace('        self._runs_dir = runs_dir\n        self._runs_dir.mkdir', '        self._runs_dir = runs_dir_path if runs_dir_path is not None else profile_runs_dir()\n        self._runs_dir.mkdir')
text = text.replace('        self._runs_dir = runs_dir or RUNS_DIR', '        self._runs_dir = runs_dir if runs_dir is not None else profile_runs_dir()')
