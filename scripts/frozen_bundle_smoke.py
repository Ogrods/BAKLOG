...
def run_smoke(bundle_dir: Path, *, expected_version: str | None=None) -> dict:
    ...
    report['checks']['static'] = {'server_exe': server_exe.is_file(), 'tray_exe': tray_exe.is_file(), 'curated_fallback': fallback.is_file(), 'fetcher_manifest_count': fetcher_count, 'bundled_env': env_ok}
    if not static_ok:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='baklog-bundle-smoke-') as td:
            ...
            proc = subprocess.Popen([str(server_exe)], cwd=str(bundle_dir), env=env, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, creationflags=subprocess.CREATE_NEW_PROCESS_GROUP if sys.platform == 'win32' else 0)
            ...
            try:
                req = urllib.request.Request('http://127.0.0.1:8765/api/mirror/import', data=b'{}', method='POST', headers={'Content-Type': 'application/json'})
                with urllib.request.urlopen(req, timeout=5) as resp:
                    import_status = resp.status
            except urllib.error.HTTPError as exc:
                import_status = exc.code
            report['checks']['mirror_routes'] = {'get_mirror_status': mirror_status, 'get_mirror_pro_error': (mirror_body or {}).get('error'), 'post_import_status': import_status}
    ...
    report['checks']['config'] = {'version': cfg_version, 'frozen': cfg_frozen, 'auth_required': config.get('authRequired'), 'has_capabilities': isinstance(config.get('capabilities'), dict), 'has_pro_settings': isinstance(config.get('proSettings'), dict)}
    ...
    if not cfg_frozen:
        ...
        return report
    dispatch = subprocess.run([sys.executable, str(_REPO / 'scripts' / 'frozen_fetcher_smoke.py'), '--exe', str(server_exe), '--data-dir', str(_REPO), '--dispatch-only'], capture_output=True, text=True, encoding='utf-8', errors='replace', cwd=str(_REPO))
...
def main() -> int:
    ...
    ap.add_argument('--bundle-dir', type=Path, default=_REPO / 'release' / 'BAKLOG', help='PyInstaller onedir output (contains BAKLOG.exe)')
