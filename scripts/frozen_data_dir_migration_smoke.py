...
def run_smoke(bundle_dir: Path) -> dict:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='baklog-migrate-smoke-') as td:
            ...
            (data_dir / '.env').write_text('BAKLOG_SUPABASE_URL=https://stale.supabase.co\nBAKLOG_SUPABASE_ANON_KEY=stale-anon\n', encoding='utf-8')
            env = {**os.environ, 'LOCALAPPDATA': str(localappdata), 'BAKLOG_NO_BROWSER': '1'}
            env.pop('BAKLOG_DATA_DIR', None)
            env.pop('BAKLOG_PORTABLE', None)
            proc = subprocess.Popen([str(exe)], cwd=str(bundle_dir), env=env, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, creationflags=subprocess.CREATE_NEW_PROCESS_GROUP if sys.platform == 'win32' else 0)
            ...
            report['auth_env_synced'] = bundle_auth.get('BAKLOG_SUPABASE_URL') == data_auth.get('BAKLOG_SUPABASE_URL') and bundle_auth.get('BAKLOG_SUPABASE_ANON_KEY') == data_auth.get('BAKLOG_SUPABASE_ANON_KEY') and (data_auth.get('BAKLOG_SUPABASE_URL') != 'https://stale.supabase.co')
            report['ok'] = all((report['migrated_index'], report['migrated_games'], report['legacy_profiles_gone'], report['auth_env_synced']))
            return report
    finally:
...
def main() -> int:
    ...
    parser.add_argument('--bundle-dir', type=Path, default=_REPO / 'release' / 'BAKLOG', help='PyInstaller onedir output (contains BAKLOG.exe)')
