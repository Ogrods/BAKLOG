...
def _run(exe, key, extra, env, timeout):
    ...
    try:
        ...
        return {
            ...
            "exit_label": EXIT_LABELS.get(proc.returncode, f"code_{proc.returncode}"),
...
def main():
    ...
    report = {
        ...
        "summary": {
            "dispatch_pass": len(dispatch_rows) - len(dispatch_fail),
            "dispatch_fail": len(dispatch_fail),
            "live_pass": len(live_rows) - len(live_fail),
            "live_fail": len(live_fail),
