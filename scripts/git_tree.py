...
def git(*args: str) -> str:
    try:
        proc = subprocess.run(['git', *args], cwd=str(ROOT), capture_output=True, text=True, encoding='utf-8', errors='replace', creationflags=_NO_WINDOW)
...
def parse_branch_row(line: str) -> dict[str, Any] | None:
    ...
    if not name:
        return None
    return {'name': name, 'sha': fields[1].strip(), 'upstream': fields[2].strip(), 'date': fields[3].strip(), 'subject': fields[4].strip()}

def collect_branches(base: str, current: str) -> list[dict[str, Any]]:
    fmt = _UNIT.join(['%(refname:short)', '%(objectname:short)', '%(upstream:short)', '%(committerdate:iso8601)', '%(contents:subject)'])
...
def collect_commits(limit: int=MAX_COMMITS) -> list[dict[str, Any]]:
    ...
    for line in raw.splitlines():
        ...
        commits.append({'sha': fields[0].strip(), 'parents': parents, 'refs': refs, 'subject': fields[3].strip(), 'date': fields[4].strip()})
    return commits
...
def collect_prs() -> dict[str, Any]:
    ...
    try:
        proc = subprocess.run(['gh', 'pr', 'list', '--state', 'open', '--json', 'number,title,headRefName,baseRefName,isDraft,url,updatedAt', '--limit', '100'], cwd=str(ROOT), capture_output=True, text=True, encoding='utf-8', errors='replace', creationflags=_NO_WINDOW)
    except (OSError, ValueError):
        return {'available': False, 'items': []}
...
def build_snapshot() -> dict[str, Any]:
    ...
    return {'generated_at': _dt.datetime.now().astimezone().isoformat(timespec='seconds'), 'repo': ROOT.name, 'base': base, 'current': current, 'dirty': bool(dirty_lines), 'dirty_count': len(dirty_lines), 'branches': collect_branches(base, current), 'commits': collect_commits(), 'graph_text': collect_graph_text(), 'prs': collect_prs(), 'max_commits': MAX_COMMITS}
...
def main(argv: list[str] | None=None) -> int:
    ...
    parser.add_argument('--fragment', action='store_true', help='Print only the inner HTML section (for embedding into tracker.html).')
    parser.add_argument('-o', '--output', default=None, help=f'Output path for the full page (default: {OUTPUT_NAME} at repo root).')
