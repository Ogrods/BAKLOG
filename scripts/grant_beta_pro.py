...
def _created_before(user, cutoff):
    if cutoff is None:
        return True
...
def main(argv=None):
    ...
    parser.add_argument(
        "--before", metavar="ISO-DATE", help="Only users created on or before this UTC date (YYYY-MM-DD)."
    )
    ...
    for user in selected:
        ...
        if args.apply and uid:
            admin_request("PUT", f"{url}/auth/v1/admin/users/{uid}", key=key, body={"app_metadata": meta})
            changed += 1
    if args.apply:
