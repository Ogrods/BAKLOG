"""Invite a hosted Supabase account (maintainer one-off).

Optional — beta testers can use Create account in the app. Use this when you
want Supabase to email a password-setup link instead.

Emails in ``packaging/pro_invitees.txt`` receive Pro automatically on sign-in
(see ``shared.comp_pro``); ``--grant-pro`` on invite is optional overlap.
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help=f"Invite link redirect URL (default: {DEFAULT_REDIRECT}).",
    )
    parser.add_argument(
        "--grant-pro",
        action="store_true",
        help="Also set plan=pro on invite (listed emails get Pro on login anyway).",
    )
    parser.add_argument(
        "--no-list-pro",
        action="store_true",
        help="Ignore pro_invitees.txt for this invite only.",
    )
    ...
    grant_pro = should_grant_pro(
        email,
        explicit=args.grant_pro,
        pro_invitees=frozenset() if args.no_list_pro else None,
    )
    grant_note = ""
