...
def main(argv=None):
    ...
    parser.add_argument(
        "--redirect-to", default=DEFAULT_REDIRECT, help=f"Invite link redirect URL (default: {DEFAULT_REDIRECT})."
    )
    parser.add_argument(
        "--grant-pro", action="store_true", help="Also set plan=pro on invite (listed emails get Pro on login anyway)."
    )
