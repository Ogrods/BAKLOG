#!/usr/bin/env python3
"""Automated server-side checks for the fetcher overhaul live test plan."""
...
def section_c(r: Results) -> None:
    ...
    # SSE stream endpoint should accept connection while run is active
    stream_ok = False
    if run_id:
        try:
    ...
    snap2 = runs()
    cancelled = any(
        h.get("id") == run_id and h.get("status") == "cancelled"
        for h in (snap2.get("history") or [])
    )
