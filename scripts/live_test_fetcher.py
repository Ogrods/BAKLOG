...
def wait_done(run_id: str, timeout: float=300.0) -> dict | None:
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        snap = runs()
...
def post_run(key: str) -> tuple[int, dict | str]:
    return api('POST', f'/api/run/{key}')

def poll_until(predicate, timeout: float=30.0, interval: float=0.05):
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        snap = runs()
        if predicate(snap):
...
def get_server_pid() -> int | None:
    out = subprocess.run(['powershell', '-Command', '(Get-NetTCPConnection -LocalPort 8765 -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty OwningProcess)'], capture_output=True, text=True, cwd=ROOT)
    try:
...
def section_b(r: Results) -> None:
    ...
    snap = poll_until(lambda s: (s.get('active') or {}).get('key') == 'steamCovers' and any((q.get('key') == 'hltb' for q in s.get('queue') or [])), timeout=15)
...
def section_d(r: Results) -> None:
    ...
    poll_until(lambda s: (s.get('active') or {}).get('key') == 'steamCovers' and any((q.get('key') == 'hltb' for q in s.get('queue') or [])), timeout=20)
    ...
    proc = subprocess.Popen(['python.exe', 'server.py'], cwd=ROOT, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, creationflags=subprocess.CREATE_NEW_PROCESS_GROUP if sys.platform == 'win32' else 0)
    time.sleep(2.5)
    for _ in range(20):
        try:
    ...
    pytest = subprocess.run([sys.executable, '-m', 'pytest', 'tests/test_run_manager.py::test_launch_timeout_marks_failed_and_admits_next', 'tests/test_run_manager.py::test_cancel_during_launch_does_not_leave_running_status', '-q'], cwd=ROOT, capture_output=True, text=True)
...
def section_e(r: Results) -> None:
    ...
    out = subprocess.run(['powershell', '-Command', '(Get-Process python* -ErrorAction SilentlyContinue).Count'], capture_output=True, text=True)
