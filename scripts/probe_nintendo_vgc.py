...
def diff_vgc_vs_catalog(vgc_rows: list[dict], catalog_games: list[dict]) -> dict:
    ...
    legacy_only = [g.get('name') or '' for g in catalog_games if g.get('nintendo_legacy') and _norm_title(g.get('name') or '') not in vgc_by_title]
    legacy_only.sort(key=_norm_title)
    return {'vgc_count': len(vgc_rows), 'catalog_count': len(catalog_games), 'catalog_fresh_count': sum((1 for g in catalog_games if not g.get('nintendo_legacy') and (not g.get('stale')))), 'catalog_legacy_count': sum((1 for g in catalog_games if g.get('nintendo_legacy'))), 'shared_title_count': len(shared), 'only_vgc_titles': only_vgc, 'only_catalog_titles': only_catalog, 'legacy_not_in_vgc_titles': legacy_only, 'sample_only_vgc': [{'name': vgc_by_title[t]['name'], 'application_id': vgc_by_title[t].get('application_id'), 'platform': vgc_by_title[t].get('platform')} for t in only_vgc[:15]], 'sample_only_catalog': [{'name': cat_by_title[t].get('name'), 'id': cat_by_title[t].get('id'), 'nintendo_legacy': bool(cat_by_title[t].get('nintendo_legacy'))} for t in only_catalog[:15]]}
...
def main() -> int:
    ...
    if not prof.exists() or not any(prof.iterdir()):
        print('Nintendo is not connected (no saved browser profile). Open Connections → Nintendo → Connect, then re-run.', file=sys.stderr)
        return 1
    try:
