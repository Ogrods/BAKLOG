...
This script creates rows via the admin API (blocks Create account). Only use when
you explicitly want admin pre-create without the signup UI.
...
Examples:
  .\\.venv\\Scripts\\python.exe scripts\\provision_pro_user.py --email you@example.com
  .\\.venv\\Scripts\\python.exe scripts\\provision_pro_user.py --email you@example.com --apply
  .\\.venv\\Scripts\\python.exe scripts\\provision_pro_user.py --from-invitees --apply
"""
...
def provision_email(base: str, key: str, email: str, *, apply: bool) -> str:
    """Return action label: created | upgraded | already_pro | dry_run_create."""
    target = email.strip()
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Pre-create hosted BAKLOG users with plan=pro (no email)."
    )
    ...
    parser.add_argument(
        "--from-invitees",
        action="store_true",
        help="Provision every address in packaging/pro_invitees.txt.",
    )
    parser.add_argument(
        "--delete",
        action="store_true",
        help="Delete listed user(s) so Create account works again (requires --apply).",
    )
    ...
    if args.apply:
        print(
            f"Done: created={counts['created']} upgraded={counts['upgraded']} "
            f"already_pro={counts['already_pro']}"
        )
