...
def _request_graceful_shutdown() -> bool:
    if not _port_open():
        return False
    req = urllib.request.Request(f'http://{HOST}:{PORT}/api/shutdown', method='POST', headers={'X-BAKLOG-Local': '1'}, data=b'')
    try:
...
def _port_pids() -> set[int]:
    ...
    if sys.platform == 'win32':
        ...
        try:
            out = subprocess.run(['lsof', '-ti', f'tcp:{PORT}'], capture_output=True, text=True, timeout=10, check=False)
...
def _cmdline_pids() -> set[int]:
    ...
    if sys.platform == 'win32':
        ...
        try:
            out = subprocess.run(['powershell', '-NoProfile', '-Command', ps_cmd], capture_output=True, text=True, timeout=15, check=False, creationflags=_NO_WINDOW)
        except (OSError, subprocess.TimeoutExpired):
            return pids
        pids.update((int(t) for t in (out.stdout or '').split() if t.isdigit()))
    else:
...
def main() -> int:
    ...
    parser.add_argument('--dedupe', action='store_true', help='Keep the live server on the port; kill only extra strays + stale pid file.')
