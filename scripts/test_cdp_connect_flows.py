...
CDP_PROVIDERS: dict[str, dict[str, object]] = {'gog': {'label': 'GOG (web)', 'fetchers': ['gog', 'wishlistGog'], 'games_json': ['games_gog.json', 'games_wishlist_gog.json'], 'login_hint': 'Land on gog.com library/account before the window closes.'}, 'battlenet': {'label': 'Battle.net', 'fetchers': ['battlenet'], 'games_json': ['games_battlenet.json'], 'login_hint': 'Open account.battle.net/games so the session is fully active.', 'test_reconnect': True}, 'nintendo': {'label': 'Nintendo', 'fetchers': ['nintendo'], 'games_json': ['games_nintendo.json'], 'login_hint': 'Let ec.nintendo.com/my/transactions/ finish loading.'}, 'humble': {'label': 'Humble Bundle', 'fetchers': ['humble', 'wishlistHumble'], 'games_json': ['games_humble.json', 'games_wishlist_humble.json'], 'login_hint': 'Complete CAPTCHA; library page opens after sign-in.'}, 'ea': {'label': 'EA App', 'fetchers': ['ea'], 'games_json': ['games_ea.json'], 'login_hint': 'After sign-in, EA deals page confirms the session.'}}
...
def api(method: str, path: str, body: dict | None=None, *, strict_local: bool=False, timeout: float=30.0) -> tuple[int, dict | str]:
    data = None
    if body is not None:
...
def chrome_available() -> bool:
    ...
    for name in ('C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe', 'C:\\Program Files (x86)\\Google\\Chrome\\Application\\chrome.exe', 'C:\\Program Files\\Microsoft\\Edge\\Application\\msedge.exe'):
...
def cmd_preflight(run: TestRun) -> int:
    ...
    run.profile_id = pid
    run.preflight = {'server_up': ok_server, 'chrome_available': ok_chrome, 'profile': profile_msg, 'url': f'{BASE}/#connections'}
...
def save_results(run: TestRun) -> None:
    ...
    payload = {'started_at': run.started_at, 'finished_at': run.finished_at, 'profile_id': run.profile_id, 'profile_label': run.profile_label, 'preflight': run.preflight, 'providers': [asdict(p) for p in run.providers]}
    RESULTS_PATH.write_text(json.dumps(payload, indent=2) + '\n', encoding='utf-8')
    print(f'\nResults written to {RESULTS_PATH.relative_to(ROOT)}')

def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('command', choices=('preflight', 'status', 'fetch', 'all'), help='preflight | status | fetch | all')
    parser.add_argument('--provider', help='Single provider key for fetch')
    parser.add_argument('--launch-connect', action='store_true', help='Open headed browser for disconnected providers (manual sign-in still required)')
    parser.add_argument('--wait-connect', type=float, default=0.0, metavar='SECS', help='After --launch-connect, poll auth status up to SECS (default 0 = do not wait)')
