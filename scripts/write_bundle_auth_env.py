...
def render_env_lines(values: dict[str, str]) -> list[str]:
    lines = ['# BAKLOG account auth (bundled for beta builds).', '# Anon key is public; included so the local app can require sign-in.']
    for key in BUNDLE_KEYS:
        if key in values:
...
def main(argv: list[str] | None=None) -> int:
    ...
    if missing:
        print('Missing auth env for frozen bundle: ' + ', '.join(missing) + '. Set them in packaging/bundle-auth.env, repo .env, or CI env.', file=sys.stderr)
        return 1
