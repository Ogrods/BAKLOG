...
from shared.install_paths import bundle_root, data_root, is_frozen, load_built_manifest, serve_built_frontend, static_root
...
def _warn_built_manifest_version_mismatch() -> None:
    ...
    if built_ver and built_ver != app_ver:
        print(f'WARNING: dist/manifest.json version {built_ver} != app {app_ver} — run npm run build before testing packaged frontend', flush=True)
ROOT = data_root()
try:
...
_DEV_SERVER_BUSY_MSG = f'BAKLOG dev server is already running on http://{HOST}:{PORT} — stop that instance first (it owns the port).'
MAX_HISTORY = 200
...
from shared.log_redact import redact_diagnostics_payload as _redact_diagnostics_payload, redact_log_line as _redact_log_line
from shared.server_epic_oauth import register_epic_oauth_state as _register_epic_oauth_state
from shared.server_stream_tickets import STREAM_ATTACH_LONG_WAIT_SEC as _STREAM_ATTACH_LONG_WAIT_SEC, STREAM_ATTACH_POLL_SEC as _STREAM_ATTACH_POLL_SEC, STREAM_ATTACH_SHORT_WAIT_SEC as _STREAM_ATTACH_SHORT_WAIT_SEC, commit_stream_ticket as _commit_stream_ticket, consume_stream_ticket as _consume_stream_ticket, mint_stream_ticket as _mint_stream_ticket, peek_stream_ticket as _peek_stream_ticket, stream_ticket_from_handler as _stream_ticket_from_handler
...
from shared.profile_paths import PROFILE_CACHE_JSON_FILES, cache_json_path, catalog_path, clear_request_profile_id, free_claims_path, get_active_profile_id, personal_backup_dir, personal_path, profile_root, runs_dir, set_request_profile_id, sponsors_path
...
def _python_executable() -> str:
    ...
    if override:
        return override
    candidates = [ROOT / '.venv' / 'Scripts' / 'python.exe', ROOT / '.venv' / 'bin' / 'python', ROOT / '.venv' / 'bin' / 'python3']
    for c in candidates:
        if c.exists():
            return str(c)
    exe = sys.executable
    ...
    if 'WindowsApps/python.exe' in exe_norm or exe_norm.endswith('/WindowsApps/python'):
        try:
            ...
            proc = subprocess.run(['py', '-3.13', '-c', 'import sys; print(sys.executable)'], capture_output=True, text=True, timeout=15, check=False, creationflags=flags)
...
def _load_fetchers() -> dict[str, dict[str, Any]]:
    ...
    for entry in entries:
        ...
        fetchers[key] = {'label': label, 'argv': _fetcher_argv(key, script, extra_args), 'refreshArgs': [str(a) for a in refresh_args], 'metaKey': entry.get('metaKey', key), 'group': entry.get('group', 'library'), 'color': entry.get('color'), 'requires': requires, 'platforms': [str(p) for p in platforms], 'maxRunSeconds': max_run_seconds}
    return fetchers
...
_DEFAULT_INTERNAL_JOBS: dict[str, dict[str, Any]] = {'claimSources': {'label': 'Fetch claim sources', 'script': 'fetchers/fetch_claim_sources.py', 'group': 'claims', 'description': 'Auto-discover free claims from Epic, GamerPower, and ITAD giveaways RSS', 'args': [], 'options': {'--dry-run': {'type': 'bool', 'default': False}, '--source': {'type': 'enum', 'values': ['all', 'epic', 'gamerpower', 'itad'], 'default': 'all'}}}, 'buildClaims': {'label': 'Build free claims feed', 'script': 'fetchers/build_free_claims.py', 'group': 'claims', 'description': 'Merge manual + approved auto items, enrich with Steam metadata, publish feed', 'args': [], 'options': {'--dry-run': {'type': 'bool', 'default': False}, '--no-profile': {'type': 'bool', 'default': False}, '--allow-empty': {'type': 'bool', 'default': False}, '--require-manual-approval': {'type': 'bool', 'default': False}}}}
...
def _normalize_internal_job_entry(raw: dict[str, Any]) -> tuple[str, dict[str, Any]] | None:
    ...
    if not isinstance(options, dict):
        options = {}
    return (key, {'label': str(raw.get('label') or key), 'script': script, 'group': str(raw.get('group') or 'internal'), 'description': str(raw.get('description') or ''), 'args': [str(a) for a in args], 'options': options})
...
def _send_bytes(handler: SimpleHTTPRequestHandler, status: int, body: bytes, *, content_type: str, filename: str | None=None) -> None:
    handler.send_response(status)
...
def _maybe_serve_empty_sponsors_json(handler: SimpleHTTPRequestHandler, path: str) -> bool:
    ...
    return True
_EMPTY_CACHE_META_JSON: dict[str, dict[str, Any]] = {'hltb_map.json': {'fetched_at': None}, 'steam_review_map.json': {'fetched_at': None}, 'cross_store_images_meta.json': {'fetched_at': None, 'no_steam_match': []}, 'steam_tags_meta.json': {'fetched_at': None}, 'protondb_map.json': {'fetched_at': None}, 'fx_rates.json': {'fetched_at': None, 'rates': {}}}
...
def _read_json_body(handler: SimpleHTTPRequestHandler, *, max_bytes: int=PERSONAL_MAX_BYTES) -> tuple[dict[str, Any] | None, str | None]:
    try:
...
def _api_error(handler: SimpleHTTPRequestHandler, status: int, code: str, exc: BaseException | None=None) -> None:
    if exc is not None:
        try:
...
def _wait_for_stream_target(run_id: str, *, since: int) -> tuple[Run | None, dict[str, Any] | None]:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def _reject_if_csrf(self) -> bool:
        if _csrf_allowed(self):
            return False
        _send_json(self, HTTPStatus.FORBIDDEN, {'error': 'cross-origin request blocked — open BAKLOG from http://127.0.0.1 and retry'})
        return True
    ...
    def do_POST(self) -> None:
        ...
        if path == '/api/auth/stream-ticket':
            from shared.supabase_auth import auth_enabled, verify_bearer_user
            if auth_enabled() and ADMIN_ENABLED and (verify_bearer_user(self.headers.get('Authorization')) is None) and _request_host_is_local(self) and (self.headers.get(_BAKLOG_LOCAL_HEADER) == '1'):
        ...
        if path.startswith('/api/auth/') and path.endswith('/enable'):
            ...
            return
        if path in ('/api/auth/master-password', '/api/auth/secrets/export', '/api/auth/secrets/reset') or path.startswith('/api/auth/secrets/import'):
            if self._reject_if_csrf_strict():
                return
    ...
    def _handle_support_get(self, path: str) -> None:
        ...
        if handle_update_support_get(path, current_version=_app_version, has_in_flight_runs=lambda: bool(MANAGER._in_flight_targets()), has_active_sessions=has_active_sessions, send_json=lambda status, payload: _send_json(self, status, payload)):
            return
        _send_json(self, HTTPStatus.OK, _redact_diagnostics_payload(build_diagnostics_payload(data_root=ROOT, version=_app_version(), load_run_history=_load_run_history)))

    def _handle_fetchers(self) -> None:
        try:
            data = {'server_platform': sys.platform, 'fetchers': [{'key': k, 'label': v['label'], 'cmd': _fetcher_cmd_label(v['argv']), 'metaKey': v.get('metaKey', k), 'group': v.get('group', 'library'), 'color': v.get('color'), 'requires': v.get('requires') or [], 'missing_requirements': _missing_requirements(v.get('requires') or []), 'supports_refresh': bool(v.get('refreshArgs')), 'platforms': v.get('platforms') or [], 'available': platform_supported(v.get('platforms'))} for k, v in FETCHERS.items()]}
    ...
    def _handle_profiles_create(self) -> None:
        if _profile_admin_blocked():
            _send_json(self, HTTPStatus.FORBIDDEN, {'error': 'profile management is disabled while account sign-in is enabled'})
            return
    ...
    def _handle_profiles_set_active(self) -> None:
        if _profile_admin_blocked():
            _send_json(self, HTTPStatus.FORBIDDEN, {'error': 'profile switching is disabled while account sign-in is enabled'})
            return
        payload, err = _read_json_body(self)
        if err:
            _send_json(self, HTTPStatus.BAD_REQUEST, {'error': err})
        ...
        if not profile_id:
            ...
            return
        try:
            from shared.profiles import clear_pin_failures, pin_rate_limit_error, profile_requires_pin, record_pin_failure, set_active_profile, verify_profile_pin
            ...
            if has_active_sessions():
                _send_json(self, HTTPStatus.CONFLICT, {'error': 'Finish or cancel the sign-in window before switching profiles.'})
                return
    ...
    def _handle_profiles_delete(self, profile_id: str) -> None:
        ...
        if MANAGER.has_runs_for_profile(profile_id):
            _send_json(self, HTTPStatus.CONFLICT, {'error': 'This profile has a fetch running or queued. Cancel its runs or let them finish before deleting.'})
            return
    ...
    def _handle_personal_put(self) -> None:
        ...
        if claimed is not None:
            ...
            if str(claimed) != active:
                _send_json(self, HTTPStatus.CONFLICT, {'error': 'profile mismatch', 'active': active, 'claimed': str(claimed)})
                return
    ...
    def _handle_submit(self, rest: str) -> None:
        ...
        if not platform_supported(fetcher_platforms):
            ...
            _send_json(self, HTTPStatus.BAD_REQUEST, {'error': f"{FETCHERS[key]['label']} is only available on {allowed} (this server runs on {sys.platform})."})
            return
        ...
        try:
            ...
            _send_json(self, HTTPStatus.CONFLICT, {'error': str(exc)})
            return
        _send_json(self, HTTPStatus.ACCEPTED, {'run_id': run.id, 'key': run.key, 'label': run.label, 'status': run.status})
    ...
    def _handle_update_post(self, path: str) -> None:
        ...
        handle_update_post(path, current_version=_app_version, has_in_flight_runs=lambda: bool(MANAGER._in_flight_targets()), has_active_sessions=has_active_sessions, read_json_body=lambda: _read_json_body(self, max_bytes=4096), send_json=lambda status, payload: _send_json(self, status, payload), trigger_shutdown=_trigger_dev_shutdown)
    ...
    def _handle_auth_status(self) -> None:
        try:
            ...
            _send_json(self, HTTPStatus.OK, {'server_platform': sys.platform, 'providers': get_status(), 'secrets_corrupt': secrets_store_corrupt()})
        except Exception as exc:
    ...
    def _handle_auth_stream(self, session_id: str) -> None:
        ...
        with _sse_lock:
            if _sse_connections >= MAX_SSE_CONNECTIONS:
                _send_json(self, HTTPStatus.SERVICE_UNAVAILABLE, {'error': f'too many stream connections (max {MAX_SSE_CONNECTIONS})'})
                return
            _sse_connections += 1
    ...
    def _handle_stream(self, run_id: str) -> None:
        ...
        if run is None and terminal is not None:
            ...
            self.end_headers()
            try:
                self._sse_write('status', {'status': terminal.get('status'), 'started_at': terminal.get('started_at'), 'ended_at': terminal.get('ended_at'), 'exit_code': terminal.get('exit_code')})
                self._sse_write('done', {'status': terminal.get('status'), 'exit_code': terminal.get('exit_code'), 'started_at': terminal.get('started_at'), 'ended_at': terminal.get('ended_at')})
            except (BrokenPipeError, ConnectionResetError, ConnectionAbortedError, OSError):
                pass
            finally:
                with _sse_lock:
        ...
        try:
            self._sse_write('status', {'status': run.status, 'started_at': run.started_at, 'ended_at': run.ended_at, 'exit_code': run.exit_code})
            for msg in replay:
                self._sse_write('line', msg, event_id=msg.get('seq'))
            if already_done:
                self._sse_write('done', {'status': run.status, 'exit_code': run.exit_code, 'started_at': run.started_at, 'ended_at': run.ended_at})
                return
            last_ping = time.time()
            while True:
                try:
...
def _start_background_scheduler() -> None:
    ...
    try:
        ...
        SCHEDULER = BackgroundScheduler(manager=MANAGER, fetchers=FETCHERS, missing_requirements=lambda reqs: _missing_requirements(reqs))
        SCHEDULER.start()
    except Exception as exc:
...
def _maybe_import_legacy_env() -> None:
    ...
    if err:
        ...
        print(f"[auth] Imported {count} provider(s) from .env into profile '{DEFAULT_PROFILE_ID}' (plaintext credentials removed from .env)", flush=True)
...
def main() -> None:
    ...
    with httpd:
        ...
        if _SERVER_ENV_PROFILE_OVERRIDE:
            print(f'NOTE: BAKLOG_PROFILE={_SERVER_ENV_PROFILE_OVERRIDE!r} was set in the server shell; ignoring it so the profile menu owns the active profile (now {get_active_profile_id()!r}). Per-run fetchers still pin their own profile.', flush=True)
