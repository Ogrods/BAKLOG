...
def ensure_comp_pro_on_login(user_id: str, email: str) -> tuple[bool, bool]:
    """Upgrade listed emails to Pro on sign-in.

    Returns ``(should_be_pro, upgraded_now)``. ``upgraded_now`` is True when
    Supabase ``app_metadata.plan`` was written this call.
    """
...
# Back-compat names for invite_beta_user.py
