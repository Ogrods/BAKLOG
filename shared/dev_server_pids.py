...
def pid_alive(pid: int) -> bool:
    ...
    if sys.platform == 'win32':
        try:
            out = subprocess.run(['tasklist', '/FI', f'PID eq {pid}', '/FO', 'CSV', '/NH'], capture_output=True, text=True, timeout=5, check=False, creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0))
...
def pid_listening_on_port(host: str=DEFAULT_HOST, port: int=DEFAULT_PORT) -> int | None:
    if sys.platform != 'win32':
        return None
    try:
        out = subprocess.run(['netstat', '-ano', '-p', 'TCP'], capture_output=True, text=True, timeout=5, check=False, creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0))
    except (OSError, subprocess.TimeoutExpired):
        return None
    needle = f'{host}:{port}'
...
def reclaim_stale_server(host: str, port: int, pid_file: str | os.PathLike[str]) -> bool:
    me = os.getpid()
    for pid in (read_pid_file(pid_file), pid_listening_on_port(host, port)):
        ...
        if not pid_is_python_server(pid):
            continue
        print(f'[server] port {port} held by orphaned instance (pid {pid}) - reclaiming it', file=sys.stderr, flush=True)
        terminate_pid(pid)
        return True
    return False

def reclaim_or_exit(host: str, port: int, pid_file: str | os.PathLike[str], busy_msg: str) -> None:
