...
def maybe_refresh_local_license(*, force: bool=False) -> None:
    ...
    if result.get('ok'):
        write_license_document({**doc, 'plan': PLAN_PRO, 'key': stored_key.strip(), 'validated_at': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())})
        return
    write_license_document({'plan': PLAN_FREE, 'key': stored_key.strip(), 'validated_at': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())})
...
def activate_local_license_key(key: str) -> tuple[bool, str]:
    ...
    write_license_document({'plan': PLAN_PRO, 'key': cleaned, 'validated_at': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())})
    global _LICENSE_REFRESH_AT
