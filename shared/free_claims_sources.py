...
EPIC_FREE_GAMES_URL = 'https://store-site-backend-static.ak.epicgames.com/freeGamesPromotions?locale=en-US&country=US&allowCountries=US'
...
def parse_epic_element(element: dict, *, now: datetime | None=None) -> dict | None:
    ...
    if not title:
        return None
    return {'id': f'epic-{slug}', 'store': 'epic', 'title': title, 'claim_url': f'https://store.epicgames.com/en-US/p/{slug}', 'ends_at': _to_iso_z(_parse_iso_dt(offer.get('endDate'))), 'header_image': _epic_header_image(element), 'blurb': None, 'source': 'epic'}
...
def parse_gamerpower_item(raw: dict) -> dict | None:
    ...
    return {'id': f'gamerpower-{gp_id}', 'store': platforms_to_store(str(raw.get('platforms') or '')), 'title': title, 'claim_url': claim_url, 'ends_at': _to_iso_z(end_dt), 'header_image': raw.get('image') or raw.get('thumbnail'), 'blurb': raw.get('description'), 'source': 'gamerpower'}
...
def _itad_store_from_text(text: str) -> str:
    lower = (text or '').lower()
    for store, needles in (('steam', ('steam',)), ('epic', ('epic',)), ('gog', ('gog',)), ('itch', ('itch.io', 'itch')), ('ubisoft', ('ubisoft',)), ('humble', ('humble',))):
...
def parse_itad_rss(xml_text: str) -> list[dict]:
    ...
    for item in root.findall('./channel/item'):
        ...
        items.append({'id': f'itad-{digest}', 'store': _itad_store_from_text(f'{title} {description}'), 'title': title, 'claim_url': link, 'ends_at': None, 'header_image': None, 'blurb': description or None, 'source': 'itad'})
    return items