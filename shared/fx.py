"""Exchange rates (Frankfurter) with on-disk cache and amount conversion."""
...
def round_amount(amount: float, currency: str) -> float:
    """Round converted amounts per currency (JPY/KRW: whole units)."""
...
def parse_price_amount(value: Any) -> float | None:
    """Extract a numeric amount from a formatted price string or number."""
    if value is None:
        return None
...
def _fetch_from_api() -> dict[str, Any]:
    req = urllib.request.Request(
        FRANKFURTER_LATEST,
        headers={"Accept": "application/json", "User-Agent": "steam-backlog/1.0"},
    )
...
def ensure_fx_rates(
    *,
    ...
    """Return cached rates, refreshing from Frankfurter when missing or older than 24h."""
    ...
    print(
        f"FX rates refreshed ({doc.get('date') or 'latest'}, base {doc.get('base')}, "
        f"{len(doc.get('rates') or {})} pairs).",
        flush=True,
    )
    return doc
...
def convert(
    ...
    """Convert amount between currencies using a Frankfurter-style rates document."""
