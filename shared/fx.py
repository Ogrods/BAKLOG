...
def load_fx_rates(*, profile_id=None):
...
def _fetch_from_api():
    req = urllib.request.Request(
        FRANKFURTER_LATEST, headers={"Accept": "application/json", "User-Agent": "steam-backlog/1.0"}
    )
...
def _warn_stale_fx_cache(age):
    print(
        f"WARN: FX rates are {int(age / 86400)}d old (>{STALE_WARN_SECONDS // 86400}d); re-run fetch_itad to refresh.",
        file=sys.stderr,
        flush=True,
    )


def _return_cached_or_raise(cached, age, *, warn_stale, err=None):
...
def save_fx_rates(doc, *, profile_id=None):
    path = fx_rates_path(profile_id=profile_id)
...
def ensure_fx_rates(*, force=False, warn_stale=True, profile_id=None):
    ...
    print(
        f"FX rates refreshed ({doc.get('date') or 'latest'}, base {doc.get('base')}, {len(doc.get('rates') or {})} pairs).",
        flush=True,
    )
    return doc


def convert(amount, from_ccy, to_ccy, rates_doc):
