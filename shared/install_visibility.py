...
def install_trust_fields() -> dict[str, Any]:
    ...
    if sys.platform == 'win32':
        fields['trust_note'] = 'Unsigned beta: SmartScreen may warn on first launch — More info, then Run anyway. In-app zip updates do not refresh Add/Remove Programs; re-run Setup when you want ARP to match.'
    elif sys.platform == 'darwin':
        fields['trust_note'] = 'Unsigned beta: Gatekeeper may block after in-app update — right-click Open once, or run: xattr -cr /path/to/BAKLOG.app (or the BAKLOG folder) in Terminal.'
    else:
...
def install_visibility_fields(current_version: str) -> dict[str, Any]:
    ...
    return {'install_source': detect_install_source(), 'arp_version': arp, 'arp_version_mismatch': arp_version_mismatch(current_version, arp), **install_trust_fields()}