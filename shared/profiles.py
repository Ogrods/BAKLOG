...
from shared.profile_paths import DEFAULT_PROFILE_ID, get_active_profile_id, is_legacy_layout, load_index, mutate_index, normalize_profile_id, profile_data_dir, unique_profile_id_for_doc
...
_AUTH_SKIP_DIR_NAMES = frozenset({'Extensions', 'Cache', 'Code Cache', 'GPUCache', 'Service Worker', 'IndexedDB', 'blob_storage', 'Default'})
...
def _copy_tree_if_missing(src: Path, dst: Path, *, skip_dir_names: frozenset[str] | None=None) -> int:
    if not src.exists():
        return 0
    copied = 0
    if src.is_file():
...
def create_profile(label: str) -> dict[str, Any]:
    ...
    try:
        ...
        with mutate_index() as doc:
            doc['profiles'] = [p for p in doc.get('profiles', []) if not (isinstance(p, dict) and p.get('id') == profile_id)]
...
def rename_profile(profile_id: str, label: str, *, current_pin: str | None=None) -> dict[str, Any]:
    profile_id = normalize_profile_id(profile_id)
    label = _validate_label(label)
    with mutate_index() as doc:
...
def set_profile_pin(profile_id: str, pin: str, current_pin: str | None=None) -> None:
    ...
    with mutate_index() as doc:
        ...
        if profile_has_pin(profile_id, doc):
            ...
            clear_pin_failures(profile_id)
        salt = os.urandom(16)
        p['pin'] = {'salt': base64.b64encode(salt).decode('ascii'), 'hash': base64.b64encode(_hash_pin(pin, salt)).decode('ascii'), 'n': _PIN_SCRYPT_N}
...
def _public_profile_row(p: dict[str, Any], doc: dict[str, Any]) -> dict[str, Any]:
    pid = str(p.get('id') or '')
    return {'id': pid, 'label': p.get('label'), 'created_at': p.get('created_at'), 'hasPin': profile_has_pin(pid, doc) if pid else False}
...
def profiles_status() -> dict[str, Any]:
    ...
    return {'active': active, 'active_label': active_label, 'legacy': is_legacy_layout(active), 'profiles': [_public_profile_row(p, doc) for p in profiles if isinstance(p, dict) and p.get('id')]}