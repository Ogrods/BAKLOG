...
def handle_auth_secrets_export(handler: SimpleHTTPRequestHandler) -> None:
    s = _srv()
...
def handle_auth_secrets_reset(handler: SimpleHTTPRequestHandler) -> None:
    s = _srv()
    payload, err = s._read_json_body(handler, max_bytes=s._AUTH_JSON_MAX_BYTES)
    if err:
        s._send_json(handler, HTTPStatus.BAD_REQUEST, {'error': err})
    ...
    try:
        ...
        if not secrets_store_corrupt():
            s._send_json(handler, HTTPStatus.BAD_REQUEST, {'error': 'secrets store is readable; reset refused', 'code': 'not_corrupt'})
            return
        reset_secrets_store()
