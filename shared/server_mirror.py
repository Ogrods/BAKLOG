...
from shared.cloud_mirror import download_remote_mirror_artifact, import_remote_mirror_to_profile, list_remote_mirror_snapshot, mirror_read_allowed, read_mirror_upload_state
...
def handle_mirror_get(handler: Any) -> None:
    ...
    try:
        if artifact:
            body = download_remote_mirror_artifact(authorization=authorization, artifact_path=artifact, profile_id=profile)
        else:
            snap = list_remote_mirror_snapshot(authorization=authorization, profile_id=profile)
            local_state = read_mirror_upload_state(profile_id=profile)
            payload: dict[str, Any] = {'artifacts': snap.get('artifacts') or [], 'localUploadState': local_state}
            profiles = snap.get('profiles')
            if isinstance(profiles, list) and profiles:
                payload['profiles'] = profiles
            srv._send_json(handler, HTTPStatus.OK, payload)
            return
...
def handle_mirror_import_post(handler: Any) -> None:
    ...
    if paths_raw is not None:
        ...
        paths = paths_raw
    try:
        result = import_remote_mirror_to_profile(authorization=authorization, paths=paths, include_personal=include_personal, allow_empty_catalogs=allow_empty_catalogs)
    except PermissionError as exc:
        srv._send_json(handler, HTTPStatus.UNAUTHORIZED, {'error': str(exc)})
        return
    except ValueError as exc:
