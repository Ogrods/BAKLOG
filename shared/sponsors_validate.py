...
def _validate_sponsor_creative(ad_id: str, item: Any) -> str | None:
    ...
    if cover is not None and str(cover).strip():
        c = str(cover).strip()
        if not (c.startswith('http://') or c.startswith('https://') or (c.startswith('/') and (not c.startswith('//')))):
...
def _validate_sponsors_v1(doc: dict[str, Any]) -> str | None:
    ...
    for i, item in enumerate(items):
        ...
        if placements is not None and placements != '':
            ...
            valid = {'deal-rail', 'dash-deal-rail', 'spotlight', 'picks', 'table', 'dash-picks', 'dash-feature-banner', 'dash-versus', 'coop-online', 'coop-couch', 'claimable'}
            for p in raw:
