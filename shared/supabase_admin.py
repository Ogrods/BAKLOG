...
def admin_request(method: str, url: str, *, key: str, body: dict | None=None) -> dict | list:
    data = None
...
def invite_user_by_email(base: str, key: str, email: str, *, redirect_to: str) -> dict:
...
def create_user_by_email(base: str, key: str, email: str, *, plan: str='pro', email_confirm: bool=True) -> dict:
    body: dict = {'email': email.strip(), 'email_confirm': email_confirm, 'app_metadata': {'plan': plan}}
...
def delete_user_by_id(base: str, key: str, user_id: str) -> None:
    ...
    req = urllib.request.Request(f'{base}/auth/v1/admin/users/{uid}', method='DELETE', headers={'apikey': key, 'Authorization': f'Bearer {key}'})
    try:
        with urllib.request.urlopen(req, timeout=60) as resp:
            resp.read()
    except urllib.error.HTTPError as exc:
