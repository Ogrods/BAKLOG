...
def public_auth_config() -> dict[str, Any]:
    return {'supabaseUrl': _supabase_url(), 'supabaseAnonKey': _anon_key(), 'authRequired': auth_enabled(), 'localProfiles': local_profiles_enabled(), 'authConfirmRedirectUrl': os.environ.get('BAKLOG_AUTH_CONFIRM_REDIRECT_URL', 'https://baklog.app/auth/confirmed').strip(), 'authResetRedirectUrl': os.environ.get('BAKLOG_AUTH_RESET_REDIRECT_URL', 'https://baklog.app/auth/reset').strip()}
...
def _decode_hs256(raw: str) -> dict[str, Any] | None:
    ...
    try:
        issuer = _jwt_issuer()
        payload = jwt.decode(raw, secret, algorithms=list(_HS256_ALGORITHMS), audience=_AUDIENCE, issuer=issuer, options={'require': list(_JWT_REQUIRED_CLAIMS)})
...
def _decode_jwks(raw: str) -> dict[str, Any] | None:
    ...
    for attempt in range(3):
        ...
        try:
            ...
            issuer = _jwt_issuer()
            payload = jwt.decode(raw, signing_key.key, algorithms=list(_JWKS_ALGORITHMS), audience=_AUDIENCE, issuer=issuer, options={'require': list(_JWT_REQUIRED_CLAIMS)})
        except InvalidTokenError:
            return None
        except Exception:
