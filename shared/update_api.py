...
def _mgr(*, current_version: Callable[[], str], has_in_flight_runs: Callable[[], bool], has_active_sessions: Callable[[], bool] | None=None):
    return get_update_manager(current_version=current_version, has_in_flight_runs=has_in_flight_runs, has_active_sessions=has_active_sessions)

def handle_update_post(path: str, *, current_version: Callable[[], str], has_in_flight_runs: Callable[[], bool], has_active_sessions: Callable[[], bool] | None=None, read_json_body: Callable[[], tuple[dict[str, Any] | None, str | None]], send_json: Callable[[HTTPStatus, dict[str, Any]], None], trigger_shutdown: Callable[[], None]) -> None:
    mgr = _mgr(current_version=current_version, has_in_flight_runs=has_in_flight_runs, has_active_sessions=has_active_sessions)
...
def handle_update_support_get(path: str, *, current_version: Callable[[], str], has_in_flight_runs: Callable[[], bool], has_active_sessions: Callable[[], bool] | None=None, send_json: Callable[[HTTPStatus, dict[str, Any]], None]) -> bool:
    ...
    if path == '/api/update-check':
        ...
        send_json(HTTPStatus.OK, build_update_check_payload(current_version(), fetchers_in_flight=has_in_flight_runs(), sign_in_active=sign_in_active))
        return True
