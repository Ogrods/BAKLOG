...
from shared.update_platform import apply_script_name, is_in_app_apply_platform, launch_apply_subprocess, server_binary_name
from shared.update_ready_state import clear_apply_result, clear_ready_state, read_apply_result, scan_ready_state, write_ready_state
from shared.update_release import ReleaseArtifacts, UpdateSecurityError, fetch_release_artifacts, fetch_url_to_file, verify_file_sha256
...
class UpdateManager:
    ...
    def __init__(self, *, current_version: Callable[[], str], has_in_flight_runs: Callable[[], bool], has_active_sessions: Callable[[], bool] | None=None, work_root: Path | None=None) -> None:
    ...
    def _rehydrate_from_disk(self) -> None:
        ...
        with self._lock:
            ...
            self._artifacts = ReleaseArtifacts(tag=f'v{version}', version=version, html_url=html_url or '', zip_url=zip_url or '', sha256=sha256)
            self._status = UpdateStatus(phase='ready', progress_bytes=size, total_bytes=size, version=version, error=None, ready=True, can_apply=True)
    ...
    def start_download(self) -> dict[str, Any]:
        ...
        self._set_status(phase='downloading', progress_bytes=0, total_bytes=None, version=artifacts.version, error=None, ready=False, can_apply=False)
        self._thread = threading.Thread(target=self._download_worker, args=(artifacts,), name='baklog-update-download', daemon=True)
        self._thread.start()
    ...
    def discard_ready_update(self) -> dict[str, Any]:
        ...
        self._set_status(phase='idle', progress_bytes=0, total_bytes=None, version=None, error=None, ready=False, can_apply=False)
    ...
    def apply_ready_update(self) -> dict[str, Any]:
        ...
        manifest = {'install_dir': str(install_dir), 'zip_path': str(zip_path), 'sha256': sha256, 'version': artifacts.version, 'server_pid': os.getpid(), 'tray_pid': int(os.environ.get('BAKLOG_TRAY_PID', '0') or 0)}
        ...
        try:
            ...
            if zip_path.is_file() and artifacts.sha256:
                try:
                    ...
                    with self._lock:
                        self._zip_path = zip_path
                    write_ready_state(self._work_root, version=artifacts.version, sha256=artifacts.sha256, zip_path=zip_path, zip_url=artifacts.zip_url, html_url=artifacts.html_url)
                    self._set_status(phase='ready', ready=True, can_apply=True, progress_bytes=zip_path.stat().st_size, total_bytes=zip_path.stat().st_size, error=None)
                    return
                except UpdateSecurityError:
            ...
            if artifacts.sha256:
                ...
                self._set_status(phase='error', error='Release missing sha256 sidecar — download kept but apply blocked', ready=False, can_apply=False, progress_bytes=written, total_bytes=written)
                with self._lock:
                    self._zip_path = zip_path
                return
            with self._lock:
                self._zip_path = zip_path
            write_ready_state(self._work_root, version=artifacts.version, sha256=artifacts.sha256 or '', zip_path=zip_path, zip_url=artifacts.zip_url, html_url=artifacts.html_url)
            self._set_status(phase='ready', ready=True, can_apply=True, progress_bytes=written, total_bytes=written, error=None)
        except UpdateSecurityError as exc:
            self._cleanup_partial()
...
def get_update_manager(*, current_version: Callable[[], str], has_in_flight_runs: Callable[[], bool], has_active_sessions: Callable[[], bool] | None=None) -> UpdateManager:
    global _MANAGER
    if _MANAGER is None:
        _MANAGER = UpdateManager(current_version=current_version, has_in_flight_runs=has_in_flight_runs, has_active_sessions=has_active_sessions)
    return _MANAGER
