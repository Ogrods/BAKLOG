APPLY_BLOCKED_MESSAGES = {
    ...
    "missing_apply_script": "This install cannot auto-update yet. Download once from the release page to enable in-app updates.",
    "platform_zip_missing": "No install package for your system on this release yet. Use the release page to update manually.",
    "sha256_missing": "This release is missing a verification file. Use the release page to update safely.",
}
_UPDATE_ERROR_ALIASES = (
    ("Updates apply only to installed BAKLOG builds", "dev_runtime", APPLY_BLOCKED_MESSAGES["dev_runtime"]),
    (
        "In-app apply is not supported on this platform",
        "platform_not_supported",
        APPLY_BLOCKED_MESSAGES["platform_not_supported"],
    ),
    ("Move BAKLOG out of a temporary folder before updating", "temp_install", APPLY_BLOCKED_MESSAGES["temp_install"]),
    (
    ...
    ("Already on latest release", "already_latest", "You are already on the latest release."),
    (
        "No verified update package is ready",
        "not_ready",
        "No verified update is ready to install. Check for updates first.",
    ),
    ("Release sha256 unavailable", "sha256_missing", APPLY_BLOCKED_MESSAGES["sha256_missing"]),
    ("download cancelled", "download_cancelled", "Update download cancelled."),
    ("missing from install", "missing_apply_script", APPLY_BLOCKED_MESSAGES["missing_apply_script"]),
    (
...
def humanize_update_error(error):
    text = (error or "").strip()
    if not text:
...
def enrich_update_api_payload(payload):
