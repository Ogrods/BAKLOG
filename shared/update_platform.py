...
_PLATFORM_ARTIFACTS: dict[str, tuple[str, str]] = {'win32': ('BAKLOG-win64.zip', 'BAKLOG-win64.sha256'), 'darwin': ('BAKLOG-macos.zip', 'BAKLOG-macos.sha256')}
_PLATFORM_BUNDLE_FILES: dict[str, tuple[str, ...]] = {'win32': ('BAKLOG.exe', 'BAKLOG Tray.exe'), 'darwin': ('BAKLOG', 'BAKLOG Tray')}
_PLATFORM_APPLY_SCRIPT: dict[str, str] = {'win32': 'apply_update.ps1', 'darwin': 'apply_update.sh'}
...
def launch_apply_subprocess(*, script: Path, manifest_path: Path, install_dir: Path) -> subprocess.Popen:
    ...
    if platform == 'win32':
        cmd = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', str(script), '-ManifestPath', str(manifest_path)]
