...
from shared.update_platform import allowed_asset_names, release_platform, required_bundle_files, server_binary_name, stable_sha256_name, stable_zip_name
...
def build_release_artifacts(release: dict[str, Any], platform: str | None=None) -> ReleaseArtifacts:
    ...
    return ReleaseArtifacts(tag=tag, version=version, html_url=html_url, zip_url=zip_url, sha256=sha256, sha256_url=sha256_url, release_notes=notes, published_at=published)
...
def fetch_url_to_file(url: str, dest: Path, *, max_bytes: int=MAX_DOWNLOAD_BYTES, on_progress: Callable[[int, int | None], None] | None=None) -> int:
    import urllib.error
    import urllib.request
    if not is_allowed_download_url(url):
