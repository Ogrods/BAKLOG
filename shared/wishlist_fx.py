...
def display_currency_for_profile(*, profile_id: str | None=None) -> str:
    ...
    return country_to_currency(country)
_FX_FIELDS = ('currency', 'price', 'price_initial', 'price_amount', 'price_amount_initial', 'currency_native', 'price_native', 'price_initial_native', 'fx_converted')

def _snapshot(game: dict[str, Any]) -> tuple[Any, ...]:
    return tuple((game.get(k) for k in _FX_FIELDS))

def apply_fx_to_game(game: dict[str, Any], target_ccy: str, rates_doc: dict[str, Any]) -> bool:
    ...
    if native_ccy == target_ccy:
        ...
        for key in ('currency_native', 'price_native', 'price_initial_native', 'price_amount', 'price_amount_initial', 'fx_converted'):
            game.pop(key, None)
...
def apply_fx_to_games(games: list[dict[str, Any]], target_ccy: str, rates_doc: dict[str, Any]) -> int:
    n = 0
    for g in games:
...
def apply_fx_to_catalog_payload(payload: dict[str, Any], target_ccy: str, rates_doc: dict[str, Any]) -> int:
...
def apply_fx_to_wishlist_file(filename: str, target_ccy: str, rates_doc: dict[str, Any], *, profile_id: str | None=None) -> int:
...
def apply_fx_to_all_wishlist_files(target_ccy: str | None=None, rates_doc: dict[str, Any] | None=None, *, profile_id: str | None=None) -> tuple[int, int]:
...
def refresh_wishlist_fx_after_itad(country: str, *, profile_id: str | None=None) -> tuple[int, int]:
