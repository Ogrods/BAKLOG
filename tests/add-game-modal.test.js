...
beforeEach(async () => {
  ...
  ({ bindAddGameModal, setAddGameTarget } = await import('../js/add-game-modal.js'));
  ({ loadManualGames, getPersonal } = await import('../js/personal-storage.js'));
...
function dupWarnVisible() {
  return !document.getElementById('addGameDuplicateWarn').classList.contains('hidden');
}
...
describe('add-game target routing', () => {
  it('library title-only add lands in state.allGames under the chosen platform', () => {
    ...
    expect(manual[0]).toMatchObject({ store: 'gog', name: 'Outer Wilds', manual: true });
    expect(manual[0].id).toBe('manual-outer-wilds');
    expect(state.allGames.some(g => g.id === 'manual-outer-wilds' && g.store === 'gog')).toBe(true);
  ...
  it('itch add lands in state.itchGames (regression: not the library catalog)', () => {
    ...
    const manual = loadManualGames();
    expect(manual).toHaveLength(1);
    expect(manual[0]).toMatchObject({ store: 'itch', manual: true, name: 'Celeste Classic' });
    expect(state.itchGames.some(g => g.id === 'manual-celeste-classic')).toBe(true);
...
describe('add-game via Steam match', () => {
  it('imports cover/reviews and stores store=steam with appid id', async () => {
    const match = { id: 620, name: 'Portal 2', tiny_image: 'http://x/tiny.jpg' };
    vi.stubGlobal('fetch', vi.fn(async (url) => {
      const u = String(url);
      if (u.includes('storesearch')) {
        return { ok: true, json: async () => ({ items: [match] }) };
      }
      if (u.includes('appreviews')) {
        return {
          ok: true,
          json: async () => ({
            success: 1,
            query_summary: { total_reviews: 100, total_positive: 98, review_score_desc: 'Overwhelmingly Positive' },
          }),
        };
      }
    ...
    expect(g).toMatchObject({ store: 'steam', id: '620', manual: true, name: 'Portal 2' });
...
describe('duplicate detection gate', () => {
  beforeEach(() => {
    state.allGames = [{ store: 'steam', id: 42, name: 'Hades', playtime_minutes: 0 }];
  });
  ...
  it('Add anyway bypasses the dup check and saves the manual row', () => {
    ...
    expect(loadManualGames()).toHaveLength(1);
    expect(loadManualGames()[0]).toMatchObject({ store: 'gog', name: 'Hades', manual: true });
  });
  ...
  it('typing in the title input clears any armed bypass without throwing', () => {
    ...
    expect(() => document.getElementById('addGameTitle').dispatchEvent(evt)).not.toThrow();
  });
  ...
  it('Go to existing closes the modal and keeps the existing game visible', () => {
    ...
    const existing = { store: 'steam', id: 7, name: 'Bastion', playtime_minutes: 0 };
    ...
    expect(loadManualGames()).toHaveLength(0);
    expect(document.getElementById('addGameModal').classList.contains('hidden')).toBe(true);
  });

  it('duplicate copy names the itch library for itch-target dupes', () => {
    state.allGames = [];
    state.itchGames = [{ store: 'itch', id: 'i1', name: 'Untitled Goose Game', playtime_minutes: 0 }];
    setAddGameTarget('itch');
    setPlatform('itch');
    setTitle('Untitled Goose Game');
    clickSkipSteam();
    expect(dupWarnVisible()).toBe(true);
    expect(document.getElementById('addGameDuplicateText').textContent).toContain('itch library');
  });
});
