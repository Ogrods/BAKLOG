...
def _cleanup_leaks(*, baseline_threads: set[str], baseline_children: set[int]) -> None:
...
def test_leak_detector_cleans_child_process():
    proc = subprocess.Popen([sys.executable, '-c', 'import time; time.sleep(120)'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0))
    try:
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter('always')
            _cleanup_leaks(baseline_threads=set(), baseline_children=set())
