...
def _request(base: str, method: str, path: str, *, body: dict | None=None) -> tuple[int, dict | str]:
    headers = {}
    data = None
    if method != 'GET':
...
def admin_server(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    (prof / 'index.json').write_text(json.dumps({'active': 'default', 'profiles': [{'id': 'default', 'label': 'Default'}]}), encoding='utf-8')
...
def test_internal_job_skips_fetcher_key_collision(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setitem(server.FETCHERS, 'customJob', server.FETCHERS['claims'])
    overlay = server.INTERNAL_JOBS_OVERLAY
    overlay.parent.mkdir(parents=True, exist_ok=True)
    overlay.write_text(json.dumps({'jobs': [{'key': 'customJob', 'label': 'Should skip', 'script': 'fetchers/fetch_free_claims.py'}]}), encoding='utf-8')
    try:
...
def test_free_claims_put_rejects_bad_claim_url(admin_server: tuple[str, Path]) -> None:
    base, _ = admin_server
    code, data = _request(base, 'PUT', '/api/internal/free-claims', body={'items': [{'id': 'bad-url', 'store': 'steam', 'title': 'Bad', 'claim_url': 'javascript:alert(1)'}]})
    assert code == 400
...
def test_internal_enrich_requires_local_header(admin_server: tuple[str, Path]) -> None:
    ...
    req = urllib.request.Request(f'{base}/api/internal/free-claims/enrich', method='POST', headers={'Content-Type': 'application/json'}, data=payload)
    with pytest.raises(urllib.error.HTTPError) as exc:
        urllib.request.urlopen(req, timeout=10)
    assert exc.value.code == 403

def test_internal_enrich_rejects_oversized_batch(admin_server: tuple[str, Path], monkeypatch: pytest.MonkeyPatch) -> None:
    base, _ = admin_server
...
def test_free_claims_put_writes_file(admin_server: tuple[str, Path]) -> None:
    ...
    payload = {'items': [{'id': 'steam-test', 'store': 'steam', 'title': 'Test Game', 'claim_url': 'https://store.steampowered.com/app/570'}]}
...
def test_free_claims_get_returns_field_overrides(admin_server: tuple[str, Path], tmp_path: Path) -> None:
    ...
    approved_path = tmp_path / 'curated' / 'free_claims.approved.json'
    approved_path.parent.mkdir(parents=True, exist_ok=True)
    approved_path.write_text(json.dumps({'ids': ['epic-a'], 'field_overrides': {'epic-a': {'title': 'Custom Title', 'claim_url': 'https://store.epicgames.com/en-US/p/custom'}}}), encoding='utf-8')
    code, data = _request(base, 'GET', '/api/internal/free-claims')
    assert code == 200
    assert data.get('field_overrides') == {'epic-a': {'title': 'Custom Title', 'claim_url': 'https://store.epicgames.com/en-US/p/custom'}}
...
def test_free_claims_approved_put_writes_field_overrides(admin_server: tuple[str, Path], tmp_path: Path) -> None:
    base, _ = admin_server
    approved_path = tmp_path / 'curated' / 'free_claims.approved.json'
    payload = {'ids': ['epic-approved'], 'field_overrides': {'epic-approved': {'title': 'Edited', 'claim_url': 'https://store.epicgames.com/en-US/p/edited', 'ends_at': '2099-01-01T00:00:00Z'}, 'ignored-id': {'title': 'Should Drop'}}}
    code, data = _request(base, 'PUT', '/api/internal/free-claims/approved', body=payload)
    assert code == 200
    saved = json.loads(approved_path.read_text(encoding='utf-8'))
    assert saved['field_overrides'] == {'epic-approved': {'title': 'Edited', 'claim_url': 'https://store.epicgames.com/en-US/p/edited', 'ends_at': '2099-01-01T00:00:00Z'}, 'ignored-id': {'title': 'Should Drop'}}

def test_free_claims_approved_put_writes_claim_urls_field_overrides(admin_server: tuple[str, Path], tmp_path: Path) -> None:
    base, _ = admin_server
    approved_path = tmp_path / 'curated' / 'free_claims.approved.json'
    payload = {'ids': ['itad-f520d9928848'], 'field_overrides': {'itad-f520d9928848': {'claim_urls': {'ios': 'https://apps.apple.com/app/id123', 'android': 'https://play.google.com/store/apps/details?id=abc'}}}}
    code, data = _request(base, 'PUT', '/api/internal/free-claims/approved', body=payload)
...
def test_free_claims_approved_put_writes_dismissed(admin_server: tuple[str, Path], tmp_path: Path) -> None:
    base, _ = admin_server
    approved_path = tmp_path / 'curated' / 'free_claims.approved.json'
...
def test_free_claims_approved_put_dismissed_validation(admin_server: tuple[str, Path]) -> None:
    base, _ = admin_server
    code, data = _request(base, 'PUT', '/api/internal/free-claims/approved', body={'ids': ['epic-a'], 'dismissed': ['ok', '']})
    assert code == 400
    assert 'dismissed[1]' in str(data.get('error', ''))

def test_free_claims_get_returns_premium_only_ids(admin_server: tuple[str, Path], tmp_path: Path) -> None:
    base, _ = admin_server
    approved_path = tmp_path / 'curated' / 'free_claims.approved.json'
    approved_path.parent.mkdir(parents=True, exist_ok=True)
...
def test_free_claims_approved_put_writes_premium_only_ids(admin_server: tuple[str, Path], tmp_path: Path) -> None:
    base, _ = admin_server
    approved_path = tmp_path / 'curated' / 'free_claims.approved.json'
    payload = {'ids': ['epic-approved', 'gamerpower-42'], 'premium_only_ids': ['epic-approved', 'gamerpower-42', 'not-approved']}
    code, data = _request(base, 'PUT', '/api/internal/free-claims/approved', body=payload)
    assert code == 200
    saved = json.loads(approved_path.read_text(encoding='utf-8'))
    assert saved['premium_only_ids'] == ['epic-approved', 'gamerpower-42']

def test_free_claims_approved_put_premium_only_validation(admin_server: tuple[str, Path]) -> None:
    base, _ = admin_server
    code, data = _request(base, 'PUT', '/api/internal/free-claims/approved', body={'ids': ['epic-a'], 'premium_only_ids': ['ok', '']})
    assert code == 400
    assert 'premium_only_ids[1]' in str(data.get('error', ''))

def test_free_claims_preview_stamps_premium_only(admin_server: tuple[str, Path]) -> None:
    base, _ = admin_server
    payload = {'manual_items': [], 'auto_items': [{'id': 'auto-pro', 'store': 'steam', 'title': 'Bonus DLC', 'claim_url': 'https://example.com/bonus'}], 'approved_ids': ['auto-pro'], 'premium_only_ids': ['auto-pro']}
...
def test_free_claims_approved_put_field_overrides_validation(admin_server: tuple[str, Path]) -> None:
    base, _ = admin_server
    code, data = _request(base, 'PUT', '/api/internal/free-claims/approved', body={'ids': ['epic-a'], 'field_overrides': {'epic-a': {'bad_key': 'nope'}}})
    assert code == 400
    assert 'unknown key' in str(data.get('error', ''))

def test_sponsors_put_validation(admin_server: tuple[str, Path]) -> None:
    base, _ = admin_server
    code, data = _request(base, 'PUT', '/api/internal/sponsors', body={'items': [{'id': 'sp1', 'title': 'Ad', 'url': 'ftp://bad.example'}]})
    assert code == 400
...
def test_sponsors_put_rejects_bad_cover(admin_server: tuple[str, Path]) -> None:
    base, _ = admin_server
    code, data = _request(base, 'PUT', '/api/internal/sponsors', body={'items': [{'id': 'sp1', 'title': 'Ad', 'cover': '//evil.example/x.png'}]})
    assert code == 400
    assert 'cover must be' in str(data.get('error', ''))

def test_sponsors_put_accepts_cover_and_placements(admin_server: tuple[str, Path], tmp_path: Path) -> None:
    base, _ = admin_server
    ...
    payload = {'items': [{'id': 'ad-hero', 'kind': 'sponsor', 'title': 'Emberfall', 'url': 'https://example.com/deal', 'cover': '/assets/ads-sample/hero-emberfall.webp', 'placements': 'spotlight, picks', 'enabled': True}]}
...
def test_sponsors_put_accepts_dash_deal_rail_placement(admin_server: tuple[str, Path], tmp_path: Path) -> None:
    base, _ = admin_server
    sponsors_path = _profile_sponsors_path(tmp_path)
    sponsors_path.parent.mkdir(parents=True, exist_ok=True)
    sponsors_path.write_text(json.dumps({'items': []}), encoding='utf-8')
    payload = {'items': [{'id': 'house-pro-promo', 'kind': 'house', 'title': 'Power-user conveniences', 'url': 'https://baklog.app/', 'placements': 'dash-deal-rail', 'enabled': True}]}
    code, data = _request(base, 'PUT', '/api/internal/sponsors', body=payload)
    assert code == 200
    saved = json.loads(sponsors_path.read_text(encoding='utf-8'))
...
def test_sponsors_put_writes_file(admin_server: tuple[str, Path], tmp_path: Path) -> None:
    ...
    sponsors_path.parent.mkdir(parents=True, exist_ok=True)
    sponsors_path.write_text(json.dumps({'items': []}), encoding='utf-8')
    payload = {'items': [{'id': 'house-test', 'kind': 'house', 'title': 'Back BAKLOG', 'tagline': 'Support local-first backlog', 'cta': 'Learn more', 'url': 'https://baklog.app/#waitlist', 'enabled': True, 'priority': 0}]}
    code, data = _request(base, 'PUT', '/api/internal/sponsors', body=payload)
    assert code == 200
    assert data.get('items') == 1
    saved = json.loads(sponsors_path.read_text(encoding='utf-8'))
...
def test_sponsors_put_accepts_v2_schema(admin_server: tuple[str, Path], tmp_path: Path) -> None:
    base, _ = admin_server
    sponsors_path = _profile_sponsors_path(tmp_path)
    sponsors_path.parent.mkdir(parents=True, exist_ok=True)
    sponsors_path.write_text(json.dumps({'version': 2, 'ads': {}, 'locations': {}}), encoding='utf-8')
    payload = {'version': 2, 'ads': {'ad-hero': {'kind': 'sponsor', 'title': 'Emberfall', 'url': 'https://example.com/deal', 'cover': '/assets/ads-sample/hero-emberfall.webp', 'enabled': True}}, 'locations': {'dash-spotlight': ['ad-hero'], 'lib-pick': ['ad-hero']}}
    code, data = _request(base, 'PUT', '/api/internal/sponsors', body=payload)
    assert code == 200
...
def test_sponsors_put_rejects_unknown_location(admin_server: tuple[str, Path]) -> None:
    base, _ = admin_server
    payload = {'version': 2, 'ads': {'ad1': {'title': 'Ad', 'kind': 'sponsor'}}, 'locations': {'not-a-real-slot': ['ad1']}}
    code, data = _request(base, 'PUT', '/api/internal/sponsors', body=payload)
    assert code == 400
    assert 'unknown key' in str(data.get('error', ''))
...
def test_free_claims_enrich_returns_items_without_writing_feed(admin_server: tuple[str, Path], tmp_path: Path) -> None:
    base, _ = admin_server
    built_path = tmp_path / 'landing' / 'free-claims.json'
    fallback_path = tmp_path / 'curated' / 'free_claims.fallback.json'
    payload = {'items': [{'id': 'itch-demo', 'store': 'itch', 'title': 'Demo on itch.io', 'claim_url': 'https://example.itch.io/demo', 'blurb': 'A free itch.io giveaway'}]}
...
def test_free_claims_enrich_persists_auto_feed(admin_server: tuple[str, Path], tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    base, _ = admin_server
    ...
    code, data = _request(base, 'POST', '/api/internal/free-claims/enrich', body={'items': [{'id': 'itad-b07aac9ebd26', 'store': 'epic', 'title': 'Wytchwood', 'claim_url': 'https://example.com/w'}]})
    assert code == 200
...
def test_free_claims_preview_dry_run_merge(admin_server: tuple[str, Path], tmp_path: Path) -> None:
    base, _ = admin_server
    built_path = tmp_path / 'landing' / 'free-claims.json'
    payload = {'manual_items': [{'id': 'manual-1', 'store': 'steam', 'title': 'Manual Game', 'claim_url': 'https://store.steampowered.com/app/1'}], 'auto_items': [{'id': 'epic-approved', 'store': 'epic', 'title': 'Approved Epic', 'claim_url': 'https://store.epicgames.com/en-US/p/approved', 'source': 'epic', 'ends_at': '2026-12-01T00:00:00Z'}, {'id': 'gog-hidden', 'store': 'gog', 'title': 'Hidden GOG', 'claim_url': 'https://www.gog.com/game/hidden', 'source': 'gamerpower'}], 'approved_ids': ['epic-approved']}
    code, data = _request(base, 'POST', '/api/internal/free-claims/preview', body=payload)
    assert code == 200
    items = data.get('items')
    assert isinstance(items, list)
...
def test_free_claims_preview_excludes_dismissed_key_matched_duplicate(admin_server: tuple[str, Path]) -> None:
    base, _ = admin_server
    payload = {'manual_items': [], 'auto_items': [{'id': 'epic-rogue-waters-9764d6', 'store': 'epic', 'title': 'Rogue Waters', 'claim_url': 'https://store.epicgames.com/en-US/p/rogue-waters-9764d6', 'ends_at': '2099-01-01T00:00:00Z'}], 'approved_ids': ['itad-0c69ed1f1bd8'], 'field_overrides': {'itad-0c69ed1f1bd8': {'title': 'Rogue Waters'}}, 'dismissed': ['epic-rogue-waters-9764d6']}
    code, data = _request(base, 'POST', '/api/internal/free-claims/preview', body=payload)
    assert code == 200
    assert data.get('items') == []

def test_free_claims_preview_excludes_blocked_ids(admin_server: tuple[str, Path]) -> None:
    base, _ = admin_server
    payload = {'manual_items': [], 'auto_items': [{'id': 'epic-blocked', 'store': 'epic', 'title': 'Blocked Game', 'claim_url': 'https://store.epicgames.com/en-US/p/blocked', 'ends_at': '2099-01-01T00:00:00Z'}], 'approved_ids': ['epic-blocked'], 'blocked': ['epic-blocked']}
    code, data = _request(base, 'POST', '/api/internal/free-claims/preview', body=payload)
    assert code == 200
    assert data.get('items') == []

def test_free_claims_preview_bypasses_supabase_auth(admin_server: tuple[str, Path], monkeypatch: pytest.MonkeyPatch) -> None:
    import shared.supabase_auth as supabase_auth
    base, _ = admin_server
    monkeypatch.setattr(supabase_auth, 'auth_enabled', lambda: True)
    code, data = _request(base, 'POST', '/api/internal/free-claims/preview', body={'manual_items': [], 'auto_items': [], 'approved_ids': []})
    assert code == 200
    assert isinstance(data.get('items'), list)

def test_runs_status_bypasses_supabase_auth_under_admin(admin_server: tuple[str, Path], monkeypatch: pytest.MonkeyPatch) -> None:
    import shared.supabase_auth as supabase_auth
    base, _ = admin_server
    monkeypatch.setattr(supabase_auth, 'auth_enabled', lambda: True)
...
def test_unknown_internal_post_returns_404_not_401(admin_server: tuple[str, Path], monkeypatch: pytest.MonkeyPatch) -> None:
    import shared.supabase_auth as supabase_auth
    base, _ = admin_server
    monkeypatch.setattr(supabase_auth, 'auth_enabled', lambda: True)
...
def test_free_claims_preview_trailing_slash(admin_server: tuple[str, Path]) -> None:
    base, _ = admin_server
    code, data = _request(base, 'POST', '/api/internal/free-claims/preview/', body={'manual_items': [], 'auto_items': [], 'approved_ids': []})
    assert code == 200
    assert isinstance(data.get('items'), list)