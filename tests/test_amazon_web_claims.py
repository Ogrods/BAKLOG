...
from clients.amazon_web_client import _CODE_FIELD_NAMES, _capture_claims_from_response, _drain_claim_candidates, claim_to_record, extract_claims_list, filter_codeless_claims, is_codeless_claim, scrub_claim_codes, try_parse_claims_from_text

def test_scrub_claim_codes_removes_credential_fields():
    claims = [{'itemTitle': 'Game', 'itemId': 'id-1', 'redemptionCode': 'AAAA-BBBB', 'gameCode': 'CCCC', 'claimCode': 'DDDD', 'activationCode': 'EEEE', 'code': 'FFFF'}]
...
def test_extract_claims_list_nested_shape():
    payload = {'data': {'claims': {'claims': [{'itemTitle': 'Alpha', 'itemId': 'amzn1.pg.item.aaa'}]}}}
...
def test_is_codeless_claim_rejects_epic_key_drop():
    claim = {'itemTitle': 'Tomb Raider IV-VI Remastered', 'itemId': 'amzn1.pg.item.cba06ec0', 'destinationAccountType': 'EPIC', 'destinationAccount': 'player1'}
...
def test_is_codeless_claim_rejects_redemption_code_field():
    claim = {'itemTitle': 'Some Game', 'itemId': 'amzn1.pg.item.x', 'redemptionCode': 'AAAA-BBBB'}
    assert is_codeless_claim(claim) is False

def test_is_codeless_claim_accepts_amazon_fulfilled():
    claim = {'itemTitle': 'Lake', 'itemId': 'amzn1.pg.item.lake', 'orderId': 'amzn1.pg.order.1', 'orderState': 'FULFILLED'}
...
def test_filter_codeless_dedupes_by_product_id():
    claims = [{'itemTitle': 'A', 'itemId': 'id-1'}, {'itemTitle': 'A duplicate', 'itemId': 'id-1'}, {'itemTitle': 'B', 'itemId': 'id-2'}]
...
def test_capture_handler_never_reads_body_on_reader_thread():
    resp = _FakeResponse('https://luna.amazon.com/graphql', 200, '{"data":{"claims":{"claims":[{"itemTitle":"X","itemId":"y"}]}}}')
    candidates: list = []
...
def test_capture_handler_ignores_non_claim_and_non_200_responses():
    ...
    raw_claims: list[dict] = []
    captured = {'done': False}
    _capture_claims_from_response(_FakeResponse('https://luna.amazon.com/static/app.js', 200, ''), candidates, raw_claims, captured)
    _capture_claims_from_response(_FakeResponse('https://luna.amazon.com/graphql', 500, ''), candidates, raw_claims, captured)
    assert candidates == []
...
def test_drain_candidates_empty_claims_payload_counts_as_capture():
    resp = _FakeResponse('https://luna.amazon.com/graphql', 200, '{"data":{"claims":{"claims":[]}}}')
    captured = {'done': False}
    raw_claims: list[dict] = []
