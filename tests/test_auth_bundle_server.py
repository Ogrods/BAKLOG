...
def _post_json(base: str, path: str, body: dict) -> tuple[int, dict | bytes, dict]:
    ...
    req = urllib.request.Request(f'{base}{path}', data=data, headers={'Content-Type': 'application/json', server._BAKLOG_LOCAL_HEADER: '1'}, method='POST')
    try:
...
def _post_bytes(base: str, path: str, body: bytes) -> tuple[int, dict]:
    req = urllib.request.Request(f'{base}{path}', data=body, headers={'Content-Type': 'application/octet-stream', server._BAKLOG_LOCAL_HEADER: '1'}, method='POST')
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
...
def _post_json_origin_only(base: str, path: str, body: dict) -> tuple[int, dict | bytes, dict]:
    data = json.dumps(body).encode('utf-8')
    req = urllib.request.Request(f'{base}{path}', data=data, headers={'Content-Type': 'application/json', 'Origin': base}, method='POST')
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
            raw = resp.read()
            headers = dict(resp.headers)
...
def test_export_content_disposition(auth_bundle_server: str) -> None:
    ...
    status, body, headers = _post_json(auth_bundle_server, '/api/auth/secrets/export', {'passphrase': 'server-test-pass'})
    assert status == 200
...
def test_import_bad_passphrase_403(auth_bundle_server: str) -> None:
    ...
    import base64
    status, body, _headers = _post_json(auth_bundle_server, '/api/auth/secrets/import', {'passphrase': 'wrong-passphrase-here', 'blob': base64.b64encode(blob).decode('ascii')})
    assert status == 403
    assert body['code'] == 'bad_passphrase'

def test_secrets_export_blocked_without_local_header(auth_bundle_server: str) -> None:
    status, body, _ = _post_json_origin_only(auth_bundle_server, '/api/auth/secrets/export', {'passphrase': 'server-test-pass'})
    assert status == 403
...
def test_secrets_import_blocked_without_local_header(auth_bundle_server: str) -> None:
    status, body, _ = _post_json_origin_only(auth_bundle_server, '/api/auth/secrets/import', {'passphrase': 'server-test-pass', 'blob': ''})
    assert status == 403
    assert 'cross-origin' in str(body.get('error', '')).lower()

def test_master_password_blocked_without_local_header(auth_bundle_server: str) -> None:
    status, body, _ = _post_json_origin_only(auth_bundle_server, '/api/auth/master-password', {'password': 'new-test-passphrase-for-unit-tests'})
    assert status == 403
    assert 'cross-origin' in str(body.get('error', '')).lower()