...
def test_browser_auth_worker_inherits_profile_context(profile_env: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    captured: list[str] = []
...
def test_fresh_reconnect_clears_browser_session(profile_env: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    cleared: list[str] = []
...
def test_plain_connect_does_not_clear_browser_session(profile_env: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    cleared: list[str] = []
    monkeypatch.setattr(auth_manager, 'clear_browser_session', lambda p: cleared.append(p))
    done = _stub_browser_worker(monkeypatch)
...
def test_clear_browser_session_removes_profile_dir(profile_env: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_start_browser_auth_rejects_overlapping_provider_session(profile_env: Path, monkeypatch: pytest.MonkeyPatch) -> None:
