...
def test_is_running_from_temp_dir_false_when_not_frozen(monkeypatch, tmp_path):
...
def test_is_running_from_temp_dir_true_under_temp(monkeypatch, tmp_path):
...
def test_is_running_from_temp_dir_false_outside_temp(monkeypatch):
    monkeypatch.setattr(server_support, "is_frozen", lambda: True)
...
def test_is_running_from_temp_dir_marker_rar(monkeypatch, tmp_path):
    monkeypatch.setattr(server_support, "is_frozen", lambda: True)
...
def test_check_data_location_prints_warning(monkeypatch, tmp_path, capsys):
    monkeypatch.setattr(server_support, "is_frozen", lambda: True)
...
def test_reconcile_startup_removes_missing_target(monkeypatch):
    monkeypatch.setattr(sys, "platform", "win32")
    monkeypatch.setattr(startup, "_win_run_command", lambda: '"C:\\missing\\BAKLOG.exe" --tray')
    disabled = []
...
def test_reconcile_startup_keeps_existing_target(monkeypatch, tmp_path):
    monkeypatch.setattr(sys, "platform", "win32")
...
def test_reconcile_startup_noop_without_entry(monkeypatch):
    monkeypatch.setattr(sys, "platform", "win32")
    monkeypatch.setattr(startup, "_win_run_command", lambda: None)
    assert startup.reconcile_startup() is False


def test_parse_win_run_target_quoted():
    target = startup._parse_win_run_target('"C:\\Apps\\BAKLOG.exe" --tray')
    assert target == Path("C:\\Apps\\BAKLOG.exe")


def test_private_profile_hint_on_empty_library(monkeypatch, capsys):
...
def test_update_available_compares_semver():
...
def test_github_releases_api_url_matches_community_json():
    import json
