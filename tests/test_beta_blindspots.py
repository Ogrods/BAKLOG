...
def test_check_data_location_prints_warning(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    monkeypatch.setattr(server_support, 'is_frozen', lambda: True)
...
def test_private_profile_hint_on_empty_library(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
