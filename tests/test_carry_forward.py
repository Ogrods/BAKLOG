...
from fetchers._base import LAST_SEEN_FIELD, STALE_FIELD, STALE_SINCE_FIELD, apply_carry_forward, carry_forward_missing, refuse_drift_result, row_key_by_appid, row_key_by_id
...
def test_carry_forward_missing_clears_stale_when_row_returns() -> None:
    ...
    existing = [{'id': '2', 'name': 'Beta', STALE_FIELD: True, STALE_SINCE_FIELD: '2026-06-01T00:00:00+00:00', LAST_SEEN_FIELD: '2026-06-01T00:00:00+00:00'}]
    out = carry_forward_missing(fresh, existing, key_fn=row_key_by_id, now_iso='2026-06-08T12:00:00+00:00')
    assert len(out) == 1
    row = out[0]
...
def test_multi_source_match_key_dedup_not_double_counted() -> None:
    ...
    existing = [{'id': 10, 'gog_id': 10, 'name': 'Fresh', 'source': 'local', 'hltb_main_hours': 5}, {'id': 99, 'gog_id': 99, 'name': 'Carried', 'source': 'local'}]
...
def test_drift_guard_runs_before_carry_forward_semantics() -> None:
    ...
    carried = carry_forward_missing([{'id': '1', 'name': 'A'}], [{'id': str(i), 'name': f'G{i}'} for i in range(2, 11)], key_fn=row_key_by_id)
