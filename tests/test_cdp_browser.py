...
import pytest
from auth.cdp_browser import _BROWSER_LAUNCH_HINT, CdpContext, CdpPage, _cdp_websocket_error, _chromium_executable_candidates, _should_preserve_popup, auth_banner_init_script, find_chromium_executable, is_blank_browser_url, launch_persistent_profile
...
class TestCookieHeader:
    ...
    def test_filters_by_domain(self) -> None:
        cookies = [{'name': 'a', 'value': '1', 'domain': '.battle.net'}, {'name': 'b', 'value': '2', 'domain': 'example.com'}, {'name': 'c', 'value': '3', 'domain': 'store.steampowered.com'}]
...
class TestNetworkExtraInfo:
    ...
    def test_extra_info_merges_authorization_and_redispatches(self) -> None:
        ...
        page._handle_network_event('Network.requestWillBeSent', {'requestId': 'req-1', 'request': {'url': 'https://service-aggregation-layer.juno.ea.com/graphql', 'headers': {'accept': 'application/json'}}})
        assert seen == [False]
        page._handle_network_event('Network.requestWillBeSentExtraInfo', {'requestId': 'req-1', 'headers': {'Authorization': 'Bearer ea-test-token'}})
...
class TestGracefulClose:
    ...
    def test_close_sends_browser_close_even_when_proc_already_exited(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ctx = _bare_context()
        calls: list[str] = []
    ...
    def test_close_kills_debug_port_when_proc_already_exited(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ctx = _bare_context()
        calls: list[str] = []
        killed: list[int] = []
