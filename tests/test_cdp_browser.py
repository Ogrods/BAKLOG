...
class TestShouldPreservePopup:
    @pytest.mark.parametrize("url", ["", "about:blank", "chrome://newtab/"])
    def test_blank_preserves(self, url):
        assert _should_preserve_popup(url)

    def test_account_ubisoft_preserves(self):
        assert _should_preserve_popup("https://account.ubisoft.com/en-US/login")

    def test_logged_in_merges(self):
        assert not _should_preserve_popup("https://connect.ubisoft.com/logged-in.html")

    def test_ubisoft_connect_games_merges(self):
        assert not _should_preserve_popup("https://www.ubisoft.com/en-us/ubisoft-connect/games")


class TestRegisterPageDebugger:
    def test_register_page_skips_debugger_for_storefront_auth(self):
        ctx = _bare_context()
        ...
        ctx._send = fake_send
        ctx._init_scripts = []
        ctx._apply_init_script = lambda page, source: None
...
class TestNetworkExtraInfo:
    def test_extra_info_merges_authorization_and_redispatches(self):
        ctx = _bare_context()
        ...
        page._handle_network_event(
            ...
            {"requestId": "req-1", "headers": {"Authorization": "Bearer ea-test-token"}},
        )
        assert seen == [False, True]

    def test_websocket_403_message(self):
        err = _cdp_websocket_error(Exception("Handshake status 403 Forbidden - remote-allow-origins"))
    ...
    def test_websocket_generic_message(self):
...
class TestHttpClient:
    def test_post_forwards_cookies_and_json(self, monkeypatch):
        import auth.cdp_browser as cdp

        ctx = _bare_context()
        ctx.cookies = lambda: [{"name": "remid", "value": "abc", "domain": ".ea.com"}]
        captured = {}
        ...
        def fake_post(url, *, cookies, headers, data, json, timeout):
            captured.update({"url": url, "cookies": cookies, "headers": headers, "data": data, "json": json})
            return _FakeResp()

        monkeypatch.setattr(cdp.requests, "post", fake_post)
        client = cdp.CdpHttpClient(ctx)
        resp = client.post("https://example.com/gql", headers={"accept": "application/json"}, data='{"q":1}')
    ...
    def test_uses_page_session_for_network_getallcookies(self):
        ctx = _bare_context()
    ...
    def test_falls_back_to_storage_getcookies(self):
        ctx = _bare_context()
        ctx.pages = [_FakePage("SESSION-1")]
        calls = []
...
class TestRunnerCdpCompat:
    def test_runner_does_not_call_is_closed_as_method(self):
...
class TestAuthBanner:
    def test_init_script_embeds_message_and_setter(self):
    ...
    def test_set_auth_banner_pushes_to_live_pages_only(self):
        ctx = _bare_context()
        open_page = _FakePage("S-OPEN")
        closed_page = _FakePage("S-CLOSED")
        closed_page.is_closed = True
        evaluated = []
        open_page.evaluate = lambda fn, timeout=5: evaluated.append(fn)
        closed_page.evaluate = lambda fn, timeout=5: evaluated.append("CLOSED")
    ...
    def test_set_auth_banner_ignores_blank_message(self):
        ctx = _bare_context()
        page = _FakePage("S-1")
        page.evaluate = lambda fn, timeout=5: (_ for _ in ()).throw(AssertionError("called"))
        ctx.pages = [page]
        ctx.set_auth_banner("")


class TestLaunchArgs:
    def test_off_screen_headed_uses_window_position(self, tmp_path):
        ...
        try:
            with pytest.raises(LaunchArgsCaptured) as exc:
                launch_persistent_profile(profile, headless=False, window_position=(-32000, -32000))
        finally:
            monkeypatch.undo()
        args = exc.value.args
    ...
    def test_headed_default_uses_start_maximized(self, tmp_path):
        exe = tmp_path / "chrome.exe"
        exe.write_bytes(b"")
        profile = tmp_path / "profile"
...
class TestProfileLockRelease:
    def test_release_chromium_profile_lock_kills_matching_pids(self, monkeypatch, tmp_path):
        import auth.cdp_browser as cdp
...
class TestGracefulClose:
    def test_close_sends_browser_close_before_terminate(self):
        ctx = _bare_context()
        calls = []
    ...
    def test_close_sends_browser_close_even_when_proc_already_exited(self, monkeypatch):
        ctx = _bare_context()
        calls = []
        ...
        monkeypatch.setattr("auth.cdp_browser._pids_listening_on_local_port", lambda _port: [])
        ctx._proc = _ExitedProc()
        ctx._ws = _FakeWs()
        ctx._port = 9222
        ctx._send = fake_send
        ctx.close()
    ...
    def test_close_kills_debug_port_when_proc_already_exited(self, monkeypatch):
        ctx = _bare_context()
        calls = []
        killed = []
        ...
        monkeypatch.setattr("auth.cdp_browser._pids_listening_on_local_port", lambda port: [4242])
        monkeypatch.setattr("auth.cdp_browser._kill_pids", lambda pids: killed.extend(pids) or list(pids))
        ctx._proc = _ExitedProc()
        ctx._ws = _FakeWs()
        ctx._port = 9222
        ctx._send = fake_send
...
class TestFindBrowser:
    ...
    def test_macos_chrome_candidate(self, monkeypatch, tmp_path):
    ...
    def test_linux_which_fallback(self, monkeypatch, tmp_path):
        monkeypatch.delenv("BAKLOG_CHROME_PATH", raising=False)
        ...
        monkeypatch.setattr(cdp.shutil, "which", lambda name: str(chrome) if name == "google-chrome-stable" else None)
        assert str(find_chromium_executable()) == str(chrome)

    def test_posix_candidates_include_edge_and_brave(self, monkeypatch):
        monkeypatch.setattr(sys, "platform", "linux")
...
class TestBrowserSessionGone:
    def test_exited_launcher_proc_is_not_gone_when_socket_live(self):
    ...
    def test_detects_all_pages_closed(self):
        from auth.cdp_browser import ConnectBrowserClosed, abort_if_browser_closed, browser_session_gone
    ...
    def test_live_browser_not_gone(self):
        from auth.cdp_browser import browser_session_gone
