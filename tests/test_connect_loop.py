...
def test_run_connect_poll_returns_when_check_succeeds(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def check() -> dict[str, str] | None:
        ...
        if calls['n'] >= 2:
            return {'TOKEN': 'ok'}
        return None
    creds = run_connect_poll(context=_FakeContext(), session=None, deadline=clock.time() + 60, poll_sec=0.1, check=check, timeout_message='timed out')
...
def test_run_connect_poll_raises_on_deadline(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match='no session'):
        run_connect_poll(context=_FakeContext(), session=None, deadline=clock.time() + 10, poll_sec=0.1, check=lambda: None, timeout_message='no session')