...
def _post_origin_only(base: str, path: str, body: dict) -> tuple[int, dict]:
    req = urllib.request.Request(f'{base}{path}', data=json.dumps(body).encode('utf-8'), headers={'Content-Type': 'application/json', 'Origin': 'http://127.0.0.1:8765'}, method='POST')
    try:
...
def test_profile_switch_accepts_local_header(csrf_server: str) -> None:
    req = urllib.request.Request(f'{csrf_server}/api/profiles/active', data=json.dumps({'id': 'default'}).encode('utf-8'), headers={'Content-Type': 'application/json', server._BAKLOG_LOCAL_HEADER: '1'}, method='POST')
    with urllib.request.urlopen(req, timeout=5) as resp:
