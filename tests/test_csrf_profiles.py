...
def _post_origin_only(base, path, body):
...
def test_profile_create_rejects_origin_without_local_header(csrf_server):
    status, body = _post_origin_only(csrf_server, "/api/profiles", {"label": "Work"})
    assert status == 403
    assert "cross-origin" in body.get("error", "").lower()


def test_profile_switch_accepts_local_header(csrf_server):
    req = urllib.request.Request(
