...
def test_port_busy_true_when_connect_succeeds(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(dsp.socket, 'create_connection', lambda *_a, **_k: MagicMock(__enter__=lambda s: s, __exit__=lambda *a: None))
...
def test_clear_stale_pid_file_removes_dead_pid(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_clear_stale_pid_file_keeps_live_server(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    pid_file = tmp_path / '.baklog_server.pid'
...
def test_reclaim_stale_server_kills_orphan_from_pid_file(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    pid_file = tmp_path / '.baklog_server.pid'
    pid_file.write_text('4242', encoding='utf-8')
    killed: list[int] = []
    monkeypatch.setattr(dsp, 'pid_is_python_server', lambda _pid: True)
...
def test_reclaim_stale_server_skips_non_server_pid(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    pid_file = tmp_path / '.baklog_server.pid'
    pid_file.write_text('4242', encoding='utf-8')
    monkeypatch.setattr(dsp, 'pid_is_python_server', lambda _pid: False)
...
def test_reclaim_or_exit_returns_when_port_free(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_reclaim_or_exit_exits_when_busy_and_no_reclaim(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr(dsp, 'clear_stale_pid_file', lambda _f: False)
...
def test_reclaim_or_exit_reclaims_then_returns(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    calls = {'busy': 0}
