...
def test_extract_ea_accepts_sniffed_bearer_when_apq_stale(monkeypatch) -> None:
    monkeypatch.setattr('clients.ea_session.probe_ea_token', lambda _t, _c: {'ok': False, 'error': 'EA GraphQL HTTP 400: PersistedQueryNotFound'})
    sniffer = HeaderSniffer(url_substr=EA_GRAPHQL_HOST, fields={'authorization': 'EA_BEARER_TOKEN'}, normalize=normalize_bearer)
