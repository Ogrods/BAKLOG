...
def test_cookie_mode_posts_subscription_apq_probe(monkeypatch):
    calls = []
    ...
    assert calls[0]["json"]["extensions"]["persistedQuery"]["sha256Hash"] == USER_SUBSCRIPTION_HASH
    assert "Authorization" not in session.headers


def test_bearer_mode_requires_token_without_cookies():
    try:
        EaClient("")
        raised = False
    except EaAuthError:
...
def test_cookie_mode_raises_on_unauthenticated(monkeypatch):
...
def test_get_owned_games_falls_back_when_apq_hash_stale():
    ...
    assert [c["op"] for c in calls] == ["getPreloadedOwnedGames", "getPreloadedOwnedGames"]
