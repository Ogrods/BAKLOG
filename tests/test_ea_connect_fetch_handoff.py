...
def isolated_profile(tmp_path, monkeypatch):
    ...
    (tmp_path / "profiles" / "index.json").write_text(
        json.dumps({"active": "handoff", "profiles": [{"id": "handoff", "label": "H", "created_at": "t"}]}),
        encoding="utf-8",
    )
...
def _owned_game():
    return {
        "originOfferId": "OFW-HANDOFF-1",
        "product": {
            "name": "Battlefield 2042",
            "gameSlug": "battlefield-2042",
            "baseItem": {"gameType": "GAME", "isLauncher": False},
            "gameProductUser": {"ownershipMethods": ["PURCHASE"]},
        },
    }


def test_resolve_session_skips_browser_when_snapshot_fresh(isolated_profile, monkeypatch):
    import fetchers.fetch_ea as fetch_ea

    snap = isolated_profile / "cache" / "ea" / "connect_snapshot.json"
    snap.write_text(
        json.dumps(
            {"captured_at": datetime.now(UTC).isoformat(), "owned_items": [_owned_game()], "browser_auth_ok": True}
        ),
        encoding="utf-8",
    )
...
def test_resolve_session_launches_browser_when_snapshot_stale(isolated_profile, monkeypatch):
    import fetchers.fetch_ea as fetch_ea

    snap = isolated_profile / "cache" / "ea" / "connect_snapshot.json"
    snap.write_text(
        json.dumps(
            {"captured_at": "2000-01-01T00:00:00+00:00", "owned_items": [_owned_game()], "browser_auth_ok": True}
        ),
        encoding="utf-8",
    )
    ...
    fake_result = type(
        "R", (), {"token": "cookie", "cookies": [], "owned_items": [_owned_game()], "debug": {"browser_auth_ok": True}}
    )()
    ctx = MagicMock()
    ctx.pages = [MagicMock()]
    with (
        patch("fetchers.fetch_ea.launch_ea_profile") as mock_launch,
        patch("fetchers.fetch_ea.capture_ea_browser_session", return_value=fake_result),
    ):
...
def test_fetch_blocks_during_active_auth_with_message(monkeypatch):
    import fetchers.fetch_ea as fetch_ea
...
def test_main_success_from_connect_snapshot(isolated_profile, monkeypatch):
    import fetchers.fetch_ea as fetch_ea
    ...
    with (
        patch("fetchers.fetch_ea.launch_ea_profile") as mock_launch,
        patch("fetchers.fetch_ea.sniff_ea_bearer") as mock_sniff,
    ):
        monkeypatch.setattr(sys, "argv", ["fetch_ea", "--skip-hltb"])
        code = fetch_ea.main()
        mock_launch.assert_not_called()
