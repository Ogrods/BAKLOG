...
from clients.ea_session import DEFAULT_TRIGGER_URLS, EA_COOKIE_SESSION, EA_SESSION_COOKIE, _ea_session_failure, drain_ea_graphql_hook, ea_graphql_authenticated, ea_graphql_owned_items, is_ea_login_page, normalize_bearer, probe_ea_token, sniff_ea_bearer
...
def test_sniff_ea_bearer_from_request_handler(monkeypatch) -> None:
    ...
    class Page:
        ...
        def goto(self, *_a, **_k) -> None:
            for h in handlers:
                h(MagicMock(url='https://service-aggregation-layer.juno.ea.com/graphql?x=1', headers={'authorization': 'Bearer sniffed-token'}))
...
def test_sniff_cookie_session_when_browser_hook_authenticated(monkeypatch) -> None:
    monkeypatch.setattr('clients.ea_session.install_ea_graphql_hook', lambda _c: None)
    monkeypatch.setattr('clients.ea_session.ensure_ea_graphql_hook', lambda _p: None)
    monkeypatch.setattr('clients.ea_session.drain_ea_graphql_hook', lambda _p: (True, [], {'browser_auth_ok': True, 'hook_authenticated': True}))
...
def test_drain_ea_graphql_hook_merges_auth_and_owned(monkeypatch) -> None:
    ...
    monkeypatch.setattr('clients.ea_session.read_captured_ea_graphql', lambda _p: [{'payload': me_payload}, {'payload': owned_payload}, {'payload': unauth_payload}])
...
def test_drain_ea_graphql_hook_unauthenticated_only(monkeypatch) -> None:
    monkeypatch.setattr('clients.ea_session.read_captured_ea_graphql', lambda _p: [{'payload': _load_fixture('ea_graphql_not_authenticated.json')}])
    auth_ok, owned, stats = drain_ea_graphql_hook(object())
...
def test_capture_ea_browser_session_success(monkeypatch) -> None:
    ...
    monkeypatch.setattr('clients.ea_session.install_ea_graphql_hook', lambda _c: None)
    monkeypatch.setattr('clients.ea_session.ensure_ea_graphql_hook', lambda _p: None)
    monkeypatch.setattr('clients.ea_session.drain_ea_graphql_hook', lambda _p: (True, ea_graphql_owned_items(owned_payload), {'hook_authenticated': True}))
    clock = {'now': 1000.0}
