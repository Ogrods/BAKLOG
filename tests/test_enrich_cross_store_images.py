...
ARMATUS_ROW = {'store': 'wishlist', 'wishlist_store': 'xbox', 'id': 'xbox-9NQPJ4M6SMDF', 'xbox_product_id': '9NQPJ4M6SMDF', 'name': 'Armatus', 'header_image': None, 'library_image': None}
...
def test_needs_images_false_when_urls_present() -> None:
    g = {'header_image': 'https://cdn.akamai.steamstatic.com/steam/apps/1/header.jpg', 'library_image': 'https://cdn.akamai.steamstatic.com/steam/apps/1/library_600x900_2x.jpg'}
    assert enrich.needs_images(g) is False

def test_needs_lowres_upgrade_true_for_native_store_art() -> None:
    g = {'header_image': 'https://images-eds-ssl.xboxlive.com/image/foo', 'library_image': 'https://images-eds-ssl.xboxlive.com/image/bar'}
    assert enrich.needs_lowres_upgrade(g) is True

def test_needs_lowres_upgrade_false_for_steamstatic() -> None:
    g = {'header_image': 'https://cdn.akamai.steamstatic.com/steam/apps/1/header.jpg', 'library_image': 'https://cdn.akamai.steamstatic.com/steam/apps/1/library_600x900_2x.jpg', 'image_source': 'steam_search'}
...
def test_enriches_armatus_wishlist_row(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(enrich, 'write_catalog_text', lambda rel, text: wishlist_path.write_text(text, encoding='utf-8'))
...
def test_itch_non_game_skipped(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    itch_path.write_text(json.dumps({'games': [{'id': 'tool-1', 'name': 'Map Editor', 'classification': 'tool', 'header_image': None, 'library_image': None}, {'id': 'game-1', 'name': 'Indie Game', 'classification': 'game', 'header_image': None, 'library_image': None}]}, indent=2), encoding='utf-8')
    meta_path = tmp_path / 'cross_store_images_meta.json'
    searches: list[str] = []
...
NATIVE_ROW = {'store': 'xbox', 'id': 'xbox-native-1', 'name': 'Native Capsule Game', 'header_image': 'https://images-eds-ssl.xboxlive.com/image/small-capsule', 'library_image': 'https://images-eds-ssl.xboxlive.com/image/small-cover'}
...
def test_upgrade_lowres_skips_cached_checked_rows(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    xbox_path.write_text(json.dumps({'games': [dict(NATIVE_ROW)]}, indent=2), encoding='utf-8')
    meta_path = tmp_path / 'cross_store_images_meta.json'
    meta_path.write_text(json.dumps({'lowres_checked': ['xbox:xbox-native-1'], 'no_steam_match': []}, indent=2), encoding='utf-8')
    searches: list[str] = []
