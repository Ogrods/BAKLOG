...
def workspace(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    (tmp_path / 'cache' / 'steam_review_map.json').write_text(json.dumps({'gog:1': 4242}), encoding='utf-8')
    (tmp_path / 'games_steam.json').write_text(json.dumps({'fetched_at': '2026-01-01T00:00:00Z', 'games': [{'id': 570, 'name': 'Dota 2', 'playtime_minutes': 0}]}), encoding='utf-8')
    (tmp_path / 'games_gog.json').write_text(json.dumps({'fetched_at': '2026-01-01T00:00:00Z', 'games': [{'id': '1', 'name': 'Mapped GOG', 'playtime_minutes': 0}, {'id': '2', 'name': 'Unmapped', 'playtime_minutes': 0}]}), encoding='utf-8')
    return tmp_path
...
def test_effective_tier_prefers_confirmed_over_pending() -> None:
    ...
    fields = summary_to_row_fields({'tier': 'platinum', 'confidence': 'high', 'total': 42, 'score': 0.91, 'trendingTier': 'gold'})
    assert fields['protondb_tier'] == 'platinum'
    assert fields['protondb_report_count'] == 42

def test_enricher_writes_steam_and_mapped_gog_rows(workspace: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    calls: list[int] = []

    def fake_fetch(appid: int):
        calls.append(appid)
        if appid == 570:
            return {'tier': 'gold', 'confidence': 'strong', 'total': 10, 'score': 0.8, 'trendingTier': 'gold'}
        if appid == 4242:
            return False
        return None
