...
def workspace(tmp_path, monkeypatch):
    ...
    (tmp_path / "games_steam.json").write_text(
        json.dumps(
            {"fetched_at": "2026-01-01T00:00:00Z", "games": [{"id": 570, "name": "Dota 2", "playtime_minutes": 0}]}
        ),
        encoding="utf-8",
    )
...
def test_effective_tier_prefers_confirmed_over_pending():
    ...
    fields = summary_to_row_fields(
        {"tier": "platinum", "confidence": "high", "total": 42, "score": 0.91, "trendingTier": "gold"}
    )
    assert fields["protondb_tier"] == "platinum"
    assert fields["protondb_report_count"] == 42


def test_enricher_writes_steam_and_mapped_gog_rows(workspace, monkeypatch):
    calls = []

    def fake_fetch(appid):
        calls.append(appid)
        if appid == 570:
