        ...
        json.dumps(
            {
                ...
                "games": [
                    {"id": "1", "name": "Coop Title", "playtime_minutes": 0},
                    {"id": "2", "name": "No Match", "playtime_minutes": 0, "genres": ["GOG-classic"]},
                    {"id": "3", "name": "Unmapped", "playtime_minutes": 0},
                ],
            }
        ),
        encoding="utf-8",
...
def test_enricher_writes_coop_and_fills_missing_genres(workspace, monkeypatch):
    ...
    def fake_get_app_details(self, appid, refresh=False):
...
def test_enricher_dry_run_does_not_write(workspace, monkeypatch):
    for mod in ("enrichers.enrich_steam_tags", "clients.steam_client"):
        sys.modules.pop(mod, None)
    import enrichers.enrich_steam_tags as enrich_steam_tags
    from clients.steam_client import SteamClient
    ...
    def fake_get_app_details(self, appid, refresh=False):
        return {
            "success": True,
            "data": {"categories": [{"description": "Online Co-op"}], "genres": [{"description": "Action"}]},
        }
...
def test_enricher_bails_without_mapping(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr("shared.profile_paths.profile_root", lambda profile_id=None: tmp_path)
    monkeypatch.setenv("STEAM_API_KEY", "test_key")
    monkeypatch.setenv("STEAM_ID", "76561197960287930")
