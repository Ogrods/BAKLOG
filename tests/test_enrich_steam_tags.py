...
def workspace(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr('shared.profile_paths.get_active_profile_id', lambda: DEFAULT_PROFILE_ID)
    monkeypatch.setattr('auth.manager.get_active_profile_id', lambda: DEFAULT_PROFILE_ID)
    (tmp_path / 'cache').mkdir()
    (tmp_path / 'cache' / 'steam_review_map.json').write_text(json.dumps({'gog:1': 4242, 'gog:2': 0}), encoding='utf-8')
    (tmp_path / 'games_gog.json').write_text(json.dumps({'fetched_at': '2026-01-01T00:00:00Z', 'games': [{'id': '1', 'name': 'Coop Title', 'playtime_minutes': 0}, {'id': '2', 'name': 'No Match', 'playtime_minutes': 0, 'genres': ['GOG-classic']}, {'id': '3', 'name': 'Unmapped', 'playtime_minutes': 0}]}), encoding='utf-8')
    monkeypatch.setenv('STEAM_API_KEY', 'test_key')
    monkeypatch.setenv('STEAM_ID', '76561197960287930')
    return tmp_path

def test_enricher_writes_coop_and_fills_missing_genres(workspace: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    canned = {4242: {'success': True, 'data': {'type': 'game', 'name': 'Coop Title', 'categories': [{'description': 'Single-player'}, {'description': 'Online Co-op'}], 'genres': [{'description': 'Action'}, {'description': 'RPG'}], 'release_date': {'date': 'Mar 15, 2024'}, 'metacritic': {'score': 88}, 'controller_support': 'full', 'developers': ['Enrich Dev'], 'publishers': ['Enrich Pub'], 'achievements': {'total': 30}}}}
    ...
    meta = json.loads((workspace / 'cache' / 'steam_tags_meta.json').read_text(encoding='utf-8'))
    assert meta['rows_updated'] >= 1
    assert meta['rows_with_appid'] >= 1
    assert 'fetched_at' in meta

def test_enricher_dry_run_does_not_write(workspace: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    for mod in ('enrichers.enrich_steam_tags', 'clients.steam_client'):
        sys.modules.pop(mod, None)
    import enrichers.enrich_steam_tags as enrich_steam_tags
    from clients.steam_client import SteamClient
    ...
    def fake_get_app_details(self, appid: int, refresh: bool=False) -> dict | None:
        return {'success': True, 'data': {'categories': [{'description': 'Online Co-op'}], 'genres': [{'description': 'Action'}]}}
    monkeypatch.setattr(SteamClient, 'get_app_details', fake_get_app_details)
...
def test_enricher_bails_without_mapping(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr('shared.profile_paths.profile_root', lambda profile_id=None: tmp_path)
    monkeypatch.setenv('STEAM_API_KEY', 'test_key')
    monkeypatch.setenv('STEAM_ID', '76561197960287930')
