...
from clients.epic_client import EpicAuthError, EpicClient, EpicCorrectiveActionError, corrective_action_in_text, epic_error_fields, is_corrective_action
CORRECTIVE_PAYLOAD = {'errorCode': 'errors.com.epicgames.oauth.corrective_action_required', 'message': 'Corrective action is required to continue.', 'metadata': {'correctiveAction': 'PRIVACY_POLICY_ACCEPTANCE', 'continuation': 'continuation-token-123'}, 'correlationId': 'abc-correlation'}
INVALID_GRANT_PAYLOAD = {'errorCode': 'errors.com.epicgames.account.oauth.invalid_grant', 'message': 'Sorry the originating session for the exchange code was not found.'}
...
def test_login_refresh_reraises_corrective_action(tmp_path: Path, monkeypatch) -> None:
    ...
    client._request_token = lambda _data: (_ for _ in ()).throw(EpicCorrectiveActionError('Epic privacy policy acceptance required', corrective_action='PRIVACY_POLICY_ACCEPTANCE'))
    with pytest.raises(EpicCorrectiveActionError):
        client.login()