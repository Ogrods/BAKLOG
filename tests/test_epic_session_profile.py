"""Epic OAuth refresh token is stored in the encrypted secrets blob, scoped to
the active profile — never as a plaintext session.json on disk."""
...
def isolated_profiles(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Use a deterministic master password so secrets.bin can be written without
    # touching the real OS keyring, and start from a clean decrypted cache.
    reset_cache()
...
def test_epic_session_stored_in_secrets_not_plaintext(
    ...
    # No plaintext session file is left behind.
    assert not (cache_dir / "session.json").is_file()
    assert not (profile_paths.ROOT / "cache" / "epic" / "session.json").is_file()

    # The refresh token lives in the encrypted secrets blob and round-trips.
...
def test_epic_migrates_plaintext_session_into_secrets(
    isolated_profiles: None,
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    # Seed a legacy plaintext session under the old global cache dir.
    ...
    # Migration moved it into secrets and removed the plaintext copies.
