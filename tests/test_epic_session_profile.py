...
def test_epic_session_stored_in_secrets_not_plaintext(isolated_profiles, monkeypatch):
    create_profile("Work")
...
def test_epic_migrates_plaintext_session_into_secrets(isolated_profiles, monkeypatch, tmp_path):
    create_profile("Work")
    monkeypatch.setenv("BAKLOG_PROFILE", "work")
    reset_cache()
