...
def test_wishlist_graphql_ok_ignores_toast_only_graphql():
...
def test_wishlist_graphql_ok_accepts_batched_array():
    batch = [
...
_DEHYDRATED_SNIPPET = '{"state":{"data":{"Wishlist":{"wishlistItems":{"elements":[{"id":"wish-1","offerId":"offer-aaa","namespace":"ns-1"}]}}},"status":"success"},"queryKey":["getWishlist",["accountId","acct"],"hash"]},{"state":{"data":[{"Catalog":{"catalogOffer":{"title":"Batman","id":"offer-aaa","namespace":"ns-1","keyImages":[{"type":"OfferImageWide","url":"https://cdn/w.jpg"}]}}}]}'


def test_extract_wishlist_payloads_from_dehydrated_html():
...
def test_enrich_wishlist_elements_with_catalog_offer():
    payloads = extract_wishlist_payloads_from_html(_DEHYDRATED_SNIPPET)
...
def test_is_epic_graphql_url_matches_known_hosts():
...
def test_cloudflare_interstitial_real_challenge():
    cf_html = "<html><head><title>Just a moment...</title></head><body><div class='cf_challenge_container'>Checking your browser</div></body></html>"
...
def test_cloudflare_interstitial_not_store_home_js_bundle():
    snippet = "<html><body><script>a.src='/cdn-cgi/challenge-platform/scripts/jsd/main.js';</script>Sign in<button>Wishlist</button></body></html>"
...
def test_store_home_dump_html_not_cloudflare_false_positive():
