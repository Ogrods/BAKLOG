...
def _launcher_row(pid, *, name="Launcher Game", asin="B00LAUNCH", last_played="2024-01-01"):
    return {
        "store": "amazon",
        "id": pid,
        "amazon_id": pid,
        ...
        "genres": ["Action"],
        "header_image": "https://launcher.example/icon.jpg",
    }


def _web_row(pid, *, name="Launcher Game", asin="B00LAUNCH"):
    return {
        "store": "amazon",
        "id": pid,
        "amazon_id": pid,
...
class TestEffectiveRowSource:
    def test_legacy_row_treated_as_launcher(self):
        assert fa._effective_row_source({"name": "Old"}) == "launcher"

    def test_tagged_web(self):
...
class TestMergeAmazonSources:
    ...
    def test_asin_collapse_prefers_launcher(self):
    ...
    def test_launcher_winner_keeps_web_enrichment(self):
    ...
    def test_name_collapse_when_asin_missing(self):
...
class TestRefuseAmazonSourceDrift:
    def test_web_slice_shrink_triggers_exit_3(self, tmp_path, monkeypatch):
        catalog = tmp_path / "games_amazon.json"
        catalog.write_text(
            json.dumps({"games": [_web_row(f"w{i}", name=f"Web {i}", asin=f"B{i:03d}") for i in range(10)]}),
            encoding="utf-8",
        )
    ...
    def test_launcher_slice_unchanged_does_not_block_web_run(self, tmp_path, monkeypatch):
        catalog = tmp_path / "games_amazon.json"
        catalog.write_text(
            json.dumps({"games": [_launcher_row(f"l{i}", name=f"Launcher {i}", asin=f"L{i:03d}") for i in range(20)]}),
            encoding="utf-8",
        )
        monkeypatch.setattr(fa, "catalog_file", lambda p: catalog if p == fa.GAMES_AMAZON_JSON else tmp_path / p)
        assert (
            fa.refuse_amazon_source_drift(5, source="web", allow_drift=False, output_path=fa.GAMES_AMAZON_JSON) is None
        )


class TestCarryForwardLogic:
    def test_legacy_rows_carried_on_web_run(self):
    ...
    def test_same_source_not_carried(self):
...
class TestResolveSourceLauncherDisabled:
    def test_launcher_db_ready_false_when_disabled(self, monkeypatch):
        monkeypatch.setattr(fa, "is_local_provider_disabled", lambda provider: True)
        assert fa._launcher_db_ready(None) is False

    def test_auto_skips_launcher_when_user_disabled(self, monkeypatch):
    ...
    def test_auto_launcher_when_not_disabled(self, monkeypatch):
    ...
    def test_auto_launcher_when_both_launcher_and_web_ready(self, monkeypatch):
        monkeypatch.setattr(fa, "is_local_provider_disabled", lambda provider: False)
        monkeypatch.setattr(fa, "_launcher_db_ready", lambda sql_dir: True)
        monkeypatch.setattr(fa, "_web_profile_ready", lambda: True)
        assert fa.resolve_source("auto", None) == "launcher"
