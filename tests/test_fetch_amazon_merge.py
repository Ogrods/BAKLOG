"""Tests for Amazon launcher/web union-merge in fetch_amazon.py."""
...
def _launcher_row(
    pid: str,
    *,
    name: str = "Launcher Game",
    asin: str | None = "B00LAUNCH",
    last_played: str | None = "2024-01-01",
) -> dict:
    return {
        "store": "amazon",
        "id": pid,
    ...
    def test_web_slice_shrink_triggers_exit_3(self, tmp_path: Path, monkeypatch) -> None:
        ...
        catalog.write_text(
            json.dumps(
                {
                    "games": [
                        _web_row(f"w{i}", name=f"Web {i}", asin=f"B{i:03d}")
                        for i in range(10)
                    ]
                }
            ),
        ...
        assert (
            fa.refuse_amazon_source_drift(
                2,
                source="web",
                allow_drift=False,
                output_path=fa.GAMES_AMAZON_JSON,
            )
            == 3
        )
    ...
    def test_launcher_slice_unchanged_does_not_block_web_run(self, tmp_path: Path, monkeypatch) -> None:
        ...
        catalog.write_text(
            json.dumps(
                {
                    "games": [
                        _launcher_row(f"l{i}", name=f"Launcher {i}", asin=f"L{i:03d}")
                        for i in range(20)
                    ]
                }
            ),
