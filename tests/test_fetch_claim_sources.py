...
def _claim(item_id, source='gamerpower'):
    return {'id': item_id, 'store': 'steam', 'claim_url': f'https://example.com/{item_id}', 'title': item_id, 'source': source}
...
def test_vanished_source_allowed_when_rows_carried_forward(monkeypatch, out_path):
    prior = {'sources': {'gamerpower': 1, 'itad': 1}, 'items': [_claim('gamerpower-old', 'gamerpower'), _claim('itad-old', 'itad')]}
    out_path.write_text(json.dumps(prior), encoding='utf-8')
...
def test_collect_claims_keeps_cross_source_same_title(monkeypatch):
    ...
    def fake_epic(**kw):
        return [{'id': 'epic-brocco', 'store': 'other', 'title': 'Mr.Brocco & Co', 'claim_url': 'https://example.com/epic', 'source': 'epic'}]

    def fake_gamerpower():
        return [{'id': 'gamerpower-brocco', 'store': 'other', 'title': 'Mr.Brocco And Co (IndieGala) Giveaway', 'claim_url': 'https://example.com/gp', 'source': 'gamerpower'}]
