...
def test_ea_connected_requires_mark_connected_blob(monkeypatch):
    ...
    monkeypatch.setattr("auth.manager.get_provider_blob", lambda _p: {"status": "connected", "EA_BEARER_TOKEN": "tok"})
    assert fetch_ea._ea_connected() is True


def test_fetch_blocks_while_auth_session_active(monkeypatch):
    import fetchers.fetch_ea as fetch_ea
...
def test_stored_token_skips_sniff_when_owned_api_works(monkeypatch):
    monkeypatch.setattr("fetchers.fetch_ea._ea_connected", lambda: True)
    monkeypatch.setattr(
        "fetchers.fetch_ea.resolve_env", lambda key, **_k: "stored-tok" if key == "EA_BEARER_TOKEN" else ""
    )
...
def test_stored_token_browser_fallback_when_apq_stale(monkeypatch, tmp_path):
    prof = tmp_path / "ea"
    prof.mkdir(parents=True)
    (prof / "Default").mkdir()
    monkeypatch.setattr("fetchers.fetch_ea.profile_dir", lambda _p: prof)
    ...
    monkeypatch.setattr("fetchers.fetch_ea.probe_ea_token", lambda *_a, **_k: {"ok": True, "library_via_browser": True})
    sample_cookies = [{"name": "remid", "value": "x"}]
    monkeypatch.setattr("fetchers.fetch_ea._load_ea_profile_cookies", lambda: sample_cookies)
    ...
    ctx.pages = [MagicMock()]
    with (
        patch("fetchers.fetch_ea.launch_ea_profile") as mock_launch,
        patch("fetchers.fetch_ea.sniff_ea_bearer", return_value=fake_result),
    ):
...
def test_resolve_session_skips_browser_when_snapshot_fresh(monkeypatch, tmp_path):
...
def test_resolve_session_skips_browser_for_auth_only_snapshot(monkeypatch, tmp_path):
    from datetime import UTC, datetime
    ...
    snap = tmp_path / "connect_snapshot.json"
    snap.write_text(
        json.dumps({"captured_at": datetime.now(UTC).isoformat(), "owned_items": [], "browser_auth_ok": True}),
        encoding="utf-8",
    )
    monkeypatch.setattr("clients.ea_session.ea_connect_snapshot_path", lambda: snap)
...
def test_main_session_expired_marks_invalid_exit_4(monkeypatch):
    import fetchers.fetch_ea as fetch_ea
...
def test_main_empty_owned_exit_4(monkeypatch):
    import fetchers.fetch_ea as fetch_ea
    from fetchers._progress import EXIT_CODE_AUTH
    ...
    with (
        patch("fetchers.fetch_ea._resolve_session") as mock_resolve,
        patch("fetchers.fetch_ea.fetch_owned_games_browser", return_value=[]),
    ):
...
def test_cdp_closed_is_exit_1_not_4(monkeypatch):
    import fetchers.fetch_ea as fetch_ea
    ...
    with (
        patch("fetchers.fetch_ea._resolve_session", side_effect=boom),
        patch(
