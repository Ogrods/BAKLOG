...
def test_stored_token_browser_fallback_when_apq_stale(monkeypatch, tmp_path) -> None:
    ...
    fake_result = type('R', (), {'token': 'stored-tok', 'cookies': sample_cookies, 'owned_items': [{'originOfferId': '2', 'product': {'name': 'Browser'}}], 'debug': {'final_url': 'https://www.ea.com/sales/deals'}})()
    ctx = MagicMock()
    ctx.pages = [MagicMock()]
    with patch('fetchers.fetch_ea.launch_ea_profile') as mock_launch, patch('fetchers.fetch_ea.sniff_ea_bearer', return_value=fake_result):
...
def test_resolve_session_skips_browser_when_snapshot_fresh(monkeypatch, tmp_path) -> None:
    ...
    snap.write_text(json.dumps({'captured_at': datetime.now(UTC).isoformat(), 'owned_items': [{'originOfferId': '1', 'product': {'name': 'Game'}}], 'browser_auth_ok': True}), encoding='utf-8')
...
def test_resolve_session_skips_browser_for_auth_only_snapshot(monkeypatch, tmp_path) -> None:
    ...
    import fetchers.fetch_ea as fetch_ea
    snap = tmp_path / 'connect_snapshot.json'
    snap.write_text(json.dumps({'captured_at': datetime.now(UTC).isoformat(), 'owned_items': [], 'browser_auth_ok': True}), encoding='utf-8')
    monkeypatch.setattr('clients.ea_session.ea_connect_snapshot_path', lambda: snap)
...
def test_main_empty_owned_exit_4(monkeypatch) -> None:
    ...
    with patch('fetchers.fetch_ea._resolve_session') as mock_resolve, patch('fetchers.fetch_ea.fetch_owned_games_browser', return_value=[]):
...
def test_cdp_closed_is_exit_1_not_4(monkeypatch) -> None:
    ...
    with patch('fetchers.fetch_ea._resolve_session', side_effect=boom), patch('fetchers.fetch_ea.mark_invalid', side_effect=lambda *_a, **_k: (_ for _ in ()).throw(AssertionError('should not mark invalid'))):
        monkeypatch.setattr(sys, 'argv', ['fetch_ea', '--skip-hltb'])
        code = fetch_ea.main()
    assert code == 1