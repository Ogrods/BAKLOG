...
import argparse
from fetchers.fetch_epic import _can_reuse_cached_epic_row, _is_epic_catalog_excluded, _is_game_item, _is_non_game_title, _needs_catalog_fetch
...
def test_is_game_item_accepts_noise_soundtrack_for_tagging() -> None:
    item = {'title': 'HD Wallpaper', 'keyImages': [{'type': 'OfferImageWide', 'url': 'https://example/x.jpg'}], 'categories': []}
    assert _is_game_item(item)

def test_is_epic_catalog_excluded_addon_only() -> None:
    item = {'title': 'Sand Patch Grade', 'keyImages': [{'type': 'OfferImageWide', 'url': 'https://example/x.jpg'}], 'categories': [{'path': 'addons'}]}
...
def test_is_game_item_accepts_games_category_dlc() -> None:
    item = {'title': 'ARK Ragnarok', 'keyImages': [{'type': 'OfferImageWide', 'url': 'https://example/x.jpg'}], 'categories': [{'path': 'games/edition/base|games/edition/addons|addons/durable'}]}
    assert _is_game_item(item)
...
def test_can_reuse_cached_row_rejects_live_placeholder_mismatch() -> None:
    ...
    catalog_item = {'title': 'Sand Patch Grade', 'keyImages': [{'type': 'OfferImageWide', 'url': 'https://example/x.jpg'}], 'categories': [{'path': 'addons'}]}
...
def test_can_reuse_cached_row_allows_skip_hltb_without_hltb_fields() -> None:
    ...
    catalog_item = {'title': 'Botany Manor', 'keyImages': [{'type': 'OfferImageWide', 'url': 'https://example/x.jpg'}], 'categories': [{'path': 'games/edition/base'}]}
...
def test_get_playtime_parses_and_filters_zero_and_junk() -> None:
    client = _playtime_client([{'artifactId': 'a', 'totalTime': 3600}, {'artifactId': 'b', 'totalTime': 0}, {'artifactId': 'c'}, {'noId': True}, 'junk'])
