...
def test_fetch_games_prefetch_uses_heartbeat_guard():
...
def test_missing_credentials_message_mentions_connections(monkeypatch):
...
def test_fetch_store_data_falls_back_to_cached_reviews():
    steam = MagicMock()
...
def test_main_survives_store_error_with_cached_catalog(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        fetch_games, "catalog_file", lambda p: catalog if p == fetch_games.GAMES_STEAM_JSON else tmp_path / p
    )
