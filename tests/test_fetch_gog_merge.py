...
class TestResolveSource:
    ...
    def test_auto_local_without_web_creds(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(fg, 'is_local_provider_disabled', lambda _p: False)
        monkeypatch.setattr(fg, '_galaxy_db_ready', lambda _p: True)
    ...
    def test_auto_local_when_ready_sentinel_poisoned_env(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    def test_local_data_present_ignores_ready_sentinel_in_blob(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        galaxy_db = tmp_path / 'galaxy-2.0.db'
        galaxy_db.write_text('', encoding='utf-8')
        monkeypatch.setattr('clients.gog_galaxy_client.default_galaxy_db', lambda: galaxy_db)
