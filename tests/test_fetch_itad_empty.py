...
def _isolated(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        fetch_itad, "ItadClient", lambda *a, **k: pytest.fail("ItadClient should not be built for an empty wishlist")
    )
    monkeypatch.setattr("sys.argv", ["fetchers.fetch_itad.py"])
    return catalogs


def test_empty_wishlist_warns_and_exits_clean(_isolated, capsys):
...
def test_empty_wishlist_does_not_overwrite_existing_prices(_isolated):
