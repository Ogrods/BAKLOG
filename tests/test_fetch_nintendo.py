...
def test_merge_transactions_tags_voucher_as_noise() -> None:
    from fetchers.fetch_nintendo import _merge_transactions
    rows = _merge_transactions([{'title': 'Nintendo eShop Card $20', 'content_type': 'voucher', 'transaction_type': 'purchase', 'transaction_id': 'v1', 'date': '2024-01-01'}])
    assert len(rows) == 1
