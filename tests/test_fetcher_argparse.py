...
def _run_help(script: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run([sys.executable, rel, '--help'], cwd=ROOT, capture_output=True, text=True, timeout=30, env=env)
_NO_DUPLICATE_ALLOW_DRIFT = ('fetchers/fetch_ea.py', 'fetchers/fetch_itch.py', 'fetchers/fetch_gog.py', 'fetchers/fetch_amazon.py')
