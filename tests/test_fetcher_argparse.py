...
def _run_help(script):
    ...
    return subprocess.run(
        [sys.executable, rel, "--help"], cwd=ROOT, capture_output=True, text=True, timeout=30, env=env
    )
...
def test_fetch_ea_help_exits_zero():
...
def test_fetch_scripts_do_not_duplicate_allow_drift_in_source():
    for script in _NO_DUPLICATE_ALLOW_DRIFT:
        ...
        assert manual == 0, f"{script} registers --allow-drift manually; use add_allow_empty_arg only"


def test_fetch_nintendo_help_lists_rebuild_flag():
    proc = _run_help("fetch_nintendo.py")
    assert proc.returncode == 0, proc.stderr or proc.stdout
    assert "--rebuild" in (proc.stdout or "")


def test_fetch_humble_accepts_skip_hltb_flag():
...
def test_every_manifest_fetcher_help_exits_zero(entry):
...
@pytest.mark.parametrize("entry", MANIFEST_ENTRIES, ids=[e["key"] for e in MANIFEST_ENTRIES])
def test_manifest_default_args_advertised_in_help(entry):
