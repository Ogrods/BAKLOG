...
def test_every_fetcher_dispatches_via_argv_and_env():
    raw = json.loads(__import__('fetchers.registry', fromlist=['MANIFEST_PATH']).MANIFEST_PATH.read_text(encoding='utf-8'))
...
def test_every_fetcher_reconfigures_stdout_line_buffered():
    ...
    offenders: list[str] = []
    targets = sorted({*_REPO.glob('fetchers/fetch_*.py'), *_REPO.glob('enrichers/enrich_*.py'), _REPO / 'fetchers' / '_base.py'})
    for path in targets:
