...
import pytest
import server
from fetchers.registry import AUTH_PROVIDER_BY_KEY, ENRICH_FETCHER_KEYS, ENRICH_RELOAD_WISHLIST_KEYS, LIBRARY_JSON_BY_KEY, MANIFEST_PATH, WISHLIST_JSON_BY_KEY, WISHLIST_META_KEY_BY_FETCHER, validate_manifest
...
def test_reload_mapping(entry: dict) -> None:
    ...
    if key in ENRICH_KEYS or key in ('itad', 'claims'):
        return
    assert key in LIBRARY_STORE_JSON or key in WISHLIST_FETCHER_JSON, f'{key} has no reloadAfterFetcher mapping in app.js'
...
REFUSE_EMPTY_SCRIPTS = {'fetchers/fetch_games.py', 'fetchers/fetch_gog.py', 'fetchers/fetch_epic.py', 'fetchers/fetch_psn.py', 'fetchers/fetch_xbox.py', 'fetchers/fetch_gog_wishlist.py', 'fetchers/fetch_epic_wishlist.py', 'fetchers/fetch_psn_wishlist.py', 'fetchers/fetch_ubisoft_wishlist.py', 'fetchers/fetch_xbox_wishlist.py', 'fetchers/fetch_nintendo_wishlist.py', 'fetchers/fetch_humble.py', 'fetchers/fetch_humble_wishlist.py', 'fetchers/fetch_wishlist.py', 'fetchers/fetch_itad.py', 'fetchers/fetch_itch.py', 'fetchers/fetch_battlenet.py', 'fetchers/fetch_nintendo.py', 'fetchers/fetch_ubisoft.py', 'fetchers/fetch_ea.py'}
MANUAL_EMPTY_EXIT_SCRIPTS = {'fetchers/fetch_amazon.py'}
DRIFT_GUARD_BY_SCRIPT: dict[str, str] = {'fetchers/fetch_amazon.py': 'refuse_amazon_source_drift', 'fetchers/fetch_gog.py': 'refuse_gog_source_drift', 'fetchers/fetch_itch.py': 'refuse_itch_source_drift'}
NO_EMPTY_GUARD_SCRIPTS = {'enrichers/enrich_hltb.py', 'enrichers/enrich_steam_reviews.py', 'enrichers/enrich_cross_store_images.py', 'enrichers/enrich_steam_tags.py', 'enrichers/enrich_protondb.py'}
...
ENRICH_CACHE_LOADERS: dict[str, str] = {'hltb': 'loadHltbCache', 'steamReviews': 'loadSteamReviewCache', 'steamCovers': 'loadSteamCoversMeta', 'steamTags': 'loadSteamTagsMeta', 'protondb': 'loadProtondbCache'}

def test_reload_after_fetcher_calls_enrich_cache_loaders() -> None:
    text = (ROOT / 'js' / 'library-load.js').read_text(encoding='utf-8')
    branch = re.search("ENRICH_FETCHER_KEYS\\.has\\(key\\)\\)\\s*\\{([\\s\\S]*?)\\} else if \\(key === 'claims'\\)", text)
    assert branch, 'enrich branch missing in reloadAfterFetcher'
    body = branch.group(1)
    for key in ENRICH_FETCHER_KEYS:
        fn = ENRICH_CACHE_LOADERS[key]
        assert f'if (key === "{key}")' in body or f"if (key === '{key}')" in body, f'{key}: no per-key branch in enrich reload'
