"""Every manifest fetch script must resist the 180s stdout stall watchdog."""
...
# Single HTTP round-trip; no numbered enrichment loop.
...
def test_fetcher_emits_stall_heartbeats(entry: dict) -> None:
    ...
    assert has_timer or has_pulse, (
        f"{entry['script']} must use HeartbeatTimer and/or run_with_heartbeat "
        "so long network loops keep stdout alive under the 180s stall watchdog"
    )
...
def test_fetcher_progress_prints_flush(entry: dict) -> None:
    ...
    assert "flush=True" in text, (
        f"{entry['script']} must pass flush=True on progress prints "
        "(or every numbered progress print) for pipe capture"
    )
