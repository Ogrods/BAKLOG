...
def test_has_valid_claim_links_epic_mobile():
    assert has_valid_claim_links(
        {"store": "epic_mobile", "claim_urls": {"android": "https://play.google.com/store/apps/details?id=x"}}
    )
...
def test_infer_store_from_text_mobile_epic():
    store = _infer_store_from_text(
        "epic", "TMNT free on Mobile from EGS on Epic Game Store", None, "https://isthereanydeal.com/giveaways/1/"
    )
    assert store == "epic_mobile"


def test_preview_publish_items_epic_mobile_manual(tmp_path, monkeypatch):
