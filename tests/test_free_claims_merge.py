...
from shared.free_claims_sources import carry_claim_enrichment, claim_match_keys, dedup_claim_items_by_id, merge_manual_and_auto, norm_title, parse_epic_element, parse_gamerpower_item, parse_itad_rss, platforms_to_store, should_skip_itad_title

def _epic_element(*, discount: int, title: str='Free Game', slug: str='free-game-123') -> dict:
    return {'title': title, 'offerMappings': [{'pageSlug': slug, 'pageType': 'productHome'}], 'keyImages': [{'type': 'OfferImageWide', 'url': 'https://example.com/wide.jpg'}], 'promotions': {'promotionalOffers': [{'promotionalOffers': [{'startDate': '2020-01-01T00:00:00.000Z', 'endDate': '2099-12-31T23:59:59.000Z', 'discountSetting': {'discountType': 'PERCENTAGE', 'discountPercentage': discount}}]}], 'upcomingPromotionalOffers': []}}
...
def test_gamerpower_parses_end_date_and_na():
    active = parse_gamerpower_item({'id': 42, 'title': 'Test Game (Steam) Giveaway', 'status': 'Active', 'platforms': 'PC, Steam', 'open_giveaway_url': 'https://www.gamerpower.com/open/test', 'end_date': '2026-06-08 23:59:00', 'image': 'https://example.com/img.jpg', 'description': 'Free on Steam.'})
    no_end = parse_gamerpower_item({'id': 43, 'title': 'No End Game (GOG) Giveaway', 'status': 'Active', 'platforms': 'PC, GOG', 'open_giveaway_url': 'https://www.gamerpower.com/open/no-end', 'end_date': 'N/A'})
...
def test_dedup_by_id_keeps_cross_source_same_title():
    items = dedup_claim_items_by_id([{'id': 'gamerpower-1', 'store': 'epic', 'title': 'Relaxing Simulator', 'claim_url': 'https://www.gamerpower.com/open/relaxing', 'source': 'gamerpower'}, {'id': 'epic-relaxing-simulator', 'store': 'epic', 'title': 'Relaxing Simulator', 'claim_url': 'https://store.epicgames.com/en-US/p/relaxing-simulator', 'source': 'epic'}])
    assert len(items) == 2
    sources = {item['source'] for item in items}
    assert sources == {'gamerpower', 'epic'}

def test_dedup_by_id_collapses_same_id_prefers_epic():
    items = dedup_claim_items_by_id([{'id': 'epic-relaxing-simulator', 'store': 'epic', 'title': 'Relaxing Simulator (GamerPower)', 'claim_url': 'https://www.gamerpower.com/open/relaxing', 'source': 'gamerpower'}, {'id': 'epic-relaxing-simulator', 'store': 'epic', 'title': 'Relaxing Simulator', 'claim_url': 'https://store.epicgames.com/en-US/p/relaxing-simulator', 'source': 'epic'}])
    assert len(items) == 1
    assert items[0]['source'] == 'epic'

def test_merge_manual_wins_on_duplicate_title_and_id():
    manual = [{'id': 'epic-manual', 'store': 'epic', 'title': 'Manual Override', 'claim_url': 'https://example.com/manual'}]
    auto = [{'id': 'epic-manual', 'store': 'epic', 'title': 'Manual Override', 'claim_url': 'https://store.epicgames.com/en-US/p/auto', 'source': 'epic'}, {'id': 'gog-auto', 'store': 'gog', 'title': 'GOG Freebie', 'claim_url': 'https://www.gog.com/game/freebie', 'source': 'gamerpower'}]
...
def test_carry_claim_enrichment_fills_missing_fields_only() -> None:
    fresh = {'id': 'itad-b07aac9ebd26', 'store': 'epic', 'title': 'Wytchwood', 'claim_url': 'https://example.com/w', 'header_image': None}
    existing = {'id': 'itad-b07aac9ebd26', 'header_image': 'https://cdn.example/portrait.jpg', 'review_percent': 93, 'steam_appid': 729000, 'genres': ['Adventure']}
