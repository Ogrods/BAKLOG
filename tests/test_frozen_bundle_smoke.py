...
def _stub_bundle(tmp_path, *, with_env=True):
    ...
    (manifest_dir / "manifest.json").write_text('{"fetchers":[{"key":"steam"},{"key":"gog"}]}', encoding="utf-8")
    if with_env:
...
def test_env_has_auth_keys_detects_missing(tmp_path):
...
def test_manifest_fetcher_count(tmp_path):
    bundle = _stub_bundle(tmp_path)
    assert smoke._manifest_fetcher_count(bundle) == 2


def test_run_smoke_fails_without_env(tmp_path, monkeypatch):
    bundle = _stub_bundle(tmp_path, with_env=False)
