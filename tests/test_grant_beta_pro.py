...
def test_main_dry_run_lists_users(monkeypatch, capsys) -> None:
    ...
    def fake_list(_base: str, _key: str) -> list[dict]:
        return [{'id': '11111111-1111-4111-8111-111111111111', 'email': 'beta@example.com', 'app_metadata': {'plan': 'free'}, 'created_at': '2026-01-01T00:00:00Z'}]
    monkeypatch.setattr(grant, 'list_users', fake_list)
    monkeypatch.setattr(grant, 'admin_request', lambda *a, **k: (_ for _ in ()).throw(AssertionError('should not write')))
...
def test_main_apply_updates_user(monkeypatch, capsys) -> None:
    ...
    def fake_list(_base: str, _key: str) -> list[dict]:
        return [{'id': '11111111-1111-4111-8111-111111111111', 'email': 'beta@example.com', 'app_metadata': {'plan': 'free', 'beta': True}, 'created_at': '2026-01-01T00:00:00Z'}]
