...
def test_parse_order_detail_games_only() -> None:
    data = json.loads(ORDER_FIXTURE.read_text(encoding='utf-8'))
...
def test_parse_order_detail_include_nongames() -> None:
    data = json.loads(ORDER_FIXTURE.read_text(encoding='utf-8'))
    items = _parse_order_detail(data, include_nongames=True)
    assert any((it.name == 'Sample Ebook Only' for it in items))

def test_build_library_row_schema() -> None:
    item = LibraryItem(machine_name='sample_game', name='Sample Game', image_url='https://cdn.humblebundle.com/images/icons/sample_game.png', store_url='https://www.humblebundle.com/store/sample_game', gamekey='AbCdEf', redeemed=False, steam_app_id=None)
...
def test_build_wishlist_row_schema() -> None:
    item = WishlistItem(product_id='hollow_knight', title='Hollow Knight', image_url='https://cdn.humblebundle.com/images/hollow_knight.jpg', store_url='https://www.humblebundle.com/store/hollow_knight', price='$14.99', price_initial='$14.99', discount_percent=None, currency='USD')
