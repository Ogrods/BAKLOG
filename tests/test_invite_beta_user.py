...
def test_invite_dry_run_new_user(monkeypatch, capsys):
...
def test_invite_send_new_user(monkeypatch, capsys):
    monkeypatch.setenv("SUPABASE_URL", "https://demo.supabase.co")
    monkeypatch.setenv("SUPABASE_SERVICE_ROLE_KEY", "service-key")
    calls = []
...
def test_invite_listed_pro_invitee_auto_grants(monkeypatch, capsys, tmp_path):
    monkeypatch.setenv("SUPABASE_URL", "https://demo.supabase.co")
    monkeypatch.setenv("SUPABASE_SERVICE_ROLE_KEY", "service-key")
...
def test_invite_non_listed_stays_free(monkeypatch, capsys, tmp_path):
    monkeypatch.setenv("SUPABASE_URL", "https://demo.supabase.co")
    monkeypatch.setenv("SUPABASE_SERVICE_ROLE_KEY", "service-key")
    pro_file = tmp_path / "pro_invitees.txt"
    pro_file.write_text("paid@example.com\n", encoding="utf-8")
    ...
    monkeypatch.setattr(invite, "find_user_by_email", lambda *_a, **_k: None)
    monkeypatch.setattr(
        invite, "invite_user_by_email", lambda *a, **k: {"id": "33333333-3333-4333-8333-333333333333", "email": a[2]}
    )
    monkeypatch.setattr(invite, "set_user_plan", lambda *args: writes.append(args))
...
def test_invite_existing_user_grant_pro(monkeypatch, capsys):
    monkeypatch.setenv("SUPABASE_URL", "https://demo.supabase.co")
    monkeypatch.setenv("SUPABASE_SERVICE_ROLE_KEY", "service-key")
    existing = {
