...
def test_get_library_records_with_cave_stats(tmp_path):
...
def test_default_butler_db_windows_includes_itch_folder(monkeypatch):
...
def test_default_butler_db_darwin_path(monkeypatch):
...
def test_default_butler_db_linux_prefers_local_share(monkeypatch, tmp_path):
...
def test_default_butler_db_linux_falls_back_to_config(monkeypatch, tmp_path):
    monkeypatch.setattr(sys, "platform", "linux")
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.setattr("clients.itch_local_client.Path.home", lambda: home)
