...
from auth.manager import _provider_state, _with_profile_secrets, enable_local, get_status, migrate_existing_itch_local_opt_in, seed_new_profile_auth_defaults
...
def test_itch_local_requires_enabled_even_when_butler_db_present(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_migration_opts_in_existing_profile_with_itch_library(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr('auth.manager.platform_supported', lambda platforms: True)
    monkeypatch.setattr('auth.manager._local_data_present', lambda provider, blob: True)
...
def test_create_profile_local_providers_disconnected(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr('auth.manager.platform_supported', lambda platforms: True)
    monkeypatch.setattr('auth.manager._local_data_present', lambda provider, blob: True)
    create_profile('Work')
