...
def test_merge_vgc_with_transactions_joins_by_title():
    vgc = [_sample_vgc()]
    tx = [
        {
            "name": "Zelda Tears",
...
def test_merge_vgc_primary_lists_all_entitlements_before_orphan_receipts():
    vgc = [_sample_vgc(application_id="0100a", name="Alpha"), _sample_vgc(application_id="0100b", name="Beta")]
    tx = [
        {"name": "Alpha", "id": "tx-a", "nintendo_id": "tx-a", "purchase_date": "2024-01-01", "tags": []},
        {"name": "Receipt Only", "id": "tx-only", "nintendo_id": "tx-only", "purchase_date": "2025-01-01", "tags": []},
    ]
    merged = merge_vgc_with_transactions(vgc, tx)
    assert len(merged) == 3
...
def test_merge_vgc_with_transactions_adds_vgc_only_entitlements():
...
def test_merge_vgc_fuzzy_matches_trademark_and_edition_titles():
...
def test_match_nintendo_title_key_strips_deluxe_suffix():
    assert match_nintendo_title_key("Game Digital Deluxe Edition") == "game"


def test_is_nintendo_playable_game_rejects_skins_and_dlc():
    ...
    assert (
        is_nintendo_playable_game({"name": "Persona 5 Tactica: Orpheus Picaro & Izanagi Picaro", "tags": []}) is False
    )
    assert (
        is_nintendo_playable_game({"name": "SONIC SUPERSTARS Digital Art Book with Mini Digital Sound", "tags": []})
        is False
    )


def test_nintendo_store_url_prefers_application_id():
    url = nintendo_store_url("01008F600B514000", "Some Game")
    assert url.endswith("/game/01008F600B514000/")


def test_find_existing_row_matches_application_id_after_id_migration():
    ...
    cached = find_existing_row(
        {"id": "0100abc", "application_id": "0100abc", "nintendo_id": "tx-old", "name": "Shared Game"},
        existing=existing,
        by_title=by_title,
        by_app_id=by_app_id,
