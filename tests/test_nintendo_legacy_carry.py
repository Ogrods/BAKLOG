...
from fetchers.fetch_nintendo import NINTENDO_LEGACY_FIELD, _nintendo_drift_baseline, carry_forward_nintendo_legacy, load_nintendo_dropped_ids, maybe_repair_nintendo_catalog_on_disk, refuse_nintendo_source_drift, repair_nintendo_stale_catalog

def test_carry_forward_nintendo_legacy_tags_legacy_not_stale() -> None:
    fresh = [{'id': '1', 'name': 'Alpha'}]
    existing = [{'id': '1', 'name': 'Alpha'}, {'id': '2', 'name': 'Beta', STALE_FIELD: True, STALE_SINCE_FIELD: '2026-01-01T00:00:00+00:00', 'hltb_main_hours': 10}]
    out = carry_forward_nintendo_legacy(fresh, existing, dropped_ids=set(), key_fn=row_key_by_id, now_iso='2026-06-25T12:00:00+00:00')
    assert len(out) == 2
...
def test_carry_forward_nintendo_legacy_skips_dropped_ids() -> None:
    fresh = [{'id': '1', 'name': 'Alpha'}]
    existing = [{'id': '1', 'name': 'Alpha'}, {'id': '2', 'name': 'Dropped'}, {'id': '3', 'name': 'Legacy'}]
    out = carry_forward_nintendo_legacy(fresh, existing, dropped_ids={'2'}, key_fn=row_key_by_id, now_iso='2026-06-25T12:00:00+00:00')
...
def test_nintendo_drift_baseline_prefers_fresh_count(tmp_path: Path, monkeypatch) -> None:
    ...
    catalog.write_text(json.dumps({'game_count': 120, 'fresh_count': 45, 'games': [{'id': '1'}, {'id': '2', 'nintendo_legacy': True}]}), encoding='utf-8')
...
def test_nintendo_drift_baseline_falls_back_to_non_legacy_rows(tmp_path: Path, monkeypatch) -> None:
    catalog = tmp_path / 'games_nintendo.json'
    catalog.write_text(json.dumps({'game_count': 5, 'games': [{'id': '1'}, {'id': '2', 'nintendo_legacy': True}, {'id': '3', 'stale': True}]}), encoding='utf-8')
    monkeypatch.setattr('fetchers.fetch_nintendo.catalog_file', lambda _p: catalog)
    assert _nintendo_drift_baseline(Path('games_nintendo.json')) == 1

def test_refuse_nintendo_drift_uses_fresh_baseline_not_total(tmp_path: Path, monkeypatch) -> None:
    catalog = tmp_path / 'games_nintendo.json'
    catalog.write_text(json.dumps({'game_count': 120, 'fresh_count': 45, 'games': []}), encoding='utf-8')
    monkeypatch.setattr('fetchers.fetch_nintendo.catalog_file', lambda _p: catalog)
    rows = [{'id': str(i)} for i in range(40)]
    assert refuse_nintendo_source_drift(rows, label='Nintendo library rows', allow_drift=False, output_path=Path('games_nintendo.json')) is None
    small = [{'id': str(i)} for i in range(20)]
    assert refuse_nintendo_source_drift(small, label='Nintendo library rows', allow_drift=False, output_path=Path('games_nintendo.json')) == 3
...
def test_load_nintendo_dropped_ids_reads_personal(tmp_path: Path, monkeypatch) -> None:
    ...
    personal.write_text(json.dumps({'personal': {'__nintendo_dropped_ids_v1': ['tx-1', 'tx-2']}, 'prefs': {}, 'manual': [], 'libraryFirstSeen': {}}), encoding='utf-8')
...
def test_dropped_ids_excluded_from_fresh_slice() -> None:
    fresh = [{'id': '1', 'name': 'Alpha'}, {'id': '2', 'name': 'Dropped but still on API'}]
    out = carry_forward_nintendo_legacy(fresh, [], dropped_ids={'2'}, key_fn=row_key_by_id, now_iso='2026-06-25T12:00:00+00:00')
...
def test_carry_forward_nintendo_legacy_skips_title_match_when_id_churns() -> None:
    ...
    existing = [{'id': 'old-tx', 'name': 'Mario Kart 8 Deluxe', STALE_FIELD: True, 'hltb_main_hours': 12}]
    out = carry_forward_nintendo_legacy(fresh, existing, dropped_ids=set(), key_fn=row_key_by_id, now_iso='2026-06-25T12:00:00+00:00')
    assert len(out) == 1
...
def test_repair_nintendo_stale_catalog_promotes_legacy() -> None:
    games = [{'id': '1', 'name': 'Fresh'}, {'id': '2', 'name': 'Old', STALE_FIELD: True, STALE_SINCE_FIELD: '2026-01-01T00:00:00+00:00'}]
...
def test_maybe_repair_nintendo_catalog_on_disk_writes(tmp_path: Path, monkeypatch) -> None:
    catalog = tmp_path / 'games_nintendo.json'
    catalog.write_text(json.dumps({'games': [{'id': '1', 'name': 'A'}, {'id': '2', 'name': 'B', STALE_FIELD: True}]}), encoding='utf-8')
    monkeypatch.setattr('fetchers.fetch_nintendo.catalog_file', lambda _rel: catalog)
    monkeypatch.setattr('fetchers.fetch_nintendo.write_catalog_text', lambda _rel, text: catalog.write_text(text, encoding='utf-8'))
