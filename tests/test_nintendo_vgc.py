...
import pytest
from clients.nintendo_vgc import NintendoVgcAuthError, NintendoVgcCaptureError, _merge_vgc_payload, _portal_html_has_vgc_data, _portal_html_looks_unsigned, fetch_vgc_portal_html, map_vgc_view, parse_vgc_embedded_json, region_from_vgc_state, resolve_nintendo_icon_url
...
def test_map_vgc_view_switch_and_dlc() -> None:
    base = map_vgc_view({'id': 'vgc-1', 'applicationId': '0100abc', 'applicationName': 'Zelda™ Tears', 'apparentPlatform': 'NX', 'hasNxApplication': True, 'icon': {'url': 'https://atum-img.test/i/c/abc_${size}', 'sizes': [128, 256, 512]}})
...
def test_map_vgc_view_exposes_entitlement_metadata() -> None:
    view = {'id': 'vgc-1', 'applicationId': '0100abc', 'applicationName': 'Zelda™ Tears', 'apparentPlatform': 'NX', 'publisher': 'Nintendo', 'hasNxApplication': True, 'hasApplication': True, 'isLending': True, 'containsReleased': True, 'icon': {'url': 'https://img.test/a_${size}', 'upgradedIconUrl': 'https://img.test/b_${size}', 'sizes': [256, 512]}}
...
def test_merge_vgc_payload_dedupes_application_id() -> None:
    payload = {'data': {'account': {'vgc': {'vgcViews': {'views': [{'applicationId': 'a1', 'applicationName': 'One'}, {'applicationId': 'a1', 'applicationName': 'One dup'}, {'applicationId': 'a2', 'applicationName': 'Two'}]}}}}}
    collected: list = []
    seen: set[str] = set()
...
def test_diff_vgc_vs_catalog_legacy_gap() -> None:
    ...
    catalog = [{'name': 'Shared Title', 'id': 'tx-1'}, {'name': 'Old Purchase', 'id': 'tx-2', 'nintendo_legacy': True}, {'name': 'Receipt Only', 'id': 'tx-3'}]
