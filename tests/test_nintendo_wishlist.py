...
from fetchers.fetch_nintendo_wishlist import _build_row, _signed_out, _wishlist_graphql_ok, _wishlist_session_authenticated, parse_wishlist_sources
...
def test_build_row_schema() -> None:
    ...
    item = WishlistItem(product_id='71000000012345', title='Test Adventure', image_url='https://assets.nintendo.com/test.jpg', store_url='https://www.nintendo.com/us/store/products/test-adventure-switch/', release_date=None, genres=['Adventure'], price='$49.99', price_initial='$59.99', discount_percent=17, currency='USD')
...
def test_build_row_tags_dlc() -> None:
    from fetchers.fetch_nintendo_wishlist import WishlistItem
    item = WishlistItem(product_id='71000000099999', title='Adventure Expansion Pass', image_url=None, store_url='https://www.nintendo.com/us/store/products/adventure-dlc-switch/', release_date=None, genres=[], price='$19.99', price_initial=None, discount_percent=None, currency='USD', is_dlc=True)
    row = _build_row(item, None)
    assert row['type'] == 'dlc'
    assert row['tags'] == ['dlc']
...
def test_wishlist_item_is_dlc_from_product_type() -> None:
    ...
    item = _item_from_dict({'nsUid': '71000000088888', 'title': 'Bonus Tracks Pack', 'productType': 'DLC', 'url': '/us/store/products/bonus-tracks-switch/'})
...
def test_parse_wishlist_from_graphql_payload() -> None:
    payload = {'data': {'customer': {'id': 'cust-1', 'wishList': {'items': [{'id': 'wl-1', 'product': {'nsUid': '71000000012345', 'title': 'GraphQL Adventure', 'url': '/us/store/products/graphql-adventure-switch/', 'image': {'url': 'https://assets.nintendo.com/test.jpg'}}}], 'hasNextPage': False}}}}
...
def test_empty_authenticated_wishlist_counts_as_session() -> None:
    payload = {'data': {'customer': {'id': 'cust-1', 'wishList': {'items': [], 'hasNextPage': False}}}}
    assert _wishlist_graphql_ok(payload)
    assert _wishlist_session_authenticated([payload])
