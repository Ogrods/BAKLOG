...
def _spec_resolved_hiddenimports() -> set[str]:
    ...
    extra_fetchers = [ROOT / 'fetchers' / 'fetchers.fetch_free_claims.py', ROOT / 'fetchers' / 'fetchers.fetch_claim_sources.py', ROOT / 'fetchers' / 'fetchers.build_free_claims.py']
    ...
    out = {'baklog_fetcher_dispatch', 'clients', *(mod(p) for p in client_scripts), 'fetchers', 'fetchers.registry', *(mod(p) for p in fetcher_scripts), *(mod(p) for p in extra_fetchers if p.is_file()), 'enrichers', *(mod(p) for p in enricher_scripts), 'auth', 'auth.cdp_browser', 'auth.manager', 'auth.secrets', 'shared.install_paths', 'shared.data_dir_migration', 'shared.bundled_auth_env', 'shared.built_frontend', 'shared.legacy_env', 'shared.uninstall_cleanup', 'keyring.backends.macOS' if sys.platform == 'darwin' else 'keyring.backends.Windows', 'cryptography.hazmat.primitives.ciphers.aead'}
    return out
...
def test_pyinstaller_datas_include_curated_feeds() -> None:
    text = SPEC.read_text(encoding='utf-8')
    assert '("curated"), "curated")' in text.replace(' ', '') or ('root / "curated"' in text and '"curated"' in text), 'packaging/baklog.spec datas must bundle curated/ (free_claims.fallback.json offline)'

def test_inno_installer_branding_assets() -> None:
    packaging = ROOT / 'packaging'
    iss = (packaging / 'baklog.iss').read_text(encoding='utf-8')
    for key in ('SetupIconFile=installer-icon.ico', 'WizardImageFile=installer-wizard-large.bmp', 'WizardSmallImageFile=installer-wizard-small.bmp'):
