...
def test_pro_settings_put_requires_pro(local_server, monkeypatch):
    ...
    status, raw = _request(base, '/api/pro-settings', method='PUT', headers={'X-BAKLOG-Local': '1', 'Content-Type': 'application/json'}, body=body)
...
def test_pro_settings_put_blocked_without_local_header(auth_server):
    ...
    body = json.dumps({'cloudMirrorEnabled': True}).encode('utf-8')
    status, _raw = _request(base, '/api/pro-settings', method='PUT', headers={'Content-Type': 'application/json', 'Host': 'public.example.com'}, body=body)
    assert status == HTTPStatus.FORBIDDEN