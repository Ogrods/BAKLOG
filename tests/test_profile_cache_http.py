...
@pytest.fixture()
def profile_cache_server(tmp_path, monkeypatch):
...
def test_cache_json_served_from_active_profile_not_repo_root(profile_cache_server, tmp_path):
    create_profile("Work")
...
def test_missing_profile_cache_returns_empty_stub_not_root_cache(profile_cache_server, tmp_path):
    create_profile("Work")
    set_active_profile("work")
    server._refresh_personal_paths()
    root_cache = tmp_path / "cache"
    root_cache.mkdir(parents=True, exist_ok=True)
    (root_cache / "steam_review_map.json").write_text(
        json.dumps({"fetched_at": "root", "marker": "root"}), encoding="utf-8"
    )
...
def test_missing_profile_cross_store_meta_stub_shape(profile_cache_server, tmp_path):
    create_profile("Work")
    set_active_profile("work")
    server._refresh_personal_paths()
...
def test_profiles_url_prefix_blocked(profile_cache_server, tmp_path):
    create_profile("Work")
    set_active_profile("work")
    server._refresh_personal_paths()
