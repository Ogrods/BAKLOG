"""HTTP serving of profile-scoped enrichment cache files."""
...
def test_cache_json_served_from_active_profile_not_repo_root(
    ...
    (root_cache / "hltb_map.json").write_text(
        json.dumps({"fetched_at": "root", "marker": "root"}),
        encoding="utf-8",
    )

    work_cache = profile_paths.cache_json_path("hltb_map.json", profile_id="work")
    work_cache.parent.mkdir(parents=True, exist_ok=True)
    work_cache.write_text(
        json.dumps({"fetched_at": "2026-06-03T00:00:00+00:00", "marker": "work"}),
        encoding="utf-8",
    )
