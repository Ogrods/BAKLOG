...
def _write_blob(profile_id: str, key: str) -> None:
    ...
    saved = (secrets_mod.AUTH_DIR, secrets_mod.SECRETS_FILE, secrets_mod.MASTER_KEY_FILE, secrets_mod._cache, secrets_mod.PROFILE_ID_OVERRIDE)
    ...
    try:
        set_provider_blob('steam', {'STEAM_API_KEY': key, 'STEAM_ID': '1', 'status': 'connected'})
    finally:
        secrets_mod.AUTH_DIR, secrets_mod.SECRETS_FILE, secrets_mod.MASTER_KEY_FILE, secrets_mod._cache, secrets_mod.PROFILE_ID_OVERRIDE = saved

def test_concurrent_profile_credentials_env_isolation(isolated_profiles: None) -> None:
    create_profile('Work')
    ...
    def read(profile_id: str) -> None:
        try:
            ...
            errors.append(exc)
    threads = [threading.Thread(target=read, args=('default',)), threading.Thread(target=read, args=('work',)), threading.Thread(target=read, args=('default',)), threading.Thread(target=read, args=('work',))]
    for t in threads:
        t.start()
    for t in threads:
        t.join(timeout=10)
