...
@pytest.fixture()
def profile_delete_server(tmp_path, monkeypatch):
...
def _delete_profile(base, profile_id):
    req = urllib.request.Request(
        f"{base}/api/profiles/{profile_id}", method="DELETE", headers={server._BAKLOG_LOCAL_HEADER: "1"}
    )
    try:
...
def test_delete_profile_blocked_when_runs_in_flight(profile_delete_server):
...
def test_delete_profile_ok_when_no_runs(profile_delete_server):
    base, fake = profile_delete_server
    create_profile("Work")
    fake.blocked = None
    work_dir = profile_paths.profile_data_dir("work")
