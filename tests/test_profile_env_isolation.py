"""Per-profile credential checks do not inherit process .env."""
...
def test_missing_requirements_ignores_process_env(
...
def test_subprocess_env_uses_profile_blob_only(
    isolated_profiles: None,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    create_profile("Work")
...
def test_named_profile_ignores_process_env_in_get_credentials(
    isolated_profiles: None,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    create_profile("Work")
...
def test_default_profile_merges_process_env(
    isolated_profiles: None,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_named_profile_blob_credentials_still_resolve(
    isolated_profiles: None,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    create_profile("Work")
    ...
    set_provider_blob(
        "steam",
        {"STEAM_API_KEY": "profile-steam-key", "STEAM_ID": "1", "status": "connected"},
    )
...
def test_named_profile_auth_status_not_unverified_from_process_env(
    isolated_profiles: None,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    create_profile("Work")
...
def test_default_profile_unverified_when_only_process_env(
    isolated_profiles: None,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    set_active_profile("default")
