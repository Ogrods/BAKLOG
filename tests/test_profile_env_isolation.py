...
def test_missing_requirements_ignores_process_env(isolated_profiles: None, monkeypatch: pytest.MonkeyPatch) -> None:
    create_profile('Work')
...
def test_subprocess_env_uses_profile_blob_only(isolated_profiles: None, monkeypatch: pytest.MonkeyPatch) -> None:
    create_profile('Work')
    set_active_profile('work')
    monkeypatch.setenv('BAKLOG_PROFILE', 'work')
...
def test_named_profile_ignores_process_env_in_get_credentials(isolated_profiles: None, monkeypatch: pytest.MonkeyPatch) -> None:
    create_profile('Work')
    set_active_profile('work')
    monkeypatch.setenv('BAKLOG_PROFILE', 'work')
    monkeypatch.setenv('STEAM_API_KEY', 'from-process-env')
...
def test_default_profile_merges_process_env(isolated_profiles: None, monkeypatch: pytest.MonkeyPatch) -> None:
    set_active_profile('default')
    monkeypatch.delenv('BAKLOG_PROFILE', raising=False)
    monkeypatch.setenv('STEAM_API_KEY', 'default-env-key')
    assert get_credentials('steam').get('STEAM_API_KEY') == 'default-env-key'

def test_named_profile_blob_credentials_still_resolve(isolated_profiles: None, monkeypatch: pytest.MonkeyPatch) -> None:
    create_profile('Work')
    set_active_profile('work')
    monkeypatch.setenv('BAKLOG_PROFILE', 'work')
    monkeypatch.setenv('STEAM_API_KEY', 'from-process-env')
...
def test_named_profile_auth_status_not_unverified_from_process_env(isolated_profiles: None, monkeypatch: pytest.MonkeyPatch) -> None:
    create_profile('Work')
    set_active_profile('work')
    monkeypatch.setenv('BAKLOG_PROFILE', 'work')
    monkeypatch.setenv('STEAM_API_KEY', 'from-process-env')
...
def test_default_profile_unverified_when_only_process_env(isolated_profiles: None, monkeypatch: pytest.MonkeyPatch) -> None:
    set_active_profile('default')
    monkeypatch.delenv('BAKLOG_PROFILE', raising=False)
    monkeypatch.setenv('STEAM_API_KEY', 'from-process-env')
    monkeypatch.setenv('STEAM_ID', '76561198000000000')
...
def test_subprocess_env_decrypts_target_when_other_profile_active(isolated_profiles: None, monkeypatch: pytest.MonkeyPatch) -> None:
    create_profile('Work')
    create_profile('Play')
    set_active_profile('work')
    monkeypatch.setenv('BAKLOG_PROFILE', 'work')
...
def test_subprocess_env_omits_unset_provider_keys(isolated_profiles: None, monkeypatch: pytest.MonkeyPatch) -> None:
    create_profile('Work')
