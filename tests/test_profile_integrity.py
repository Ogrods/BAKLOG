...
def test_reconcile_materializes_index_when_profile_dir_exists_without_index(isolated_profiles: Path) -> None:
...
def test_quarantined_delete_dir_not_readopted(isolated_profiles: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    trash_dirs = [p for p in (isolated_profiles / 'profiles').iterdir() if p.is_dir() and p.name.startswith('.trash-work-')]
    assert trash_dirs
    with patch('shared.supabase_auth.auth_enabled', return_value=False):
        reconcile_profile_store()
    assert 'work' not in {p['id'] for p in profile_paths.load_index()['profiles']}

def test_create_profile_reserves_index_before_mkdir(isolated_profiles: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_delete_profile_blocks_effective_active_via_env(isolated_profiles: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    profiles.create_profile('Work')
    doc = profile_paths.load_index()
    doc['active'] = 'default'
    profile_paths.save_index(doc)
