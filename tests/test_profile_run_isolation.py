"""Run access and history stay scoped to the active profile in local multi-profile mode."""
...
def test_run_accessible_hides_other_profile_run(isolated_profiles: Path) -> None:
    ...
    work_run = server.Run(
        "steam",
        runs_dir=profile_paths.runs_dir(profile_id="work"),
        profile_id="work",
    )
...
def test_rebind_drops_other_profile_runs_from_memory(
    ...
    default_run = server.Run(
        "steam",
        runs_dir=profile_paths.runs_dir(profile_id="default"),
        profile_id="default",
    )
    with mgr._lock:
        mgr._runs_by_id[work_run.id] = work_run
...
def test_runs_snapshot_filters_other_profile_history(
    multi_profile_server: str,
    isolated_profiles: Path,
) -> None:
    base = multi_profile_server
    create_profile("Work")
