...
@pytest.fixture()
def multi_profile_server(isolated_profiles: Path, monkeypatch: pytest.MonkeyPatch):
...
def _request(base: str, method: str, path: str) -> tuple[int, str | dict]:
    headers = {}
    if method != 'GET':
...
def test_run_accessible_hides_other_profile_run(isolated_profiles: Path) -> None:
    create_profile('Work')
    create_profile('Play')
...
def test_rebind_drops_other_profile_runs_from_memory(isolated_profiles: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    create_profile('Work')
    mgr = server.RunManager(runs_dir=profile_paths.runs_dir(profile_id='default'))
    work_run = server.Run('steam', runs_dir=profile_paths.runs_dir(profile_id='work'), profile_id='work')
...
def test_cancel_and_stream_return_404_for_other_profile_run(multi_profile_server: str) -> None:
...
def test_runs_snapshot_filters_other_profile_history(multi_profile_server: str, isolated_profiles: Path) -> None:
    base = multi_profile_server
    create_profile('Work')
    create_profile('Play')
    mgr = server.MANAGER
    work_summary = {'id': 'workhist1', 'key': 'steam', 'label': 'Steam', 'status': 'done', 'exit_code': 0, 'profile_id': 'work'}
    play_summary = {'id': 'playhist1', 'key': 'gog', 'label': 'GOG', 'status': 'done', 'exit_code': 0, 'profile_id': 'play'}
