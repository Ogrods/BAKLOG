...
class _FakeManager:
    ...
    def cancel_all_and_wait(self, timeout=10.0):
        self.calls.append("cancel_all_and_wait")
        return {"cancelled": [{"id": "stub", "key": "demo", "status": "cancelled"}], "stragglers": []}
...
@pytest.fixture()
def switch_server(tmp_path, monkeypatch):
...
def test_profile_switch_cancels_runs_before_rebind(switch_server):
