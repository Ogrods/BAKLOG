...
def _stat(*, title_id: str, name: str, hours: int=0, minutes: int=0, play_count: int | None=None, last: datetime | None=None, first: datetime | None=None) -> SimpleNamespace:
    return SimpleNamespace(title_id=title_id, name=name, play_duration=timedelta(hours=hours, minutes=minutes) if hours or minutes else None, play_count=play_count, last_played_date_time=last, first_played_date_time=first, image_url=None)
...
def _trophy(*, comm_id: str='NPWR22022_00', name: str='Fortnite', title_id: str='PPSA01922_00', platform: str='PS5', earned: SimpleNamespace | None=None, defined: SimpleNamespace | None=None) -> SimpleNamespace:
    return SimpleNamespace(np_communication_id=comm_id, title_name=name, np_title_id=title_id, title_platform=[SimpleNamespace(value=platform)], title_icon_url=None, progress=10, last_updated_datetime=datetime(2024, 2, 17, 20, 21, 43), earned_trophies=earned, defined_trophies=defined)

class _FakePsnMe:

    def __init__(self, stats: list, trophies: list, entitlements: list | None=None) -> None:
        self._stats = stats
...
def test_cross_gen_playtime_summed() -> None:
    ...
    stats = [_stat(title_id='CUSA07022_00', name='Fortnite', hours=2304, minutes=16), _stat(title_id='PPSA01922_00', name='Fortnite', hours=168)]
    client = object.__new__(PsnClient)
...
def test_cross_gen_last_first_play_count_merged() -> None:
    stats = [_stat(title_id='CUSA07022_00', name='Fortnite', hours=100, play_count=50, last=datetime(2024, 6, 1, tzinfo=UTC), first=datetime(2018, 1, 1, tzinfo=UTC)), _stat(title_id='PPSA01922_00', name='Fortnite', hours=10, play_count=5, last=datetime(2025, 1, 1, tzinfo=UTC), first=datetime(2020, 1, 1, tzinfo=UTC))]
    client = object.__new__(PsnClient)
...
def test_edition_suffix_groups_via_dedupe_key() -> None:
    ...
    entry = PsnGameEntry(id='x', np_communication_id='x', title_id='A', concept_id=None, name='Game Name', image_url=None, platforms=['PS5'], trophy_progress=None, trophies_earned=None, trophies_total=None, has_platinum=False, platinum_earned=False, playtime_minutes=0, last_played=None, first_played=None, play_count=None, store_url=None)
...
def test_trophy_counts_and_platinum_flags() -> None:
    ...
    client = object.__new__(PsnClient)
    client._client = _FakePsnMe([], [_trophy(comm_id='NPWR99999_00', name='Platinum Game', earned=earned, defined=defined)])
...
def test_entitlement_beta_and_non_game_skipped() -> None:
    client = object.__new__(PsnClient)
    client._client = _FakePsnMe([], [], entitlements=[{'isGame': False, 'titleMeta': {'titleId': 'CUSA00001_00', 'name': 'Not A Game'}}, {'isBeta': True, 'titleMeta': {'titleId': 'CUSA00002_00', 'name': 'Beta Build'}}, {'titleMeta': {'titleId': 'CUSA00003_00', 'name': 'Real Game', 'imageUrl': 'https://example.com/title.jpg'}, 'conceptMeta': {'conceptId': '123', 'name': 'Real Game'}, 'gameMeta': {'name': 'Real Game', 'iconUrl': 'https://example.com/icon.jpg'}}])
    games = client.collect_library()
