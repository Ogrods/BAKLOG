...
def test_steam_store_api_rejects_csv_appids() -> None:
    single = requests.get('https://store.steampowered.com/api/appdetails', params={'appids': 70, 'l': 'english'}, timeout=15)
    batch = requests.get('https://store.steampowered.com/api/appdetails', params={'appids': '70,220,240', 'l': 'english'}, timeout=15)
...
@pytest.mark.release_smoke
def test_steam_get_app_details_batch_one_call_per_appid(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
