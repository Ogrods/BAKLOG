...
def runs_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setitem(server.FETCHERS, 'demo2', {'label': 'Demo2', 'argv': [server.sys.executable, '-c', "print('two')"], 'refreshArgs': [], 'metaKey': 'demo2', 'group': 'library', 'color': '#fff', 'requires': []})
    monkeypatch.setitem(server.FETCHERS, 'demo3', {'label': 'Demo3', 'argv': [server.sys.executable, '-c', "print('three')"], 'refreshArgs': [], 'metaKey': 'demo3', 'group': 'library', 'color': '#fff', 'requires': []})
...
def test_cancel_schedules_sync_completion_when_worker_dead(runs_env, monkeypatch: pytest.MonkeyPatch) -> None:
    completed: list[str] = []
...
def test_force_finalize_orphaned_active_run(runs_env, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_orphaned_reaper_spares_live_process(runs_env, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(server, 'STUCK_NO_PROC_GRACE_SEC', 0.01)
...
def test_orphaned_reaper_one_cycle_grace(runs_env, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(server, 'STUCK_NO_PROC_GRACE_SEC', 0.01)
    mgr, runs_dir = runs_env
    run = server.Run('demo', runs_dir=runs_dir)
    run.status = 'running'
...
def test_stall_watchdog_emits_notice(runs_env, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setitem(server.FETCHERS, 'sleepy', {'label': 'Sleepy', 'argv': [server.sys.executable, '-c', "import time; time.sleep(1.5); print('done')"], 'refreshArgs': [], 'metaKey': 'sleepy', 'group': 'library', 'color': '#fff', 'requires': []})
...
def test_reap_orphan_on_startup(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    active.write_text(json.dumps({'runs': [{'id': 'deadbeef', 'pid': 999999, 'key': 'demo', 'label': 'Demo', 'started_at': 1.0}]}), encoding='utf-8')
    monkeypatch.setattr(server, 'RUNS_DIR', runs_dir)
    monkeypatch.setattr(server, 'ACTIVE_RUNS_FILE', active)
    monkeypatch.setattr(server, 'RUN_HISTORY_FILE', runs_dir / 'history.json')
    monkeypatch.setitem(server.FETCHERS, 'demo', {'label': 'Demo', 'argv': [server.sys.executable, '-c', 'pass'], 'refreshArgs': [], 'metaKey': 'demo', 'group': 'library', 'requires': []})
...
def test_max_runtime_cap_kills_run(runs_env, monkeypatch: pytest.MonkeyPatch):
    ...
    mgr, _ = runs_env
    monkeypatch.setitem(server.FETCHERS, 'trickle', {'label': 'Trickle', 'argv': [server.sys.executable, '-c', "import time\nwhile True:\n print('tick')\n time.sleep(0.2)"], 'refreshArgs': [], 'metaKey': 'trickle', 'group': 'library', 'color': '#fff', 'requires': []})
...
def test_per_fetcher_max_runtime_override(runs_env, monkeypatch: pytest.MonkeyPatch):
    ...
    mgr, _ = runs_env
    monkeypatch.setitem(server.FETCHERS, 'long_enrich', {'label': 'Long enrich', 'argv': [server.sys.executable, '-c', "print('start'); import time; time.sleep(10)"], 'refreshArgs': [], 'metaKey': 'long_enrich', 'group': 'enrich', 'color': '#fff', 'requires': [], 'maxRunSeconds': 7200.0})
...
def test_stall_kill_after_single_stdout_line(runs_env, monkeypatch: pytest.MonkeyPatch):
    ...
    mgr, _ = runs_env
    monkeypatch.setitem(server.FETCHERS, 'one_line', {'label': 'One line', 'argv': [server.sys.executable, '-c', "print('started'); import time; time.sleep(5)"], 'refreshArgs': [], 'metaKey': 'one_line', 'group': 'library', 'color': '#fff', 'requires': []})
...
def test_heartbeat_keeps_long_run_alive(runs_env, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(server, 'MAX_RUN_SECONDS', 9999.0)
    mgr, _ = runs_env
    monkeypatch.setitem(server.FETCHERS, 'heartbeating', {'label': 'Heartbeating', 'argv': [server.sys.executable, '-c', "import time\nfor i in range(15):\n print('  · still working', i, flush=True)\n time.sleep(0.1)\nprint('done', flush=True)"], 'refreshArgs': [], 'metaKey': 'heartbeating', 'group': 'enrich', 'color': '#fff', 'requires': []})
    run = mgr.submit('heartbeating')
    assert run._finished.wait(timeout=20)
    replay = run.replay_lines()
    texts = [m.get('text', '') for m in replay]
    assert not any(('force-killing' in t for t in texts)), 'heartbeating run was force-killed despite emitting periodic stdout'
...
def test_durable_queue_skips_when_key_already_done(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    hist.write_text(json.dumps([{'id': 'finished-1', 'key': 'demo', 'status': 'done', 'label': 'Demo'}]), encoding='utf-8')
...
def test_shutdown_cancels_in_flight_run(runs_env, monkeypatch: pytest.MonkeyPatch) -> None:
    mgr, _runs_dir = runs_env
    monkeypatch.setitem(server.FETCHERS, 'slow', {'label': 'Slow', 'argv': [server.sys.executable, '-c', 'import time; time.sleep(10)'], 'refreshArgs': [], 'metaKey': 'slow', 'group': 'library', 'color': '#fff', 'requires': []})
    run = mgr.submit('slow')
    deadline = time.time() + 5
    while time.time() < deadline:
...
def test_execute_runs_from_repo_root_for_nondefault_profile(runs_env, monkeypatch: pytest.MonkeyPatch) -> None:
    import auth.manager
    mgr, _runs_dir = runs_env
    monkeypatch.setattr(server, 'get_active_profile_id', lambda: 'work')
    monkeypatch.setattr(auth.manager, 'subprocess_env_for_profile', lambda pid: {'BAKLOG_PROFILE': pid, 'PYTHONUNBUFFERED': '1'})
    ...
    def capturing_popen(argv, **kwargs):
        ...
        return real_popen([server.sys.executable, '-c', "print('ok')"], stdout=kwargs.get('stdout'), stderr=kwargs.get('stderr'), text=kwargs.get('text', True), encoding=kwargs.get('encoding'), errors=kwargs.get('errors'), bufsize=kwargs.get('bufsize', -1), cwd=kwargs.get('cwd'))
...
def internal_jobs(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setitem(server.INTERNAL_JOBS, 'buildClaims', {'label': 'Build free claims feed', 'script': 'fetchers/build_free_claims.py', 'group': 'claims', 'args': [], 'options': {}})
    monkeypatch.setitem(server.INTERNAL_JOBS, 'claimSources', {'label': 'Fetch claim sources', 'script': 'fetchers/fetch_claim_sources.py', 'group': 'claims', 'args': [], 'options': {}})
...
def test_enrich_lane_parallel_to_fetcher(runs_env) -> None:
    mgr, runs_dir = runs_env
    monkeypatch_item = {'label': 'Enrich demo', 'argv': [server.sys.executable, '-c', "print('enrich')"], 'refreshArgs': [], 'metaKey': 'enrich_demo', 'group': 'enrich', 'color': '#fff', 'requires': []}
