...
FETCHERS = {'steam': {'group': 'library', 'metaKey': 'steam', 'requires': [], 'platforms': [], 'refreshArgs': ['--refresh']}, 'gog': {'group': 'library', 'metaKey': 'gog', 'requires': [], 'platforms': [], 'refreshArgs': []}, 'hltb': {'group': 'enrich', 'metaKey': 'hltb', 'requires': [], 'platforms': []}, 'itad': {'group': 'prices', 'metaKey': 'itad', 'requires': [], 'platforms': []}}
...
def _make(manager, *, is_pro=True, missing=lambda reqs: list(reqs)):
    return sched.BackgroundScheduler(manager=manager, fetchers=FETCHERS, missing_requirements=missing, is_pro_fn=lambda: is_pro)
...
def test_skips_fetchers_with_missing_credentials(monkeypatch):
    _set_ages(monkeypatch, {'steam': sched.DEFAULT_STALE_AGE_SEC + 100})
    fetchers = {'steam': {'group': 'library', 'metaKey': 'steam', 'requires': [{'env': 'STEAM_API_KEY'}], 'platforms': [], 'refreshArgs': ['--refresh']}}
    mgr = FakeManager()
    s = sched.BackgroundScheduler(manager=mgr, fetchers=fetchers, missing_requirements=lambda reqs: ['STEAM_API_KEY'] if reqs else [], is_pro_fn=lambda: True)
...
def test_skips_auto_fetch_disabled_launcher(monkeypatch):
    monkeypatch.setattr(sched, '_catalog_age_sec', lambda mk, pid, now: {'amazon': sched.DEFAULT_STALE_AGE_SEC + 9999, 'steam': 10}.get(mk, 10))
    fetchers = {'steam': {'group': 'library', 'metaKey': 'steam', 'requires': [], 'platforms': []}, 'amazon': {'group': 'library', 'metaKey': 'amazon', 'requires': [], 'platforms': [], 'autoFetch': False}}
    mgr = FakeManager()
    s = sched.BackgroundScheduler(manager=mgr, fetchers=fetchers, missing_requirements=lambda reqs: list(reqs), is_pro_fn=lambda: True)
    assert s.tick(now=time.time()) is None
    assert mgr.submitted == []
...
def test_probe_skipped_when_scheduler_disabled(monkeypatch):
    mgr = FakeManager()
    s = _make(mgr)
    monkeypatch.setattr(s, '_load_config', lambda _pid: {'enabled': False, 'stale_age_sec': sched.DEFAULT_STALE_AGE_SEC, 'stagger_sec': sched.DEFAULT_STAGGER_SEC, 'probe_interval_sec': sched.DEFAULT_PROBE_INTERVAL_SEC, 'quiet_start_hour': None, 'quiet_end_hour': None})
    with patch('auth.connection_probe.run_connection_probe') as probe:
