...
def _post(base: str, path: str, body: dict) -> tuple[int, dict]:
    ...
    req = urllib.request.Request(f'{base}{path}', data=data, method='POST', headers={'Content-Type': 'application/json', 'X-BAKLOG-Local': '1'})
    try:
...
def test_catalog_import_round_trip(catalog_server: str, tmp_path: Path) -> None:
    payload = {'catalogs': {'games_steam.json': {'games': [{'store': 'steam', 'id': '570', 'name': 'Dota 2'}], 'fetched_at': '2026-06-25T00:00:00Z'}, 'itad_prices.json': {'currency': 'USD', 'prices': {}}}}
