...
def _post(base, path, body):
    ...
    req = urllib.request.Request(
        f"{base}{path}", data=data, method="POST", headers={"Content-Type": "application/json", "X-BAKLOG-Local": "1"}
    )
    try:
...
def test_catalog_filename_whitelist():
...
def test_catalog_import_rejects_bad_filename(catalog_server):
    status, body = _post(catalog_server, "/api/catalogs/import", {"catalogs": {"secrets.bin": {"games": []}}})
    assert status == 400
