...
@pytest.fixture()
def personal_server(tmp_path, monkeypatch):
...
def _request(base, method, path, body=None, *, extra_headers=None):
    data = None
    headers = {}
    if body is not None:
...
def test_personal_restores_from_backup_on_corrupt_primary(personal_server, monkeypatch):
...
def test_personal_corrupt_without_backup_returns_503(personal_server):
    path = server.personal_path()
    path.parent.mkdir(parents=True, exist_ok=True)
...
def test_personal_get_empty_doc(personal_server):
    status, doc = _request(personal_server, "GET", "/api/personal")
    assert status == 200
    assert doc["personal"] == {}
    assert doc["prefs"] == {}
...
def test_personal_put_rejects_empty_overwrite_of_meaningful_doc(personal_server):
...
def test_personal_put_allows_empty_overwrite_with_opt_in_header(personal_server):
    seed = {"personal": {"steam:1": {"status": "backlog"}}, "prefs": {}, "manual": [], "libraryFirstSeen": {}}
    _request(personal_server, "PUT", "/api/personal", seed)
    empty = {"personal": {}, "prefs": {}, "manual": [], "libraryFirstSeen": {}}
    status, saved = _request(
        personal_server, "PUT", "/api/personal", empty, extra_headers={server._BAKLOG_ALLOW_EMPTY_HEADER: "1"}
    )
    assert status == 200
    assert saved["personal"] == {}


def test_personal_put_round_trip(personal_server):
    payload = {
        "personal": {"steam:570": {"status": "backlog"}},
        "prefs": {"picksTab": "topRated"},
        "manual": [{"store": "manual", "id": "demo", "name": "Demo"}],
        "libraryFirstSeen": {"steam:570": 1700000000000},
    }
...
def test_personal_hidden_flag_round_trips(personal_server):
    payload = {
        "personal": {
...
def test_personal_put_invalid_payload(personal_server):
...
def test_personal_put_matching_profile_stamped(personal_server):
...
def test_personal_put_profile_mismatch_rejected(personal_server, tmp_path, monkeypatch):
    monkeypatch.setattr(profile_paths, "ROOT", tmp_path)
    prof = tmp_path / "profiles"
...
def test_personal_post_beacon_writes(personal_server):
...
def test_personal_put_without_profile_back_compat(personal_server):
...
def test_missing_library_json_returns_empty_catalog(personal_server, tmp_path, monkeypatch):
    monkeypatch.setattr(profile_paths, "ROOT", tmp_path)
...
def test_profiles_create_and_switch(personal_server, tmp_path, monkeypatch):
    monkeypatch.setattr(profile_paths, "ROOT", tmp_path)
...
def test_scoped_catalog_served_from_profile_dir(personal_server, tmp_path, monkeypatch):
