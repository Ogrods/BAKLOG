"""Tests for POST /api/shutdown (tray graceful server quit)."""
...
def test_shutdown_ok_with_local_header(shutdown_server: str) -> None:
    ...
    # Background thread should stop serve_forever shortly.
    deadline = time.monotonic() + 3.0
    while time.monotonic() < deadline:
        time.sleep(0.05)
    # No assertion on httpd state — fixture teardown handles cleanup.
