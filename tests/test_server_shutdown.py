...
@pytest.fixture()
def shutdown_server(tmp_path, monkeypatch):
...
def _post(base, path, *, local_header=False):
    import urllib.error
...
def test_shutdown_requires_local_header(shutdown_server):
    status, body = _post(shutdown_server, "/api/shutdown")
    assert status == 403
    assert "cross-origin" in body.get("error", "").lower()


def test_shutdown_ok_with_local_header(shutdown_server):
