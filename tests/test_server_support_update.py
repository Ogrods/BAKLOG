...
def test_build_update_check_payload_ok() -> None:
    release = {'tag_name': 'v1.2.3', 'html_url': 'https://github.com/Ogrods/BAKLOG/releases/tag/v1.2.3', 'body': '## Notes\n- fix bug', 'published_at': '2026-06-26T00:00:00Z', 'assets': [{'name': 'BAKLOG-win64.zip', 'browser_download_url': 'https://github.com/Ogrods/BAKLOG/releases/download/v1.2.3/BAKLOG-win64.zip'}, {'name': 'BAKLOG-win64.sha256', 'browser_download_url': 'https://github.com/Ogrods/BAKLOG/releases/download/v1.2.3/BAKLOG-win64.sha256'}]}
...
def test_build_update_check_payload_darwin_without_mac_asset(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    release = {'tag_name': 'v1.2.3', 'html_url': 'https://github.com/Ogrods/BAKLOG/releases/tag/v1.2.3', 'assets': [{'name': 'BAKLOG-win64.zip', 'browser_download_url': 'https://github.com/Ogrods/BAKLOG/releases/download/v1.2.3/BAKLOG-win64.zip'}]}
    with patch('shared.server_support.fetch_latest_github_release', return_value=release):
        with patch('shared.server_support.is_frozen', return_value=True):
            with patch('shared.install_paths.runtime_label', return_value='installed'):
                with patch('shared.server_support._apply_script_present', return_value=True):
