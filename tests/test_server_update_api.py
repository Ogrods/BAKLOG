...
def _post(base: str, path: str, *, origin: str | None=None, local_header: bool=False) -> tuple[int, dict]:
    import urllib.error
...
def test_update_check_includes_download_metadata(update_api_server: str, monkeypatch: pytest.MonkeyPatch) -> None:
    fake_release = {'tag_name': 'v9.9.9', 'html_url': 'https://github.com/Ogrods/BAKLOG/releases/tag/v9.9.9', 'assets': [{'name': 'BAKLOG-win64.zip', 'browser_download_url': 'https://github.com/Ogrods/BAKLOG/releases/download/v9.9.9/BAKLOG-win64.zip'}, {'name': 'BAKLOG-win64.sha256', 'browser_download_url': 'https://github.com/Ogrods/BAKLOG/releases/download/v9.9.9/BAKLOG-win64.sha256'}]}
...
def test_update_apply_result_public_read(update_api_server: str, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    work_root.mkdir()
    (work_root / 'apply-result.json').write_text('{"ok": false, "error": "copy failed", "restored_from_backup": true}', encoding='utf-8')
    mgr = UpdateManager(current_version=lambda: '0.8.25', has_in_flight_runs=lambda: False, work_root=work_root)
...
def test_update_discard_ready_endpoint(update_api_server: str, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    from shared.update_manager import UpdateManager, reset_update_manager_for_tests
    reset_update_manager_for_tests()
