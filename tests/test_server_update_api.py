...
@pytest.fixture()
def update_api_server(tmp_path, monkeypatch):
...
def test_update_download_cross_origin_blocked(update_api_server):
...
def test_update_download_requires_local_header(update_api_server):
    status, body = _post(update_api_server, "/api/update/download", origin=update_api_server)
    assert status == 403


def test_update_download_blocked_when_not_frozen(update_api_server, monkeypatch):
...
def test_update_status_is_public_read(update_api_server):
    status, body = _get(update_api_server, "/api/update/status")
    assert status == 200
    assert body.get("phase") == "idle"


def test_update_check_includes_download_metadata(update_api_server, monkeypatch):
    fake_release = {
        "tag_name": "v9.9.9",
...
def test_update_apply_result_public_read(update_api_server, tmp_path, monkeypatch):
    ...
    (work_root / "apply-result.json").write_text(
        '{"ok": false, "error": "copy failed", "restored_from_backup": true}', encoding="utf-8"
    )
...
def test_update_discard_ready_endpoint(update_api_server, monkeypatch, tmp_path):
    from shared.update_manager import UpdateManager, reset_update_manager_for_tests
