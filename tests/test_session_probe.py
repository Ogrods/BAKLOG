...
class TestProbeGogSession:
    ...
    def test_ok_when_validate_succeeds(self):
    ...
    def test_error_when_validate_raises(self):
        with patch("auth.session_probe.GogClient") as cls:
    ...
    def test_probe_browser_session_routes_gog(self):
    ...
    def test_probe_browser_session_unknown_provider(self):
        assert probe_browser_session("steam", {"STEAM_API_KEY": "k"}) is None


class TestProbeXboxWishlistSession:
    def test_signed_out_headless(self):
        state = {"user": {"isSignedIn": False}}
        with patch("auth.xbox_wishlist_session.capture_xbox_wishlist_preloaded_state", return_value=state):
            err = probe_xbox_wishlist_session({"XBOX_WISHLIST_PROFILE": "ready"})
        assert err is not None
        assert "isSignedIn=false" in err

    def test_ok_when_headless_signed_in(self):
        state = {"user": {"isSignedIn": True}, "pageRequestMetadata": {"/wishlist": {}}}
        with patch("auth.xbox_wishlist_session.capture_xbox_wishlist_preloaded_state", return_value=state):
            assert probe_xbox_wishlist_session({"XBOX_WISHLIST_PROFILE": "ready"}) is None

    def test_capture_failure_surfaces(self):
        with patch(
...
class TestQuietProbes:
    ...
    def test_itad_quiet_maps_tri_state(self):
        from auth.api_keys import KEY_INVALID, KEY_UNREACHABLE, KEY_VALID
    ...
    def test_probe_provider_quiet_routes(self):
