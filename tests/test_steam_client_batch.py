...
@pytest.fixture()
def steam_client(tmp_path):
    return SteamClient("key", "76561198000000000", cache_dir=tmp_path / "steam")


def test_get_app_details_batch_fetches_each_appid(steam_client, monkeypatch):
    appids = [1, 2, 3, 4, 5]
    calls = []
...
def test_get_app_details_batch_uses_cache(steam_client):
