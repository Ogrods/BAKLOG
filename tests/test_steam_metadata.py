...
class TestCoopFlags:
    ...
    def test_lan_counts_as_online(self):
        assert coop_flags_from_categories(_cats("LAN Co-op")) == (True, False)

    def test_bare_coop_is_unknown(self):
...
class TestEarlyAccess:
    ...
    def test_early_access_genre(self):
...
class TestEnrichmentFromAppdetails:
    def test_none_payload_returns_empty_shape(self):
...
class TestApplyEnrichmentToRow:
    def test_always_writes_coop_flags(self):
        row = {"name": "X"}
        enrichment = {
            "coop_online": True,
            "coop_local": False,
    ...
    def test_does_not_clobber_existing_genres(self):
    ...
    def test_fills_missing_metacritic(self):
        row = {"name": "X"}
        enrichment = {"coop_online": False, "coop_local": False, "metacritic_score": 90}
        apply_enrichment_to_row(row, enrichment)
        assert row["metacritic_score"] == 90

    def test_fills_empty_genres_list(self):
        row = {"name": "X", "genres": []}
        enrichment = {"coop_online": False, "coop_local": False, "genres": ["RPG"]}
        apply_enrichment_to_row(row, enrichment)
        assert row["genres"] == ["RPG"]

    def test_does_not_clobber_existing_release_date(self):
    ...
    def test_always_write_fields_set_means_correct_partitioning(self):
        assert ALWAYS_WRITE_FIELDS == frozenset({"coop_online", "coop_local"})
        assert FILL_IF_MISSING_FIELDS == frozenset(
            {
                "genres",
                "release_date",
                "metacritic_score",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"categories": [{"description": "Online Co-op"}, {"description": "Shared/Split Screen Co-op"}]}, True, True),
    ],
)
def test_appdetails_to_coop_end_to_end(appdetails, expected_online, expected_local):
