...
def test_pid_listening_on_port_parses_netstat(monkeypatch):
    sample = "\n".join(
        [
...
def test_pid_listening_on_port_none_when_no_listener(monkeypatch):
    monkeypatch.setattr(dev_server_pids.sys, "platform", "win32")
    monkeypatch.setattr(
...
def test_collect_targets_unions_and_sorts(monkeypatch):
...
def test_dry_run_lists_without_killing(monkeypatch, capsys):
    killed = []
...
def test_force_kills_targets_and_clears_pid_file(monkeypatch, tmp_path, capsys):
    killed = []
...
def test_dedupe_keeps_live_server_and_kills_extras(monkeypatch, tmp_path, capsys):
    killed = []
    pid_file = tmp_path / ".baklog_server.pid"
...
def test_dedupe_clears_pid_file_not_matching_keeper(monkeypatch, tmp_path):
    killed = []
    pid_file = tmp_path / ".baklog_server.pid"
...
def test_dedupe_with_no_live_server_kills_all(monkeypatch, tmp_path):
    killed = []
    pid_file = tmp_path / ".baklog_server.pid"
...
def test_dedupe_protects_keeper_launch_tree(monkeypatch, tmp_path):
    killed = []
    pid_file = tmp_path / ".baklog_server.pid"
...
def test_main_dedupe_dispatch(monkeypatch):
    called = {"dedupe": 0}
