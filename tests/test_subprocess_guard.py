...
def test_related_pids_includes_ancestors_and_descendants(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # shell(20476) -> launcher(3208) -> server child(7528) -> fetcher(8000)
    # plus an unrelated stray launch: shell(50) -> launcher(51) -> server(52)
    ...
    # keeper + its launcher + shell ancestor + its fetcher descendant
    assert {7528, 3208, 20476} <= related
    assert 8000 in related
    # an unrelated stray launch must NOT be protected
