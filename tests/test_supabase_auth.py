...
def _token(
    ...
    *,
    ...
    payload = {"sub": sub, "aud": aud, "iss": iss, "exp": int(time.time()) + exp_delta, "email": "invitee@example.com"}
...
def test_verify_rejects_wrong_issuer(monkeypatch):
    secret = "test-jwt-secret"
    monkeypatch.setenv("BAKLOG_SUPABASE_URL", "https://x.supabase.co")
    monkeypatch.setenv("BAKLOG_SUPABASE_ANON_KEY", "anon-key")
    monkeypatch.setenv("BAKLOG_SUPABASE_JWT_SECRET", secret)
...
def test_public_config_masks_secret(monkeypatch):
    monkeypatch.setenv("BAKLOG_SUPABASE_URL", "https://x.supabase.co")
    monkeypatch.setenv("BAKLOG_SUPABASE_ANON_KEY", "anon-key")
    monkeypatch.setenv("BAKLOG_SUPABASE_JWT_SECRET", "secret")
...
def test_public_config_auth_redirect_overrides(monkeypatch):
    monkeypatch.setenv("BAKLOG_SUPABASE_URL", "https://x.supabase.co")
    monkeypatch.setenv("BAKLOG_SUPABASE_ANON_KEY", "anon-key")
...
def test_decode_jwks_es256(monkeypatch):
...
def test_warmup_jwks_client_noop_without_auth(monkeypatch):
    from shared.bundled_auth_env import warmup_auth_verification

    warmup_auth_verification()


def test_warmup_jwks_skips_network_when_hs256_secret(monkeypatch):
    monkeypatch.setenv("BAKLOG_SUPABASE_URL", "https://x.supabase.co")
    monkeypatch.setenv("BAKLOG_SUPABASE_ANON_KEY", "anon-key")
    monkeypatch.setenv("BAKLOG_SUPABASE_JWT_SECRET", "secret")
...
def test_auth_enabled_without_jwt_secret(monkeypatch):
    monkeypatch.setenv("BAKLOG_SUPABASE_URL", "https://x.supabase.co")
    monkeypatch.setenv("BAKLOG_SUPABASE_ANON_KEY", "anon-key")
    monkeypatch.delenv("BAKLOG_SUPABASE_JWT_SECRET", raising=False)
    assert supabase_auth.auth_enabled() is True
