...
def test_handle_update_post_download_humanizes_error(monkeypatch) -> None:
    ...
    handle_update_post('/api/update/download', current_version=lambda: '0.8.25', has_in_flight_runs=lambda: True, read_json_body=lambda: ({}, None), send_json=lambda status, payload: sent.append((status, payload)), trigger_shutdown=lambda: None)
...
def test_handle_update_post_dismiss_requires_version(monkeypatch) -> None:
    sent: list[tuple[HTTPStatus, dict]] = []
    handle_update_post('/api/update/dismiss', current_version=lambda: '0.8.25', has_in_flight_runs=lambda: False, read_json_body=lambda: ({}, None), send_json=lambda status, payload: sent.append((status, payload)), trigger_shutdown=lambda: None)
    assert sent[0][0] == HTTPStatus.BAD_REQUEST