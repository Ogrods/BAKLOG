...
def _artifacts(version: str='0.8.26') -> ReleaseArtifacts:
    return ReleaseArtifacts(tag=f'v{version}', version=version, html_url=f'https://github.com/Ogrods/BAKLOG/releases/tag/v{version}', zip_url=f'https://github.com/Ogrods/BAKLOG/releases/download/v{version}/BAKLOG-win64.zip', sha256='')
...
def test_download_worker_marks_ready_after_verify(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr('shared.update_manager.is_frozen', lambda: True)
    monkeypatch.setattr(sys, 'platform', 'win32')
    monkeypatch.setattr('shared.update_manager.is_running_from_temp_dir', lambda _p: False)
    payload = b'fake-zip-bytes'
    digest = hashlib.sha256(payload).hexdigest()
    artifacts = ReleaseArtifacts(tag='v0.8.26', version='0.8.26', html_url='https://example.com/release', zip_url='https://github.com/Ogrods/BAKLOG/releases/download/v0.8.26/BAKLOG-win64.zip', sha256=digest)
...
def test_apply_ready_update_launches_helper_darwin(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr('shared.update_manager.is_frozen', lambda: True)
    ...
    with mgr._lock:
        ...
        mgr._artifacts = ReleaseArtifacts(tag='v0.8.26', version='0.8.26', html_url='https://example.com', zip_url='https://github.com/Ogrods/BAKLOG/releases/download/v0.8.26/BAKLOG-macos.zip', sha256=digest)
...
def test_apply_ready_update_launches_helper(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr('shared.update_manager.is_frozen', lambda: True)
    monkeypatch.setattr(sys, 'platform', 'win32')
    monkeypatch.setattr('shared.update_manager.is_running_from_temp_dir', lambda _p: False)
    monkeypatch.setattr('shared.update_manager.frozen_bundle_dir', lambda: tmp_path)
    ...
    with mgr._lock:
        ...
        mgr._status.ready = True
        mgr._zip_path = zip_path
        mgr._artifacts = ReleaseArtifacts(tag='v0.8.26', version='0.8.26', html_url='https://example.com', zip_url='https://github.com/Ogrods/BAKLOG/releases/download/v0.8.26/BAKLOG-win64.zip', sha256=digest)
    launched: list[bool] = []
...
def test_apply_rejects_zip_outside_work_root(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr('shared.update_manager.is_frozen', lambda: True)
    monkeypatch.setattr(sys, 'platform', 'win32')
    monkeypatch.setattr('shared.update_manager.is_running_from_temp_dir', lambda _p: False)
...
def test_apply_requires_sha256(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr('shared.update_manager.is_frozen', lambda: True)
    monkeypatch.setattr(sys, 'platform', 'win32')
    monkeypatch.setattr('shared.update_manager.is_running_from_temp_dir', lambda _p: False)
    monkeypatch.setattr('shared.update_manager.frozen_bundle_dir', lambda: tmp_path)
    ...
    with mgr._lock:
        ...
        mgr._artifacts = ReleaseArtifacts(tag='v0.8.26', version='0.8.26', html_url='https://example.com', zip_url='https://github.com/Ogrods/BAKLOG/releases/download/v0.8.26/BAKLOG-win64.zip', sha256='')
    payload = mgr.apply_ready_update()
    assert payload['ok'] is False
    assert 'sha256' in payload['error'].lower()

def test_rehydrate_ready_state_on_init(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_discard_ready_update_clears_disk(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    payload = b'verified-package'
    digest = hashlib.sha256(payload).hexdigest()
    version_dir = tmp_path / '0.8.27'
    version_dir.mkdir()
...
def test_start_download_blocked_when_sign_in_active(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(sys, 'platform', 'win32')
    monkeypatch.setattr('shared.update_manager.is_running_from_temp_dir', lambda _p: False)
    mgr = UpdateManager(current_version=lambda: '0.8.25', has_in_flight_runs=lambda: False, has_active_sessions=lambda: True)
    with patch('shared.update_manager.fetch_release_artifacts', return_value=_artifacts()):
        payload = mgr.start_download()
    assert payload['ok'] is False
