...
import pytest
from shared.update_platform import apply_script_name, is_in_app_apply_platform, launch_apply_subprocess, required_bundle_files, server_binary_name, stable_sha256_name, stable_zip_name, tray_binary_name
from shared.update_release import is_allowed_download_url

@pytest.mark.parametrize(('platform', 'zip_name', 'sha_name', 'server', 'tray', 'script'), [('win32', 'BAKLOG-win64.zip', 'BAKLOG-win64.sha256', 'BAKLOG.exe', 'BAKLOG Tray.exe', 'apply_update.ps1'), ('darwin', 'BAKLOG-macos.zip', 'BAKLOG-macos.sha256', 'BAKLOG', 'BAKLOG Tray', 'apply_update.sh')])
