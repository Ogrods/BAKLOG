...
def test_write_and_scan_ready_state(tmp_path):
    version = "0.8.27"
    payload = b"zip-bytes"
    ...
    write_ready_state(
        ...
        sha256=digest,
        zip_path=zip_path,
        zip_url="https://example.com/BAKLOG-win64.zip",
...
def test_clear_ready_state_removes_version_dir(tmp_path):
    version = "0.8.27"
    payload = b"x"
    digest = hashlib.sha256(payload).hexdigest()
    version_dir = tmp_path / version
...
def test_read_apply_result(tmp_path):
