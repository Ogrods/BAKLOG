...
def _temporary_dashboard_view(base_url: str):
    ...
    try:
        ...
        if original_view != 'dashboard':
            ...
            req = urllib.request.Request(url, data=payload, headers={'Content-Type': 'application/json', 'Origin': base_url.rstrip('/')}, method='PUT')
...
def capture_dashboard(url: str, out: Path, *, width: int=1400, height: int=900) -> None:
    ...
    try:
        with _temporary_dashboard_view(base):
            common = [str(chrome), '--headless=new', '--disable-gpu', '--hide-scrollbars', f'--user-data-dir={profile}', f'--window-size={width},{height}', '--virtual-time-budget=45000', f'--screenshot={out}', bootstrap]
