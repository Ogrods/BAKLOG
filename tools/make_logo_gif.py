...
PILLS = ({'name': 'bl', 'rect': (2, 52, 46, 24), 'knob': (14, 64), 'radius': 12}, {'name': 'br', 'rect': (52, 52, 46, 24), 'knob': (64, 64), 'radius': 12}, {'name': 'top', 'rect': (27, 24, 46, 24), 'knob': (39, 36), 'radius': 12})
VIEWBOX = (2, 24, 96, 52)
KNOB_R = 8
WORDMARK = 'BAKLOG'
LETTER_SPACING = 0.18
...
FPS = 30
FRAME_MS = 33
DROP_START_ABOVE = 0.65
ROOT = Path(__file__).resolve().parents[1]
OUT_DIR = ROOT / 'marketing' / 'assets' / 'logo'
FONT_CANDIDATES = (ROOT / 'assets' / 'fonts' / 'SpaceGrotesk-Bold.ttf', Path(os.environ.get('WINDIR', 'C:/Windows')) / 'Fonts' / 'segoeuib.ttf', Path(os.environ.get('WINDIR', 'C:/Windows')) / 'Fonts' / 'arialbd.ttf')
...
def blue_preset(out_w: int=400, out_h: int=400, supersample: int=4) -> GifPreset:
    return GifPreset(out_w=out_w, out_h=out_h, supersample=supersample, log_fill='#38bdf8', log_edge='#22d3ee', knob_edge='#0c4a6e', bg_top='#0f172a', bg_bottom='#0b1220', wordmark_color='#7dd3fc', wordmark_shadow='#0b1220', wordmark_size=0.108, wordmark_gap=0.028, layout_mode='wordmark', shadow_rgba=(0, 0, 0, 48), word_shadow=True)

def midnight_preset(out_w: int, out_h: int, supersample: int, layout_mode: LayoutMode) -> GifPreset:
    return GifPreset(out_w=out_w, out_h=out_h, supersample=supersample, log_fill='#ffffff', log_edge='#e5e7eb', knob_edge='#52525b', bg_top='#000000', bg_bottom='#000000', wordmark_color='#ffffff', wordmark_shadow='#000000', wordmark_size=0.108 if out_h >= 200 else 0.095, wordmark_gap=0.028, layout_mode=layout_mode, shadow_rgba=(0, 0, 0, 32), word_shadow=False, drop_start_above=DROP_START_ABOVE)
...
class MarkLayout:
    ...
    def to_canvas(self, sx: float, sy: float) -> tuple[float, float]:
        vx, vy, _, _ = VIEWBOX
        return (self.origin_x + (sx - vx) * self.scale, self.origin_y + (sy - vy) * self.scale)
...
def make_pill_sprite(pill: dict, layout: MarkLayout, preset: GifPreset) -> Image.Image:
    ...
    draw.rounded_rectangle((px0, py0, px1, py1), radius=r * layout.scale, outline=edge, width=max(1, int(layout.scale * 0.8)))
    ...
    knob_draw.ellipse((klx - kr, kly - kr, klx + kr, kly + kr), outline=_hex_rgb(preset.knob_edge) + (90,), width=max(1, int(layout.scale * 0.5)))
    return layer
...
def make_gradient_pill_sprite(slot_center: tuple[float, float], layout: MarkLayout, preset: GifPreset, stops: tuple[tuple[float, str], ...]) -> Image.Image:
    import numpy as np
    rw, rh = (46, 24)
    knob_off_x = -11.0
    r = 12
...
def _draw_static_wordmark(bg: Image.Image, layout: MarkLayout, preset: GifPreset, font: ImageFont.FreeTypeFont | ImageFont.ImageFont) -> None:
    ch = preset.canvas_h
...
def composite_pill(base: Image.Image, sprite: Image.Image, cx: float, cy: float, angle_deg: float, preset: GifPreset, scale_x: float=1.0, scale_y: float=1.0, shadow: bool=True) -> None:
    w, h = sprite.size
...
def render_frame(layout: MarkLayout, preset: GifPreset, sprites: list[Image.Image], states: list[PillState], word_alpha: float=0.0, word_drop: float=0.0, font: ImageFont.FreeTypeFont | ImageFont.ImageFont | None=None, motto_alpha: float=0.0, motto_font: ImageFont.FreeTypeFont | ImageFont.ImageFont | None=None) -> Image.Image:
    ch = preset.canvas_h
    bg = draw_background(preset).convert('RGBA')
    for sprite, state in zip(sprites, states):
        composite_pill(bg, sprite, state.x, state.y, state.angle, preset, state.scale_x, state.scale_y)
...
def motion_toss(layout: MarkLayout, preset: GifPreset, n_frames: int) -> list[list[PillState]]:
    ...
    starts = [(finals[0][0] - cw * 0.55, finals[0][1] + ch * 0.35), (finals[1][0] + cw * 0.55, finals[1][1] + ch * 0.35), (finals[2][0], finals[2][1] - ch * 0.45)]
...
def motion_drop(layout: MarkLayout, preset: GifPreset, n_frames: int) -> list[list[PillState]]:
    ...
    for fi in range(n_frames):
        ...
        if fi > max(delays) + fall_dur + 4:
            ...
            for j in range(3):
                states[j] = PillState(states[j].x + wob * 0.3, states[j].y + wob * 0.15, states[j].angle + wob * 0.02, states[j].scale_x, states[j].scale_y)
        frames.append(states)
    return frames
...
def motion_scatter(layout: MarkLayout, preset: GifPreset, n_frames: int) -> list[list[PillState]]:
    cw, ch = (layout.canvas_w, layout.canvas_h)
    finals = [layout.pill_center(p) for p in PILLS]
    starts = [(finals[0][0] - cw * 0.42, finals[0][1] + ch * 0.08), (finals[1][0] + cw * 0.42, finals[1][1] + ch * 0.08), (finals[2][0] + cw * 0.12, finals[2][1] - ch * 0.38)]
...
ORBIT_GRADIENT_STOPS = ((0.0, '#38bdf8'), (0.5, '#22d3ee'), (1.0, '#a855f7'))
...
def _orbit_progress(hop_t: float, start_slot: int) -> float:
    ...
    span = ORBIT_HOP_MOVE - ORBIT_STAGGER * 2
    return clamp01((hop_t - delay) / max(span, 1))

def _polar_from_centroid(cx: float, cy: float, px: float, py: float) -> tuple[float, float]:
...
def motion_orbit(layout: MarkLayout, preset: GifPreset, n_frames: int, straight: bool=True) -> list[list[PillState]]:
    finals = [layout.pill_center(p) for p in PILLS]
...
def apply_wordmark_phase(pill_frames: list[list[PillState]], settle_end: int, include_wordmark: bool=True, include_motto: bool=False, word_dur: int=14, word_pause: int=4, motto_dur: int=12, hold_dur: int=16) -> tuple[list[list[PillState]], list[float], list[float], list[float]]:
...
def save_gif(path: Path, frames: list[Image.Image], preset: GifPreset) -> None:
    ...
    quantized[0].save(path, save_all=True, append_images=quantized[1:], duration=FRAME_MS, loop=0, disposal=2, optimize=False)

def build_orbit_animation(preset: GifPreset, include_wordmark: bool=True, loop_frames: int=ORBIT_LOOP_FRAMES, gradient_stops: tuple[tuple[float, str], ...] | None=None, straight: bool=True) -> list[Image.Image]:
    ...
    if gradient_stops is None:
        ...
        for st in pill_frames:
            fr = render_frame(layout, preset, sprites, st, word_alpha=1.0 if include_wordmark else 0.0, word_drop=0.0, font=font)
            frames.append(fr.convert('RGB'))
        return frames
    slot_centers = [layout.pill_center(p) for p in PILLS]
    slot_sprites = [make_gradient_pill_sprite(slot_centers[s], layout, preset, gradient_stops) for s in range(3)]
    hop_move = ORBIT_HOP_MOVE
    ...
    for fi, st in enumerate(pill_frames):
        ...
        for i in range(3):
            ...
            if in_hold or end_slot == start_slot:
                ...
                sprite = Image.blend(slot_sprites[start_slot], slot_sprites[end_slot], move_p)
            state = st[i]
            composite_pill(bg, sprite, state.x, state.y, state.angle, preset, state.scale_x, state.scale_y)
...
def build_animation(preset: GifPreset, motion_fn: Callable, include_wordmark: bool=True, include_motto: bool=False) -> list[Image.Image]:
    layout = compute_layout(preset)
    sprites = [make_pill_sprite(p, layout, preset) for p in PILLS]
    word_fs = int(preset.canvas_h * preset.wordmark_size)
    font = resolve_font(word_fs) if include_wordmark else None
    ...
    states, alphas, drops, mottos = apply_wordmark_phase(pill_frames, settle_end, include_wordmark=include_wordmark, include_motto=include_motto, word_pause=0 if not include_motto else 4, motto_dur=0 if not include_motto else 12)
    frames: list[Image.Image] = []
    for i, st in enumerate(states):
        fr = render_frame(layout, preset, sprites, st, alphas[i], drops[i], font, motto_alpha=mottos[i], motto_font=motto_font)
        frames.append(fr.convert('RGB'))
    return frames
...
def run_log_jammin_batch(arg: str) -> None:
    ...
    variants = (('logo-toss-a', motion_toss), ('logo-drop-b', motion_drop), ('logo-scatter-c', motion_scatter))
...
def run_blue_batch() -> None:
    ...
    print(f'Canvas {preset.canvas_w}x{preset.canvas_h} -> {preset.out_w}x{preset.out_h} @ {FPS}fps')
    jobs = (('logo-orbit-blue.gif', True, True), ('logo-orbit-blue-plain.gif', False, True), ('logo-orbit-blue-circle.gif', True, False))
    for filename, wordmark, straight in jobs:
        plain_preset = preset
        if not wordmark:
            plain_preset = GifPreset(out_w=preset.out_w, out_h=preset.out_h, supersample=preset.supersample, log_fill=preset.log_fill, log_edge=preset.log_edge, knob_edge=preset.knob_edge, bg_top=preset.bg_top, bg_bottom=preset.bg_bottom, layout_mode='mark_center', shadow_rgba=preset.shadow_rgba)
        print(f'  {filename}...', end=' ', flush=True)
        frames = build_orbit_animation(plain_preset, include_wordmark=wordmark, gradient_stops=ORBIT_GRADIENT_STOPS, straight=straight)
        save_gif(OUT_DIR / filename, frames, plain_preset)
        print(f'{len(frames)} frames')

def run_midnight_batch() -> None:
    jobs = (('logo-drop-midnight-sm.gif', 120, 120, 2, 'wordmark', True), ('logo-drop-midnight-sm-plain.gif', 120, 120, 2, 'mark_center', False), ('logo-drop-midnight-sm-plain-wide.gif', 213, 120, 2, 'mark_wide', False), ('logo-drop-midnight.gif', 400, 400, 4, 'wordmark', True), ('logo-drop-midnight-plain.gif', 400, 400, 4, 'mark_center', False), ('logo-drop-midnight-plain-wide.gif', 711, 400, 4, 'mark_wide', False))
    print(f'Midnight drop batch -> {OUT_DIR}')
    for filename, ow, oh, ss, mode, wordmark in jobs:
        preset = midnight_preset(ow, oh, ss, mode)
        print(f'  {filename} ({ow}x{oh}, {mode})...', end=' ', flush=True)
        frames = build_animation(preset, motion_drop, include_wordmark=wordmark, include_motto=False)
