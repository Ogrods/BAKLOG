...
JS_DIR = ROOT / 'js'
STATE_FIELDS = ['allGames', 'personal', 'prefs', 'sortKey', 'sortDir', 'pickedKey', 'libraryMeta', 'crossStoreHiddenKeys', 'crossStoreOwnedStores', 'wishlistGames', 'itchGames', 'itadByKey', 'activeView', 'selectedKeys', 'cleanupModeActive', 'focusedRowIndex', 'ownedNormNames', 'filtersDrawerOpen', 'genreChipsExpanded']
...
def main() -> None:
    ...
    app_body = re.sub('^let state\\.personal = loadPersonal\\(\\);\\s*\\nlet state\\.prefs = loadPrefs\\(\\);\\s*\\n', '', app_body, flags=re.MULTILINE)
    ...
    new_html = re.sub('<script>.*?</script>\\s*</body>', '  <script type="module" src="js/app.js"></script>\n</body>', html, count=1, flags=re.DOTALL)
