...
def _request_graceful_shutdown() -> bool:
    if not _port_open():
        return True
    req = urllib.request.Request(f'http://{HOST}:{PORT}/api/shutdown', method='POST', headers={_BAKLOG_LOCAL_HEADER: '1', 'Content-Length': '0'})
    try:
...
def _run_headless(controller: ServerController) -> int:
    print('[tray] pystray/Pillow not installed — running headless.\n       Install the tray UI with: pip install pystray Pillow', file=sys.stderr, flush=True)
...
def _start_update_notify(icon) -> None:
    ...
    def _poll() -> None:
        ...
        while time.monotonic() < deadline:
            ...
            return
        try:
            req = urllib.request.Request(f'http://{HOST}:{PORT}/api/update-check', headers={'Accept': 'application/json'})
        ...
        try:
            status_req = urllib.request.Request(f'http://{HOST}:{PORT}/api/update/status', headers={'Accept': 'application/json'})
            with urllib.request.urlopen(status_req, timeout=10) as status_resp:
                status_payload = json.loads(status_resp.read().decode('utf-8'))
            if isinstance(status_payload, dict) and status_payload.get('phase') == 'ready' and (status_payload.get('can_apply') is True):
                ready_version = str(status_payload.get('version') or latest).strip()
                _tray_notify(icon, 'Update ready to install', f'BAKLOG v{ready_version} is downloaded. Open BAKLOG and choose Install & restart.')
        except Exception:
            pass
        ...
        while True:
            ...
            if proc is not None and proc.poll() is not None:
                ...
                if not notified and (not controller.is_running()):
                    notified = True
                    _tray_notify(icon, 'BAKLOG server stopped', 'The local server exited unexpectedly. Use Open BAKLOG to restart.')
...
def run_tray() -> int:
    ...
    def _on_open_data_folder(icon, _item) -> None:
        try:
            open_data_folder()
        except OSError as exc:
            _tray_notify(icon, 'Open folder failed', str(exc))

    def _on_restart(icon, _item) -> None:
        if not controller.restart():
            if controller.is_running() and (not controller.owns_server()):
                _tray_notify(icon, 'Restart skipped', 'The server was not started by BAKLOG tray — restart it manually.')
            else:
    ...
    def _toggle_startup(icon, _item) -> None:
        toggle_startup()
    menu_items = [pystray.MenuItem('Open BAKLOG', _on_open, default=True), pystray.MenuItem('Open data folder', _on_open_data_folder), pystray.MenuItem('Restart server', _on_restart)]
    if startup_supported():
        menu_items.append(pystray.MenuItem('Start at login', _toggle_startup, checked=lambda item: is_startup_enabled()))
